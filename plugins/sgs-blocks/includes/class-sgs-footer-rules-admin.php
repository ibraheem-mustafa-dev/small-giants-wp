<?php
/**
 * Admin UI for the Conditional Footer rules engine (FR-S3-3, Spec 17 Wave 2).
 *
 * Merged onto the native "Advanced Footers" CPT screen (D-merge, 2026-09-17):
 * this class no longer registers its own top-level submenu/page. Instead it
 * hooks `admin_notices`, scoped to the `edit-sgs_footer` list-table screen
 * (`edit.php?post_type=sgs_footer`), and prints the rules table + "Add Rule"
 ...
 * {@see Sgs_Footer_Rules::add_rule()} and {@see Sgs_Footer_Rules::remove_rule()}.
 *
 * Mirror of FR-S3-2 (Sgs_Header_Rules_Admin). Separate admin-post actions, no
 * shared global state with the header admin class.
 *
 * UK English throughout. Plain language for operators — "Show this footer
...
final class Sgs_Footer_Rules_Admin {
	 ...
	 * Footers" list table. Scoped to the `edit-sgs_footer` screen only — every
	...
	public static function render_admin_notice(): void {
		...
		if ( ! $screen instanceof \WP_Screen || 'edit-' . Sgs_Block_CPTs::FOOTER_CPT !== $screen->id ) {
			return;
		}
		...
		echo '<h2>' . \esc_html__( 'Footer display rules', 'sgs-blocks' ) . '</h2>';
		echo '<p>' . \esc_html__( 'Show different footer patterns on different pages. Rules are checked top-to-bottom — the first matching rule wins. The default rule at the bottom always matches and cannot be removed.', 'sgs-blocks' ) . '</p>';

		if ( 'added' === $status ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . \esc_html__( 'Footer rule added.', 'sgs-blocks' ) . '</p></div>';
		}
		if ( 'removed' === $status ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . \esc_html__( 'Footer rule removed.', 'sgs-blocks' ) . '</p></div>';
		}
		if ( '' !== $error ) {
	...
	private static function render_rules_table(): void {
		...
		echo '<th>' . \esc_html__( 'Show this footer', 'sgs-blocks' ) . '</th>';
		...
		foreach ( $rules as $rule ) {
			...
			$is_default   = Sgs_Footer_Rules::DEFAULT_RULE_ID === $id;
	...
	private static function render_add_form(): void {
		...
		echo '<tr><th scope="row"><label for="sgs_pattern_slug">' . \esc_html__( 'Footer pattern', 'sgs-blocks' ) . '</label></th><td>';
		echo '<select id="sgs_pattern_slug" name="pattern_slug" required>';
		foreach ( self::footer_pattern_choices() as $slug => $title ) {
			echo '<option value="' . \esc_attr( $slug ) . '">' . \esc_html( $title ) . '</option>';
		}
		echo '</select>';
		echo '<p class="description">' . \esc_html__( 'Choose which footer pattern to render when this rule matches.', 'sgs-blocks' ) . '</p>';
		echo '</td></tr>';
		...
		echo '<tr><th scope="row"><label for="sgs_condition_type">' . \esc_html__( 'Show this footer when', 'sgs-blocks' ) . '</label></th><td>';
	...
	/**
	 * Available footer pattern choices for the dropdown.
	 *
	 * @return array<string,string> Map of slug => title.
	 */
	public static function footer_pattern_choices(): array {
		$out = array();
		if ( class_exists( '\\WP_Block_Patterns_Registry' ) ) {
			...
			foreach ( $patterns as $pattern ) {
				...
				if ( in_array( 'core/template-part/footer', $blocktypes, true ) ) {
		...
		if ( empty( $out ) ) {
			...
			$out[ Sgs_Footer_Rules::DEFAULT_PATTERN_SLUG ] = \__( 'Default SGS footer', 'sgs-blocks' );
		}
		return $out;
	}
	...
	public static function handle_add(): void {
		if ( ! \current_user_can( self::CAP ) ) {
			\wp_die( \esc_html__( 'You do not have permission to add a footer rule.', 'sgs-blocks' ), '', array( 'response' => 403 ) );
		}
		...
		$result = Sgs_Footer_Rules::add_rule(
			array(
	...
	public static function handle_remove(): void {
		if ( ! \current_user_can( self::CAP ) ) {
			\wp_die( \esc_html__( 'You do not have permission to remove a footer rule.', 'sgs-blocks' ), '', array( 'response' => 403 ) );
		}
	...
	/**
	 * Redirect back to the "Advanced Footers" list-table screen (where the
	...
	private static function redirect_back( array $args ): void {
		$url = \add_query_arg(
			array_merge( array( 'post_type' => Sgs_Block_CPTs::FOOTER_CPT ), $args ),
			\admin_url( 'edit.php' )
		);
