		...
		array(
			...
			'inline_style' => '
				...
				@media (forced-colors:active) {
					.wp-block-sgs-team-member.is-style-elevated:not(:focus-visible) {
...
function sgs_register_team_member_variations( array $variations, \WP_Block_Type $block_type ): array {
	...
	$sgs_variations = array(
		...
		array(
			...
			'description' => __( 'Photo, name and role only — bio and social links hidden. For dense team grids.', 'sgs-blocks' ),
			'icon'        => 'id',
			'scope'       => array( 'inserter' ),
			'attributes'  => array(
		...
		array(
			...
			'description' => __( 'Full profile in a boxed card with a border — same fields as Standard, different style.', 'sgs-blocks' ),
			'icon'        => 'businessperson',
			'scope'       => array( 'inserter' ),
			'attributes'  => array(
