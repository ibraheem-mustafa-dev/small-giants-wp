...
def main():
    ...
    # Extract rows from registry, filter to css:* and anim:* only
    rows = registry.get("rows", [])
    css_anim_rows = {r.get("setting_key"): r for r in rows if r.get("setting_key", "").startswith(("css:", "anim:"))}

    if not css_anim_rows:
        sys.exit("FATAL: No css:* or anim:* rows found in setting-registry.json")
    ...
    for cluster_name in cluster_order:
        ...
        for member_idx, member in enumerate(members):
            ...
            if member_key not in css_anim_rows:
    ...
    # Main coverage check: every css:* and anim:* row must be either a member or absorbed
    uncovered = []
    for setting_key in css_anim_rows.keys():
    ...
    # Output
    total_css_anim = len(css_anim_rows)
    ...
    if args.json:
        result = {
            ...
            "total_members": total_css_anim - total_absorbed,
            "errors": errors,
            "uncovered": uncovered,
        }
