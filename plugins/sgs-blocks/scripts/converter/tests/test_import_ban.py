...
# -- STOP-28 exemption removed (EXECUTION Step 16, 2026-07-05) -------------
# The frozen orchestrator.converter_v2 package is deleted; the ban is now
# unconditional for every file under converter/, including entry.py. A
# marked or unmarked frozen-tree import in entry.py is banned exactly like
# anywhere else — there is no longer a narrow exemption to plant-test.

def test_entry_py_frozen_import_is_banned_like_any_other_file(tmp_path):
    """A (former-shape) STOP-28-marked frozen-tree import in entry.py is
    banned unconditionally now — the marker comment carries no meaning."""
    src = (
        "def f():\n"
...
def test_real_entry_py_has_no_frozen_imports():
    """The real converter/entry.py, scanned in place, is all-clear: it no
    longer imports anything from the (deleted) frozen tree."""
