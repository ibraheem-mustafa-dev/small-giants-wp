...
// 2ad141986 — titleColourHover/subtitleColourHover written into the SAME
// shared $card_grid_hover_decls array as backgroundColourHover/textColourHover/
// borderColourHover, emitted once on .sgs-card-grid__item:hover instead of
// each on its own .sgs-card-grid__title / .sgs-card-grid__subtitle selector.
if ( '' !== ( $attributes['titleColourHover'] ?? '' ) ) {
	$card_grid_hover_decls[] = 'color:' . sgs_colour_value( $attributes['titleColourHover'] );
}
if ( '' !== ( $attributes['subtitleColourHover'] ?? '' ) ) {
	$card_grid_hover_decls[] = 'color:' . sgs_colour_value( $attributes['subtitleColourHover'] );
}
