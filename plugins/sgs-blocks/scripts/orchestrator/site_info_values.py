...
Plan step A2a (D1133). A Claude Design draft keeps its phone number, review link, social links and map link
...
def site_info_values(draft_html: str) -> dict[str, str]:
    ...
    """
    out: dict[str, str] = {}
    for key, base, _suffix, value in script_candidates(draft_html):
...
def resolve_site_info_bindings(draft_html: str) -> tuple[str, dict[str, int]]:
    ...
    def swap(match: re.Match[str]) -> str:
        ...
        if not _BARE_NAME_RE.match(expr) or expr not in values:
