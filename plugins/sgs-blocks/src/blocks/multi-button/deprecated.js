 ...
 *
 * v3 — Gap-consolidation deprecation (2026-06-07).
 *      gap/gapTablet/gapMobile were type:number + separate gapUnit string.
 *      Consolidated to type:string storing the full CSS value (e.g. "12px")
 *      so the shared ContainerWrapperControls SpacingControl can own the Gap
 *      panel. gapUnit is removed. migrate() combines number + unit into string.
 *
...
/**
 * v3: numeric gap + gapUnit → combined CSS string gap.
 * This is a dynamic block (save returns null) so the save shape is identical
 * to v2; only the attribute schema + migrate() differ.
 */
const v3 = {
	isEligible( attributes ) {
		// Only trigger migration for posts that still store gap as a number
		// (the old type:number schema). Already-migrated string values skip this.
	...
	migrate( attributes ) {
		...
		// Convert numeric values to "Npx" / "Nem" / "Nrem" strings.
		// null gapTablet means "inherit desktop" — map to empty string.
		const gap      = null != attributes.gap      ? String( attributes.gap )      + unit : '12px';
		const gapTablet = null != attributes.gapTablet ? String( attributes.gapTablet ) + unit : '';
		const gapMobile = null != attributes.gapMobile ? String( attributes.gapMobile ) + unit : '8px';

		const { gapUnit: _removed, ...rest } = attributes;
		return {
			...rest,
			gap,
			gapTablet,
