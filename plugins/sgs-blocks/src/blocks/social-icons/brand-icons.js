/**
 * Editor-canvas brand-mark SVGs — mirrors `brand-icons.php` EXACTLY so the
 * block editor preview shows the same glyph the frontend renders.
 *
 * Lucide has no Google or WhatsApp brand mark, and its `twitter` entry is
 * the retired bird logo (not the current X wordmark) while `tiktok` has no
 * entry at all. This map supplies the same four brand overrides render.php
 * applies; every other platform keeps resolving through `IconPreview`'s
 * Lucide lookup, unchanged.
 *
...
export const BRAND_ICON_PATHS = {
	...
	// PLATFORM_ICONS/$platform_icons key this platform 'twitter' (never
	// renamed to 'x' — that would be an attribute-value rename across every
...
/**
 * Render a single social-icon glyph for the editor canvas, preferring the
 * brand-mark override above when this platform has one — falls back to the
...
export default function BrandIconGlyph( { platform, size = 24, colourMode = 'theme', fallback } ) {
	...
	return (
		<svg
			className={ `sgs-brand-icon sgs-brand-icon-${ platform }` }
			xmlns="http://www.w3.org/2000/svg"
			width={ size }
			height={ size }
			viewBox="0 0 24 24"
