<?php
/**
 * Brand-mark SVGs for platforms Lucide has no recognisable icon for.
 *
 * Lucide (`includes/lucide-icons.php`) ships no Google or WhatsApp brand
 * mark, and its `twitter` entry is the retired bird logo (not the current X
 * wordmark) while `tiktok` has no entry at all (the block falls back to the
 * generic 'music' note glyph). This map supplies a single-colour brand path
 * for each of those four platforms so a visitor can recognise the channel;
 * every other platform in `render.php`'s `$platform_icons` already has an
 * outline brand glyph of its own from Lucide (facebook/instagram/linkedin/
 * youtube) and keeps using that.
 *
 * Path data: Simple Icons (https://simpleicons.org), CC0 1.0 Universal
 * licence — https://github.com/simple-icons/simple-icons/blob/develop/LICENSE.md.
 * Fetched 2026-09-24 from https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/<slug>.svg
 * (google, whatsapp, tiktok, x), each a single filled `<path>` on a
 * `0 0 24 24` viewBox with no colour baked in — `fill="currentColor"` here so
 * the block's existing glyph-colour custom properties (`--sgs-social-glyph`,
...
/**
 * Return a brand-mark SVG for a platform, or '' when none is defined.
 *
 * Deliberately covers only the platforms Lucide can't express recognisably
...
function sgs_social_icons_get_brand_icon( string $platform ): string {
	...
	if ( null === $icons ) {
		$icons = array(
			...
			// 'x' — that would be an attribute-value rename across every
