 ...
 * own directory rather than added to the shared `src/components/` barrel —
 * avoids touching a file other concurrent sessions may also be editing. If a
 * THIRD block needs this pattern, promote the (then-duplicated-3x) component
 * to `src/components/` in its own follow-up change.
 *
 ...
 * `attrPropRe` (the same JSX string-literal-prop resolution the existing
 * `RUnitControl` pattern in this same edit.js already relies on) so a
 * computed `setAttributes({ [key]: val })` write still registers as a
 * controlled attribute for the dead-control guard.
 */
import {
	ToggleControl,
