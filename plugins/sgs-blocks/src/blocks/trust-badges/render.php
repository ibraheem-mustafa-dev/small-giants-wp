...
// ─── Wrapper classes ──────────────────────────────────────────────────────────
$extra_classes = 'sgs-trust-badges sgs-trust-badges--' . $badge_style . ' sgs-trust-badges--' . $badge_size;
...
foreach ( $items as $item ) {
	...
	if ( 'icon-circle' === $badge_style ) {
		...
		// ── text-only pill ─────────────────────────────────────────────────────
		$inner_html = sprintf(
			'<span class="sgs-trust-badges__badge-label"%s>%s</span>',
			$label_style_attr,
			esc_html( $item_label )
		);
		...
		if ( $item_url ) {
			$items_html .= sprintf(
				'<a href="%s" class="sgs-trust-badges__badge"%s target="_blank" rel="noopener noreferrer">%s</a>',
				esc_url( $item_url ),
				$item_attrs,
				$inner_html
			);
			...
			$items_html .= sprintf(
				'<div class="sgs-trust-badges__badge"%s>%s</div>',
				$item_attrs,
				$inner_html
			);
		}
		...
		if ( $media_url ) {
			$badge_content .= sprintf(
				'<img src="%s" alt="%s" class="sgs-trust-badges__badge-img" loading="lazy" />',
				esc_url( $media_url ),
				esc_attr( $media_alt )
			);
		}
...
if ( $block_title ) {
	$title_html = sprintf(
		'<p class="sgs-trust-badges__title"%s>%s</p>',
		$title_style_attr,
...
if ( $auto_scroll ) {
	$badges_html = '<div class="sgs-trust-badges__track">' . $items_html . '</div>';
} else {
