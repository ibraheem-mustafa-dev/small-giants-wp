/**
 * WhatsApp CTA block deprecations.
 *
 * v2 — Static save.js era. Blocks saved with full HTML before the block
 *      was converted to dynamic (server-rendered via render.php).
 * v1 — First version: save returned null, nothing rendered on frontend.
 *      Blocks created during this era have empty innerHTML.
 */
...
const v2 = {
	...
	save( { attributes } ) {
		...
		if ( ! showOnMobile ) {
			visibilityClasses.push( 'sgs-whatsapp-cta--hide-mobile' );
		}
		if ( ! showOnDesktop ) {
			visibilityClasses.push( 'sgs-whatsapp-cta--hide-desktop' );
		}
		...
		return (
			<div { ...blockProps }>
				<a
					...
					aria-label={
						label ? undefined : 'Chat on WhatsApp'
					}
				>
					<svg
