/**
 * SGS Nav Bar/Drawer Menu (shared) — shared editor helpers.
 *
 ...
 *
 * ⚠ `BURGER_SCOPE_PX` / `burgerScopeOf()` are BAR-only (D1059 split,
 * 2026-09-14 reconciliation) — `collapsePoint` is not declared on
 * `sgs/nav-drawer-menu`'s block.json, so only `nav-bar-menu`'s own
 * `MenuSettingsPanel.js` imports these two exports. Left in this shared
 * file rather than split out because they are a small, side-effect-free
 * constant + pure function; webpack tree-shakes the unused export out of
 * the drawer block's bundle, so there is no dead-control or bundle-size
 * cost to keeping one `utils.js` instead of two near-identical copies.
 *
 * @package SGS\Blocks
 */
