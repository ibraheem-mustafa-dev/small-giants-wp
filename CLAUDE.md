...
- No jQuery — vanilla JS only frontend; `viewScriptModule` (ES modules) for interactive blocks. **Motion is governed by the three-tier doctrine (Spec 38 §1, D406 + D422): Tier V (vanilla/CSS) is the default for every effect; Tier G (GSAP) is the bounded exception for what vanilla genuinely cannot do; Tier H (helper/utility — a CLOSED list, currently Lenis alone, admitted per the §1.2a four-part test) covers a single-purpose library that is neither vanilla nor GSAP. All npm-bundled, all conditionally loaded — a page using none ships zero bytes of any. No CDN ever.**
