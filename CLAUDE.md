...
Stable facts: WP 7.0 compatible; per-client theming via `theme-snapshot.json`; the modular `converter/` engine is THE ONLY converter (the frozen convert.py/converter_v2 tree was DELETED at D276, 2026-07-05 — no flag, no fallback); `/sgs-update` is **12-stage** (aggressive prune-orphans + attr-orphan detection + retired-blocks delete; **Stage 12 regenerates the motion/FX artefacts from the DB — added 2026-08-01, D436**). ⚠ This line said "10-stage" while the pipeline was already 11, then 12 — **do not trust a cached stage count here; read `sgs-update-v2.py`'s stage map**; single-command deploy `plugins/sgs-blocks/scripts/build-deploy.py`.
