...
4b. **A CONTROL THAT "DOESN'T WORK" — FIND A BLOCK WHERE IT ALREADY WORKS AND DIFF (Bean-locked 2026-08-31).** Before designing anything, ask *which blocks already have this attribute or control?* — `python ~/.claude/skills/sgs-wp-engine/scripts/sgs-db.py sql "SELECT block_slug, attr_name, css_property, css_element FROM block_attributes WHERE css_property='<prop>'"` — then read the WORKING block's `render.php` + `style.css` and compare against yours. The answer is almost always already in the tree. Measured 2026-08-31: every "the atoms don't mesh" problem in the media layer resolved this way in minutes. `sgs/hero`'s split-media object-fit already worked; the difference was that hero scopes its selector PER ELEMENT (`.{uid} .sgs-hero__split-media--image`) while the new layer emitted one fixed custom-property name per block — so a two-element block silently rendered both slots with the second slot's value. The DB query also surfaced two blocks (`sgs/brand-strip` `logoFit`, `sgs/trust-bar` `badgeImageObjectFit`) that a hand-written survey of "media blocks" had missed entirely.
...
**Tools for this method (use the right one; don't hand-roll):** `/systematic-debugging` (root-cause gate), `/qc-council` (multi-rater validate fix-shapes from the FULL logs + code — converged-evidence beats single-model), `/verify-loop` (2-attestation), `/diagnostics` + `/lint` (static checks), `/subagent-driven-development` + `/dispatching-parallel-agents` (orchestrate independent work), `/brainstorming` (design before build), `/library-docs` (gold-standard reference), `/wp-blocks` + `/sgs-db` + `/sgs-wp-engine` + `/sgs-clone` (SGS ground truth — for a composite VARIANT's grid/structure, query `variant_slots` + `blocks.variant_attr` via `/sgs-db` BEFORE reasoning, never guess from attrs), the `wp-sgs-developer` agent (when relevant). Captured: `feedback_read_ground_truth_before_concluding`, `feedback_qc_council_cross_family_triangulation_finds_bugs`, `empty-section-false-pixel-diff-win`, `feedback_ground_in_variant_db_for_variant_block_setups`.
...
| [`.claude/specs/31-UNIVERSAL-CLONING-PIPELINE.md`](.claude/specs/31-UNIVERSAL-CLONING-PIPELINE.md) Appendix D | Stage map (the two hand-maintained mirrors `cloning-pipeline-flow.md`/`-stages.md` were archived 2026-07-28 for chronic drift — per-stage truth is the code) |
...
Before adding any hardcoded lookup dict in pipeline scripts, check sgs-framework.db. **Counts are DB-authoritative — query `/sgs-db`, never cache a number here (this line's D99-era snapshot drifted 6-of-9 by 2026-07-22).** The tables + what each is for: `property_suffixes` + its `kind_override` column (replaces `_KIND_BY_SUFFIX` dict per D99), `block_supports`, `modifier_suffixes`, `slots` (element + section scopes; replaces retired `slot_synonyms` + `legacy_role_lookup`), `roles` (base + `scalar-media` added D128; replaces `slot_synonyms.role_classification` per D99), `block_attributes` (+ its Front-1 declarative routing columns `css_property`/`css_element`/`css_state`/`css_tier` — seeded + load-bearing, see Spec 31 §3.A/§4), `block_capabilities`, **`blocks.variant_attr` column** (BUILT, FR-31-20; names the variant-selector attr per block), **`variant_slots` table** (BUILT, FR-31-20; stores each variant's DISCRIMINATING slots via set-difference; populated by `/sgs-update` from `supports.sgs.variants` in block.json), **`block_composition.container_kind` column** (BUILT + populated 2026-06-02, D152; values `section|layout|content`; the container roster has `wraps_block` + `container_kind` populated). Refactor to DB reads. ⚠ **There is no `plugins/sgs-blocks/scripts/db_lookup.py`** — this line implied one for months and a 2026-08-24 agent went looking. The only module of that name is `scripts/converter/db/db_lookup.py`, which runs six schema-migration functions against the shared live DB **as an import side effect** — do NOT import it from a read-only reporter. For a plain read, open `~/.claude/skills/sgs-wp-engine/sgs-framework.db` read-only (`sqlite3.connect(f'file:{db}?mode=ro', uri=True)`), the convention used by `audit-declared-vs-seeded-roles.py`, `generate-db-catalogue.py` and `audit-feature-parity.py`. Full mechanism: Spec 31 §13.5 + §13.6.
...
**Counts (blocks, block_attributes, slots, roles, patterns, etc.) drift in prose — the DB is authoritative; never hard-code a count here.** Query live via `/sgs-db` or `/wp-blocks`, or read `.claude/specs/02-SGS-BLOCKS-REFERENCE.md` (regenerated by `/sgs-update`). DB schema + per-table roles + the recent column/table additions (`blocks.tier`, `block_composition.container_kind`, `blocks.variant_attr`/`variant_slots`, `property_suffixes.kind_override`, `slots`/`roles`, **`preset_implications`** — the preset-selector implied-CSS map, auto-derived from each block's style.css via `supports.sgs.presetSelectors`, Spec 31 §13.4 FR-31-23) are catalogued in **Spec 31 §13 (data layer)** + `.claude/decisions.md`.

Stable facts: WP 7.1 compatible (canary upgraded 7.0.2 -> 7.1 on 2026-08-20); per-client theming via `theme-snapshot.json`; the modular `converter/` engine is THE ONLY converter (the frozen convert.py/converter_v2 tree was DELETED at D276, 2026-07-05 — no flag, no fallback); `/sgs-update`'s stage map is declared in its own module docstring — `plugins/sgs-blocks/scripts/sgs-update-v2.py`, with `--stage` validated by `choices=range(1, 14)` in the same file. **Read it there, never here — a cached line number has already drifted three times.** ⚠ This line drifted **three** times ("10-stage" while the pipeline was 11; "12-stage" once Stages 13-14 landed; then a 14-slot/13-function gap). **2026-08-10 (D555): the retired Stage 3 slot was DELETED outright and stages 4-14 renumbered down one — the pipeline is now a clean contiguous 1-13**, so a retired stage no longer taxes every reader. Anyone holding pre-2026-08-10 numbers must subtract one from any stage ≥ 3. A count in prose is a copy that rots; the script is the only source. (Note: "Stage 11.6" belongs to the CLONING pipeline's `sgs-clone-orchestrator.py` — a different pipeline, not a `/sgs-update` sub-stage.) Single-command deploy `plugins/sgs-blocks/scripts/build-deploy.py`.
