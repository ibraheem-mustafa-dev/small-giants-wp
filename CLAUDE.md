...
## Active focus (2026-06-03 — container/wrapper standardisation: WS-1 A1+A2 shipped, triage grounded)

**LATEST (2026-06-03, D159/D160): WS-1 A1+A2 SHIPPED (commit `2f86d9e6`)** — `sgs/container` gained a `contentWidth` capability (guarded `__inner` render wrapper: full-bleed box + capped centred content) + the converter now transfers each slug-None section's own max-width→widthMode (absent→full/alignfull, present→custom) + lifts the folded `__inner` max-width→contentWidth. Live-DOM verified @1440 (4 body sections 1200/inner-dropped → 1425 full-bleed/content 1040|960). **The #1-#8 page-triage was GROUNDED** (5 investigations + qc-council; the council corrected 3 of 5 guessed fixes) — full grounded spec in `.claude/plans/2026-06-03-cloning-fidelity-triage-and-composite-remodel.md` (the live cloning-fidelity plan) + D160. **LOCKED next sequence (Bean-directed, WS-4-FIRST):** WS-1c (complete sgs/container A3-A6) → **WS-4 wrapper mirror across ALL ~28 composites, KIND-scoped** (foundational — wrapper-dependent fixes overwrite if done first) → wrapper-dependent fixes (#4 product-card fill, #7); in parallel: #3 heading/label textAlign parity, #6 notice-banner, #4a grid-lift, #8 slider rebuild (built-but-buggy, uncommitted). Several #1-#8 are block-QUALITY gaps, not converter bugs.

**Earlier (2026-06-02, D152): the DB substrate (the scratch-handoff "Workstream A") SHIPPED (commit `0d746073`):** `block_composition.container_kind` column built + populated (28-block container roster, values `section|layout|content`); `sync-container-wrapping-blocks.py` rewritten to validated "wraps children" detection + 3-KIND model (report-only; the propagation WRITER is WS-4); trust-bar/modal block.json gained `containerKind:"section"`. (NB: "Workstream A" = the D150-scratch DB-substrate task, NOT plan-WS-1.) Phase 2 hybrid migration continues as the cloning pixel-diff thread; the standardisation programme is the primary architectural goal.
...
- **Canonical spec:** [Spec 22 — Universal Block-Equivalent Extraction](.claude/specs/22-UNIVERSAL-BLOCK-EQUIVALENT-EXTRACTION.md) — now extended with §FR-22-21 (universal wrapper-conversion procedure + 3-layer model OUTER/CONTENT-WIDTH/PER-GRID-ITEM + composite-mirror rule).
- **Standardisation programme plan:** [`.claude/plans/2026-06-02-container-wrapper-standardisation.md`](.claude/plans/2026-06-02-container-wrapper-standardisation.md) — 5 workstreams: **WS-1** sgs/container 3-layer completion (A1-A6), **WS-2** converter/router truth (B1-B5), **WS-3** de-cheat R-22-1 (C1-C8), **WS-4** composite wrapper mirror + auto-propagation across all ~28 composites (D1-D3), **WS-5** docs. **WS-1 A1+A2 SHIPPED (D159, `2f86d9e6`); WS-1c + WS-2/3/4/5 pending.** Live triage + WS-4 build spec now in `.claude/plans/2026-06-03-cloning-fidelity-triage-and-composite-remodel.md`.
- **Active Phase 2 plan (still live — cloning pixel-diff thread):** [`.claude/plans/2026-05-28-phase-2-hybrid-block-migration.md`](.claude/plans/2026-05-28-phase-2-hybrid-block-migration.md) — 4 streams; Streams B/C/D deferred. Phase 2 acceptance: per-section ≤5% × 3 viewports + Bean visual sign-off (R-22-13).
...
- **Latest cloning-thread work (2026-06-01, D145/D146):** `sgs/button` replaces `core/button` everywhere + `sgs/multi-button` grouping (D146 / commit `270cd995`, Spec 11 complete); `is-style-*` carry onto cloned blocks + tag-authoritative content-leaf routing (D145 / commit `b93a3b51`). Both pending cloning-thread ratification.
...
### Composite-mirror rule (R-22-9 extension, D152, locked 2026-06-02)
No composite block evades R-22-9. Every composite block with a built-in wrapper (e.g. sgs/hero, sgs/cta-section, sgs/trust-bar) MUST mirror `sgs/container`'s wrapper capabilities (padding, max-width, contentWidth, gap, background) rather than diverging. Capability propagation route: `block.json` `supports.sgs.containerKind` → `block_composition.container_kind` (populated by `/sgs-update`) → converter wrapper logic reads the column and applies the 3-layer model (OUTER/CONTENT-WIDTH/PER-GRID-ITEM). Missing capabilities = gap candidates to add to the composite, never converter workarounds. Canonical procedure: Spec 22 §FR-22-21. Memory: `feedback_no_composite_evades_universal_rule`, `feedback_pipeline_transfers_draft_css_not_converter_detection_hacks`.
