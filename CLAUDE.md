...
## Active focus (2026-06-02 onwards — container/wrapper standardisation programme)

**Container/wrapper CSS-transfer system deep-analysed → 5-workstream standardisation programme (D152). Workstream A SHIPPED (commit `0d746073`): `block_composition.container_kind` column built + populated (28-block container roster, values `section|layout|content`); `sync-container-wrapping-blocks.py` rewritten to validated "wraps children" detection + 3-KIND model; trust-bar/modal block.json gained `containerKind:"section"`; seed-composition-roles.py cleaned stale rows. Phase 2 hybrid migration (Stream A) continues as the cloning pixel-diff thread; the standardisation programme is the NEW primary architectural goal.**
...
- **Standardisation programme plan:** [`.claude/plans/2026-06-02-container-wrapper-standardisation.md`](.claude/plans/2026-06-02-container-wrapper-standardisation.md) — 5 workstreams (WS-1 audit, WS-2 sgs/container capability parity, WS-3 converter wrapper logic, WS-4 pixel-diff closure, WS-5 docs+QA). WS-1 SHIPPED. WS-2–5 pending.
