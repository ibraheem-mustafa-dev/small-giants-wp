...
Before adding any hardcoded lookup dict in pipeline scripts, check sgs-framework.db: `property_suffixes` (124) + `kind_override` column (17 populated, replaces `_KIND_BY_SUFFIX` dict per D99), `block_supports` (1160 active post-D100 prune), `modifier_suffixes` (19), `slots` (103 = 99 element + 4 section post-D111 2026-05-30; was 105 at D99 — section pruned 16→4, element grew 89→99, replaces retired `slot_synonyms` + `legacy_role_lookup`), `roles` (21 — 20 base + `scalar-media` added D128, replaces `slot_synonyms.role_classification` per D99), `block_attributes` (2,935 — live; query `/sgs-db`), `block_capabilities` (88), **`blocks.variant_attr` column** (BUILT, FR-22-20, Commits 1–5/6 SHIPPED 2026-06-01; names the variant-selector attr per block), **`variant_slots` table** (BUILT, FR-22-20; stores each variant's DISCRIMINATING slots via set-difference; populated by `/sgs-update` from `supports.sgs.variants` in block.json), **`block_composition.container_kind` column** (BUILT + populated 2026-06-02, D152; values `section|layout|content`; 31-block container roster has `wraps_block` + `container_kind` populated; NOT walker-read yet — standardisation WS-3). Refactor to `db_lookup.py` reads. Full mechanism: Spec 22 §FR-22-20 + §FR-22-21.
