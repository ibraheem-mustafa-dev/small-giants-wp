# site-header — visual-diff report (2026-08-06)
...
71 container-mirror attrs + `tagName` wired (enum narrowed to header|div) + `align-items` on the `.sgs-header-icons` descendant moved into `:where()`.

**Root tag:** `<header>` -> `<header>` (unchanged; enum + default corrected from the mirrored `section` so the wiring could not change it)
