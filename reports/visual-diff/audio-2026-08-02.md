...
# sgs/audio — focus-ring token repoint (D467 sweep)

## What changed

One line in `src/blocks/audio/style.css`, inside the existing `:focus-visible` rule. No
...
`curl https://sandybrown-nightingale-600381.hostingersite.com/wp-content/plugins/sgs-blocks/build/blocks/audio/style-index.css`:
...
= `#f5d050` on this client. `sgs/audio` was not present on the homepage at verification time
