# physics-canvas — visual-diff report (2026-08-06)
...
63 container-mirror attrs + `tagName` wired. The mirror added the `bgSvg*` family, which legitimately qualifies the block for the `draw` FX effect — both generated FX artefacts regenerated atomically in this same commit.

**Root tag:** `<div>` -> `<section>` (DELIBERATE — section-KIND composite, Bean's call)
...
```
url      : https://sandybrown-nightingale-600381.hostingersite.com/physics-canvas-canary/
selector : .wp-block-sgs-physics-canvas
