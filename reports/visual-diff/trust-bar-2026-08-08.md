---
doc_type: reference
title: "Visual-diff / LANDED report — sgs/trust-bar Settings/Styles inspector tab split"
block: sgs/trust-bar
date: 2026-08-08
wave: "Spec 35 Task 3 batch A — 01-tab-group backlog, behaviour→Settings / appearance→Styles"
verdict: PASS
first_paint_capture_passed: true
source_sha: e94ebf176aaa8118
---

# sgs/trust-bar — inspector tab split (EDITOR-ONLY change)

**Verdict: PASS**, on the specific ground this gate protects: **the frontend render surface did
not change.** `src/blocks/trust-bar/edit.js` is the ONLY file touched — `render.php`,
...
Now: **3 panel(s) in Settings, 12 in Styles**, using
...
Panel splitting: Style — mixed badgeStyle, a structural mode selector (behaviour), with badge size and label typography (appearance).
...
- **Frontend is provably unaffected** — no render-path file was edited. Verified with
  `git status` scoped to `src/blocks/trust-bar/`: `edit.js` is the only modified entry.
...
**Result: 12 of this block's OWN panels now appear in the Styles tab**, with the behaviour and
