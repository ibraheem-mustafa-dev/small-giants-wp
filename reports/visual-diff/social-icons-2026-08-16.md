# Visual diff — sgs/social-icons — 2026-08-16

verdict: PASS
intent_capture_passed: true
source_sha: fbc8a88f8b4bb1ea

## What changed

Migrated the block's own colour attributes (`iconColour` / `iconColourHover`) onto the shared `SgsColourPanel` as one row with two states, `linked: true`, replacing the scattered inline pickers; set `supports.color.background`/`text`/`gradients` to `false`. Note `colourMode='brand'` paints per-platform brand colours the client cannot set, so in that mode the row renders its hover state only rather than showing a dead resting control.
...
Corroborating: `git diff --cached -- src/blocks/social-icons/render.php` is **0 lines** — the only file
