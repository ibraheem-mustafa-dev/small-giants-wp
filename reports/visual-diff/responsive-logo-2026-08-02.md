...
# sgs/responsive-logo — focus-ring token repoint (D467 sweep)

## What changed

One line in `src/blocks/responsive-logo/style.css`, inside the existing `:focus-visible` rule. No
...
`curl https://sandybrown-nightingale-600381.hostingersite.com/wp-content/plugins/sgs-blocks/build/blocks/responsive-logo/style-index.css`:
...
= `#f5d050` on this client. `sgs/responsive-logo` was not present on the homepage at verification time
