---
doc_type: reference
title: "Visual-diff / LANDED report — sgs/option-picker Settings/Styles inspector tab split"
block: sgs/option-picker
date: 2026-08-08
wave: "Spec 35 Task 3 batch A — 01-tab-group backlog, behaviour→Settings / appearance→Styles"
verdict: PASS
first_paint_capture_passed: true
source_sha: 82c40faa1d5410aa
---

# sgs/option-picker — inspector tab split (EDITOR-ONLY change)

**Verdict: PASS**, on the specific ground this gate protects: **the frontend render surface did
not change.** `src/blocks/option-picker/edit.js` is the ONLY file touched — `render.php`,
...
Now: **4 panel(s) in Settings, 5 in Styles**, using
...
Panel splitting: Label — mixed the label text and visibility toggle (content) with typography, margin and colour (appearance). Split across both tabs, the Styles half still gated on showLabel.
...
- **Frontend is provably unaffected** — no render-path file was edited. Verified with
  `git status` scoped to `src/blocks/option-picker/`: `edit.js` is the only modified entry.
...
**Result: 5 of this block's OWN panels now appear in the Styles tab**, with the behaviour and
