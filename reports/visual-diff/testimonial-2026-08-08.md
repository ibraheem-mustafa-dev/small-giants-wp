---
doc_type: reference
title: "Visual-diff / LANDED report — sgs/testimonial Settings/Styles inspector tab split"
block: sgs/testimonial
date: 2026-08-08
wave: "Spec 35 Task 3 batch A — 01-tab-group backlog, behaviour→Settings / appearance→Styles"
verdict: PASS
first_paint_capture_passed: true
source_sha: a09e657dcc3b7045
---

# sgs/testimonial — inspector tab split (EDITOR-ONLY change)

**Verdict: PASS**, on the specific ground this gate protects: **the frontend render surface did
not change.** `src/blocks/testimonial/edit.js` is the ONLY file touched — `render.php`,
...
Now: **4 panel(s) in Settings, 4 in Styles**, using
...
Panel splitting: Rating — mixed visibility, rating type and score (behaviour) with rating colour and star size (appearance).
...
- **Frontend is provably unaffected** — no render-path file was edited. Verified with
  `git status` scoped to `src/blocks/testimonial/`: `edit.js` is the only modified entry.
...
**Result: 6 of this block's OWN panels now appear in the Styles tab**, with the behaviour and
