# Visual Diff Report — sgs/team-member (Wave 5 media-slot migration)
...
Indirect verification via Wave 6 deploy verify on sandybrown post 29 (which exercises hero — same migration pattern as team-member):
...
Low. Migration is additive: new attribute added, legacy retained, deprecation entry handles existing posts. Editor + frontend tested for hero (proof-of-pattern); team-member follows the same recipe.

## Per-instance verification needed

Live verification on a page that uses `sgs/team-member` is deferred to next session's UC3 smoke run on Mama's full homepage (Task 5 of `next-session-prompt.md`). Existing instances on production are protected by the back-compat path; new instances only appear when an operator opts in via the Media Picker.
