# nav-menu — visual-diff report (2026-08-06)
...
19 container-mirror attrs + `justify-content: space-between` moved into `:where()` so the new `justifyContent` attr can win.

**Root tag:** `<nav>` -> `<nav>` (unchanged)
...
selector : .sgs-site-header .sgs-nav-menu
