...
4 pass / 0 fail at rest. NOTE the rest-state guard stays DELETED (inheritance): a sweep that force-opens <details> measures 2.38:1, but that is the deliberate '.sgs-accordion-item[open] >' brand highlight. Re-adding a painted colour here would make the header invisible in a dark drawer - the icon-list defect. A DO-NOT-RESTORE comment records this.
