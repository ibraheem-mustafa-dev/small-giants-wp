# card-grid — visual-diff report (2026-08-06)
...
**Change:** Removed the abandoned `overlayStyle` attribute (no editor control, no consumer; cleared by a full-repo D338 grep).

**Why first paint is unaffected:** Nothing consumed it, so nothing rendered from it.
...
Rendered HTML post-deploy: **28** `sgs-card-grid` class hits, 109,653 bytes, **zero** `overlayStyle` occurrences.
