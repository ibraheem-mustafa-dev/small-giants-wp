# Visual diff — sgs/product-card — 2026-08-16

verdict: PASS
intent_capture_passed: true
source_sha: b9a2e441e6cca870

## What changed

Added block-level "Card text colour" + "Card background colour" rows to the existing `SgsColourPanel` (native `style.color.*` path, placed ahead of the title/description/price/tag/CTA/picker rows), and set `supports.color.background`/`text` to `false`. Verified the rows apply in BOTH typed and bound (WooCommerce/CPT) mode — `render.php` reads `style.color` unconditionally, before the mode branch splits.
...
Corroborating: `git diff --cached -- src/blocks/product-card/render.php` is **0 lines** — the only file
