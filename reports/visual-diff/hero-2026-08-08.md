---
doc_type: reference
title: "Visual-diff / LANDED report — sgs/hero Settings/Styles inspector tab split"
block: sgs/hero
date: 2026-08-08
wave: "Spec 35 Task 3 batch A — 01-tab-group backlog, behaviour→Settings / appearance→Styles"
verdict: PASS
first_paint_capture_passed: true
source_sha: 3875f2f981246d83
---

# sgs/hero — inspector tab split (EDITOR-ONLY change)

**Verdict: PASS**, on the specific ground this gate protects: **the frontend render surface did
not change.** `src/blocks/hero/edit.js` is the ONLY file touched — `render.php`,
...
Now: **5 panel(s) in Settings, 12 in Styles**, using
...
Panel splitting: Three panels split — Image, Background Video and SVG Background each mixed media SELECTION (behaviour) with overlay colour/opacity (appearance). The variant conditionals travelled with both halves.
...
- **Frontend is provably unaffected** — no render-path file was edited. Verified with
  `git status` scoped to `src/blocks/hero/`: `edit.js` is the only modified entry.
...
**Result: 14 of this block's OWN panels now appear in the Styles tab**, with the behaviour and
...
## Follow-up change, same day — element-first panel naming (CO-2)

A duplicate panel NAME was corrected after the live pass surfaced it. Still editor-only:
`edit.js` remains the sole modified file, so the frontend is unchanged and
`first_paint_capture_passed` stands. `source_sha` above was recomputed for this content.
