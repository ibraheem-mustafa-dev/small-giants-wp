---
doc_type: reference
title: "Visual-diff / LANDED report — sgs/button Settings/Styles inspector tab split"
block: sgs/button
date: 2026-08-08
wave: "Spec 35 Task 3 batch A — 01-tab-group backlog, behaviour→Settings / appearance→Styles"
verdict: PASS
first_paint_capture_passed: true
source_sha: 422ffaea043ad7ce
---

# sgs/button — inspector tab split (EDITOR-ONLY change)

**Verdict: PASS**, on the specific ground this gate protects: **the frontend render surface did
not change.** `src/blocks/button/edit.js` is the ONLY file touched — `render.php`,
...
Now: **2 panel(s) in Settings, 8 in Styles**, using
...
Panel splitting: Icon — WHICH icon and its placement (position, collapse-to-icon, accessible title) is behaviour; size, gap and colour are appearance. Two same-titled Icon panels, one per tab.
...
- **Frontend is provably unaffected** — no render-path file was edited. Verified with
  `git status` scoped to `src/blocks/button/`: `edit.js` is the only modified entry.
...
**Result: 7 of this block's OWN panels now appear in the Styles tab**, with the behaviour and
