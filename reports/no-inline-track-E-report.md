...
**Leftover cleanup (commit `ae33cca6`):** removed BOTH now-dead pack-pill CSS blocks (`.pill`/`.pill-group` + `.sgs-product-card__pill(-group/-option)`) — both typed + bound render the real `sgs/option-picker`, and the editor preview now uses that markup too, so those rules styled nothing. Re-pointed the card's `pill` typography control from the removed `.sgs-product-card__pill` selector to the live `.sgs-option-picker__pill` (kept it a live control). **This also drained the `7px 13px` F3 mis-tag** (it lived only in those dead rules) — so product-card's F3 is now just the ONE `4px 10px` tag-chip row (still mis-tagged → report-only). Integrator: remove the stale `7px 13px` baseline entry.

## Cloning-capability assessment (Bean asked — findings only, no pipeline change)
**Q1 — CAN the option-picker be styled to clone the draft's selected/unselected pills?** *Mostly.* The block exposes `--sgs-op-bg / -text / -border` (resting) + `--sgs-op-sel-bg / -sel-text` (selected) + `pillStyle/pillSize/radius`, so resting bg+border+text and selected bg+text are all controllable. **Two limits for an EXACT clone of the mamas draft** (draft: cream bg + beige border resting; pink-*outline* + pale-pink-*tint* fill + dark text, NO tick selected): (a) the `outlined` variant couples selected `border-color` to `--sgs-op-sel-bg`, so "pink border + tint fill" (decoupled) isn't reachable via vars alone; (b) the picker always renders a selection tick (`::before`) the draft has none of. Full fidelity needs a small option-picker enhancement (a new style variant OR decoupled selected-border + optional-tick).
**Q2 — is the pipeline set up to clone this element?** *Structurally YES, styling NO.* `sgs/option-picker` is a DB-recognised standalone block (`slots` + aliases pill-group/pills/pack-group/…), and the card's pack pills clone as `packSizes` (labels + selected index) then render via `render_block('sgs/option-picker')`. **But the pipeline does NOT clone the draft's pill STYLING:** `packSizes` captures label+selected only (no colours); the card forwards no pill colours into `render_block()` and owns no pill-colour attrs; even a standalone picker's `pillBgColour`/`pillSelectedBgColour`/`pillSelectedTextColour` are non-liftable (role/selector NULL in DB, gated out at `styling_content.py:135-139`); the card hardcodes only `--sgs-op-border: primary`. So a clone renders SGS-brand pink pills regardless of the source draft. Making it draft-faithful = a design-gated converter+DB+block enhancement (route draft pill CSS → new card/picker style attrs + decouple selected border/tick), OUT of Track E's files-only scope.
