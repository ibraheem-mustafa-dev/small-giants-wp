...
## 2026-07-12 (D311 close) — per-block `<style>`-tag consolidation (Bean-flagged during Fix 9)

> **P-STYLE-TAG-CONSOLIDATION** — NEW 2026-07-12 (Bean surfaced it after the Fix 9 register came back "SGS blocks 100% no-inline compliant"). Bean's sharper question: the per-block scoped `<style>` TAGS the no-inline migration emits. **Live facts (page 8):** 144 `<style>` tags total — 37 in `<head>`, **107 in the body, 83 of them SGS block-scoped** (`.sgs-hero-XXX`, `.sgs-lbl-XXX.wp-block-sgs-label`, `.sgs-btn-…`, `.sgs-pc-N …`, one per block instance) — **~33KB of CSS embedded in the HTML body across ~100 tags.** **Classification:** a `<style>` TAG is NOT an inline `style="…"` ATTRIBUTE — Spec 32 §6.1(b) explicitly sanctions "the block's own scoped `<style>`" as THE no-inline mechanism (matches WP core's `layout`-support output), so by the contract-as-written these are COMPLIANT. BUT ~100 embedded `<style>` tags / 33KB is genuine bloat + non-cacheable (re-downloads every page) — a legitimate architecture concern, not a Spec-32 violation. **The WP-native fix:** the WordPress Style Engine "store" (`wp_style_engine_get_styles($css, ['context' => …])` + `wp_enqueue_stored_styles()`) collects every block's rules into ONE footer `<style>` block instead of ~100 scattered ones — how core avoids the scatter; SGS currently uses the per-block `['selector' => …]` echo mode (`SGS_Container_Wrapper`, `sgs_label_box_css_rule`, `sgs_typography_css_rule`, each block's render.php). **Switching is FRAMEWORK-WIDE + design-gated** (every block's render.php registers into the store rather than echoing its own `<style>`) → RESEARCH the WP store pattern + how Kadence/Spectra/GenerateBlocks handle it, then a design + `/qc-council` BEFORE any build. Register: `.claude/reports/2026-07-12-fix9-inline-styles-register.md` (ADDENDUM). Distinct from P-PATTERNS-USE-CORE-BLOCKS (that = core blocks in patterns leaking `style="…"` ATTRIBUTES; this = SGS blocks' own scoped `<style>` TAG count). **Status: OPEN** · **Bucket:** Framework / blocks · **Trigger:** a fresh session (Bean parked it 2026-07-12 to bank the Fix 4 win).
