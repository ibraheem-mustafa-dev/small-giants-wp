...
last_updated: 2026-06-02 (pruned — 21 archived entries moved to memory/parking-archive.md)
---
...
## 2026-06-02 — cloning thread: container roster + DB-table + save-null

> **P-CONTAINER-KIND-ROSTER** — NEW 2026-06-02 (D150). **Status: OPEN** (pipeline/converter). Workstream A — the validated block_composition table fix is SPEC'D but NOT built. Rewrite `plugins/sgs-blocks/scripts/sync-container-wrapping-blocks.py` detection to the DB-derived "wraps children" criterion + add a `container_kind` column + add `supports.sgs.containerKind:"section"` to trust-bar/modal block.json + fix composition_role leaf→content-block for post-grid/gallery/card-grid (via seed-composition-roles.py) + populate wraps_block+container_kind on the 28-block roster + extend GRID_ONLY_ATTRS → a KIND→attr-scope map. Start from `/tmp/d6_classify2.py` (the working classifier that produced the 28). Walker-safe (wraps_block/container_kind not walker-read; the 3 role flips have zero canary impact — Rater C). Single Sonnet subagent → /qc → STOP #18 sanity-check roster with Bean before --apply. Full spec: `.claude/scratch/2026-06-02-container-roster-db-table-handoff.md`.

> **P-TRUSTBAR-BOUND-GRID** — NEW 2026-06-02. **Status: OPEN** (pipeline/converter). trust-bar gap-4: in bound mode the converter-emitted `sgs/container.sgs-trust-bar__inner` grid renders 584px wide / uneven auto columns (143/132/111/160) instead of 1100 / 4×266. Root cause (Bean's diagnosis, confirmed): the grid lives on the block's own `.sgs-trust-bar--icon-circle` class (trust-bar/style.css); bound mode echoes `$content` (the emitted `__inner` container relying on draft CSS) → the block's own grid CSS + the draft's `__inner` grid CSS collide / the wrong one wins. Composite-block CSS conflict, NOT the slug-None fold. Separate diagnosis from the contentWidth work. Measure live DOM (R-22-11).

> **P-CSS-TRANSFER-CONTENTWIDTH** — NEW 2026-06-02 (extends D136 gaps 1+2). **Status: OPEN** (pipeline/converter). The faithful-transfer fix for the 4 slug-None sections (featured-product/ingredients/gift/social-proof): gap 1 = sections clamped to wideSize 1200, should be full-bleed (theme-CSS-by-position OR widthMode:full on the slug-None path); gap 2 = the FR-22-4.1 fold drops `__inner`'s max-width. Option B (Bean-locked): add a `contentWidth` capability to sgs/container that constrains its CHILDREN to a centred max-width while the box stays full-bleed (mirrors WP is-layout-constrained); the fold lifts `__inner`'s max-width into it. Designed + qc-council-validated + @1440 baseline measured (draft localhost :8137 vs page 144). Build → deploy → re-measure live DOM → /qc. Full fix-shape + baseline table: `.claude/scratch/2026-06-04-css-transfer-gaps-1-2-fix-shape.md`. Gap 3 (hero gradient) + gap 4 (=P-TRUSTBAR-BOUND-GRID) are separate.

## 2026-06-02 — theme thread wave 1–3 deferred follow-ups

> **P-PRODUCT-CARD-PHASE-DE** — UPDATED 2026-06-02. **Status: OPEN** (framework). Product-card Phase A (option-picker) + Phase B (variation-sets meta + panel + custom-fields fix) + **Phase C (Bound mode — wrapper+bridge over WooCommerce + add-to-cart) SHIPPED + live-verified + committed+pushed `6bcdf48c`, D151.** Remaining: **Phase D** = clone-emit (converter outputs `sgs/option-picker` for a pill group — TRUTH-SPEC + slot_synonyms/slots + converter, per D144.4). **Phase E** = collection/query block (Spec 24 FR-24-4/5). **Plus a Phase-2 data-model task:** Phase C wired the pill→price/image swap but it is DORMANT — `_sgs_variation_sets` stores only `{key,label}` per option (no per-option price/image), so the swap has no data to act on. The SKU-matrix / per-option-value data model (Spec 24 FR-24-14, `_sgs_sku_matrix` registered but Phase-2-deferred) must land to activate the visible swap. Full design: Spec 24 §FR-24 + D144/D149/D151.
...
> **P-CSS-TRANSFER-FIDELITY** — NEW 2026-06-02 (D136). A draft-vs-clone computed-style audit proved the cloning pipeline does NOT faithfully transfer the draft's CSS — the framework IMPOSES values the draft never had + DROPS draft structure. **4 gaps:** (1) imposed `max-width:1200` on full-bleed sections — fix = theme-CSS `.entry-content > .wp-block-sgs-container{max-width:none}` (full-bleed by position); (2) the FR-22-4.1 fold DROPS the `__inner` content wrappers (max-width:960) — fix = preserve the two-level structure; (3) hero gradient overlay (`#C56A7A→#E68A95`) imposed over the draft's solid pink; (4) `grid-template-columns` not transferred (trust-bar 266×4 → uneven; brand 122/782 → 450/450). The principle (Bean, locked): faithful CSS transfer is the pipeline's whole point; converter detect-mode band-aids are the wrong layer (2 rejected). Static-div editor bug already fixed (committed). The `widthMode:'full'` slug-resolved band-aid (e27ff591) is PARTIAL — superseded by the faithful-transfer fix. Full detail: decisions D136 + `.claude/next-session-prompt.md`.
> **Status:** OPEN
