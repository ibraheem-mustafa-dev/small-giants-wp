...
> **P-SPEC37-PER-SITE-DECLIENT** — NEW 2026-07-22. FR-37-6's file step shipped (`parts/header.html` is now a 1-line shell), but "both sites render from CPTs" is NOT met and **client data still lives in framework theme files** — `theme/sgs-theme/patterns/footer-indus-foods.php` leaks "Indus Foods Footer" + a hardcoded Google Place CID to every install (Spec 37 §3.9 / §3.9a). Work: (a) move/delete the client-named framework pattern; (b) author each live site's header + footer as CPT posts, set active, verify live (the D359 flow). The CPT is the per-site store now; the JSON snapshot handles tokens. **Status: OPEN** · **Bucket:** Framework · **Trigger:** next session (Task 1). Blocks FR-37-6 full closure + the Indus deploy.

> **P-NAV-INDUS-CUTOVER** — NEW 2026-07-22 (carried from FR-36-18). Re-author the Indus (palestine-lives.org) header onto `sgs/nav-menu` + `sgs/nav-drawer`. A complete step-by-step recon plan exists (2026-07-22, in the `memory/` session notes): live source is the theme FILE not a DB record; menu is `wp_navigation` post 100 (mega-menu Sectors/Brands); attribute traps — `collapsePoint` defaults 768 vs adaptive-nav's 1024 (must set 1024), `overflowBehaviour` has NO `sgs/nav-menu` equivalent (7 items may wrap at 1024–1280 — catch in the overflow gate), `ref:100` must be explicit on both nav-menu instances. ⛔ Do NOT delete `sgs/adaptive-nav` until green (rollback). ⛔ A plain `--target palestine-lives` deploy would push Mama's header onto Indus (shared file) — de-client first. **Status: OPEN** · **Bucket:** Framework · **Trigger:** after P-SPEC37-PER-SITE-DECLIENT.
