...
> **P-TESTIMONIAL-CONVERTER-FR2220** — NEW 2026-06-11 (D206). The testimonial D8 rebuild (typed-attr, 7 variants, on `feat/block-quality-mirror`) is converter-READY (`blocks.variant_attr` + `variant_slots` map + the +27 typed attrs registered via `/sgs-update`) but the cloning converter does NOT yet (a) route the draft `__quote`/`__author`/`__stars`/`__summary`/`__org`/`__logo`/`__work`/`__date`/etc. onto the typed attrs, nor (b) auto-detect the variant (FR-22-20 — testimonial = the 2nd variant block onboarded after hero; the generalisation past hero is still pending). The block is fully functional standalone; this is the cloning-thread wiring needed to clone a testimonial section faithfully. **Status: READY-TO-BUILD** (was DEFERRED) · **Bucket:** Pipeline / converter · **Trigger:** cloning Stage-2 routing wave.
> > **UPDATE 2026-06-11 (D212) — ROOT-CAUSED + design-gated + build plan written.** Block-quality is now MERGED to main (D209). The empty-slide symptom on live page 8 is caused by `block_composition.has_inner_blocks=1` being STALE for `sgs/testimonial` (hardcoded in `seed-composition-roles.py:60`; the D8 rebuild registered the typed attrs/variants via `/sgs-update` but never flipped this flag) → the converter descends + emits `sgs/star-rating`+2×`sgs/text` children → the typed render.php ignores them → empty. **The wiring IS the existing UNIVERSAL DB-driven lift** (Spec 22 §FR-22-2 + Spec 00 §3.1 — `equivalent_block_for` + scalar-lift via `block_attributes.role/canonical_slot/derived_selector` + `slots`): testimonial's `quote`/`reviewerName`/`ratingStars` simply lack the rows `sgs/text.text`/`sgs/quote.body` already carry. **NO bespoke handler, NO new table** (FR-22-2.3). `/adversarial-council` (6 personas) gated the design; its security (esc_html names + clamp stars), Rule-4 (card border/bg dropped), flag-child-only, and variant-safety must-fixes are folded into the 7-step plan: **`.claude/plans/2026-06-11-testimonial-universal-lift-build.md`** (delegated to a fresh focused session). Closes ledger SP-C/D/E when live-verified.
...
> **P-BATCH-GA-14-SKILLS** — from archived `2026-05-24-strategic-plan.md` Phase 4 (full detail in the archived plan + `plans/archive/2026-05-24-phase-4-skill-optimisation.md`). **Status: OPEN** — `/skill-optimiser` mode-2 + `/batch-gap-analysis` over the 14 WP/SGS skills; deliverable = 14 per-skill JSON evaluations + 1 review report + S-grade confirmations queued; runs in main conversation, no subagent dispatch (blub.db 176). KJC: all 14 inline at once OR ~6 critical first then iterate — Bean decides. **Bucket:** Tooling / lifecycle.
...
## 2026-06-11 — Spec 30 P1 open follow-ups (theme thread)

> **P-WC-GALLERY-VARIATION-SWAP** — NEW 2026-06-11 (Spec 30 P1 close). **Status: OPEN** (framework). At P1 the PDP gallery uses the stable classic `woocommerce/product-image-gallery` (the Beta `woocommerce/product-gallery` rendered an empty large-image shell on WC 10.8.1). The classic gallery does NOT swap the main image per selected variation — the Beta block's variation-awareness was tied to the core add-to-cart form that the `sgs/buybox` bridge replaces. Multi-image gallery itself works (thumbnails + click-swap + zoom, live-verified product 540). **Decision needed at P2:** either (a) drive `selectedImageId` into the Beta gallery's Interactivity context from the buybox bridge (no clean external API; version-fragile), or (b) accept static-per-variation gallery + rely on the price/stock swap (current). Probe findings: `.claude/reports/spec30-p1/STEP5-BRIDGE-DESIGN.md` + the gallery probe in agent report. **Bucket:** Framework / shop layer.

> **P-WC-NOTIFY-ME-CAPTURE** — NEW 2026-06-11 (Spec 30 P1 close, FR-30-7). **Status: DEFERRED** (framework). The buybox "notify me when back in stock" email capture was shipped DEFERRED — no PECR-compliant capture path exists yet (needs `sanitize_email` + `is_email` + nonce + ≤3/IP/hour rate-limit + un-pre-ticked consent checkbox + privacy-policy link, email+product_id+timestamp only, data stays in WP). The `notifyMeLabel` inspector control ships now (operators can pre-configure the label) but renders nothing until the capture path lands; it sits in `scripts/dead-controls-baseline.json` with a remove-on-ship note. Build the guarded capture endpoint + UI as its own unit before un-deferring. **Bucket:** Framework / shop layer.
