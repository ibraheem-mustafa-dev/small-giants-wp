...
Every `sgs/container` on the `/motion-canary-*` fixture pages is `isValid: false` in the block
editor — 7 of 21 blocks on page 2024. WP reports *"Content generated by `save` function: (empty)"*
vs *"Content retrieved from post body: `<div class="wp-block-sgs-container"></div>`"*. The block's
`save.js` returns `<InnerBlocks.Content />` (no wrapper div), so the stored markup was written by
something that emitted a wrapper the block never produces — i.e. the fixtures were generated
programmatically with hand-built markup rather than through the editor.

**Frontend is unaffected** and was measured green (render.php drives output; the travel probe
passes against these very pages). It is the EDITOR that is broken: an invalid block renders no
inspector at all, so a client cannot open or change these sections, and any control added to
`sgs/container` is unreachable on them.

Pre-existing, not caused by the D416 work — `container/save.js` last changed at `e1459e6d`, and
none of that session's commits touched a container file. Verified before the finding was recorded.

**Why it matters beyond the fixtures:** if the generator that produced these pages is the same one
used for any other programmatic page build, every page it writes has the same defect. Worth
checking what wrote them before assuming it is contained.

**Trigger:** Before the canary pages are used for any editor-side verification, or before that
page generator is reused.

### P-FX-PANEL-UNGUARDED-BY-EVERY-CONTROL-GATE — the fx inspector panel is linted by nothing
**Status:** OPEN · **Bucket:** tooling · **Parked:** 2026-07-30

`check-dead-controls.js:514`, `check-control-ux.js:455` and `audit-inspector-conformance.js:270`
all exclude `src/blocks/extensions/` or enumerate blocks by `block.slug` and so never reach it.
The fx panel has therefore never been linted by any of them, which is why `fxTrigger` sat
registered-and-rendered-by-nothing without any gate noticing.

Spec 38 §7:472-473 claims the conformance gate "covers every new panel automatically". For this
panel that is **false** — the two controls added at D416 ship unguarded.
...
`--self-test` that injects a dead control and proves the guard fails.
