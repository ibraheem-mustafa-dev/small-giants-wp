...
## 2026-07-06 (blocks) — two adjacent dead-schema findings from the container-equivalent de-dupe audit

> **P-CONTAINER-DEAD-SCHEMA** — NEW D281. Surfaced by the 30-block container-equivalent dupe audit (2026-07-06, which confirmed hero `splitGap` was the ONLY true attr dupe on the roster — now fixed `2f4a1e4a`). Two separate dead-schema issues found, NOT dupes, worth a cleanup pass: (1) **`customWidth`/`customWidthUnit` orphan** — declared in `block.json` for 25+ roster composites (hero, trust-bar, cta-section, quote, card-grid, …) but has ZERO consumers (no edit.js control, no render.php read; only the unrelated button/text/heading/form blocks use `customWidth` for a real own-width concept). Not on `sgs/container` itself. A pre-v0.4 `widthMode`-era leftover copy-pasted everywhere — dead schema bloat, safe to strip from the composites. (2) **`accordion` + `product-faq` declare `supports.spacing.blockGap:true` alongside custom `gap`/`gapTablet`/`gapMobile`** but neither declares `supports.layout` (which WP-native blockGap needs to paint) — likely an HC2-style dead native control writing an unconsumed `--wp--style--block-gap`. NOT render-traced to full depth (flag, not asserted). **Status: OPEN** · **Bucket:** Blocks / dead schema · **Trigger:** next block-cleanup pass. Also: the audit was time-boxed — only block.json was grep-swept for all 30; edit.js/render.php read in full for hero, spot-checked elsewhere. A differently-NAMED bespoke dupe inside another block's edit.js could exist un-caught — a follow-up 100%-confidence pass is optional.
