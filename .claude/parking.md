...
> **P-CONFORMANCE-GOLDEN-DRIFT** — NEW 2026-07-26 (D383). `scripts/tests/test_converter_conformance.py` (Gate A) has 27 golden-mismatch failures across ~13 blocks (form/hero/info-box/modal/multi-button/notice-banner/option-picker/tab/team-member/testimonial/trust-bar/…). PROVEN pre-existing + unrelated to A1/A2: identical failure count with `box_family` seeded ON and OFF. The goldens are stale (converter evolved, goldens never re-seeded). The test's OWN docstring forbids a blind re-seed — a re-baseline is only valid "after a fresh landed deploy proof" per block, because a blind `seed_conformance_goldens.py` bakes whatever real regression is hiding in as "correct". Overlaps the existing `P-ORACLE` stale-golden reseed (4 fossil goldens from D381) — same discipline. Safe close = verify each divergence is intended (per-block deploy proof), THEN re-seed. The structural harness failure (missing-fixture count) was already fixed this session (`68a70260`). **Status: OPEN** · **Bucket:** tooling · **Trigger:** the Spec-31 completion track (Track 1c) reaches the phase-f-canary-deploy + `check_landed()` wiring — golden re-baseline lands with it (they share the "needs a landed deploy proof" gate).

## 2026-07-24 (D378) — mega-menu foundation re-scoped to a core; follow-on items deferred

> **P-MEGA-FOLLOWON-DEFERRALS** — NEW 2026-07-24 (D378). The mega-menu was re-scoped after a 7-persona adversarial council to ship a smallest CORE first (`general`/`columns`, light-only, caret-only). These items are DECLARED in the BUILD-SPEC §0.5 and DROP onto the proven core spine in a follow-on — nothing cut, sequenced. **Deferred:** (1) the 5 motion effects (staggered reveal, sliding indicator, cursor spotlight, magnet label, card hover-lift — the caret flip ships in the core); (2) the `media-cards` + `brands` structural variations; (3) the night/day `dark` colour set + its 4-branch selector cascade (the `colourScheme` attr + light-safe rule ship in the core); (4) the aside `feature` + `preview` formats (core ships a static `cta` aside); (5) full manifest GAP/ORPHAN-0 conformance (core declares an ADVISORY manifest); (6) the TRUE safe-triangle hover path (core ships the 170ms bridge, CF-13 — record against FR-36-4). Also follow-on: the D294 drift-guard computed-CSS golden-parity test for the standalone `sgs/mega-panel`; and the competitive gaps the council named (conditional/role menus, WooCommerce category mega, RTL, import/export — all Spec 36 §7 Phase 3). **Status: DEFERRED** · **Bucket:** framework · **Trigger:** the core lands + passes its live-a11y QC (Gate 3), then build the follow-on onto the spine. Canonical: BUILD-SPEC §0.5.A + §0.6.

## 2026-07-25 (D379) — mega editor-canvas preset preview not applying; Gate-3 live checks owed

> **P-MEGA-EDITOR-PRESET-PREVIEW** — NEW 2026-07-25 (D379). Switching the mega-panel Style preset (Columns/Cards/Minimal) IN THE EDITOR shows NO visual change, though the FRONTEND renders each preset correctly (render.php per-uid `<style>`). **Root cause PROVEN via live iframe evals (Bean-directed live test):** WP 7.0's iframed editor canvas is NOT applying this block's `editor.css` — only `style.css` reaches the canvas (the base panel styling I saw came from style.css; injecting a preset rule INTO the canvas made it apply). **Proven fix (NOT yet landed):** move the mega-panel preset LAYOUT rules from `editor.css` → `style.css`, generic selectors, `.sgs-mega-panel__content > *` for wrapper-aware flex sizing (WP wraps each InnerBlock in `.block-editor-block-list__block`), **NO `!important`** (a no-`!important` rule beats WP's block-list layout by specificity — 0,3,0 > 0,1,0 — confirmed applying at `498.5px 498.5px`; `!important` in style.css would break the frontend `@media`/`@container` mobile-stack). Also: fix the render.php mobile stack to collapse the CARDS grid to 1 column on mobile; and revert the now-inert `!important` added to `editor.css` (`e5f70680`). A block `?ver` cache-bust (or the 0.1.1 bump already made) is needed for the CSS to propagate. **Status: OPEN** · **Bucket:** Framework · **Trigger:** next session Task 1 (~20 min — diagnosis done). Canonical: LEDGER "Your next session" + D379.
...
<!-- P-SPEC37-CORE-BUILD, P-FOOTER-COLUMNS-DISCARDED-ATTRS, P-TEMPLATELOCK-REORDER-GAP — RESOLVED 2026-07-22 (D359), moved to memory/parking-archive.md. -->

> **P-SPEC37-PER-SITE-DECLIENT** — NEW 2026-07-22. FR-37-6's file step shipped (`parts/header.html` is now a 1-line shell), The framework de-client is now COMPLETE — `theme/sgs-theme/patterns/footer-indus-foods.php` (which leaked "Indus Foods Footer" + a hardcoded Google Place CID to every install) was DELETED 2026-07-22 (`94ab240f`), and the repo carries no client data (Spec 37 §3.9 / §3.9a). Work: ~~(a) move/delete the client-named framework pattern~~ **DONE 2026-07-22 (`94ab240f`)**; (b) author each live site's header + footer as CPT posts — **mechanism PROVEN on BOTH sites 2026-07-22** (generic proof CPTs active: sandybrown #1570/#1571, palestine-lives #360), but the REAL branded per-site content is deferred to the Spec 33 Part 2 cloning pipeline. The CPT is the per-site store now; the JSON snapshot handles tokens. **Status: PARTIAL** · **Bucket:** Framework · **Trigger:** next session (Task 1). Blocks FR-37-6 full closure + the Indus deploy.

> **P-NAV-INDUS-CUTOVER** — NEW 2026-07-22 (carried from FR-36-18). Re-author the Indus (palestine-lives.org) header onto `sgs/nav-menu` + `sgs/nav-drawer`. A complete step-by-step recon plan exists (2026-07-22, in the `memory/` session notes): live source is the theme FILE not a DB record; menu is `wp_navigation` post 100 (mega-menu Sectors/Brands); attribute traps — `collapsePoint` defaults 768 vs adaptive-nav's 1024 (must set 1024), `overflowBehaviour` has NO `sgs/nav-menu` equivalent (7 items may wrap at 1024–1280 — catch in the overflow gate), `ref:100` must be explicit on both nav-menu instances. **⚠ SUPERSEDED IN PART 2026-07-22 (D361/D362):** the cutover MECHANISM is PROVEN on palestine-lives (generic proof header #360 on `sgs/nav-menu` ref:3 + `sgs/nav-drawer`, all gates green), the de-client is DONE, and `sgs/adaptive-nav` is now DELETED — so the "do NOT delete until green (rollback)" clause no longer applies (rollback = git history). What REMAINS is the faithful BRANDED Indus header, which is a Spec 33 Part 2 cloning job, not a manual re-author. **Status: PARTIAL** · **Bucket:** Framework · **Trigger:** after P-SPEC37-PER-SITE-DECLIENT.

> **P-SPEC37-S3-CARRIED** — NEW 2026-07-22 (D359, from the FR-37-9/10 §3 audit). Three §3 clauses the audit failed, carried as FR-37-33/34/35 (one mechanism each covers both row blocks): **FR-37-33** `layoutMode` is implicit in the `layout` string, not a first-class inspector control; **FR-37-34** the row inserter does not promote the common elements (logo/nav/search/cart/…) — §3.5's "steering" is unbuilt; **FR-37-35** no `@container` row reflow, only viewport `flex-wrap`. None is a blocker to "usable"; all are real feature work. **Status: OPEN** · **Bucket:** Framework · **Trigger:** Task 3, or when §3 conformance is prioritised.

> **P-SPEC37-OPEN-RESIDUALS** — NEW 2026-07-21, PARTIAL 2026-07-22. Carried from the Spec 37 coverage matrix + council. Still open: (a) **FR-S1-4 skip-link regression contract** (D157) has no successor — restate it in the FR-37-31 retirement. (b) **The 3 layout starter variants** fold into FR-37-8. (c) **FR-S5-3's non-carried WP-CLI commands** — FR-37-30's "reduced set" doesn't say what happens to the rest. (d) **FR-37-12 width set** dropped the **320–374 band** — add 320. (e) Spec 17's prose-only content (REST capability-gating detail; the §S9 "attribute shape FROZEN" guardrail FR-37-14 overrides — say so explicitly). **Status: PARTIAL** (FR-37-6 gut done) · **Bucket:** Framework · **Trigger:** with the FR-37-31 retirement work.

## 2026-07-21 — sgs/container content band-width "custom" control is broken in the editor


## 2026-07-21 (Spec 35 Track 1) — css_property is LIVE-CONVERTER-READ; populating it activated a dormant path

> **P-CSSPROP-RUNTIME-RESOLVER-UNDER-KEYED** — NEW 2026-07-21. **The single most load-bearing open item from this session.** `block_attributes.css_property` is not inert metadata: the live converter reads it at `plugins/sgs-blocks/scripts/converter/db/db_lookup.py:648` — *"`css_property IS NULL` means 'not corrected' — it never enters this path"*. It was NULL on every row, so that path had **never once fired**. This session populated **516 rows**, which switches it on. **But the resolver is `attr_for_property(block_slug, css_property)` — TWO arguments.** It cannot use the new `css_element` / `css_state` / `css_tier` columns even where they are populated. Under that 2-arg key, **103 groups covering 312 attrs are ambiguous, and the resolver picks the first by rowid order.** Measured: `SELECT COUNT(*) FROM (SELECT block_slug, css_property FROM block_attributes WHERE css_property IS NOT NULL GROUP BY block_slug, css_property HAVING COUNT(*)>1)`. **This reframes last session's decision to withhold the data** — withholding was the only thing preventing half-keyed rows reaching the converter; "we withheld the data" and "the gate failed" were the same fact. Ambiguity is dominated by **colour and typography, NOT container/layout** (measured: color 25 groups, background-color 15, font-size 11, border-color 8, padding only 2) — the pattern is `titleColour`/`labelColour`/`linkColour` all driving `color` on DIFFERENT elements, so **element is the discriminator that matters most**; the L1–L4 cascade/layer axis Bean identified is real but a small tail. **Work:** widen `attr_for_property` to take element/state/tier, with a documented tie-break for anything widening cannot separate. **Spec 31 surface with cloning blast radius — needs a design gate, deploy, and live-homepage verification (R-31-11/R-31-13); explicitly NOT a tail-end edit.** **UPDATE 2026-07-22 (Front 1, `7a6a7586` — code+unit verified, NOT live-clone-verified):** the resolver widening SHIPPED — `_base_domain_attrs_for_css_property` (`db_lookup.py:708-773`) now keys on element/state/tier (`css_element`/`css_state`/`css_tier` seeded + load-bearing) and FAILS LOUD (`AmbiguousCssPropAttrError`) on a genuine tie instead of rowid-first. converter suite 449 pass. **Residual keeping this OPEN:** the trigger below (does the keyed data actually IMPROVE cloning fidelity) is a live-homepage claim (R-31-11/R-31-13) and clone-verify was deferred this session — so the mechanism is shipped but the fidelity-improvement claim is unverified. **Status: OPEN (fix shipped, verify live)** · **Bucket:** Pipeline · **Trigger:** a dedicated converter session; gates any claim that the keyed data improves cloning.

> **P-VARIANT-DISCRIMINATORS-MUST-BE-STRUCTURAL** — NEW 2026-07-21. **Design-gated and Bean-approved 2026-07-21; ready to build, deliberately deferred.** `sgs/trust-bar`'s `icon-circle` variant is discriminated by FIVE **styling** attrs (`iconCircleSize`, `iconCircleBackground`, `iconColour`, `iconCircleBorderRadius`, `iconCircleShadow`) while `text-only` and `image-badge` have **zero** discriminators. Styling attrs are exactly what the CSS lift populates, so once `css_property` exists they become lift-producible and `detect_variant` can mis-score — every trust-bar risks reading as `icon-circle`. Surfaced as 5 NEW Check-#3 findings (gate was clean at session start; **these are caused by this session's data, NOT pre-existing** — a subagent claimed pre-existing and was wrong). **Note `css_element` CANNOT fix these:** an attr is lift-producible merely by HAVING a `css_property` (`lift_producible_attrs` = union of all derived attrs), so this is lift-producibility, not ambiguity.
>
> **THE FIX, concrete and already evidenced — discriminate on BEM STRUCTURE, per R-31-2 ("BEM is the only recognition signal"), which the current styling-attr model sits outside.** The lift can fabricate attribute VALUES; it cannot fabricate an ELEMENT absent from the draft. `trust-bar/render.php` already emits structurally distinct markup per variant: **`image-badge`** → `<img class="sgs-trust-bar__badge-img">` (`:344-357`); **`text-only`** → `.sgs-trust-bar__badge-label` only, no image and no icon (`:328-330`); **`icon-circle`** → `.sgs-trust-bar__badge` + icon `<span>` + `.sgs-trust-bar__label` (`:320`, note `__label` NOT `__badge-label`). Bean's framing: *"shouldn't trust-bar be able to tell between image-badge and text-only because text-only has no image?"* — yes, and that is why the two currently have no discriminators at all: what separates them is structural, and the old model had no way to express "this variant has an image". **Scope is UNIVERSAL (R-31-9), not trust-bar-only** — styling attrs are unsafe discriminators for EVERY block with variants; audit all `variant_slots` rows, not just this one. **UPDATE 2026-07-22 (Front 2, `ccfb7967` — code+unit verified, NOT live-clone-verified):** SHIPPED — trust-bar's `image-badge` got 4 genuine structural image controls (`badgeImageBorderRadius/Size/Shadow/ObjectFit`) doubling as its recogniser, AND the F6 gate was rewritten from "no lift-producible discriminator" to a **universal ambiguity rule** (2+ variants sharing an identical/empty signature = violation; one zero-signature fallback allowed) — the universal mechanism the entry demanded. F6 0 violations, negative-control-verified it still bites. **Residual keeping this OPEN:** live-clone verify deferred this session. **Status: OPEN (fix shipped, verify live)** · **Bucket:** Pipeline · **Trigger:** live-verify with `P-CSSPROP-RUNTIME-RESOLVER-UNDER-KEYED` (same converter surface, same verification run).
...
> **P-CSSLAYER-DROPPED-ON-AN-UNASKED-QUESTION** — NEW 2026-07-21. `css_layer` was descoped this session on the basis that it was populated on only 6 of 2,817 rows, all with the same value — i.e. it distinguished nothing. **That reasoning was wrong in the same way the tier bug was wrong: the number was accepted without asking what it SHOULD be.** Bean independently re-derived the point — the L1–L4 OUTER/CONTENT-WIDTH/PER-GRID-ITEM cascade is exactly what separates container attrs that legitimately share a property (`sgs/hero` has ~9 attrs on `padding`). Evidence the mechanism works: `sgs/hero contentBackground` resolves `css_element='content'` and does NOT collide, while its two NULL-element siblings do. **Correction to scale, measured:** layer/container is the SMALL tail (padding = 2 colliding groups); colour+typography dominate (48+18). So `css_layer` is worth populating properly, but **element coverage is the bigger lever**. **UPDATE 2026-07-22 (DECIDED, Front 1 `7a6a7586`):** the open question — "is layer part of the routing key or a separate axis" — is settled: **layer stays on its own axis; `css_element`/`css_state`/`css_tier` are the separate declarative routing keys** (seeded + load-bearing via `_base_domain_attrs_for_css_property`), NOT folded into a single element key. `css_layer` proper population remains the smaller-lever tail (element coverage is the bigger lever, as measured). **Status: OPEN (decision made; residual = seed `css_layer` more fully — small tail)** · **Bucket:** Pipeline · **Trigger:** a converter session that needs the layer tail (padding-family collisions).
...
> **RESIDUAL (Status: PARTIAL):** 4 conformance goldens (tab/feature-grid/form-step/modal) are FOSSILS that encoded the dropped/self-nested content and now correctly fail. They need a **LANDED-proof full-corpus re-seed** (`tests/seed_conformance_goldens.py` re-seeds all 40 from local emit + its provenance gate mandates a canary deploy + computed-parity proof FIRST — never a bare local emit). This **folds into the pending [[P-ORACLE... stale-golden reseed]]** (the other 24 failing goldens are that same pre-existing task). **Status: PARTIAL** · **Bucket:** Cloning pipeline · **Trigger:** the next LANDED-deploy/oracle re-seed session.

> **P-OLDSHAPE-AUDIT-TEXTALIGN** — NEW 2026-07-25. `scripts/audit-post-content-blocks.py`'s `NATIVE` set (~lines 61-64) is missing `textAlign`, so any block using WP-native `typography.textAlign` shows a false "undeclared-attr" finding once stored content sets it (currently affects info-box/notice-banner/team-member on sandybrown). Add `textAlign` to the NATIVE allowlist. **Status: RESOLVED 2026-07-25** — `textAlign` added to `audit-post-content-blocks.py` NATIVE set. `/handoff` to archive. · **Bucket:** Framework / tooling

> **P-CLONE-TEAM-MEMBER-ITEM-HEIGHT-DIVERGENCE** — NEW 2026-07-22, REFRAMED 2026-07-23. The "244px vs 327px height gap" is an ENVIRONMENT ARTEFACT, not a real fidelity gap: the oracle renders the DRAFT as a bare `file://` fragment (no WP theme) and the CLONE as a full themed WP page, and `oracle/batch_runner.py` hardcodes `_HEIGHT_COMPARABLE=False` so guard 4 returns passed+measured=False by design (never a pass). The parked box-model theory is physically wrong — info-box transfers the draft padding/bg/radius faithfully via the box resolver; its only DIVERGING defaults are `cardStyle:elevated` (a box-shadow, no reflow) and `effectHover:lift` (hover only) — neither changes RESTING height. **Real remaining sub-gap (small):** faithful ABSENCE transfer (R-31-9) — a draft item with no shadow/no `:hover` should map info-box to `cardStyle:flat`/`effectHover:none`. **Verify via computed-parity Stage 11.6 (content-keyed), NOT the cross-env height number; remove if the box-layout tier matches.** UPDATE 2026-07-24 (Build #3): the "small sub-gap" (faithful absence transfer of shadow/hover) is NOT a clean build — it is the ~8-block preset-default-injection design decision now tracked as `[[P-INFOBOX-PRESET-ABSENCE-TRANSFER]]` (Option A recommended, Bean gate pending). This entry stays only for the residual (verify via computed-parity Stage 11.6 once the preset fix lands). **Status: OPEN** · **Bucket:** Pipeline · **Trigger:** a clone-fidelity session.

> **P-INFOBOX-PRESET-ABSENCE-TRANSFER** — NEW 2026-07-24 (Build #3 outcome; Bean design-gate PENDING). A cloned `sgs/info-box` (the target for team-member/card/testimonial items) ALWAYS inherits its block.json default `cardStyle=elevated` (injects a box-shadow) + `effectHover=lift` regardless of the draft — because `cardStyle`/`effectHover` are DELIBERATELY un-routed preset selectors (no attrMap targets them; documented in `google-reviews/block.json:40`). So the clone shows a shadow+hover the draft never had (phantom-inject when absent) AND double-injects when a shadow IS present (a real box-shadow already routes to `native:shadow`, `info-box/block.json:83`). "Absence" is NOT representable in the Decl stream (no resolver consumes "property absent"). Pattern spans ~8 blocks (info-box/team-member/google-reviews/testimonial-slider/trustpilot-reviews/countdown-timer/post-grid + `effectHover` on card-grid/gallery/process-steps/testimonial) — any fix MUST be universal (R-31-9). **3 options for Bean’s gate (Build #3 report):** **A (RECOMMENDED, medium)** block.json declares per preset-enum its neutral value + injected property (`supports.sgs.presetInjects`), seed a new column, a new pass forces the preset to neutral WHEN the block natively transfers that property — so `native:shadow` (which already handles present+absent) is the single source of truth, sidesteps the absence-in-Decl problem; **B (larger)** enum-value→implied-property + absence-driven selection (richer schema); **C (cheapest, product decision)** change the authoring DEFAULTS to neutral — but that also changes a freshly-inserted block’s look + does not fix the double-inject. Spec 35 E5: `effectHover=lift` IS reduced-motion-gated (`info-box/style.css:206-207`) — no WCAG gap. **Status: OPEN** · **Bucket:** Pipeline · **Trigger:** Bean gates A/B/C, then a shared-mechanism build (Rule 7).



> **P-INSPECTOR-CONTROL-TYPE-94-DISAGREEMENTS** — NEW 2026-07-21 (carried from the prior handoff, now with Bean's explicit interest — *"all 3, especially the inspector control type"*). 94 attrs where the derived `inspector_control_type` disagrees with the stored value. **Of 18 hand-traced, 15 showed the PRE-EXISTING stored value was WRONG** (e.g. `sgs/media videoPosterId` stored as `Button`; `edit.js:431-435` binds it to `<MediaUpload>` with a Button nested inside its render prop). 3 are genuinely dual-bound — two components conditionally rendering the same attr (`sgs/filter-search attributeId`, `sgs/hero gridTemplateColumns`). **76 unaudited.** Bean's standing instruction: do NOT overwrite 94 rows on an 18-row sample — finish the audit first. **Why this matters for Spec 35's actual goal:** `inspector_control_type` is what tells a non-coder client which control they get; a wrong value is a wrong sidebar. This is closer to Spec 35's end-goal (complete, correct editor sidebars) than the routing work is. **Status: OPEN** · **Bucket:** Framework · **Trigger:** a dedicated audit session; strong candidate for the next Spec 35 front.
...
**What:** `<nav>` is in `SKIP_TOP_LEVEL_TAGS` so the top-level header skip works. But nested navs (inside non-header sections) currently pass-through their children as bare `<a>` tags that render as `<p>Shop</p><p>About</p>…` paragraphs. Map nested `<nav>` to `sgs/nav-menu` (NOT `core/navigation`, which is banned, nor `sgs/mega-menu`, which was deleted at FR-37-21/D362).
...
**P-HEADER-FOOTER-SITE-SUFFIX-NAMING-CONVENTION** — NEW 2026-05-24 (clone pipeline convention). Headers + footers produced from drafts by the clone pipeline MUST be saved as `sgs/header-<client-slug>` / `sgs/footer-<client-slug>` (e.g. `sgs/header-mamas-munches`, `sgs/footer-mamas-munches`, `sgs/footer-indus-foods`). Bare `sgs/header` / `sgs/footer` are framework defaults, never site-specific. **Existing misnamed patterns:** `sgs/mamas-munches-header` + `sgs/mamas-munches-footer` use the inverted order (`<client>-<role>` instead of `<role>-<client>`). Phase 2 header/footer cloner should: (a) author headers/footers under the canonical convention, (b) rename the misnamed mamas-munches pair, (c) add a `/sgs-update` Stage 9 drift rule that fails when a `sgs/header-*` / `sgs/footer-*` pattern doesn't follow the canonical order. **Spec 17 is DELETED — the convention now lives in Spec 37** (§3.9 per-site storage); this entry extends it to client-derived patterns. Note the per-site store is now the `sgs_header`/`sgs_footer` CPT, so this naming convention applies to any pattern the cloner still emits, not to per-site headers/footers themselves.
**Status:** OPEN
