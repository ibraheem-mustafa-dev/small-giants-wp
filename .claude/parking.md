...
> **P-PRODUCT-CARD-PILL-SWAP-DORMANT** — NEW 2026-06-03. **Status: DEFERRED** (framework). The product-card pill→price/image swap is wired in Phase C but DORMANT — `_sgs_variation_sets` stores only `{key,label}` per option (no per-option price/image), so selecting a pill produces no visible change on price or image. Needs the SKU-matrix / per-option-value data model: Spec 24 FR-24-14, `_sgs_sku_matrix` registered but Phase-2-deferred. Activate only once the data model lands.
