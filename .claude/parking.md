...
> **P-CONFIGURATOR-JS-WEIGHT-DEQUEUE** — NEW 2026-06-04 (D165, U10). **Status: DEFERRED** (framework/perf). U10 measured all interaction budgets PASS (INP 152ms throttled · CLS 0 on swap · LCP 1.96s · 0 XHR on change · NO React/wc-blocks bundle · block JS 4.6KB) — BUT total product-page decoded JS = 207KB (transfer 78KB gzip), exceeding the FR-27-H1 ≤150KB-uncompressed budget. The overage is NOT the configurator (its own JS ≈20KB incl. the ~12KB Interactivity runtime). It is WooCommerce core jQuery (~100KB: jquery+migrate+blockUI) + WC frontend scripts (woocommerce.min/add-to-cart/sourcebuster/order-attribution) + 5 SGS-theme animation scripts (animation-observer/parallax/ripple/scroll-progress/tilt-3d). A configurator page uses the SGS proxy (not WC's native jQuery add-to-cart), so those WC legacy frontend scripts + unused theme animation scripts can be conditionally dequeued per-page. **Fix:** detect a bound configurator card on the page → dequeue the unneeded WC frontend + theme animation handles. **RISK:** WC frontend scripts may power cart fragments / notices elsewhere — verify the cart badge + add-to-cart still work after dequeue; do on a deliberate perf pass, NOT inline. Weight-only (interaction perf already green). **Bucket:** Framework / performance.

> **P-JEST-BLOCK-EDIT-MOCK-ROT** — NEW 2026-06-04 (U12). **Status: OPEN** (testing/infra). `tests/js/block-edit.test.js` has 8 PRE-EXISTING failures: the "renders without throwing" + "InspectorControls present" tests for hero/accordion/form/card-grid throw `Element type is invalid … Check the render method of ResponsiveControl`. Root cause: the shared `@wordpress/components` test mocks in `tests/js/__mocks__/` are incomplete for the components the current edit.js files use (ResponsiveControl + deps resolve to undefined under the mock). NOT caused by the configurator work (the U12 `configurator-schema-compat.test.js` passes 14/14; no session change touched those 4 blocks' edit.js or the mocks). **Fix:** extend the `@wordpress/components` (+ block-editor) jest mock to export the components ResponsiveControl/edit.js need (or mock ResponsiveControl). Separate test-infra task. **Bucket:** Testing / QA.
