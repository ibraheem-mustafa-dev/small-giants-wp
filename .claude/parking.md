...
## 2026-07-07 (D288, button thread) — hero content padding + universal responsive routing

> **P-UNIVERSAL-RESPONSIVE-ROUTING** — NEW D288, Bean-directed = the NEXT SESSION's primary front. The cloned hero content sits cramped LEFT on desktop because the draft's responsive `.sgs-hero__content` padding is NOT transferred: draft wants `56px 48px` (`@media min-width:768px`) + `72px 64px` (`@media min-width:1280px`); the clone has `36px 16px`. This is the **L4 per-area extraction gap** (state.md-flagged) generalised into a **universal responsive-routing** design Bean specified: (1) **device-tier breakpoints FIRST** — a draft `@media (min-width:768px)`/`1024px` rule routes to the SGS device-tier responsive attrs (`*Tablet`/`*Mobile`, the 768/1023 system) so it's a proper editable block control; (2) **non-device (arbitrary) breakpoints** — a draft `@media (min-width:1280px)` (or any non-768/1024 value) routes to the relevant block's **Additional CSS field** (`sgsCustomCss`), per STOP-52 (the page must never depend on non-block-settings pipeline CSS). Applies to BOTH CSS and content routing, universally across blocks (not a hero carve-out). The vertical-centre half of the hero fix already SHIPPED D288 (verticalAlign default '' + wrapper skips align-items when blank → browser-default stretch → content vertically centred, LANDED). Only the horizontal/responsive-padding transfer remains. **Status: OPEN** · **Bucket:** Cloning pipeline / fidelity · **Trigger:** NEXT SESSION (universal responsiveness routing — device-tier then Additional-CSS). Supersedes/absorbs the narrower L4 padding note.
