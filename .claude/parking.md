...
## 2026-07-16 (D342) — ~~Call-button contrast~~ CLOSED BY BEAN, same day: NOT AN ISSUE
...
> and the value is not hardcoded, so it is not a defect to chase. Entry kept for one cycle ONLY
> to stop a future session "re-finding" it and re-parking it; move to
> `memory/parking-archive.md` at the next handoff. Kept below, struck through, for that reason.

> ~~**P-CALL-BUTTON-CONTRAST** — the Indus/Mama's header "Call" button is cream-on-pink at 2.24:1~~

> **P-CALL-BUTTON-CONTRAST** — NEW D342. Found by the Spec-34 Gate-B axe sweep of the LIVE header (not by a code read): the D331 Call button — `sgs/business-info` `displayType="phone"` + `is-style-button`, in `parts/header.html`'s bottom row — renders its label `#fbf3dc` (cream) on `#e68a95` (primary pink) = **2.24:1**, a serious WCAG 1.4.3 failure on the sandybrown canary today. **Pre-existing and NOT drawer-related** (verified: absent from the Spec-34 diff; `is_drawer_related: false`); left unfixed deliberately to avoid scope-creeping a drawer rebuild into an unrelated block's styling. **This is the D339 class exactly** — `is-style-button` in `business-info/style.css` pairs a hardcoded token background with a hardcoded token foreground, and no client palette guarantees that pair contrasts (`primary` is a light pink on mamas-munches). **Fix shape (proven, D339):** don't author the pair — resolve the button's background slug to hex and COMPUTE the label colour via `sgs_resolve_palette_hex()` + `sgs_wcag_text_colour_for_bg()` (`helpers-colour-wcag.php`), exactly as `sgs/adaptive-nav`'s `drawerBg` now does; verify across all 8 `sites/*/theme-snapshot.json` palettes (STOP-VERIFY-EVERY-CLIENT). **Status: OPEN** · **Bucket:** Framework / blocks · **Trigger:** Goal 1/4 header work, or the next a11y sweep — whichever first.
