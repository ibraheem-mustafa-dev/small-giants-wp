...
**P-SGS-UPDATE-V2-COGNITIVE-COMPLEXITY-REFACTOR** — NEW 2026-05-24 (Task H close-out). SonarLint surfaced 9 functions in `plugins/sgs-blocks/scripts/sgs-update-v2.py` (2,400-line `/sgs-update` orchestrator) with Cognitive Complexity above the 15 allowed: L142 (cc=73), L571 (cc=96), L880 `_mode_b_refresh_upstream` (cc=142), L1483 (cc=27), L1635 `stage_4_style_variation_sync` (cc=85), L1970 `stage_5_slot_synonym_auto_seed` (cc=29), L2158 (cc=18), L2548 (cc=18) — plus one more at L2160 originally flagged. Three were investigated by parallel Sonnet agents this session; reports + addenda below. The remaining six need investigation under the same shape. All 3 reviewed proposals passed `/qc-inline` (51/100 — fix-with-addenda).

**Implementation discipline (binding):** (1) /subagent-driven-development one proposal at a time per Bean 2026-05-24 — no parallel dispatch on the same file (blub.db row 240 + 254). (2) Each refactor commit gated by multi-model `/qc` panel (Sonnet + Haiku + Gemini Flash + Cerebras) before landing. (3) Each refactor's commit-gate: predicted post-fix Stage-N return dict matches baseline dict byte-for-byte; report-file output byte-equal; for Mode B `--refresh-upstream --dry-run` source-success / source-fail counts identical pre/post. Do not commit on divergence. (4) `_mode_b` effort estimate revised from 25 min → 45-60 min. (5) Implementer must re-locate functions by name (line numbers shifted ~6 lines after the 2026-05-24 quick-win additions to the constants block + import cleanup at the top).

**Proposal A — `stage_5_slot_synonym_auto_seed` (cc=29 → <15) [SMALLEST, RECOMMENDED FIRST]:** Extract 3 helpers: `_match_slot_to_block(cursor, normalised) → (confidence, slug, candidates)` (pure 3-tier query, no writes), `_apply_high_confidence_match(cursor, row_id, slug, dry_run)` (isolates the UPDATE), `_build_synonym_report(high, medium, low, counts, ts)` (assembles report string). Main loop drops from ~65 lines with nested branches to ~15 lines of straight-line dispatch. Risk: low — all helpers have clear interfaces; only mutation is one isolated UPDATE. Effort 15 min + /qc panel ~15 min = 30 min total.

**Proposal B — `stage_4_style_variation_sync` (cc=85 → ~10-15) [MEDIUM]:** Extract 4 helpers: `_write_token_row(c, tok, dry_run)` (skip / conflict-prefix / insert, returns counters), promote `_build_token_candidates(snapshot, client_slug)` from nested-def to module-level, `_process_client_snapshot(conn, client_dir, dry_run)` (read JSON + iterate tokens), `_write_stage4_report(report_path, header, per_client, summary, dry_run)`. Trimmed parent ~60 lines. Risk: `INSERT OR IGNORE` rowcount semantics at the conflict-prefix branch must be preserved; per-client `conn.commit()` placement must be preserved; `_CUSTOM_KEY_BLACKLIST` constant must move alongside the promoted helper. Effort 25 min + /qc panel ~15 min = 40 min total.

**Proposal C — `_mode_b_refresh_upstream` (cc=142 → ~18-22) [LARGEST]:** Extract 5 helpers: `_insert_or_count(c, sql, params, dry_run) → rowcount` (eliminates 9 repeated `if not dry_run / else SELECT 1` blocks), `_scrape_source_1_gutenberg(conn, github_token, wp_version, new_rows, dry_run)`, `_scrape_source_2_hooks(...)`, `_scrape_source_3_wpcli(...)`, `_scrape_source_4_since(...)`. Sources 5-10 keep their existing shared loop in the trimmed coordinator (already shared; trimming the `if doc_type=="dev-blog"` branch only). Coordinator drops to ~60 lines. Risk: shared sqlite connection — preserve per-source `conn.commit()` placement (do NOT hoist to final commit); `new_rows` dict mutation pattern — choose by-reference OR return-and-accumulate (latter is safer). Effort 45-60 min + /qc panel ~15 min = 60-75 min total.

**Six additional cc warnings without investigation yet (L142 cc=73, L571 cc=96, L1483 cc=27, L2158 cc=18, L2548 cc=18, and any drift from L142 → renamed functions):** treat as follow-on once Proposals A/B/C land + the cc=85/142 patterns prove the helper-extraction approach generalises. Investigate via the same parallel-agents-then-/qc-inline pattern used 2026-05-24.

**Trigger to action:** dedicated session after current doc-op programme closes. Open with /subagent-driven-development on Proposal A; full QC pass before Proposal B; ditto B → C.
