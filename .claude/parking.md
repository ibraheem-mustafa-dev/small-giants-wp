...
> **(3) FR-S9-6 `{desktop,tablet,mobile}` responsive-override model.** Coordinated with the D325 footer track building this as a SHARED engine — **analyse their engine first, then adopt** across all header blocks + adaptive-nav (Bean-directed, to avoid double-building). adaptive-nav uses P1 flat-tier attrs meanwhile. Do NOT add `ksort` to the shared-wrapper uid (churns every block's uid; the object-model must guarantee key-order at write-time).
