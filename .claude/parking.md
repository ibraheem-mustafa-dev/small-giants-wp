...
> **P-ORACLE-CHECKLANDED-NEEDS-CANARY-FIXTURES** — NEW 2026-07-22. The F3 LANDED runtime + multi-fixture batch runner SHIPPED (`51629e37`, unit C1a), but `ledger/coverage_check.py::check_landed()` is **deliberately NOT wired yet**: with no per-fixture canary URLs configured, it returns `NOT-RENDERED` for all 36 fixtures and would FAIL the F5 gate for every session (including Track 2). **This is the gating dependency for declaring Spec 31 100%** — the instrument exists but can prove nothing without live pages. **Work:** deploy the phase-f fixture corpus as canary pages, populate `oracle/fixture-canary-urls.json`, then apply the `check_landed()` patch (the exact function body is in the C1a agent report + the commit message). **Status: OPEN** · **Bucket:** Pipeline · **Trigger:** the Spec-31 closing session (C1b/C2).

> **P-CLONE-TEAM-MEMBER-ITEM-HEIGHT-DIVERGENCE** — NEW 2026-07-22. Surfaced by the NEW height guard (C1a) — previously invisible because the hand-authored artefact had `draft_height_px=null`, so guard 4 was **silently SKIPPED, not passed** (the old "LANDED" was itself a partial false-win). On `rt-centred-maxwidth` the clone renders **244px vs the draft's 327px** (25% / 82px, over both thresholds): each `.sgs-team-member-grid__item` clones to `sgs/info-box`, which brings its OWN default padding/elevation/hover-lift and is genuinely shorter than the draft's plain item styling. Container-level props (max-width/padding/gap/background/grid-template-columns) all match — this is purely the item-level block choice. **Work:** decide whether the correct target is a different block, an info-box variant with the draft's box model, or a faithful per-item CSS transfer. Real clone-fidelity gap, NOT a measurement bug. **Status: OPEN** · **Bucket:** Pipeline · **Trigger:** a clone-fidelity session.
...
> **P-POSTGRID-SCALEHOVER-OUT-OF-B3-SCOPE** — NEW 2026-07-22. B3 (transform/filter hover-lift) covered `scaleHover` on 10 blocks + `grayscaleHover`/`imageZoomHover`, but deliberately EXCLUDED `sgs/post-grid.scaleHover`: it is per-ITEM (`css_element='card'`, not root/self) and multi-property (`css_property='background-color,transform'`), so it does not fit the root-domain direct-state lookup B3 built. Its hover scale therefore still drops. **Work:** extend the state-lift to per-element (non-root) domains, or split the multi-property seed. **Status: OPEN** · **Bucket:** Pipeline · **Trigger:** with the next converter state/element-domain work.
