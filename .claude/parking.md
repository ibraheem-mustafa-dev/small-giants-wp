...
> **(3) FR-S9-6 `{desktop,tablet,mobile}` responsive-override model.** **CORRECTED 2026-07-13 (ground-truth check):** the D325 footer track did NOT build this — it explicitly DEFERRED FR-S9-6 too, matching the header's flat-tier level. Confirmed on-disk: ALL five row/nav blocks (`site-header`, `site-header-row`, `site-footer`, `site-footer-row`, `adaptive-nav`) use flat-tier attrs (`gapTablet`/`maxWidthMobile`), `object-model=0` everywhere. So there is **nothing to copy** — it's a **FRESH shared-engine build**. The upside (Bean's build-once intent): since all five blocks are identical flat-tier, build the shared engine ONCE and wire all of them to it together, no divergence to reconcile. Do NOT add `ksort` to the shared-wrapper uid (churns every block's uid; the object-model must guarantee key-order at write-time instead). **THIS IS THE NEXT-SESSION FRONT (Bean, 2026-07-13):** build the shared responsive-override engine + wire the row blocks (header + footer) + adaptive-nav to it.
> **Status: DEFERRED** · **Bucket:** Framework / blocks · **Trigger:** a dedicated P2b session (or fold (3) into the FR-S9-6 shared-engine adoption pass).
