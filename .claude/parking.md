...
> **P-GATE-A-CARD-RESIDUALS** — Phase 2 landed the card's typed content (productName / priceLarge 28px-Fraunces-LANDED / priceNote / description / image / trialTag emit); four residual units remain, each mapped to a named EXECUTION stage (STOP-29, never "out of scope"): (a) **ctaText/ctaUrl** — the draft CTA is a nested `sgs-button` family node; needs the FR-31-2 IDENTITY-anchored lift (attr identity `sgs/button` → lift the recognised button descendant's text/href into the card attrs) → **Step 12/15 territory**; ctaUrl's role='content' also needs a href-extraction classification decision (link-href vs the FR-31-2.2 allowlist). (b) **packSizes** — pills are array-owned (mis-bind fixed via canonical_slot='pill' override d02d6bf5) but the array LIFT needs an `array_item_schema` items schema → **Step 15 (variant/fixture data population)**. (c) ~~trialTag variant gate~~ **CLOSED 2026-07-04 (Step 15, `051b32b0`)** — variants seeded via block.json supports.sgs.variants; the trial badge paints live. (d) ~~testimonial names~~ **CLOSED 2026-07-05** — an instrument artefact (split-text pairing), fixed in computed-parity.js; the names paint live. NEW residual added: **(e) image ALT texts** — card/brand images lose alt (string image attr drops it); needs a block-side imageAlt attr + converter lift (a11y-relevant). **QC-session updates (2026-07-05, D277):** the A2 baseline now reflects (c)'s closure (shrunk 6→5 keys via a fresh LANDED run — the stale trial-tag entry removed); and (a)'s ctaUrl had a component blocker fixed — `content_attr_for_element`'s drifted in-code role tuple excluded `link-href` entirely (now DB-sourced, commit `4a35134a`) — the remaining ctaText/ctaUrl work is the multi-attr-per-element lift (the resolver returns ONE best match per element; a CTA element needs BOTH its text and href lifted). **Status: OPEN** · **Bucket:** Cloning pipeline · **Trigger:** next converter session.
