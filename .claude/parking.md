...
> **P-VERIFY-WAVE2-A1** — NEW. Wave 2 (hero FR-22-6 `1e214d49`, trust-bar rename `6d9fabfb`) + A-1 responsive lift `d9c11ed7` are COMMITTED + `/sgs-update`-persisted but NOT built/deployed/re-cloned → NOT verified on live DOM. NEXT SESSION FIRST: build+`build-deploy.py --target sandybrown --blocks-only --allow-dirty`+re-clone (page 144)+re-generate the wireframe. Confirm hero no double-structure, trust-bar mobile 2-col/desktop 4-col, per-device attrs present, no regressions. Rendered-HTML only (R-22-11/FR-22-18); roll back fast on regression (STOP #19).
> **Status:** OPEN · **Bucket:** Pipeline / converter · **Trigger:** next session, first.

> **P-TRUST-BAR-HYBRID-MIGRATION** — NEW. The renamed `sgs/trust-bar` block (was trust-badges) is a HYBRID: render.php reads scalar `items` array, ignores `$content`. For the `.sgs-trust-bar` section block-override (Req 3) to render, it needs its own FR-22-6 migration (echo $content; items→InnerBlocks of an atomic badge block). Until then, the trust-bar section still routes via the universal fold (sgs/container + leaf-guarded badges), which works. After /sgs-update the section class `.sgs-trust-bar` resolves to the block (legacy_role_lookup + blocks table) — so this migration unblocks the dedicated-block path.
> **Status:** OPEN · **Bucket:** Block migration (FR-22-6) · **Trigger:** after Wave-2 verification.
