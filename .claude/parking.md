...
## 2026-07-07 (D289) — hero content padding (L4 wiring) + residual render-precedence

> **P-HERO-PADDING-L4-WIRING** — refocused from the D288 `P-UNIVERSAL-RESPONSIVE-ROUTING` entry after the ROUTER half SHIPPED D289. **What shipped (D289, `9a22b6f2`):** the universal responsive breakpoint router — a draft `@media` rule routes by breakpoint class for every element of every block (device-tier 768/1024 → tier attrs; non-device 600/640/1280 → the block's `sgsCustomCss`). LANDED (hero h1 52px@768/1024; ingredients/gift/social non-device breakpoints captured). **What REMAINS (this entry):** the visible hero content PADDING still does not land, because the **L4 per-area extraction is UNWIRED for composites** — nothing in the live pipeline sets `ctx.area_name`, so `layer_detect` never returns `GRID_AREA`, the `grid_area` resolver + `attr_for_area_property` (which DO map `content`→`contentPadding*`) never fire, and `.sgs-hero__content`'s box-CSS is never even collected (it's a Branch-C slug-None content wrapper at `extraction.py:672`, descended for content only). `route_area_css_to_block_attrs` (`fold_helpers.py:247`) EXISTS but is unwired (zero callers). Once wired, the padding routes to `contentPadding*` (class-scoped `.uid .sgs-hero__content`, 0,2,0) and the D289 residual overrides at equal specificity (+ append). **Status: OPEN** · **Bucket:** Cloning pipeline / fidelity · **Trigger:** the L1-L4 container cascade session (wire the L4 per-area extraction; must work for ALL block types).
