...
## 2026-07-14 (D330) — plugin-side Customiser header/footer path retirement (remaining Task 2b)

> **P-CUSTOMISER-HEADER-FOOTER-RETIRE** — NEW D330. D330 retired the THEME-side header-mode system (`inc/class-header-behaviour.php` + `header-behaviour.js` + `header-modes.css` + `header-editor-panel.js`) — the one the qc-council flagged as conflicting with the new FR-S9-9 body-class layer (duplicate `--sgs-header-height` publisher). But D329 Task 2b ALSO named a SEPARATE plugin-side Customiser path that was NOT touched this session: `Sgs_Header_Customiser` + `Sgs_Footer_Customiser` + `Sgs_Header_Renderer` (in `plugins/sgs-blocks/includes/`, registered at `sgs-blocks.php` L410-413) — Customiser sections for header/footer bg/text/link colour + max-width + an "Enable sticky header" toggle. These now DUPLICATE the block controls (colour/width on `sgs/site-header` + the new FR-S9-9 sticky toggle). Retire: prove `Sgs_Header_Renderer`'s CSS-var injection dormant (it only fires when a legacy Customiser option ≠ default; the live header renders via the block), then remove the Customiser sections + the renderer, keeping the conditional-rules engine (`Sgs_Header_Rules`) surfaced via an admin page not the Customiser. **Status: OPEN** · **Bucket:** Framework / blocks · **Trigger:** a focused consolidation session (pairs with the FR-S9-8 session or its own).
...
## 2026-07-14 (D330) — 3 inert alt-header template-part stubs (theme.json cleanup)

> **P-ALT-HEADER-PART-STUBS** — NEW D330. `theme/sgs-theme/parts/header-{sticky,transparent,shrink}.html` + their `framework-header-{sticky,transparent,shrink}.php` patterns + the theme.json `templateParts` registrations (theme.json L596/601/606) are TODO(v1.1) STUBS that re-embed `sgs/framework-header-default` (no actual behaviour markup). After the D330 FR-S9-9 build, behaviour comes from the block toggles on the DEFAULT header — so these named parts are redundant + confusing (an operator picking "header-sticky" gets the default header, no behaviour). Non-conflicting (they were always inert). Retire: remove the 3 registrations from theme.json + delete the 3 part files + 3 patterns. Keep `header-minimal` (a genuine layout variant, not a behaviour). **Status: OPEN** · **Bucket:** Framework / theme · **Trigger:** a theme.json cleanup pass (pairs with P-CUSTOMISER-HEADER-FOOTER-RETIRE).
