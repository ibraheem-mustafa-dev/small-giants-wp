...
> **P-TESTIMONIAL-CONVERTER-FR2220** — NEW 2026-06-11 (D206); **PARTIAL 2026-06-12** (testimonial-empty universal-lift build). The testimonial D8 rebuild (typed-attr, 7 variants) is converter-READY. **NOW BUILT (commit `75f2ffea`):** the universal scalar-lift (`_lift_scalar_attrs_by_selector` in convert.py, G3-attrs path) routes the draft `__text`/`__quote` → `quote`, `__author` → `reviewerName`, `__stars` → `ratingStars`, driven by DB `derived_selector`+`role`+`attr_type`, gated on the `scalar-content-lift` `block_capabilities` opt-in (declared via `supports.sgs.scalarContentLift` in block.json; populated by `/sgs-update` Stage 1). Variant auto-detect (`detect_variant`/`variant_attr`) already fires and picks `classic-card` for page 8. **STILL DEFERRED:** routing the OTHER typed fields (`__summary`/`__org`/`__logo`/`__work`/`__date`/avatar) — only quote/name/stars wired; and the full FR-22-20 variant-detection generalisation past hero+testimonial. **Status: PARTIAL** · **Bucket:** Pipeline / converter · **Trigger:** cloning Stage-2 routing wave.
>
