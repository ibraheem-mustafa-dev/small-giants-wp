...
> **P-CONTAINER-WRAPPER-STANDARDISATION** — NEW 2026-06-02 (D152); **WS-1 A1+A2 SHIPPED 2026-06-03 (D159); WS-4 composite-mirror EXECUTED 2026-06-04 (D166) — 25 composites mirror `sgs/container` via the shared helper, recipe canonical (Spec 22 §FR-22-21.1).** **Status: PARTIAL** (pipeline/converter — PROGRAMME). REMAINING: (a) WS-4 finish — hero (inline w/ Bean), mobile-nav, product-card; (b) **/sgs-update Stage-11 auto-apply** upgrade (§FR-22-21.2 — currently report-only); (c) the **post-WS-4 converter work** (the mirror does NOT fix page-clone fidelity — converter routes classes to `sgs/container` not the composite BLOCK @conf 0.10): the **routing fix** (`.sgs-hero`→`sgs/hero` block before container fallback), the **converter-lift** (transfer mockup CSS onto the now-mirrored attrs — memory `universal-lift-was-premature-not-falsified`), **#6 notice-banner content-synthesis**, **#4a grid-lift**, **image sideload (#5 + hero/product imgs)**, **#8 slider live-4-card residual verify** — all to be architected by the Phase-7 /adversarial-council. (d) WS-2/WS-3 de-cheat debt. Original faithful-transfer + no-propagation framing now partly resolved (helper = single render source; Stage-11 writer built report-only).
>
> **PROGRESS (2026-06-04):** WS-4 near-complete. **hero MIRRORED + live-verified + SHIPPED** (commit `bacbde57` — section KIND, C3 double-emit guard, `overlayColour`→`backgroundOverlayColour` rename, split via extra_styles + `wrap_inner:false`; both variants render 0 fatals on live `do_blocks`). **mobile-nav EXCLUDED** (commit `391e6cb1` — `containerMirror:false`, content KIND; Popover/dialog shell + own `drawerWidth`/`drawerMaxWidth` model + view.js reads outer-element identity; same grounds as modal; roster validates 4/14/11). **content-collection REGISTERED** (29th roster block, layout KIND — commit `40a9e03d`, + the scanner `widthMode` section-mis-classification fix). **cta-section AUDITED = conforming SECTION reference (PASS all 6 recipe checks)**; 3 minor non-blocking follow-ons logged: `providesContext` not mirrored (gridItem context channel for child blocks), `backgroundMedia` not defensively nulled in the helper-attrs copy, `layout` dual-meaning attr for Stage-11 propagation to handle carefully. **product-card WS-4 PARKED (NOT done)** — a subagent built the content-mirror migration but it is NOT verifiable via a bare `do_blocks()` probe: the Bound configurator uses `sourceMode='wc'` and needs a real page-render context; the synthetic probe rendered EMPTY for BOTH the migrated AND the committed versions, so it is an invalid test surface (I initially mis-read the empty probe as a regression, reverted, then proved the committed version also probes empty while page 589 renders the full configurator — interactive+context+44 pickers). Reverted to the working D164 version; canary configurator confirmed healthy on the REAL page 589. **product-card WS-4 must be re-done as a careful theme-thread-coordinated pass, verified on page 589 with `sourceMode='wc'` (NOT a synthetic `do_blocks` probe, NOT `sourceMode='bound'`).** **STILL REMAINING:** product-card (above) + `/sgs-update` Stage-11 auto-apply upgrade + the Method-2 empirical investigation (run `/sgs-clone` + draft-vs-clone HTML diff + `/dispatching-parallel-agents`/`/brainstorming`/`/qc-council`) + the full doc reconciliation (spec/flow/stages mark-complete + concise rewrite). Composite-conversion architecture DEFINITIVELY resolved this session via a 4-agent doc audit (full KIND-scoped mirror, docs-are-truth, WS-4-before-lift) — captured as blub.db 312.
>
