...
> **P-F5-RESIDUALS** — NEW 2026-06-21 (D239); pruned 2026-06-21 (D240). F5 is **COMPLETE** and **HARDENED** (D240 — adversarial-council-driven: fatal tier-blind soundness hole + count-blindness + tuple-key vacuous pass + unhashed baselines + universal-floor wiring all FIXED). These are the **genuinely large/architectural** follow-ups that remain — none small, none blocking the stage-rebuild; several arm AS PART OF the rebuild. **Status: DEFERRED** (tooling/framework). (1) **F3-RUNTIME — the LANDED leg** (the biggest): arms `ledger/coverage_check.py::check_landed()` (raises NotImplementedError today) + the coverage-matrix's COVERED/CHEAT classification. Needs the F3 render-oracle runtime (full-37 render, cache, pixel-diff, deploy choreography) — arms when the rebuild first renders many fixtures. (2) **css_router D1 media-axis:** the coverage gate's join is now media-aware (D240), but `css_router.route_css` itself still keys D1 on `attr_path = "{block}.{prop}"` with no media axis, so two same-property/different-media declarations collide INSIDE the router (one overwrites the other) — rebuild-scope, fix at the router when Stage 0.7 is rebuilt. (3) **CSS shorthand decomposition** in the ledger (`padding` vs `padding-top`): declare_input does not decompose shorthands, so a longhand drop can hide behind a shorthand bucket — ledger enhancement, medium. (4) **Commit-time STATIC `sourceMode='bound'` scan:** check #5 (mirror/bound) is post-clone-only (check_no_mirror via pipeline-stage-gate) + the orchestrator halts a cheating clone pre-deploy — but a `sourceMode='bound'` emit can be COMMITTED without a clone. A small static converter-source scan at commit time would close it. (5) **Inline-style bucketing:** the coverage gate can't bucket `style=""` declarations (css_router only parses `<style>`), so inline declarations are baselined rather than verified. (6) **`--update-baseline` laundering guard:** the SHA-256 hashes stop silent hand-edits, but `--update-baseline` is still a deliberate visible laundering path — a structural guard (e.g. require a paired source diff) is a larger design question. (7) **fail-open canary/heartbeat:** the CC commit-hook fails open on a wrapper crash; a heartbeat/last-green-timestamp would make a silently-disabled gate visible (the `.githooks` floor already addresses the bigger "not running outside CC" gap). **Bucket:** Tooling / pipeline gates.
