...
> **P-HERO-DOUBLE-WRAPPER-AND-SPLIT-IMAGE** — NEW 2026-05-31. Post-FR-22-6 migration, hero STILL double-renders content: render.php's `$content_html = '<div class="sgs-hero__content">'.$content.'</div>'` re-wraps the converter-emitted `sgs/container.sgs-hero__content` InnerBlock → 2 `.sgs-hero__content` divs. Plus both `__split-image--mobile` + `--desktop` images render (height 1820; the responsive show/hide isn't applying — likely the A-1 slot-level responsive/visibility gap OR the converter emits both as sgs/media without the show/hide). FIX: (a) hero render.php echoes `$content` directly in the content grid-cell without a `.sgs-hero__content` re-wrap (the InnerBlock IS the content); (b) ensure the split-image mobile/desktop visibility CSS lifts/applies (ties to A-1 Phase 2 + the visibility-via-attr gap).
> **Status:** OPEN · **Bucket:** Block migration (FR-22-6) · **Trigger:** next session (verify-Wave-2 follow-on).
