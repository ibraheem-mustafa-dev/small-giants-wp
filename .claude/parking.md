...
> **P-F5-RESIDUALS** — NEW 2026-06-21 (D239). F5 (the Spec-31 §12.7 gate cluster) is **COMPLETE** — all 4 gates + check_no_mirror built, baseline-armed (STOP-14) + wired to run (STOP-6, the `f5-commit-gate.py` PreToolUse hook + `prebuild`/`prestart`). The original `P-F5-REMAINING` (6 items) is RESOLVED → its full record is D239 (decisions.md). These are the **residual deferred** follow-ups, none blocking the stage-rebuild. **Status: DEFERRED** (tooling/framework). (1) **F3-RUNTIME — the LANDED leg** (the biggest): arms `ledger/coverage_check.py::check_landed()` (today raises NotImplementedError) + the coverage-matrix's COVERED/CHEAT classification. Needs the F3 render-oracle runtime (full-37 render, cache, pixel-diff, deploy choreography). Deferred until the rebuild first renders many fixtures (per the prompt). (2) **2 D2-reparse precision gaps** in `coverage_check`: `css_router`'s D2/D0 buckets emit raw CSS text, and the regex re-parser misses compound attribute-selectors (`[data-pending="true"]`/`[hidden]` in `mamas-trust-bar-real.html`) → 2 declarations conservatively baselined. Fix = recover full compound selectors in the D2 re-parse (or have css_router emit per-declaration dispositions directly). (3) **cheat-gate tuple-keyed dict gap:** `check_hardcoded_dicts` scans string-keyed dicts; `_SUFFIX_ATTR_OVERRIDES` (convert.py) has TUPLE keys `(css_prop, suffix)` so it isn't caught (frozen convert.py — vanishes with the modular rebuild; `prop_map` IS caught, covering the R-22-1 intent). (4) **combined-pytest collision:** `pytest scripts/` over ALL gate dirs at once errors on duplicate module basenames (`models.py`/`run.py` in hyphenated non-package dirs) — run per-dir (prebuild already invokes each gate separately, so unaffected). **Bucket:** Tooling / pipeline gates.
