...
## 2026-06-07 — stale slot_synonyms references in offline scripts

> **P-STALE-SLOT-SYNONYMS-SCRIPT-REFS** — NEW 2026-06-07 (surfaced when Bean queried the slot_synonyms retirement). **Status: OPEN** (tooling). The `slot_synonyms` TABLE was dropped at D99 (live DB confirmed: gone; `slots` 101 rows + `roles` replaced it). But ~10 OFFLINE scripts still `SELECT/INSERT FROM slot_synonyms` and would ERROR if run against the live DB: `behavioural-analyser/assign-canonical.py` (+ `backfill-from-json-catalogue.py`), `drift-validator/validate.py`, `gap-detection/{apply-fanout-proposals,apply-phase-3.5-vocab,canonicalise-pass-2,canonicalise-slot-only,coverage-ab-mining}.py`, `generate-block-reference.py`, `migrations/2026-05-16-slot-synonyms-roles.py`. These are gap-detection/canonicalisation/drift tooling, NOT in the live `/sgs-clone` walker path (the walker uses `slots`). **Fix:** migrate each `slot_synonyms` query → `slots` (canonical_slot/aliases/standalone_block columns exist there, add `WHERE scope='element'`), OR retire the genuinely-dead scripts. Verify `generate-block-reference.py` specifically (it runs in /sgs-update Stage 7 — confirm whether it actually hits slot_synonyms or only references it in a comment; Stage 7 passed this session so it likely doesn't query the dropped table at runtime). **Bucket:** Tooling.
