...
## 2026-06-18 — F5 gate cluster (partial; remaining gates)

> **P-F5-REMAINING** — NEW 2026-06-18 (D236); item (1) SHIPPED 2026-06-21 (D238). F5 of the Spec-31 §12.7 Tier-1 foundation is PARTIAL: `check_no_mirror.py` is armed with a committed legacy baseline (`6193f3e9`) AND now auto-runs on every clone (D238), but the rest of the gate cluster is OPEN. **Status: OPEN** (tooling/framework). 4 gates + 1 hook remain. (1) ~~**check_no_mirror auto-wire (STOP-6):**~~ **DONE 2026-06-21 (D238, commit `2341e761`)** — `sgs-clone-orchestrator.py` `main()` now invokes `pipeline-stage-gate.py <run_dir>` after `stage_9_report()` writes `extract.json`, BEFORE deploy/+REGISTER; hard-halts on a NEW mirror-cheat; `--skip-stage-gate` opts out. Acceptance proven (real clone fires it; planted NEW violation exits 1). (2) **`check-converter-cheats.py` (§7a):** Python DB-querying whole-tree gate, 7 checks (per-block slug literal whole-tree + indirect forms; hardcoded prop→attr dict R-22-1 — BASELINE the legacy `_SUFFIX_ATTR_OVERRIDES`/`prop_map`, do NOT edit convert.py; `!important` scan of PHP/CSS render surface; breakpoint numeric-literal scan + the legacy `_BP_SUFFIX_MAP`; D2-when-D1 DB cross-join; sentinel leakage). (3) **`generate-coverage-matrix.py` (§5):** auto-generated dashboard (rows from `block_composition`, cols from DB; secondary, not the completeness gate). (4) **pipeline-close ledger checker:** the coverage-invariant join `UNACCOUNTED = draft − (transferred ∪ excluded[F4 table] ∪ gap)` (F2∪F3∪F4∪gap); UNACCOUNTED>0 / WRITTEN-not-LANDED>0 → fail. Its WRITTEN-not-LANDED leg needs F3-runtime (rendering many fixtures). THE real no-silent-drop guarantee (per the F4 /qc-council). (5) **EXCLUDED-literal gate:** the council-hardened tripwire per the F4 design doc §3 hand-off (import-graph-wide scan, content-hashed self-blessing-proof baseline, inline/anonymous-set detection, explicit F2/F3/lint delegation, Python pre-commit wiring). (6) **PreToolUse git-commit hook** in `.claude/settings.json` (close the commit-without-build bypass). Each gate is its own `/brainstorming` → `/adversarial-council`/`/qc-council` → Bean gate → SDD → qc. Per STOP-14, run each gate against current output first + baseline today's violations (the §12.6-step-1 armed-against-legacy pattern). **Bucket:** Tooling / pipeline gates.
