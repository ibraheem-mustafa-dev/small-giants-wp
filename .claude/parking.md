...
## 2026-07-09 (D293) — no-inline rollout follow-ups (minor; rollout continuation itself is in next-session-prompt, not parked)

> **P-DICT-MERGE-BOX-FAMILY-GATE** — NEW D293 (council finding #4, deferred). The box-object dict-merge in `orchestrator.py` (`attrs()` + `process_element`) gates the per-key merge branch on `isinstance(w.value, dict)`, NOT on the DB `box_family`. Harmless TODAY (only `content_band.py` emits dict Writes, box_family-gated at source), but if a future resolver emits a legitimately dict-valued NON-box attr (e.g. a media/background object `{url,id,alt}`) to the same attr twice, they'd silently per-key merge instead of collision-raising. Fix: gate the merge branch on an explicit box-family/shape flag before adding any non-box dict-valued resolver. An inline code comment marks it. **Status: DEFERRED** · **Bucket:** Cloning pipeline / fidelity · **Trigger:** before adding any non-box dict-valued attr resolver.

> **P-FRAMEWORK-CSS-KEYWORD-SANITISER-SWEEP** — NEW D293. The raw-CSS-concat CSS-injection pattern the button fix addressed (`border-style:`/`font-style:` etc. concatenated into the scoped `<style>` after only `sanitize_text_field`, which doesn't strip `;{}`) exists in OTHER blocks' render.php (heading/text now fixed; quote/media/others pending). Each block's no-inline migration MUST apply the `preg_replace('/[^a-zA-Z-]/','',$v)` keyword sanitiser (contract §D) — so this drains as the rollout proceeds. **Status: OPEN** · **Bucket:** Framework / security · **Trigger:** each block's migration (folds into the rollout).

> **P-HEADING-TYPOGRAPHY-SELECTOR-STALE** — NEW D293 (minor hygiene). After heading's wrapper-removal, the DB `block_selectors.typography` for `sgs/heading` still reads `.wp-block-sgs-heading__headline` (was already stale pre-change — didn't even match the old `__text` render). Proven NOT to affect the cloning typography lift (LANDED page-8 headings styled correctly, parity 68→69%), so non-blocking. Re-point to the root selector for tidiness. Also: the committed stray `plugins/sgs-blocks/reports/visual-diff/button-2026-07-09.md` (wrong dir — gate reads repo-root `reports/`) can be deleted. **Status: OPEN** · **Bucket:** Framework / tooling · **Trigger:** a hygiene pass.
...
> **P-RESIDUAL-RENDER-PRECEDENCE** — NEW D289 (STOP-64). The D289 residual (a wrapper-CLASS rule in `sgsCustomCss`, appended by `custom-css.php`) overrides only EQUAL-specificity, class-scoped block rules. A property the block renders at ID-specificity (`#uid`, e.g. the typography helper `#sgs-hdg .heading__text{font-size}`) is NOT overridden — proven live: hero h1 ≥1280 stays 52 not the residual's 58. Class-scoped properties (the hero content padding) are unaffected and WILL win. Reconcile the per-block render-specificity so an ID-scoped property's residual can override (or the block emits at class specificity). Same problem class as the L4 work. **Status: OPEN** · **Bucket:** Cloning pipeline / fidelity · **Trigger:** the L1-L4 / per-block render-specificity session (with P-HERO-PADDING-L4-WIRING).

> **P-RESPONSIVE-ROUTER-ROBUSTNESS** — NEW D289 (pre-existing, untriggered; documented in Spec 31 §3 F-fork + §13.4). Two hardening follow-ups the D289 whole-tier folding did NOT introduce and current mockups do NOT trigger: (1) a NO-WIDTH media condition (`@media print` / `prefers-color-scheme` / `orientation` / `prefers-reduced-motion`) whose selector matches a converted element folds into the SCREEN base for all tiers and is not captured as a residual (the fix: a no-width media type should be a passthrough residual, not folded — needs media-type awareness); (2) inverted-threshold `min-width` pairs on one property resolve by threshold, not CSS source-order. **Status: OPEN** · **Bucket:** Cloning pipeline / fidelity · **Trigger:** router hardening (low priority — no mockup triggers either).
