...
## 2026-07-10 (D303 session) — effective-value typography lift (the general mechanism) + other-client snapshot cleanup

> **P-EFFECTIVE-VALUE-TYPOGRAPHY-LIFT** — NEW D303 (Bean-directed: "we should have a mechanism that can deal with it if necessary"). The D303 session fixed the section-heading letter-spacing gap the THEME-SIDE way (removed the arbitrary `h1 -0.022em`/`h2 -0.015em` from the Mama's snapshot so headings render `normal`, matching the draft — LANDED + verified live). That fixes THIS instance, but the PIPELINE still lacks the general capability: **the converter only lifts EXPLICITLY-declared typography, so a draft value that is INHERITED or is the CSS-initial (`normal`) is never lifted — meaning any theme default that DIFFERS from the draft silently wins.** The robustness mechanism = **lift the draft's EFFECTIVE (computed / inherited-chain) letter-spacing + line-height onto every text element** (Spec 31 FR-31-5.1 extended from `text-align` to letter-spacing/line-height): in `collect_css_decls_for_element` (styling_helpers.py), after collecting the element's own declarations, for each inheritable typography prop ABSENT from the collection, resolve the effective value by walking the draft node's ancestor chain (else the CSS initial, `normal`) and emit it as a base declaration so the typography resolver lifts it onto the block → the block's own (0,2,0) rule then overrides ANY theme default. **Design cautions (why it's a deliberate fresh-session build, not a rush):** (a) it changes conformance goldens (many elements gain an explicit `letterSpacing`/`lineHeight` attr) — STOP-60 re-seed care; (b) scope to genuine text elements; (c) "always emit even `normal`" is correct (that's what overrides the theme) but verify no over-lift on non-text; (d) route by DB role (D301), never a property-name list. **Status: OPEN** · **Bucket:** Cloning pipeline / fidelity · **Trigger:** the effective-value-lift build (fresh session — the theme-side fix holds the current clone in the meantime).

> **P-SCOPED-SELECTOR-MATCH-AUDIT-AND-GATE** — NEW D303 (Bean-directed: "investigate the bugs or gaps causing issues like the multi-button issue"). The multi-button regression is a CLASS of bug, not a one-off: **a block emits a scoped-CSS rule (`.uid.block{…}` / `.uid .el{…}`) but the element does NOT actually carry `.uid` as a class** (the uid is applied as an `id` attribute, or via the wrapper's `extra_attrs.id`, or omitted from the class list) → the rule matches NOTHING → the block's styling is a **silent render no-op**, invisible to a fully GREEN build and often MASKED by presets/wrapper defaults/theme values (button/feature-grid looked fine but their scoped rules were dead). Two deliverables next session: **(1) roster-wide AUDIT** — for EVERY block that emits a scoped `<style>`, confirm every per-instance selector's uid/scope class is actually on the rendered element (the ~40 shared-wrapper blocks + the self-rollers); the D303 pass fixed the 7 it touched, but other blocks may have latent id-vs-class or omitted-class mismatches. **(2) STRUCTURAL GATE (Rule 10 — enforcement not "verify harder")** — a check that catches a scoped rule whose per-instance class token the element's class construction never adds. Options: a static analyser (parse render.php: for each emitted `.{$var}…` scoped selector, assert `$var` is pushed to the element's class list, not just `extra_attrs.id`), OR extend the live `scripts/no-inline-land-verify.js` harness to assert each block's scoped rules match ≥1 real element (computed-style non-default). Prefer the live-harness assertion (STOP-21: emit-green ≠ landed — a static check can't prove the class lands). **Status: OPEN** · **Bucket:** Cloning pipeline / test-coverage · **Trigger:** the next-session mechanism investigation (pairs with `P-EFFECTIVE-VALUE-TYPOGRAPHY-LIFT`). Memory: `normalise-scope-needs-uid-as-class-not-just-selector`.

> **P-SNAPSHOT-ARBITRARY-LETTER-SPACING** — NEW D303. The arbitrary heading `letterSpacing` (`h1 -0.022em`/`h2 -0.015em`) was copied IDENTICALLY into ALL 6 client snapshots (`sites/{mamas-munches,eye-care-ward-end,sgs-construction,sgs-healthcare,sgs-mosque}/theme-snapshot.json` + the `-typography-axis.json` variants) — a template-by-copy default, not extracted from any draft. Only mamas-munches was fixed + pushed this session. The other 5 carry the same arbitrary default (at different line offsets — ~175 + the axis files). Batch-remove `h1/h2 letterSpacing` from those snapshots when each client is next built/pushed (low risk — the per-element block lift supplies any real draft value). **Status: OPEN** · **Bucket:** Framework / theming · **Trigger:** next per-client build, or a snapshot-cleanup pass.
...
>
> **P-STYLESHEET-DEFAULTS** — RESOLVED (verified 2026-07-05, Bean-flagged + ground-truth-checked: theme-snapshot base fontSize = 16px matching the draft; the D278 fresh parity run contains ZERO 16-vs-18 font-size mismatches) → archived to `memory/parking-archive.md`.
>
> **P-FEATURE-GRID-AUTOFLEX-COLUMNS** — RESOLVED (D270, 2026-07-04) → archived to `memory/parking-archive.md`. (The parked fix-shape — "converter sets layoutMode + maps columns" — was WRONG; the real fix was render-side: the block delegates to the shared wrapper, which already emitted the correct grid. NO converter change.)
>
...
>
> **P-FINGERPRINT-MIGRATION** — DONE (D261, 2026-07-03) → archived to `memory/parking-archive.md`.
>
