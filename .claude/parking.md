...
> **P-SPEC37-CORE-BUILD** — NEW 2026-07-21. Spec 37 is written + signed-off-ready but **nothing is built**. The **6-FR minimum core** delivers the entire visible outcome ("a non-coder edits a header in an admin page and it appears on their site"): **FR-37-2** (Set as active → `sgs_active_header_cpt_id`) → **FR-37-3** (direct-render branch + own re-entrancy guard + the CPT-aware `get_header_content()` clause) → **FR-37-4** (verify the immutable fallback still fires) → **FR-37-25** (reset/clear, which doubles as the rollback) → **FR-37-5** (Active column) → **FR-37-6** (move `parts/header.html`'s 28 lines into a starter). **Order is not interchangeable, and FR-37-6 is LAST** — until direct render AND the behaviour resolver both work, `parts/header.html` is the rollback. ⛔ **FR-37-6 is GATED on FR-37-3's behaviour clause**: empty the template part first and every header behaviour dies silently (the council's headline find). Estimate: ~1 focused day header, ~45 min footer. **Status: OPEN** · **Bucket:** Framework · **Trigger:** next session.

> **P-SPEC37-OPEN-RESIDUALS** — NEW 2026-07-21. Carried from the Spec 37 coverage matrix + council, none blocking the core build. (a) **FR-S1-4 skip-link regression contract** (D157) has no successor — its warning lived in the three behaviour-stub template parts FR-37-31 deletes; restate it there. (b) **The 3 layout starter variants** (centred/minimal/full, ex-FR-S3-1) have no stated fate — presumably fold into FR-37-8. (c) **FR-S5-3's non-carried WP-CLI commands** (migrations, seeding-arm, rules) — FR-37-30 says "reduced set" without saying what happens to the rest. (d) **FR-37-12 width set** narrowed 8→3 widths, dropping the **320–374 band** the original sub-384px overflow emergency lived in — **add 320**. (e) Spec 17's **prose-only** content with no FR-ID: the REST capability-gating detail (`edit_theme_options`, 401 not 200, 50-post soft limit), the §S9 "attribute shape FROZEN" guardrail (which FR-37-14 knowingly overrides — say so explicitly rather than silently), and the §S9 tooling paragraph. **Status: OPEN** · **Bucket:** Framework · **Trigger:** with the FR-37-31 retirement work.

> **P-FOOTER-COLUMNS-DISCARDED-ATTRS** — NEW 2026-07-21. **Live bug.** `site-footer/edit.js:28-30` inserts a footer row carrying `columns: 3`, `columnsTablet: 3`, `columnsMobile: 1`, but `site-footer-row/block.json` **declares none of those three attributes** (only `gridTemplateColumns`). Per D338 WordPress **silently discards** any attribute a block does not declare — so every footer row built from that template throws those values away at save, with no error and no failing build. The project's own documented silent-discard class, live in the footer today. **Fixing it IS FR-37-11** (columns become an operator-set count). **Status: OPEN** · **Bucket:** Framework · **Trigger:** FR-37-11.

> **P-TEMPLATELOCK-REORDER-GAP** — NEW 2026-07-21. `sgs/site-header` and `sgs/site-footer` set `templateLock: 'insert'` (`site-header/edit.js:94`, `site-footer/edit.js:115`), and both files' comments claim *"operators can't add/remove/reorder rows"*. WP's `'insert'` prevents adding and removing but **still permits moving** — an operator can drag the bottom row above the top one. Fix is `'all'`; row content stays freeform because `templateLock` does not cascade (both row blocks set `false` at their own level, verified). Folded into Spec 37 §3.3a. **Status: OPEN** · **Bucket:** Framework · **Trigger:** FR-37-9/10 audit.
