...
>
> **P-L2-CONTENTWIDTH-UNIVERSAL** — the `__inner`/`__card-inner` content-band `max-width` drops (trust-bar renders full-width not capped to 1100; gift/ingredients/social/featured have NO `contentWidth` attr). Per Spec 29 L2 is UNIVERSAL across ALL container-equivalents — this is ONE gap (the container-mirror sync `sync-container-wrapping-blocks.py --apply` is report-only, so the mirrored `contentWidth` was never written to those composites) PLUS a fold-routing fix (the `__inner` fold must route content-band `max-width`→`contentWidth` even when it also carries the grid, on recognised composites). NOT 4 separate fixes. **Status: OPEN** · **Bucket:** Framework / blocks + Pipeline / converter · **Trigger:** next session Task 4.
>
> **P-FEATURE-GRID-AUTOFLEX-COLUMNS** — `sgs/feature-grid` `layoutMode` defaults to `auto-flex` (render.php:79-88 uses `repeat(auto-fill, minmax(...))` which ignores the column count) → clone renders 3-col not the draft's 4. Fix = the converter sets `layoutMode` to the fixed mode + maps the transferred columns to `columnsDesktop/Tablet/Mobile` (the attrs render.php reads in fixed mode — note this block HAS `columnsDesktop`, unlike trust-bar, so the D259 Desktop→base collapse interacts). Universal principle: a block default must not override a transferred draft value. **Status: OPEN** · **Bucket:** Pipeline / converter · **Trigger:** next session Task 4.
>
