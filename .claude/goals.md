...
| **Spec 36 — Navigation System** | **PARTIAL — Phase 1 DONE; mega PROVEN (Gate 3); drawer desktop variants BUILT, Task-5 gate owed** | Shared `store('sgs/nav')`, `sgs/nav-menu`, `sgs/nav-drawer` live on both sites. **Mega menu PROVEN live 2026-07-28 (D401, Gate 3 closed)** — fixture page 1842, all 6 motion effects measured, axe 0 on the OPEN mega + drawer. **Drawer desktop variants BUILT + council-fixed 2026-07-28 (D403/D404)** — per-device `anchor` (incl. centred pause-menu), 7 complete-clone variations, backdrop-click-to-close; **Task 5 exit gate owed** (7 exact-content fixtures + per-variant sweeps + Bean's eye — LEDGER ⭐). **Plain (non-mega) dropdowns ARE BUILT** — `nav-menu/render.php:393-425` carries a real DROPDOWN branch that reuses the mega interactivity store (hover-intent, keyboard, ESC, focus-return, single-open, WCAG 1.4.13). **Main remaining:** Phase 2 utility pieces (mini-cart/search/social/business-info FR-36-19..23) deployed-unexercised; completion map `.claude/reports/2026-07-22-spec36-completion-audit.md` + Spec 36 §6a (fresher). |
