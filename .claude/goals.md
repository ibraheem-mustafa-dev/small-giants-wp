...
| **Spec 36 — Navigation System** | **PARTIAL — Phase 1 (MVP) essentially DONE** | Verified completion map: `.claude/reports/2026-07-22-spec36-completion-audit.md` (4 DONE / 17 PARTIAL / 3 NOT-BUILT / 1 needs-live-DOM, each checked against code not Status lines). Shared `store('sgs/nav')`, `sgs/nav-menu`, `sgs/nav-drawer` live on both sites. **Main remaining work = the mega/dropdown spine** (FR-36-3/4/5/10/8/17/9a) — note the `sgs_mega_menu` CPT + resolver are BUILT but panels are never rendered into the bar (`nav-menu/render.php` flattens submenus). |
