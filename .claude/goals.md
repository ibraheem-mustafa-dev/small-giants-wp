...
**Last updated:** 2026-09-07. Status detail (D-numbers, what shipped today, current front) is
single-sourced to `.claude/LEDGER.md` — this file states GOALS + exit criteria only; it does not
duplicate day-to-day status.
...
| **Track B — 5 client builds** | Mama's Munches, Indus Foods, CMX Group, Snooza/Ophir, SGS Studio v2 — each shipped per zero-QC definition (sign-off touchpoints documented per client). Not re-verified since 2026-08-12 for any client below — check each site's own `sites/<client>/CLAUDE.md` before reporting progress. Mama's Munches is the live pipeline canary (most recently touched 2026-08-27). Indus Foods last touched 2026-09-19 (header/footer/nav-drawer/mega-menu content build on a new dedicated test site — `sites/indus-foods/CLAUDE.md`). Snooza/Ophir (`sites/snooza-chair/`) last touched 2026-08-03. SGS Studio v2 last touched 2026-04-19. **CMX Group has no `sites/` directory at all as of this check** — recorded STALLED 2026-08-12 (no contact in a long while, build overdue against an earlier hoped-for completion); re-engage before resuming work. Helping Doctors (a 6th site) remains PAUSED, not part of this 5. | `plans/archive/strategy/2026-04-21-step2-strategic-plan.md` §1, §5 |
...
| **Spec 35 / capability-routing doctrine** | Every inspector control that declares a capability actually implements it — no dead panels, no silently-discarded settings. This file states the goal only; the live register, current D-range, and open-item count are single-sourced to `.claude/LEDGER.md` — they change too fast to cache here. | `.claude/specs/35-BLOCK-INSPECTOR-UX-STANDARD.md` (active, Part O = the folded-in control-type contract) + `~/.claude/plans/go-track-1b-playful-hamster.md` (live register) — current status always in `.claude/LEDGER.md` |

## Completed

| Goal | Status | Notes |
|------|--------|-------|
| **Architecture programme (8 phases, 31 decisions)** | DONE | All phases shipped across 2026-05-21 → 2026-05-22. DB consolidation, per-client theme-snapshot model established, button presets migrated to WP 7.0 theme.json, `/sgs-update` rebuilt as v2 (stage count is declared in `sgs-update-v2.py` itself — never cached here), WP 7.0 alignment (the whole block roster (count is DB-authoritative)). See `.claude/plans/archive/2026-05-21-architecture-staging.md`. |
...
| **Spec 37 — Header/Footer Builder** | **PARTIAL — core DONE + live-verified (2026-07-22, D359-D362)** | **Shipped + LIVE-VERIFIED via the real operator path:** CPT editing home + active pointer + "Set as active" (FR-37-1/2/25), direct-render binding w/ CPT-aware resolver (FR-37-3), "Active" list-table column (FR-37-5), thin template parts (FR-37-6 file step), and **legacy nav RETIRED — `sgs/adaptive-nav` + `sgs/mega-menu` deleted (FR-37-21)**, verified on repo + canary + production. **Shipped but NOT yet live-verified:** `templateLock 'all'` (§3.3a) and the footer column count (FR-37-11 — a footer CPT rendered, but the column count + mobile stacking were never measured). **Still open:** FR-37-33/34/35 (§3 gaps), FR-37-14/15 (tri-state + scoped behaviour CSS), FR-37-7/8 (starter picker), FR-37-26..31 (Simple/Advanced surface + a11y), FR-37-22 (converter emit, gated on Spec 33 Part 2). Full spec: `.claude/specs/37-HEADER-FOOTER-BUILDER.md` |
| **Spec 36 — Navigation System** | **PARTIAL — Phase 1 DONE; mega PROVEN (Gate 3); drawer desktop variants BUILT, Task-5 gate owed; block SPLIT 2026-09-14/15 (D1059/D1060)** | `sgs/nav-menu` was split into `sgs/nav-bar-menu` (flat bar + burger + dropdown/mega) + `sgs/nav-drawer-menu` (the drawer's accordion/drill-down list) — shared `store('sgs/nav')`, both new blocks + `sgs/nav-drawer` live on both sites. **Mega menu PROVEN live 2026-07-28 (D401, Gate 3 closed)** — fixture page 1842, all 6 motion effects measured, axe 0 on the OPEN mega + drawer. **Drawer desktop variants BUILT + council-fixed 2026-07-28 (D403/D404)** — per-device `anchor` (incl. centred pause-menu), 7 complete-clone variations, backdrop-click-to-close; **Task 5 exit gate owed** (7 exact-content fixtures + per-variant sweeps + Bean's eye — LEDGER ⭐). **Plain (non-mega) dropdowns ARE BUILT** — `nav-bar-menu/render.php`'s `SGS_Nav_Menu_Bar_Renderer` carries the real DROPDOWN branch (search the `DROPDOWN —` comment; don't cite a line number, it drifts — see the project's citation rule) that reuses the mega interactivity store (hover-intent, keyboard, ESC, focus-return, single-open, WCAG 1.4.13). **Main remaining:** Phase 2 utility pieces (mini-cart/search/social/business-info FR-36-19..23) deployed-unexercised; completion map `.claude/reports/2026-07-22-spec36-completion-audit.md`; live status = `.claude/LEDGER.md` (Spec 36 §6a's status table was deleted 2026-09-09 — it carried false claims; status single-sources to LEDGER). |
...
- **WooCommerce is NOT a non-goal — the SGS shop layer is in scope.** Spec 27 (SGS Product &
  WooCommerce Layer) wraps WooCommerce (WC = single source of truth, never mirrored); the
  Phase-1 sell-loop is live on canary page 589. Remaining Phase-1 hardening:
  ...
  PDP/shop/cart/search/schema/variation-gallery/notify-me) is COMPLETE — SGS is a sellable shop.**
