...
**Fidelity = transferring the draft's CSS onto the chosen block's EDITABLE attributes** (Spec 22 §FR-22-21 universal wrapper-conversion procedure): `align` / `maxWidth` / `contentWidth` / `gap` / `gridTemplateColumns` / `gridItem*` / background / padding. (`widthMode`/`customWidth` RETIRED D230/D231 2026-06-18.) The clone reproduces the draft AND stays editable + reusable.
...
| 0.7 | CSS lift (four-destination router) | `orchestrator/css_router.py` | `stage-7.json` (D0/D1/D2/D3 counts) + `variation-d0-d2.css` | LIVE (Spec 22 §FR-22-5). *(`css-d1-assignments.json` RETIRED — the D1 sidecar reader lives only in `_retired/`; see Spec 21.)* |
