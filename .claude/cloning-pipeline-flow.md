...
The current implementation has tracked gaps (content-width attr not built, fold drops `__inner` max-width, D1 typed-attr sidecar written-but-not-consumed so layout CSS strands in variation CSS, gridItem\* never written, composites don't mirror) — see Spec 22 §FR-22-21 + the standardisation plan (`.claude/plans/2026-06-02-container-wrapper-standardisation.md`).
