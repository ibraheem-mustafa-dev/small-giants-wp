...
- **STOP-28** — one converter engine, no flag, no fallback — the modular `converter/` engine is the only converter (`plugins/sgs-blocks/scripts/converter/entry.py` has no engine fork and nothing reads an `SGS_NEW_ENGINE` flag). Satisfied by construction; never reintroduce an opt-in engine flag, a second converter or a fallback path. Verify: `git grep -n "SGS_NEW_ENGINE" -- plugins` returns only comments.
...
- **STOP-TEST-DONT-GUESS (D337)** · **STOP-REUSE-THE-WORKING-BLOCK (D337)** ·
  **STOP-READ-THE-ENV-CONFIG** — read the environment config before assuming which environment
  you are on: `plugins/sgs-blocks/scripts/build-deploy.py::TARGETS` lists the deploy targets
  (`sandybrown` = the default canary; `indus-test` = the Indus Foods test site; `eye-care-test` =
  the Eye Care test site), and each has its own credentials in `.claude/secrets/<site>.env`.
