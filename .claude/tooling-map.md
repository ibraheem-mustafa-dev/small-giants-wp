...
| converter_v2/convert.py | plugins/sgs-blocks/scripts/orchestrator/converter_v2/convert.py | Slot-aware DOM-to-WP-blocks converter. ~1900 lines after Phase 9 work. Contains DB-driven block routing, CSS-driven container detection (`_detect_grid_container_from_css`), CSS-driven styling-attr lifter (`_lift_styling_attrs`), per-child mediaType detection, BEM-modifier-first hero variant inference, walker SGS-BEM grouping-wrapper preservation (added 2026-05-17, df3a6cbf), brace-balanced @media parser (2026-05-17, 20ef1d66), block-root supports lift via `_lift_root_supports_to_style` reading block_supports table (2026-05-17, 90692106). DB-first lookups for CSS-prop suffixes + breakpoint suffixes (168fd2ca). | NEW 2026-05-15, MAJOR EXPANSION 2026-05-17 | LIVE — production path for cv2-eligible boundaries |
...
| converter_v2/db_lookup.py | plugins/sgs-blocks/scripts/orchestrator/converter_v2/db_lookup.py | DB helpers for converter — block_attrs(), block_exists(), attr_name_for_slot_or_alias(), canonical_slot_for(), modifier_kind(), parse_sgs_bem(). 2026-05-17 EXPANDED for DB-first refactor (blub.db row 260 + Rule 11 HARD-GATE in /sgs-clone SKILL.md): adds `css_property_suffixes()` reading property_suffixes table (117 rows; 66 surface for cv2 lifter after `_kind_for()` filter), `breakpoint_suffix_rules()` reading modifier_suffixes (kind='breakpoint') with module-load DB verification, `block_supports_for(slug)` reading block_supports table (370 rows) for WP native style.* attr emission. Wraps sgs-framework.db + uimax.db queries with @lru_cache. | NEW 2026-05-15, EXPANDED 2026-05-17 | LIVE — imported by convert.py |
| converter_v2/test_root_supports_lift.py | plugins/sgs-blocks/scripts/orchestrator/converter_v2/test_root_supports_lift.py | Smoke tests for `_lift_root_supports_to_style()`. 3 cases: info-box with padding+border-radius+background; hero with padding+color; container with gap only. All passing as of 2026-05-17. | NEW 2026-05-17 (90692106) | LIVE — runs in CI manual gate |
| migrations/2026-05-17-property-suffixes-per-side.py | plugins/sgs-blocks/scripts/migrations/2026-05-17-property-suffixes-per-side.py | Idempotent migration seeding 18 per-side longhand rows into property_suffixes (PaddingTop/Right/Bottom/Left, MarginTop/Right/Bottom/Left, RowGap, ColumnGap, BorderTopWidth+3, BorderTopLeftRadius+3). Without these the DB-first refactor regressed hero attrs 58→50. Idempotent — re-runs are no-ops. | NEW 2026-05-17 (168fd2ca) | LIVE — applied to canonical sgs-framework.db |
| pixel-diff.py | scripts/pixel-diff.py | Python equivalent of scripts/screenshot-diff-helper.js (Node version requires pixelmatch which isn't installed). Playwright + PIL pixel diff. Forces device_scale_factor=1 (eliminates DPR noise). `--selector` flag for per-section cropped diff (mirrors screenshot-diff-helper.js). `find_body_start_y()` heuristic for body-anchored alignment when not using selector. **2026-05-18 EXPANDED:** new `--expected-rules <path>` + `--extracted-attrs <path>` paired flags compute `attribute_coverage` block in diff.json (pure converter score). Match algorithm: suffix-anchored property_suffixes lookup, `key.endswith(suffix)` OR `key.endswith(suffix + breakpoint_tail)` for tail ∈ {mobile, tablet, desktop, hover, focus, active, disabled}. Replaces an initial substring-match that the adversarial QC caught as false-positive risk. | NEW 2026-05-15, EXPANDED 2026-05-18 (397295c3) | STANDALONE — invoked by Phase 8/9 visual-QA workflow |

## NEW 2026-05-18 — Spec 16 Phase 9 pre-work: evidence infrastructure

| Script | Path | Purpose | Status | Wiring |
|---|---|---|---|---|
| orchestrator/expected_rules.py | plugins/sgs-blocks/scripts/orchestrator/expected_rules.py | Per-section CSS-rule baseline extractor. `write_baseline(section_html, full_css, run_dir, boundary_id)` writes `expected-rules-<safe-boundary>.jsonl` listing every CSS rule whose selector targets into the section subtree. Uses in-tree `convert.parse_css` (post-2026-05-17 @media-fix) + soupsieve for selector→DOM matching. Pseudo-elements/states stripped before matching; class-token fallback for unparseable selectors. Schema per JSONL line: `{selector, declarations, source_media_condition}`. Has CLI for standalone invocation; CLI selector resolution uses `soup.select(selector, limit=1)` (handles multi-class + descendant + pseudo-state correctly). | NEW 2026-05-18 (8b69bc0a + 397295c3) | LIVE — invoked by sgs-clone-orchestrator.py when `--debug-trace` is on, alongside Trace.for_boundary |
| trace.py:for_boundary | plugins/sgs-blocks/scripts/orchestrator/trace.py | New `Trace.for_boundary(run_dir, boundary_id)` classmethod. Returns a Trace bound to `convert-trace-<safe-boundary>.jsonl` in run_dir. Same per-process singleton + soft-fail discipline as `Trace.for_run`. Filename sanitisation regex `[^A-Za-z0-9._-] → _` matches expected_rules.py for paired-file diffing. | NEW 2026-05-18 (8b69bc0a) | LIVE — called by sgs-clone-orchestrator.py:1069 per section under --debug-trace |
| convert.py:set_trace + _trace | plugins/sgs-blocks/scripts/orchestrator/converter_v2/convert.py | Module-level `_TRACE` singleton + `set_trace(tr, boundary_id)` + `_trace(stage, **kw)` wrapper. `set_trace` atomically binds db_lookup's trace (single chokepoint). 14 walker_branch_taken emit sites cover the v3-plan 6 named labels: chrome_skip / fr1_block_root / composite_element / composite_element_no_standalone (Sonnet QC diagnostic emit) / css_driven_container / atomic_paragraph/heading/image/button / atomic_text_standalone / atomic_text_fallback / sgs_bem_wrapper / pass_through / top_level_container / fallback. attr_skipped roll-up at end of `lift_subtree_into_block_attrs` (value_empty + array_no_pattern_match reasons). | NEW 2026-05-18 (8b69bc0a + 10a93d87) | LIVE — emit no-op when _TRACE is None |
| db_lookup.py:set_trace + _trace | plugins/sgs-blocks/scripts/orchestrator/converter_v2/db_lookup.py | Mirror trace pattern. 4 `db_lookup_miss` emit sites: `canonical_slot_for`, `standalone_block_for`, `attr_for_slot`, `block_attrs`. lru_cache means each miss emits once per unknown lookup per process. | NEW 2026-05-18 (8b69bc0a) | LIVE — bound atomically by convert.set_trace |
| convert_v2/__init__.py:_convert_section_body | plugins/sgs-blocks/scripts/orchestrator/converter_v2/__init__.py | Private helper extracted from `convert_section`. Trace lifetime is wrapped in try/finally at the convert_section level: `set_trace(trace, boundary_id)` at entry; `set_trace(None, "")` at finally. Guarantees module-level _TRACE resets between sections, including on exception. | NEW 2026-05-18 (10a93d87) | LIVE — called inside convert_section's try-block |
| sgs-clone-orchestrator.py:--debug-trace | plugins/sgs-blocks/scripts/sgs-clone-orchestrator.py | New CLI flag. When ON, per-section Trace.for_boundary + expected_rules.write_baseline fire alongside cv2 dispatch. OFF by default in production register-tail runs. Recommended ON for /sgs-clone debugging walkdowns. | NEW 2026-05-18 (8b69bc0a) | LIVE — guarded by `getattr(args, "debug_trace", False)` |
...
| uimax_write.py | YES (via register_patterns.py) | YES via /sgs-clone path - Phase 6 v2 Step 5 2026-05-14 refactored register_patterns._insert_uimax_pattern to route through validate_and_write. STILL wired via /sgs-update path | Both |
| uimax-write-validator.py | YES | YES via /sgs-clone - transitively via uimax_write.validate_and_write since Step 5 2026-05-14. patterns table (register_patterns.py) + component_libraries (sgs-update-uimax-sync.py) both route through validate_and_write, getting row 213 (Rosetta Stone) enforcement. Non-artefact tables (gap_candidates) are unaffected because row 213 doesn't apply to them | Both |
| inheritance.py | YES (via preflight_chain.py) | YES (transitively via supports_writer.py, wired Phase 6 v2 Step 4c 2026-05-14) | Gemini Flash (I was wrong) |
...
| register_patterns.py | plugins/sgs-blocks/scripts/orchestrator/register_patterns.py | +REGISTER tail: writes PHP pattern file + inserts row into sgs-framework.db + inserts row into uimax patterns table after successful clone run. Phase 6 v2 Step 5 2026-05-14: refactored _insert_uimax_pattern to route through uimax_write.validate_and_write (Rosetta Stone chokepoint) instead of direct sqlite3 INSERT. Lazy-loader at _uimax_write() | Spec 15 Phase 6 Step 0; Rosetta Stone chokepoint Phase 6 v2 Step 5 2026-05-14 | CURRENT | YES - called after orchestrator_main returns success | Idempotent at 3 gates; module-only, no CLI; uimax writes are now validator-gated for row 213 Rosetta Stone (every artefact carries SGS-block mapping) |
...
| variation_router.py | plugins/sgs-blocks/scripts/orchestrator/variation_router.py | Single write path for token additions/updates during /sgs-clone: writes to client's style variation JSON, hard-blocked from mutating root theme.json | Spec 15 Phase 5d.3; wired Phase 6 v2 Step 4b 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py stage_4_5_6_7_8_extract loops every is_gap_candidate=true token_resolver result, derives slug via token-lint._generate_slug, calls add_token(client_slug, role, slug, raw_value, theme_root=REPO/theme/sgs-theme, write=True); appends (role,slug) tuples to per_section_results.new_tokens_written; soft-fails on exception | Has CLI for standalone invocation; writing bespoke detail as new tokens per feedback_cloning_preserves_intentional_bespoke_detail |
...
| attribute-staged-apply.py | plugins/sgs-blocks/scripts/orchestrator/attribute-staged-apply.py | Writes extracted block attrs to a STAGING file first, emits wp eval-file deploy command after operator approval; never touches live WP DB directly | Spec 15 Phase 5b.6; wired Phase 6 v2 Step 4i 2026-05-14 | CURRENT | YES (operator-gated; lazy-loaded by sgs-clone-orchestrator.py main() after stage_9_report, registered in sys.modules and added to stage_4i.modules_loaded so post-clone operator scripts can dispatch stage_change/approve/emit_deploy_command via the orchestrator's namespace) | FR21 compliant: staging + emit only |
| functionality-bulk-apply.py | plugins/sgs-blocks/scripts/orchestrator/functionality-bulk-apply.py | Stages and emits a transactional wp eval-file command for applying a new attribute across many existing block instances atomically | Spec 15 Phase 5b.7; wired Phase 6 v2 Step 4i 2026-05-14 | CURRENT | YES (operator-gated; lazy-loaded by sgs-clone-orchestrator.py main() after stage_9_report; same operator-dispatch pattern as attribute-staged-apply) | All-or-nothing atomicity; simulates apply Python-side before operator runs deploy |
| media-sideload.py | plugins/sgs-blocks/scripts/orchestrator/media-sideload.py | Uploads image-object slots from draft data to WordPress REST /wp/v2/media; returns attachment ID + URL for downstream attribute rewrite | Spec 15 Phase 5b.5; wired Phase 6 v2 Step 4i 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py main() calls media_sideload().sideload_batch(extract_out, mockup_root=args.mockup.parent, upload=False) automatically each clone; writes manifest to run_dir/media-sideload-manifest.json. Operator promotes with --upload via the module CLI. Soft-fails | Dry-run default; --upload flag for actual POST; authenticates via Application Passwords |
| supports_writer.py | plugins/sgs-blocks/scripts/orchestrator/supports_writer.py | Omit-vs-emit decision: skips writing a per-block override when the resolved value matches the block's supports native default (let cascade apply) | Spec 15 Phase 5d.4; wired Phase 6 v2 Step 4c 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py stage_4_5_6_7_8_extract loads block.json by slug (REPO/plugins/sgs-blocks/src/blocks/<slug>/block.json), calls filter_writes(block_slug, section_attrs, block_json, theme_json); supports_decisions + supports_emitted_attributes + supports_omitted_attributes appended to per_section_results; soft-fails on missing block.json | Runs at WRITE time; pairs with inheritance.py which runs at EXTRACT time; inheritance.py transitively reachable via supports_writer's optional import |
| stage1_boundary_hook.py | plugins/sgs-blocks/scripts/orchestrator/stage1_boundary_hook.py | End-of-Stage-1 hook: classifies source naming convention, runs lingua_franca on class_signature, enriches boundary with SGS-BEM primaries + equivalent_implementations | Spec 15 Phase 5c.4; wired Phase 6 v2 Step 4e 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py stage_1_boundary calls enrich_stage1_payload(output) before write_artefact; voter.json is rewritten with enriched payload so downstream stages pick up source_convention + primary_sgs_bem + equivalent_implementations + gap_candidate_classes + lingua_franca_skipped fields. Transitively wires lingua_franca.py. Soft-fails on exception | Production classifier dispatches to /uimax-classify-naming; injectable stub for tests |
| modifier_extractors.py | plugins/sgs-blocks/scripts/orchestrator/modifier_extractors.py | Three post-extraction classifiers: button_role() -> primary/secondary/ghost, dynamic_link() -> query descriptor for :latest-post() modifiers, match_block_variation() -> closest variation slug | Spec 15 Phase 5d.5 + 5d.6 + 5d.8; wired Phase 6 v2 Step 4d 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py per-section loop after supports_writer dispatch: button_role fires when target_block name contains 'button'; dynamic_link parses every section_attr value starting with ':' (only successful parses retained); match_block_variation fires when block.json declares variations. All three soft-fail independently; outputs land on per_section_results.modifier_signals | Called between Stage 4 and Stage 7 |
| wp_integration.py | plugins/sgs-blocks/scripts/orchestrator/wp_integration.py | Three WP wiring points: validate_block_markup(), route_native_feature() for lightbox/duotone, build_deploy_command() (builds but never executes) | Spec 15 Phase 5d.7 + 5d.9 + 5d.10; wired Phase 6 v2 Step 4j 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py main() calls wp_integration().validate_block_markup(aggregate_markup) automatically each clone before the autonomy gate. Validation status / errors / warnings land on run_dir/stage-4j.json. route_native_feature + build_deploy_command remain operator-gated; lazy-loader makes them reachable from post-clone tooling. Soft-fails when /wp-blocks CLI missing | Stage 7 + Stage 8 wiring |
| lingua_franca.py | plugins/sgs-blocks/scripts/orchestrator/lingua_franca.py | Converts external source naming conventions to SGS-BEM at scrape time; preserves original in equivalent_implementations per Rosetta Stone discipline | Spec 15 Phase 5c; wired Phase 6 v2 Step 4e 2026-05-14 (transitively) | CURRENT | YES (transitively via stage1_boundary_hook.py at module import time) | Handles BEM, Tailwind, and 3 other high-priority conventions |
| critical-fix-verification.py | plugins/sgs-blocks/scripts/orchestrator/critical-fix-verification.py | 5-check acceptance harness: root theme.json hash unchanged, no canonical block mutation outside FR21, plus 3 more boundary checks | Spec 15 Phase 5f.1 FR18; wired Phase 6 v2 Step 4k 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py main() calls critical_fix_verification().run_harness(run_id=so_run_id) after the +REGISTER tail. Aggregated check matrix lands at run_dir/critical-fix-verification.json. Soft-fails on missing optional inputs (theme hash, sgs_update runner) so a successful clone never breaks on a post-flight audit hiccup | Runs all checks even on failure; surfaces full picture to operator |
| composer_fallback.py | plugins/sgs-blocks/scripts/orchestrator/composer_fallback.py | Atomic-pattern fallback emitter: walks a section's DOM and emits a wp:sgs/container composition (core/heading + core/paragraph + sgs/button + sgs/decorative-image). Used when the autonomy chain can't scaffold — the Stage 7 safety net. Exports compose_atomic_pattern + 4 _emit_* helpers + _BUTTON_HINT_RE | Extracted from sgs-clone-orchestrator.py 2026-05-14 (Phase 6 v2 Step 6c) per the deterministic-not-inline rule | CURRENT | YES - sgs-clone-orchestrator.py stage_4_5_6_7_8_extract calls composer_fallback().compose_atomic_pattern(...) when target_block == 'core/group' OR confidence == 0. Lazy-loaded via composer_fallback() helper alongside token_resolver | Function name + signature unchanged from inline form. After Phase 6 the autonomy chain should cover most cases; this becomes truly cold-path code |
...
| attribute-gap-writer.py | plugins/sgs-blocks/scripts/recogniser/attribute-gap-writer.py | FR8 compliant: single entry point to INSERT rows into uimax.attribute_gap_candidates with mandatory provenance + run_id stamp | Spec 15 Phase 5a.4; wired Phase 6 v2 Step 4f 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py stage_9_report calls attribute_gap_writer().stage(gaps, run_id=run_dir.name, write=True) after the autonomy chain. Gaps harvested from extract.per_section_results via _harvest_attribute_gap_candidates() which maps token_resolutions[is_gap_candidate=true] -> {block_slug, selector, css_property, value_seen, role_proposed, confidence}. Result lands on stage_9 output.attribute_gap_writer. Soft-fails on uimax DB errors | Sibling of functionality-gap-detector.py; never auto-deletes rows; dedupes against (block_slug, selector, css_property) so repeat clones don't proliferate rows |
| functionality-gap-detector.py | plugins/sgs-blocks/scripts/recogniser/functionality-gap-detector.py | Spec 15 Phase 5a.3: detects when a draft DOM element expects behaviour (data-action, toggle, modal, scroll-reveal) that the matched SGS block does not provide; writes to uimax.functionality_gap_candidates | Spec 15 Phase 5a.3; wired Phase 6 v2 Step 4g 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py stage_9_report calls functionality_gap_detector().detect_batch(elements, run_id, write=True) after the attribute-gap-writer dispatch. Elements harvested via _harvest_functionality_gap_elements() which uses BS4 to walk the mockup under every matched section selector and emits an element dict for any element carrying a behaviour-fingerprint attribute (17 data-* / aria-* attrs) or an inline on*-handler. stage_9_report now takes an optional mockup_path kwarg threaded from args.mockup at the driver call site. Result lands on stage_9 output.functionality_gap_detector | FR8 counter-discipline: writes only, never deletes |
| gap-review-report.py | plugins/sgs-blocks/scripts/recogniser/gap-review-report.py | Phase 5a.5: generates a markdown gap-review.md report sorted by severity for the operator after each clone run | Spec 15 Phase 5a.5; wired Phase 6 v2 Step 4h 2026-05-14 | CURRENT | YES - sgs-clone-orchestrator.py stage_9_report calls gap_review_report().write_report(buckets_output, run_dir.name, out_dir=run_dir.parent.parent) after attribute-gap-writer + functionality-gap-detector. out_dir is the pipeline-state root (the module appends `sgs-clone/<run_id>/gap-review.md`). Written path lands on stage_9 output.gap_review_report_path. Soft-fails on rendering errors | Reads router output; groups by gap_level; emits to pipeline-state/<run_id>/gap-review.md |
...
| File | Path (relative to repo root) | What it does | Phase shipped | Status | Wired into /sgs-clone live? | Notes |
|---|---|---|---|---|---|---|
| uimax_write.py | plugins/sgs-blocks/scripts/uimax-tools/uimax_write.py | Single chokepoint validate-then-write helper for all uimax table writes; calls uimax-write-validator.py first; raises ValidationError when payload violates row 213 (Rosetta Stone — every artefact-table row carries equivalent_implementations.sgs_block) | Spec 15; chokepoint propagated Phase 6 v2 Step 5 2026-05-14 | CURRENT | YES via both /sgs-update + /sgs-clone paths since Step 5 (register_patterns.py + sgs-update-uimax-sync.py both route through validate_and_write) | Wraps the validator subprocess + INSERT in one call; no caller can skip the validator without bypassing this module |
| uimax-write-validator.py | plugins/sgs-blocks/scripts/uimax-tools/uimax-write-validator.py | Pre-write validator: rejects payloads missing equivalent_implementations.sgs_block on artefact tables (blub.db row 213 — Rosetta Stone) | Spec 15; row-211 IP-defence gate removed Phase 6 v2 Step 5 2026-05-14 | CURRENT | YES via both paths (transitive of uimax_write.py) since Step 5 | CLI contract: `python uimax-write-validator.py <table> <json-payload>` → `{"valid": bool, "errors": [str], "warnings": [str]}` on stdout |
