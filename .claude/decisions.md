...
**D81 — Hybrid block render.php migration is empirically scoped (Phase 0.4 audit).** Bean directive: spec doesn't hand-curate "container-shaped composite" lists. The classification dissolved; replaced by FR-22-6 `equivalent_block_for() returns non-NULL for ≥1 attr after role-exclusion`. Phase 0.4 audit produces roster (estimated 8-15 blocks from 63 raw, after FR-22-2.2 role-exclusion filters out typography/spacing attrs). Per-block migrations parallel-session-eligible per FR-22-6.1 (no shared-file edits in parallel agents — render-helpers.php additions sequenced through main session).
