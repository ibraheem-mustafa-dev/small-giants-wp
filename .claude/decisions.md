...
**D58 — Spec relocation (Phase 9C').** The `.claude/specs/` directory is exclusively for project-scoped framework specs going forward; cross-cutting / global specs move to `~/.claude/specs/` (user repo). Comparator reports (09-GOLD-STANDARD-AUDIT, 10-COMPETITOR-RESEARCH) move to `reports/` since they are output artefacts not architectural specs. Strategy / staging docs move to `plans/strategy/` (chrome-devtools-stage-8-integration) or `plans/archive/` (legacy mobile-nav specs, cloning-skill-salvage-matrix, pattern-dedup-classify-mechanics). 33 → 19 specs in `.claude/specs/`. `wp-studio-ai-manual.md` moved to `~/.claude/skills/wp-studio/` (active operational reference, not historical). Commits `ed05757f` (file moves) + `e8958433` (10 inbound-ref files updated). Cross-repo `~/.claude/commands/dev.md:37` update lands separately in user repo.

**D59 — Per-doc-type retention TTL on `.claude/memory/` (Phase 7 F4).** `next-session-prompt-*.md` → 30-day TTL; `handoff-*.md` → 60-day TTL; everything else permanent. Manual prune at `/handoff` until APScheduler runner picks it up post-Phase-1 migration. Encoded in `.claude/docs-registry.yaml` retention_rules block. Commit `c23d8948`.

## 2026-05-24 — Source-DB retirement: blocks.db + hooks.db deleted (architecture-staging Phase 1 close-out)

**D56 — Standalone source databases `~/.wp-blockmarkup-mcp/blocks.db` (459 KB) + `~/.wp-devdocs-mcp/hooks.db` (24 MB) deleted; their MCP cache dirs (127 MB + 496 MB git clones) also deleted.** Data migrated into `sgs-framework.db` with a `source` column distinguishing 'native_wp' / 'sgs' / 'third_party' (architecture-staging Phase 1 was declared "SHIPPED" 2026-05-21; in fact the data-side migration was incomplete and the read-path port was not done at all — empirically verified 2026-05-24 via schema enumeration). Closed this session by: (a) back-filling 122 missing variations + 331 missing markup_examples + 187 missing hooks via direct ATTACH + INSERT into sgs-framework.db; (b) porting `~/.claude/hooks/wp-blocks.py`, `~/.claude/hooks/wp-docs.py`, and `plugins/sgs-blocks/scripts/sgs-update-v2.py` Mode A / Stage 3 to read sgs-framework.db with source-column filtering; (c) `rm`ing the source DBs + parent + cache dirs after all 3 ports smoke-tested clean. Mode A is now a graceful no-op when source cache absent (Mode B `--refresh-upstream` is the canonical refresh path). Tables NOT migrated: `api_usages` (523 rows) + `block_registrations` (106 rows) + `indexed_files` (14,097 rows) + `sources` (8 rows) — these are scan-cache / internal MCP bookkeeping that regenerates on re-scan; `wp-docs.py` `search-apis` + `search-blocks` subcommands return retired-status stubs. Total disk recovered this work-stream: ~647 MB. New lesson `feedback_data_migration_needs_read_path_port.md` captured (write-side + read-side BOTH need to land before source deletion). Sibling lesson `feedback_shipped_claims_need_grep_verify.md` reinforced — verify migration completeness empirically before trusting any "SHIPPED" claim in a prior plan.

## 2026-05-24 — `block_compositions` table merged into `patterns.block_composition` (architecture-staging close-out)

**D55 — Pattern composition data moved from a standalone `block_compositions` table into the `patterns.block_composition` JSON column; the standalone table dropped from sgs-framework.db.** This is the single canonical record of the migration; references in active code/docs have been stripped (only this entry mentions the dropped table by name going forward).

**What changed**
- sgs-framework.db schema: dropped the `block_compositions` table (originally 37 rows, schema `id, composition_name, block_slugs, frequency, industry, page_type, description, auto_pattern_slug`).
- Added a `block_composition` JSON column on the `patterns` table to hold the same payload shape per pattern.
- Data move: 35 of 37 rows ported in; 2 orphans (pointed to deleted patterns) dropped. Result: 35 of 53 patterns carry composition data; the remaining 18 patterns either ship without composition metadata or need pattern rows registered first.
- Writers ported: `plugins/sgs-blocks/scripts/pattern-register.py` (helper renamed `_write_block_composition`; composes a JSON payload + UPDATEs the pattern row in place); `plugins/sgs-blocks/scripts/uimax-tools/seed-block-compositions.py` (now UPSERTs the JSON column; new `skipped-no-pattern` counter for pattern files whose slug doesn't match a registered pattern row).
- Verified: `python plugins/sgs-blocks/scripts/uimax-tools/seed-block-compositions.py --dry-run` reports 35 patterns with composition + 9 would-be-new (which need pattern rows registered first).

**Why** — composition data was always pattern-scoped, so a separate table created two-step inserts and an extra FK relationship for no read-side benefit (it's a write-only catalogue; the walker never reads it at clone runtime). Putting the payload on the parent row removes the join surface and keeps the write atomic with pattern registration.

**Out of scope** — read-side wiring. Pattern composition data remains WRITE-ONLY at /sgs-clone runtime. Parent-child block relations continue to be read from `blocks.parent_block` + `slot_synonyms.standalone_block`. The Wave 2 reshape (Decision 26) targets that wiring; this decision is the table-shape close-out, not the wiring change.
...
**Decision 26 — Wave 2 reshape: G1 + G3 + G5 are ONE wiring gap, not three problems.** `/wp-blocks dump` confirmed the SGS-framework.db has all the mapping data (`property_suffixes` 117 rows, `slot_synonyms` 89 rows, `block_attributes` 1755 rows, `modifier_suffixes` 19 rows). Cross-check found that pattern composition data (now on `patterns.block_composition` JSON column) is WRITE-ONLY in the current code (only `pattern-register.py` + `seed-block-compositions.py` write; nothing reads); parent-child block relations live on `blocks.parent_block` + `slot_synonyms.standalone_block` instead. The G1+G3+G5 symptoms (empty hero CTAs / Stage 3 text-only slot resolver / per-block DOM mismatches) are all manifestations of the same underlying wiring gap: cv2 doesn't walk all classes + assign CSS ownership + record parent-child relations using the DB tables that exist. Wave 2 reshape: ONE architectural change wiring the DB tables into the walker's emit shape, not three separate per-block fixes. FR1 fast-path "fix" reframed as a one-line consistency add (variation_buf.append after lift_subtree_into_block_attrs) — universal, not hero-special.
