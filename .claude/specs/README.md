...
| 43 | [43-SGS-CHOICE-FLOW.md](43-SGS-CHOICE-FLOW.md) | `sgs/choice-flow` — one step-wizard block family (not two) covering qualification quizzes, priced configurators (reusing `sgs/option-picker`'s tile pricing per-step, never a second pricing engine) and WooCommerce variation-picking (Mama's Munches: sequential full-screen steps via `sgs_modal`). Branching via a new per-step `nextStepMap`, distinct from `sgs/form`'s existing flat conditional-field logic. `sgs_choice_flow` CPT mirrors Spec 42's `sgs_form` CPT decisions exactly. Any purchase-ending flow inherits Spec 42's full pricing-security rule set verbatim. Superseded an earlier same-session research-council conclusion (two block families bridged by a link) after the owner corrected the premise against the real Ward End Eye Care draft — see `.claude/memory/research/2026-09-14-sgs-choice-flow-architecture.md`. | active (spec only — no code shipped yet) |
| 42 | [42-SGS-FORM-CPT-AND-PRICING.md](42-SGS-FORM-CPT-AND-PRICING.md) | `sgs_form` CPT (mandatory, slug-keyed identity, replaces inline-only forms per owner decision — full rebuild, no deprecation shim, D270-consistent) + generic per-tile pricing (one signed `priceDelta` + `basePrice`, no mode enum) + running total with server-side-only authority (client never submits a total) + revision-pinning (closes the price-versioning race) + reference-lifecycle contract + a standalone FR-42-0 fixing a live fail-open `requireLogin` cache bug found during the design review. Presets (Contact/Booking/Multi-step) stay an explicitly OPEN brainstorm item — FR-42-20. Produced via a full `/research-council` run (4-reviewer council + Council Head synthesis), 2026-09-14. | active (spec only — no code shipped yet; see FR-42-19 sequencing) |
