...
| 30 | [30-SGS-WOOCOMMERCE-PAGE-TYPES.md](30-SGS-WOOCOMMERCE-PAGE-TYPES.md) | WC page types — single-product/archive/cart/checkout templates, SGS search + searchable filter, option-picker WC binding, schema | complete |
...
| 35 | [35-BLOCK-INSPECTOR-UX-STANDARD.md](35-BLOCK-INSPECTOR-UX-STANDARD.md) | SGS block inspector-UX + block standards (no-inline / dynamic / feature-parity / shrink-to-fit; Part L controls, Part G native mechanisms, Part O control-type contract). Owns inspector-UX; read with Spec 32. Current completion state is single-sourced to `.claude/LEDGER.md` | active |
| 36 | [36-SGS-NAVIGATION-SYSTEM.md](36-SGS-NAVIGATION-SYSTEM.md) (v2.5) | **THE canonical SGS Navigation System** — nav bar (`sgs/nav-bar-menu`), mega CPT, off-canvas drawer (`sgs/nav-drawer-menu` + `sgs/nav-drawer`, `sgs_drawer` CPT) and utility pieces (cart/search/social/logo/business-info); classic-menu primary; WCAG 2.1 AA; crawlable; converter-emittable | active |
| 37 | [37-HEADER-FOOTER-BUILDER.md](37-HEADER-FOOTER-BUILDER.md) (v1.3.0) | SGS Header/Footer Builder — CPT editing home (header, footer, drawer, mega), container blocks, behaviours, binding | active |
...
| 43 | [43-SGS-CHOICE-FLOW.md](43-SGS-CHOICE-FLOW.md) (v1.3.0) | `sgs/choice-flow` — the step-wizard block family for qualification quizzes, priced configurators and WooCommerce variation-picking; branching via `nextStepMap`; purchase-ending flows reuse Spec 27's add-to-cart proxy; `sgs_choice_flow` CPT. Phases 0-2 built; Phases 3-5 not started | active |
