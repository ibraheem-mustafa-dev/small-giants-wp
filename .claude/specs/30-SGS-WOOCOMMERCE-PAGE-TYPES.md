...
revision_history:
  ...
  - v2.0 2026-06-12: SPEC COMPLETE (D220). All four phases shipped and Bean R-22-13 signed off. P1 (FR-30-0/1/2/7/4, D210) → P2 (FR-30-8 value-ladder + notify-me D217, gallery D218, FR-30-10 reviews deferred as empty-state shipped) → P3 shop layer (FR-30-3/5/6, D213/D214) → P4 schema (FR-30-9 Organization/WebSite/noindex/returnPolicyCountry, D215) → FR-30-13 go-live checklist (D220). FR-30-12 product-page clone UNGATED (cloning thread's call). Each FR marked SHIPPED below. Status → complete.
related:
...
> **STATUS: COMPLETE — all phases shipped, Bean R-22-13 signed off, P2 merged to main (D220, 2026-06-12). SGS is a sellable shop.** FR-30-12 product-page clone ungated (cloning thread's call).
...
### FR-30-2 — PDP composition (single-product.html) — **SHIPPED** (D210, 2026-06-11)
...
### FR-30-3 — Shop archive UX shell — **SHIPPED** (D213, 2026-06-11)
...
### FR-30-4 — Cart / checkout / Mini-Cart styling — **SHIPPED** (D210, 2026-06-11)
...
### FR-30-5 — SGS product search block — **SHIPPED** (D214, 2026-06-12)
...
### FR-30-7 — option-picker → cart wiring — **SHIPPED** (D210, 2026-06-11; sgs/buybox wrapper block)
...
### FR-30-8 — Price display (per-unit + value-ladder) — **SHIPPED** (D210/D220; value-ladder + DMCC-compliant reference price; notify-me D217)
...
### FR-30-9 — Schema completeness per page type — **SHIPPED** (D215, 2026-06-12)
...
### FR-30-10 — Reviews (DMCC-compliant) — **SHIPPED** (D220 — empty-state toggle + DMCC guardrails; live Trustpilot sync deferred until first client with reviews)
...
### FR-30-11 — Responsive + budget verification gate — **SHIPPED** (gated each phase close; Bean R-22-13 3-breakpoint sign-off at D220)
...
### FR-30-12 — Pipeline gate (cloning thread) — **UNGATED** (D211/D220; cloning thread's call; product-page clone queued after homepage ledger families close)
...
### FR-30-13 — Go-live checklist — **SHIPPED** (D220; `.claude/go-live-checklist.md`, 31 items; Turnstile keys are the only operator action required before first real-money launch)
