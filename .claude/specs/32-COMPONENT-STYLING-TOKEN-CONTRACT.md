...
>
> Family roster below is counted over the current **83** blocks.
...
### (c) Family roster — merge vs keep-scalar (counts over 83 blocks)
...
> **Counts** come from counting blocks declaring the tier sibling
> (`git grep -l '"paddingTablet"' -- 'src/blocks/*/block.json' | wc -l` → 39; `marginTablet` → 41;
> `borderRadiusTablet` → 11). Re-run these commands as the library grows.

**KEEP scalar — 4 families (not box properties, or single-side):**

> (A header count must be re-derived from its table, never adjusted by arithmetic off a prior figure.)
...
| `shapeDivider{Top,Bottom}` + `…Colour/Flip/Height/Invert` | **6** — container, cta-section, hero, **site-footer**, **site-header**, trust-bar | Not a box property — two independent decorative SVG slots each with its own sub-settings; `{top,right,bottom,left}` is semantically wrong (no left/right divider). Keep the named-slot structure. |
...
| testimonial-slider/style.css:296 | `surface` | **OUT OF SCOPE** — owned by another workstream, left untouched | — |
