...
| **FR-32-5** per-client tokens at `settings.custom.{component}Presets` | ✅ **DONE** | The MECHANISM is complete and live-proven: editing only `buttonPresets.primary.text` in the snapshot moved the live button `rgb(58,46,38)` → `rgb(255,0,255)` with no block-code change, then reverted cleanly. **Adoption is not completeness** — the snapshots carrying `buttonPresets` are the real client builds; the template/demo sites render correctly on the FR-32-6 fallback (§8, FR-32-6 row). A count of who has *used* a mechanism is not a measure of whether it *works*. The button reads every FR-32-9 role: colours and `hover-transform` as `--sgs-btn-*`, and the geometry roles (`border-width`, `border-radius`, `font-size`, `font-weight`, `padding`, `min-height`) directly in `plugins/sgs-blocks/src/blocks/button/style.css::.sgs-button--primary` (and `--secondary`, `--outline`), each with the framework look as its fallback; a per-instance setting still wins. `border` and the hover colours accept `currentColor` and `color-mix(in srgb, <colour> N%, transparent)`, so an outline preset can follow its section's text colour |
