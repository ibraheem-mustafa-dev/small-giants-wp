...
| Variable-product configurator Phase 2 — **IN PROGRESS** (D168, 2026-06-04). SHIPPED + live-verified on canary 540: Step 0 foundations (`class-configurator-meta.php` single meta registry [SEC-4 digit-strip + SEC-8 variesBy enum + per-object auth] + `configurator-head.php` wp_head emitter scaffold + `seed-48-sku-fixture-v2.php`); **B2 swatches** + **I2 build-time WCAG auto-contrast** (`option-picker` + `configurator-term-fields.php` authoring UI + `render-helpers.php` luminance); **TAX-UI** per-card `taxDisplayMode` auto\|inc-suffix\|ex-plus-vat (FR-27-H3, qc-council-passed). | PARTIAL (D168) | `includes/class-configurator-meta.php`, `includes/configurator-head.php`, `includes/configurator-term-fields.php`, `includes/render-helpers.php`, `includes/class-product-manifest.php`, `src/blocks/option-picker/`, `src/blocks/product-card/`, `scripts/seed-48-sku-fixture-v2.php` | D168 | FR-27-B2/I2/H3 done; B3/A4/C2/E1/E2/E3/F1/PREFLIGHT/demand-analytics remaining |
