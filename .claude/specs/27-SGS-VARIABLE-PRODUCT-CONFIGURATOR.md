...
|---------|--------|---------------|-------|--------------|
| `sgs/product-card` Typed mode (InnerBlocks wrapper shell) | SHIPPED | `src/blocks/product-card/block.json` (v1.3.0), `render.php` | D148 | FR-24-2 |
...
| Add-to-cart via WC Store API (R-22-14-clean) | SHIPPED (commit `6bcdf48c`) | `render.php` (no-JS `<a>` fallback), `view.js` (pending guard) | D151 | FR-24-2 |
...
| Pill-to-price/image swap via WP Interactivity API (full live read) | DRAFT-Phase-1 (wired, dormant; live WC read-through is FR-27-A1/A2) | `src/blocks/product-card/view.js` | D151 | FR-27-A1/A2 |
...
| Variable-product configurator Phase 1 (live WC swap, accessible, secure) | DRAFT-Phase-1 | `src/blocks/product-card/`, `includes/class-product-bindings.php`, `/sgs/v1/cart/add-item` proxy | D-pending | FR-27-A/B/C/G/H |
...
- **Card:** the existing presentational cards, made dual-mode (FR-24-2). Typed mode equals the FR-22-6 InnerBlocks shape (Spec 22), so the clone pipeline is unaffected.
...
SGS thin layer
  ...
  |     forwards to WC Store API (server-authoritative; the single add-to-cart path)
...
Later content types (`sgs_testimonial`, `sgs_team`, `sgs_case_study`) follow the same declarative registration with their own field sets. The mechanism is shared (R-22-9).
...
Conditions are meta-driven, not hardcoded per content type (R-22-1, R-22-9). `contentType` whitelisted via `sgs_content_collection_post_types` filter (default: `['sgs_product', 'product']`). Count capped server-side 1-24.
...
The dual-mode card's Typed mode is exactly the FR-22-6 InnerBlocks shape the converter already emits (see Spec 22). The query-driven layer is additive. The converter keeps emitting Typed cards; Bound/collection is an operator-authoring feature, not a converter output.
...
**FR-24-12 -- Content-impact map drives card rendering, not block logic.** R-22-9.
...
5. **(Phase 2)** Best-in-class SEO + AI-discovery, server-rendered, full Merchant-Listings eligibility.
6. **(Roadmap, Phase R)** Friendly authoring that writes WC variations; agency slug-templates; AI-builder shop setup from a brief. Built when a 2nd shop client lands.
...
- **No mirror of WC commerce data.** Presentation/config only in term meta / variation postmeta / block attributes.
...
**FR-27-G1 -- Add-to-cart via the SGS proxy; WC server-authoritative.** The client POSTs `/sgs/v1/cart/add-item` (the single add-to-cart path) with the variation `id` + `quantity` + a `variation` array of `{attribute, value}` (global attrs = `pa_`-slug; product-local attrs = raw case-sensitive name), never a price. The proxy validates (G2 + caps) then forwards to WC's `/wc/store/v1/cart/add-item`, which recomputes price AND re-validates stock. Any price field is ignored.
- Done when: a tampered request (fake price / OOS / foreign ID / attribute-mismatch) is rejected or server-priced; the WC contract shape is exactly `variation:[{attribute,value}]`. Model: sonnet. Test: adversarial fixtures.
...
**FR-27-G6 -- No oversell + inventory-abuse defence (cacheable).** Oversell protection is a single server stock re-check on the explicit add-to-cart click only (page stays fully cacheable). Plus, in the proxy: per-SKU quantity cap = `min(requested, floor(stock * 0.3))`; per-fingerprint (IP+token-hash) 30 s cooldown per SKU; document `woocommerce_hold_stock_minutes` = 15. Cache freshness: purge the page on `woocommerce_variation_set_stock_status` AND `wc_scheduled_sales` completion (sale start/end) AND a manual price edit, covering edge/CDN, not just object cache. The manifest carries `generated_at`; client-side, if age is over 1 h, the add-to-cart click triggers a manifest refresh before the call.
...
**FR-27-B2 -- Swatch modes via WC attribute term meta.** Text/colour/image swatches via `term_meta` (`_sgs_swatch_color`, `_sgs_swatch_image_id`; `absint()` + `wp_attachment_is_image()` validated, `wp_get_attachment_image_src()` with graceful fallback + authoring-time validation feedback). Typed mode carries optional swatch fields on `optionItems`. Model: sonnet. Test: swatch authored + a11y + image-validation + Typed swatch.

**FR-27-B3 -- Per-unit (derived) + discount-label (cosmetic) + server-rendered % off.** Per-unit £/unit DERIVED at render from live WC price divided by `_sgs_unit_divisor` (not a stored note). Discount-type label is cosmetic `postmeta`, save-time-rejected if it contains a numeric %. "% off" computed server-side from WC regular/sale, seeded as a `pctOff` literal per variation (no client getter). Model: sonnet. Test: sale + divisor + label fixtures; assert derived-per-unit, server % off, %-reject.

**FR-27-C2 -- OOS vs nonexistent distinct + announced.** Model: sonnet. Test: OOS + nonexistent fixture; axe + SR.

**FR-27-A4 -- Per-variation gallery.** Gallery IDs in variation `postmeta` (`_sgs_variation_gallery`), prefetched per payload spec; fallback variation image then parent image. Model: sonnet. Test: gallery swap + fallbacks.

**FR-27-E1 -- ProductGroup + hasVariant JSON-LD, Merchant-complete, SSR.** `ProductGroup` (`productGroupID` = parent ID/SKU; `variesBy` via an explicit operator-set `_sgs_variesby_value` term_meta mapped to the closed enum color/size/material/pattern/suggestedAge/suggestedGender; unmapped axes omitted from `variesBy` but kept as free-text child properties; `brand` from WC brand/attribute; `aggregateRating` from `get_average_rating()`/`get_review_count()` when reviews exist; `AggregateOffer` low/high + `offerCount` = true total) + `hasVariant` (at most 50 children) each with `sku`, identifier (`gtin13` from `global_unique_id`; else `mpn` from SKU; else `"identifier_exists": false`), absolute `image` (at least 250px, fallback parent, descriptive `alt`), `isVariantOf`, nested `Offer` (`price`, `priceCurrency` = `get_woocommerce_currency()`, `priceValidUntil` = the scheduled sale-end date if on a scheduled sale, else OMITTED; never a fabricated rolling date; `availability`, canonical `url`, `itemCondition` default `NewCondition`). `shippingDetails` + `hasMerchantReturnPolicy` at ProductGroup level. Emitted via `wp_json_encode()`; all `url` via `esc_url()` + same-origin. Model: sonnet. Test: Rich Results Test (0 errors; unmapped-axis warnings OK) + Merchant preview + currency-at-request.

**FR-27-E2 -- Canonical (+ optional indexable escape hatch).** `?attribute_*` redirects to `rel=canonical` to parent; no indexable thin pages by default; a per-variation `indexVariationUrl` block attribute (default false) promotes a high-intent variation when justified; canonical hreflang-neutral. Model: haiku. Test: default canonical + opt-in promotion.

**FR-27-E3 -- Supporting schema + freshness.** `BreadcrumbList`; `og:type=product` + price/availability OG tags; a product XML sitemap with `<image:image>` per variation image (SGS-generated, mandatory; OR a Playwright-verified Yoast/RankMath integration, not just "documented delegation"); `Last-Modified` + cache-purge on the FR-27-G6 hooks so feed + page never serve a stale price. Model: sonnet. Test: breadcrumb/OG/sitemap valid + purge-on-price-change.

**FR-27-F1 -- All commerce content in SSR HTML.** Price/availability/copy/JSON-LD in the initial response (AI crawlers do not run JS). Model: sonnet. Test: `curl` (no JS) shows price/availability + JSON-LD.

#### Phase R -- Roadmap (authoring + AI-builder; build when a 2nd shop client lands)

Each Phase-R FR is fully specified so the build is unambiguous, but is not on the critical path. Until then, the 48-SKU fixture is authored once via WC's classic metabox.

**FR-27-R1 -- SGS authoring controller (wraps WC data-store classes).** `/sgs/v1/` controller using `WC_Product_Variable`/`WC_Product_Variation` `set_*()`+`save()` (NOT raw postmeta, NOT same-server REST batch as primary) + explicit post-write `wc_delete_product_transients()` + attribute-lookup regenerator + `woocommerce_update_product`. `permission_callback` = per-object `edit_post`; every write validates `X-WP-Nonce` (CSRF); per-user rate-limit; multisite blog-context guard. A golden-master diff test vs the native editor (dump postmeta + term relationships + lookup rows; diff must be empty) runs every WC major.
...
**FR-27-PREFLIGHT -- Go-live + setup pre-flight check.** Before a configurator product is publishable, a check (admin + agent-callable) verifies: WC at least 9.8; every variation has price above 0 (no £0); each has at least 1 image; published (not draft); manifest at most the cap; JSON-LD passes a local Rich-Results check. Surfaces a client-legible "ready / N issues" report. Model: sonnet. Test: a deliberately-misconfigured product surfaces each issue.
...
**FR-27-F2 -- AI-citation levers + secure feed (Phase R).** `FAQPage` JSON-LD from an `sgs/product-faq` block (NEW, defined here) or an existing `core/details`/`sgs/accordion`; `llms.txt` (product names + categories only, safe) AND `llms-full.txt` (full prices/attrs), both filtered to `post_status='publish'` AND `post_password=''` AND `catalog_visibility NOT IN (hidden,search)`, rate-limited, `X-Robots-Tag: noindex`, regenerated on `woocommerce_update_product`; `speakable`; a Merchant feed (GTIN, `item_group_id` = `productGroupID`) agreeing with on-page schema, read only from `wc_get_product()`, descriptions `wp_strip_all_tags()`, image URLs same-origin/allowlist (no SSRF), public-but-rate-limited.
- Done when: FAQ schema fires; `llms.txt`/`llms-full.txt` leak no draft/hidden products + are rate-limited + noindex; feed agrees with schema + injection/SSRF-safe. Model: sonnet. Test: schema/feed parity + draft-exfil probe + SSRF probe.
...
- **Phase 1 -- MVP (approximately 1-2 weeks / 3-4 sessions).** FR-27-A1, A2, A3, A5, B1, C1, G1, G2, G3, G6, H1, H2, H3, I-MVP. Mama's sells: live WC swap, secure no-oversell add-to-cart, accessible card, availability past the cliff, cacheable + tax-correct, lab-INP budget.
- **Phase 2 -- Display + SEO (approximately 1-1.5 weeks).** FR-27-B2, B3, C2, A4, E1, E2, E3, F1, PREFLIGHT, I2, I3.
- **Phase R -- Roadmap (build behind a 2nd shop client).** FR-27-R1-R5, F2. Multi-session; not on the critical path.
- Each phase: /qc-council per configurator/converter commit; design-reviewer + performance-auditor + a11y pass; Bean visual sign-off.
