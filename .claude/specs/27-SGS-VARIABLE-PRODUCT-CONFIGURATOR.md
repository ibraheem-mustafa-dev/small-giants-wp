...
| Variable-product configurator Phase 2 — **CLUSTER A COMPLETE** (D171, 2026-06-04); Bean R-22-13 sign-off granted. SHIPPED + live-verified on canary 540: Step 0 foundations; **B2 swatches** + **I2 build-time WCAG auto-contrast**; **TAX-UI** (FR-27-H3); **B3** per-unit "£x/bar" derived live + cosmetic badge reusing `sgs/label` pill-wrap + on-sale "Sale" badge + WC variation-editor authoring (ceb4e04a / 5fe7cfd5); **A4** per-variation gallery + thumbnail strip (delegated click + imperative rebuild) + prefetch + fixed-220px-box object-fit-cover + editable `imageHeight` attr/var + media-picker authoring (77dccc9f / 48fc54b7 / 5fe7cfd5); **C2** 3-state sold-out-vs-unavailable SR text; **Step-7 demand analytics** privacy-safe REST (`/sgs/v1/demand/attempt`, SHA-256 rate-limit, ZERO PII) + admin (771f43ad); ESCAPE-AUDIT + QA-VIS(axe-0) gates passed (28607ac4). | PARTIAL (D171) | `includes/{class-configurator-meta,configurator-head,configurator-term-fields,configurator-variation-fields,render-helpers,class-product-manifest,class-product-bindings,class-demand-analytics,demand-analytics-admin}.php`, `src/blocks/{option-picker,product-card}/`, `scripts/seed-48-sku-fixture-v2.php` | D171 | Cluster A done (B2/I2/H3/B3/A4/C2/Step-7). Remaining: Cluster B SEO (E1/E2/E3/F1) → Cluster C authoring + PREFLIGHT |
| Authoring controller + AI-builder (Phase R) | ROADMAP-Phase-R — capstone, build LAST (gated on converter-pipeline + DB + theme/blocks + WC all production-ready; D168.1) | - | D168 | FR-27-R |
