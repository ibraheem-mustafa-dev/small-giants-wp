...
references:
  ...
  - .claude/plans/archive/2026-07-13-header-footer-nav-system-design-gate.md (Bean-approved 2026-07-13 — names Part 2's concrete emit target — as `sgs/adaptive-nav` at the time; that block was DELETED at Spec 36 Phase-1 close (2026-07-20) and the current target is `sgs/nav-bar-menu`+`sgs/nav-drawer-menu`+`sgs/nav-drawer`, and makes those blocks a consumer of this spec's `theme-snapshot.json` for global-style defaults, §4b)
