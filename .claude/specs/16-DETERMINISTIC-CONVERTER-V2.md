...
status_note: |
  ...
  2026-05-25: /qc-council 4-rater verdict (D75) — CONDITIONAL APPROVE pending F1 spike (per blub.db row 276). Phase 1 scope = full universal-extraction backbone per Section R of `.claude/reports/2026-05-25-qc-council-issue-register.md`. Plan = `.claude/plans/2026-05-25-phase-1-universal-extraction.md` (19 commits). Old phase-1 plan archived. G1-G5 closure tracked across Phase 1 commits 1B/1E/1F/1G; ARRAY_LIFT_PATTERNS deletion is Commit 11 (1D); patterns.block_composition population is Commit 16 (1I).
...
> **Drift corrections 2026-05-25:**
>
> 1. Earlier wording referenced a non-existent `slot_list.py` file. The actual code lives across two files: `sgs-clone-orchestrator.py:stage_3_slot_list` (lines 1102-1204) builds a metadata SCAFFOLD only (no CSS extraction); CSS-to-attr extraction lives in `convert.py:_lift_styling_attrs` (line 3120).
>
> 2. A subsequent patch in the same session prescribed extending `convert.py:_slot_attr_prefix` (line 3093) with a stem-stripping fallback as the universal G3 fix. That prescription was **empirically falsified** — a Sonnet implementer ran the fix end-to-end and demonstrated it addresses only 7 of the 473 extraction_failed events (the rest are legitimate `value_empty` for features genuinely absent from mockups: hover states, bgVideo, transitionDuration). The fix-shape is removed from this section pending re-framing around the canonical primitive in §15. Lesson captured at `~/.claude/projects/c--Users-Bean-Projects-small-giants-wp/memory/feedback_read_full_spec_before_proposing_architectural_fix_shape.md` (D71).
>
> 3. The Stage 11 pixel-diff regression on featured-product / ingredients / gift-section / social-proof — the OPERATIONAL symptom G3 was meant to close — was closed independently on 2026-05-25 (D70) via Stage 10 inline-CSS deploy of `variation-d0-d2.css`. Mean pixel-diff 74.1% → 68.4%; body sections down 15-41pp; gift-section -39pp at desktop, social-proof -40pp at desktop. See decisions.md D70.

**Current G3 framing (post-D70 + D71 corrections):**

- **Pixel-diff side of G3** — CLOSED via D70. The CSS that mockup authors wrote (backgroundImage, overlayColour, minHeight, etc.) now reaches the live page via the inline `<style id="sgs-cv2-page-css">` injection in Stage 10.
- **Failure-count side of G3** — the original `< 30` numeric gate from the 2026-05-20 council is **empirically misframed**. 440 of 473 extraction_failed events are `value_empty` for features genuinely absent from mockups; defaulting them would be a semantic lie. The remaining ~33 failures are split across (a) ~7 visual-only slots blocked by `_slot_attr_prefix` returning None and (b) ~26 attrs with no canonical_slot in DB. Either is addressable but neither is high-leverage.
- **Universal closure path** — the architectural primitive in §15 line 969 says every composite block emits OPEN with InnerBlocks children mirroring the mockup's parent-child shape (not flat-attrs lifted from descendants). When that primitive extends from hero (already shipped at `ad706d0d`) to every composite, the trust-bar / testimonial-slider / etc. classes of `extraction_failed` dissolve because each BEM-class div becomes its own emitted block carrying the mockup className. Tracked as `P-G1-EXTEND-TO-OTHER-CONTAINER-SHAPED-COMPOSITES` in parking.md.

**No fix-shape prescription in this section.** Any further G3 work should be scoped against §15's universal-nesting primitive, not against prefix derivation or slot resolver internals.
...
> **2026-05-25 quick reference (qc-council session D75 — the actionable summary):**
>
> - **Universal-nesting primitive (line 990 below, locked binding):** every composite block emits OPEN with InnerBlocks children mirroring the mockup's parent-child shape — NOT flat-attrs lifted from descendants. Every BEM-class div in the mockup becomes its own emitted block.
> - **F1 fallback insertion point:** `plugins/sgs-blocks/scripts/orchestrator/converter_v2/convert.py:1430` — the `if not child_slugs: return []` early-return in `_lift_inner_blocks`. Replace early-return with universal-walk: walk node's direct child elements (divs + semantic tags), call back into `walk()` recursively. Single source of truth — all three flatten-path callsites (composite_element at 3989, essence_match_variation at 4060, standalone-block branch at 4114) benefit from one change.
> - **DB-driven ATOMIC_TAG_MAP (R1 of qc-council register):** walker queries `SELECT slug FROM blocks WHERE replaces=? AND source='sgs'` for the SGS replacement at tag-recognition time. 18 SGS blocks declare `replaces` (verified via sgs-db). Cheat Q14 (hardcoded `ATOMIC_TAG_MAP` at convert.py:698) replaced in Phase 1B (Commit 9 of phase-1 plan).
> - **Worked example:** brand `sgs/quote` end-to-end translation — see Section R5 of `.claude/reports/2026-05-25-qc-council-issue-register.md`. Mockup has `<div class="sgs-brand__quote"><p>...</p><p>...</p><p>...</p><p class="sgs-brand__attribution">...</p></div>`. Current converter emits self-closing `sgs/quote` with empty `body[]`. Post-F1 emits OPEN with 3 nested `sgs/text` children (each `<p>` routed via FR2 atomic-tag + DB-driven ATOMIC_TAG_MAP); attribution lifts to scalar attr on parent (existing path).
> - **Phase 1 acceptance gate:** per-section ≤30% × 3 viewports for all 7 body sections (21 cells; current baseline mean 63.2%). NOT mean — per-section. Hero attribute-count parity gate during cheat removal (per R4 mitigation).
> - **F1 spike is HARD GATE** before full Phase 1 dispatch (Commit 7 of phase-1 plan, per blub.db row 276). Brand alone; if drops ≥20pp at 1440 → proceed; otherwise halt.
...
| Extend G1 OPEN-block emission to every composite block (not just hero) | The architectural primitive: every BEM-class div in a mockup is an emitted block. Composite blocks (trust-bar, testimonial-slider, card-grid, etc.) emit OPEN with InnerBlocks children matching mockup parent-child shape — NOT flat-attrs lifted from descendants. Mockup `className` carried per child block. Closes the failure-count side of G3 automatically because each `__badge` / `__text` / `__icon` div becomes its own emitted block with the mockup class on it. <!-- 2026-05-25 D71 — replaced earlier `_slot_attr_prefix` prescription that was empirically falsified (addressed 7 of 473 failures). Pixel-diff side closed by D70 inline-CSS deploy. --> |
...
ARRAY_LIFT_PATTERNS hardcoded dict (convert.py:1008-1031) deletion is Commit 11 (1D) of `.claude/plans/2026-05-25-phase-1-universal-extraction.md` — replaced via DB-driven array extraction recipes. Per R1 of the qc-council register (D75), `count_stars` becomes universal COUNT-children-by-slug, NOT a "special extractor kind".
