...
created: 2026-06-17
status: active
status_detail: ADVERSARIAL-COUNCIL-CORRECTED (GO on a clean modular stage-by-stage rebuild, 2026-06-17) — §12 is the authoritative build direction. Foundation Phase F + modular scaffold COMPLETE (D243); build is in §12.6 step 3 (stage-by-stage), partial through D250/D254/D256; array-item content lift COMPLETE + LANDED (D258). Current front = A1 media-map + the min-width cross-device tier fix + composite-fidelity. D-ceiling D258.
...
>    - **Icon content resolves to one of THREE kinds (2026-07-03):** a **Lucide slug**, a **bare emoji glyph**, or a **raw `<svg>`** — via the shared `field_extractors.resolve_icon_kind(element)` (which composes `resolve_icon` for slug/svg + emoji detection). Emoji detection lives in this ONE shared resolver, so every icon path (leaf, array item, nested child) gets it uniformly — an emoji icon is NOT a per-block case. A resolved icon sets its block's attrs by kind: `iconName`+`iconSource='lucide'`, `emojiChar`+`iconSource='emoji'`, or `iconSvg` (when the block declares it).
>    - **The named-leaf arm (`run_mechanism_leaf`) covers the `identity` (icon) role, not only text/image/link (2026-07-03).** An **icon-bearing leaf** — a `has_inner_blocks=0` block with an `identity`-role attr (e.g. `sgs/icon`, whose `primary_content_attr` is NULL) — routes through the named-leaf arm (DB signal = an `identity` attr, never a slug literal), where the icon arm lifts its icon via `resolve_icon_kind`. Without this, an icon leaf child (e.g. an info-box's first child) emitted EMPTY — the identity role was uncovered. Decorative leaves (no primary + no identity) still fall through to the Case-4 loud gap.
...
- **Resolution — RESOLVED → F-ii (preserve), Bean-mandated (D228, §9 Q1); per FR-31-5.2:** map draft `@media` thresholds that ARE device-tier values (767/768/1023/1024) to the tier attrs; **faithfully preserve** any non-device-tier threshold as a raw uid-scoped rule (passthrough), or log to D3 if no passthrough path — **never snap, never drop**. (F-i/F-ii is LOCKED to F-ii per §9 Q1 — not an open question.) **IMPLEMENTED (D259) — the device-tier CASCADE, §13.4 FR-31-5.2:** `collect_css_decls_for_element` computes the effective value per tier by sampling the cascade at 768/1024 interior widths (Desktop→base, Tablet/Mobile→suffixed), inverting mobile-first drafts and handling min/max symmetrically; the prior substring marker-match (which silently dropped `min-width:600`) is retired. Non-device thresholds emit a non-silent F-ii residual log.
...
>
> **WHAT'S LEFT TO BUILD (prioritised; each is a named §12 stage, NOT an ad-hoc gap):** — NOTE (D254): items 1 + 7 below are now DONE; see the D254 STATUS block above. Remaining priority is A1 media-map (item 4) + the Bean-review composite-fidelity defects #3-7.
...
> 4. **media-map loader (A1)** — the new engine has no media-map driver, so image `src`s are not remapped to uploaded WP URLs. **NOW THE #1 PRIORITY (Bean-review defect #3, D254)** — only the hero image + trust-bar icons show; every other image is broken. One of the two STOP-28 gates for flipping the production default.
...
- **FR-31-5.2 — responsive `@media` → device-tier CASCADE (the F-fork implementation; D259).** A draft `@media (min-width|max-width)` rule resolves to the SGS device-tier attrs by **cascade sampling**, NOT by matching the raw threshold string to a marker table (the retired substring match SILENTLY DROPPED any non-marker threshold, e.g. `min-width:600`). The device system is fixed at **768/1024** (Mobile `<768`, Tablet `768–1023`, Desktop `≥1024`); the SGS **base (unsuffixed) attr IS the Desktop value**. Algorithm (`collect_css_decls_for_element`, `db_lookup.device_tier_samples`):
  1. Compute the **effective value at each tier's representative interior width** (Desktop 1440 / Tablet 800 / Mobile 375) by applying the element's non-media cascade + every `@media` rule that applies at that width. `min-width:X` = "X and up" therefore populates every tier whose sample ≥ X; `max-width:X` = "X and down" every tier whose sample ≤ X — **one symmetric calculation, both directions.**
  ...
  3. A threshold **outside the canonical device set {767,768,1023,1024}** falls strictly inside a tier's range → a sub-tier band the 3-tier attr model cannot represent (D228 "arbitrary visual breakpoint"). Per the §3 F-ii lock it is **preserved as an F-ii passthrough — surfaced (logged), NEVER snapped to a tier, NEVER silently dropped.** (Rendering the raw residual band via a uid-scoped passthrough-CSS channel is a tracked follow-up; the covered device tiers are captured in full by step 1–2.) The device-tier suffix vocabulary (Mobile/Tablet/Desktop) stays DB-owned via `modifier_suffixes('breakpoint')`; only the 768/1024 boundary widths are the R-31-1 permitted-constant (same class as `SKIP_TOP_LEVEL_TAGS` — no DB table of CSS pixel boundaries).
