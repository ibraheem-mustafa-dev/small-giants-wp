...
status: active
status_history:
  - 2026-07-25: v0.6 — C2 partially closed: 0 WRITTEN-not-LANDED, verified against the committed oracle artefact `plugins/sgs-blocks/scripts/tests/fixtures/phase-f/_render-oracle/batch-report.json` (re-run 2026-07-30); 33 UNVERIFIED + 33 GUARD-FAIL remain, so §5's "zero CHEAT cells, zero UNVERIFIED cells" bar is not yet met. The 35-fixture canary corpus was re-provisioned through the current converter and the live LANDED batch (375/768/1440) reports 0 WRITTEN-not-LANDED + 0 UNACCOUNTED on the sandybrown canary (R-31-11/R-31-13 — Bean's eye on product-card/notice-banner/hero URLs). The last two fidelity gaps closed (`9babcfd5`): product-card root/body padding now LANDS via a `cardPadding` content-body box-object + a DECLARATIVE `fold_helpers` per-area padding router (name-guess kept as fallback, hero parity-neutral); block-root `text-align` folds to native `textAlign` (root_supports) + the 4 dead-support blocks (notice-banner/collapsible-text/icon-list/timeline) now render it (16/16 textAlign blocks audited). sgs/quote no longer emits a nested quote for its body (seed-layer fix). Shared converter guards (db_lookup OUTER-element, outer_box box-family padding) qc-council-validated + adversarially refuted. Deferred (deploy-gated, non-blocking): sgs-quote conformance-golden re-seed (bundled with the 25 pre-existing stale goldens); tracked residuals `P-QUOTE-PATH2-SELF-NESTING`, `P-OLDSHAPE-AUDIT-TEXTALIGN`. The fresh 2026-07-30 run reads `LANDED: 31, UNVERIFIED: 33, GUARD-FAIL: 33, NOT-RENDERED: 8, unattributed: 393`. The shift vs the prior 2026-07-23 run (LANDED 37→31, GUARD-FAIL 23→33) is NOT a fidelity regression — 30 of the 33 GUARD-FAILs fall on the five `rt-*` red-team fixtures (`rt-centred-maxwidth` 11, `rt-background-url` 9, `rt-pseudo-before` 5, `rt-video-media` 3, `rt-media-600` 2), i.e. exactly the §12.7-F1 fixtures built to exercise the known HIGH gaps; only 3 touch real blocks (accordion, option-picker, tabs). **Remaining to close C2: triage those 33 UNVERIFIED + 33 GUARD-FAIL + 393 unattributed cells** (owed since `96bfeb66`). Caveat on method: `batch_runner.py` only PROBES already-deployed pages — deploying is out of its scope — so this run measures the corpus as deployed, not the current working tree.
  - 2026-07-24: v0.5 — added §13.4 FR-31-23 (preset-absence transfer): un-routed preset-selector attrs (`cardStyle`/`effectHover`) resolved from the cloned element's collected CSS via a new AUTO-DERIVE seed (`/sgs-update` parses each block's style.css → `preset_implications` table, keyed on `block.json supports.sgs.presetSelectors`) + `converter/resolvers/preset_absence.py` resolver with companion-attr reconciliation, a fidelity-floor rule (unmatched shadow → `elevated`, never neutral) and base-element/tier reconciliation scoping (`5807205c`/`4ff379f1`). 5-block roster (info-box/team-member/card-grid/testimonial/google-reviews); post-grid excluded (dynamic).
...
> **Scope (D246):** ALL draft→block transfer through the one container dispatch — content (text/media/array/child-block) AND CSS — not CSS alone. Read the section title "CSS-Transfer" as "Content + CSS Transfer."
...
|-------|---------------------|---------------|
| **0.7 `css_router.py`** | Splits the draft `<style>` into 3 destinations: **D0** global/reset, **D1** typed-attr lift→block attrs (token-snapped), **D2** wrapper-CSS scoped `.page-id-N` (**D3 gap-candidate DB retired 2026-09-02**) | Routes to D1, but its D1 output is **not consumed** by Stage 4 (the merge was deleted 2026-05-27, convert.py:807) |
| **Stage 2** block-match | Picks the block for a section; no confident match → `sgs/container` fallback | Composite recognition (`.sgs-hero`→`sgs/hero`, `.sgs-trust-bar`→`sgs/trust-bar` with `variant` set) is not a live bug — it already works. Sections that emit `sgs/container` (featured-product, brand, ingredients, gift, social-proof) have no registered composite block, so container is the correct slug-None target, not a miss. The Stage-2 work is a PORT: reproduce this already-correct recognition + variant logic inside the fresh `converter/` engine. Baseline/oracle = the draft + the empty new-engine, never the frozen output. |
...
| **Stage 9** | 5-bucket leftover router (DB-backed gap-detection **RETIRED 2026-09-02**) | The "honest gap" mechanism is preserved via in-memory ledger tracking, not DB-backed candidates. See retirement note below. |
...
> **D440 (2026-08-01): `_absorb_transparent_wrappers` (with `_is_absorbable_wrapper`) is DELETED** from `converter/services/section_passes.py` — measured to fire 0 times across 46 real invocations, and its own `margin`-disqualifies-a-wrapper rule rejected the exact `max-width`+`margin:0 auto` pattern that IS the L2 CONTENT band above. Do not re-add a margin/gap/padding disqualifier to the fold/recurse test on this basis. **D441 (2026-08-01): a relational L2 qualifier — `converter/services/l2_qualify.py` — is built and measured but UNWIRED.** Its trigger is RELATIONAL, not per-element (Bean, D439): the direct PARENT being a recognised container-kind block is what makes the child a fold candidate; the child's own identity is never the input. It reproduces the §2.7 acceptance table 5/5 on the real homepage draft. Do not build a second, competing qualifier — wire this one in (register: `plans/2026-08-01-wrapper-recognition-cascade-rework.md`).
...
|-------|-------------|------------------|
| **L1 OUTER box** | the section-root element itself | native `style.*` + **`align`**/**`maxWidth`**/`padding*`/`background*`/`min-height`. **Rule (UPDATED 2026-08-22 — `align` REMOVED from the L1 model; supersedes D230 `484d04d9` / D231 `d5416ae8`):** `max-width` ABSENT → **emit NOTHING** (`maxWidth` already defaults to `{}` = no cap = full-bleed, so absence is the honest translation of absence); PRESENT → `maxWidth` (string **literal**, exact draft value — decimals+unit preserved, no 5% theme-snap, no `int()` truncation) + responsive `maxWidthTablet/Mobile` at 767/1023. **`widthMode`/`customWidth` RETIRED.** ⛔ **`align` RETIRED FROM THIS LAYER TOO (2026-08-22).** Measured on the live canary: stripping `.alignfull` from an element in a real `.wp-block-post-content` context changed nothing — left, width and all four margins identical, and an A/B against an unaligned sibling was byte-identical. Core's breakout rule resolves `calc(var(--wp--style--root--padding-left) * -1)` against a variable that is EMPTY at `:root`, so the declaration is invalid and dropped. `align:"wide"` never had a matching rule at all (its only rule needs `.is-layout-constrained`, which `sgs/container` never emits). No SGS-BEM draft can express either — there is no such CSS property — so emitting `align` invented a WordPress idiom the draft never stated, failing the R-1 honest-mapping test. `supports.align` removed from `sgs/container`; the DB-driven gates in `outer_box.py:align_finalise()` and `assembly.py` self-disable once `block_supports` no longer carries the row. **STATUS (D250, 2026-06-30): The native WP `style.*` lift (`_lift_root_supports_to_style` / `_root_lift_rules`) is PORTED into the new engine as `converter/services/root_supports.py`** — `padding`/`background-color`/`border-radius` now emit nested `style.spacing.padding`/`style.color.background`/`style.border.radius` respectively; the conductor partitions block-supported props out of the per-declaration Decl stream before handing off to outer_box. New engine only; frozen `convert.py` unchanged. |
| **L2 CONTENT-WIDTH (inner band)** | `--content-width` custom-prop decl (deterministic) OR `max-width`+`margin:auto` signature (scraped fallback) | `contentWidth` (token `normal`→content-size / `wide`→wide-size / `full`→no cap, OR a **literal**; **default `normal`** (~content-size) as of 2026-08-21 D706 / commit `2d291992` — it was `full` from D231 until then, and this row still said so for a full day after the flip; `full` still resolves to `''` = no cap, only the DEFAULT moved. A container that is NOT page content (a `<main>`, a sidebar `<aside>`, a flex cluster) must now say `full` EXPLICITLY or it silently gains a band + gutter) + `contentBandPadding*` (via the `max-width ≡ width` CONTENT-layer suffix equivalence) + an inheritable **`text-align`** on the band → the block's WP-native `textAlign` support (D267 — see FR-31-5.1a; cascades to the band content, child-overrideable) |
...
8. **No destination?** → log to the in-memory honest-gap ledger (**DB-backed `attribute_gap_candidates` retired 2026-09-02 — the mechanism is preserved via in-memory tracking**). NEVER silent-drop, NEVER emit as inline `@media` (responsive values go to tier attrs — §3 F-fork), NEVER fall to D2 scoped CSS when a D1 attr destination exists (R-31-15c).
...
| `block_attributes.role='scalar-media'` | **RESTORED 2026-08-02 (D474)** — opens `run_mechanism_b` branch A, the ONLY path reading an image's `--mobile`/`--desktop` modifier to route art-directed pairs. ⛔ `emit_shape='nested'` superseded only the *no-child-block* half of this role; the device-tier half has NO replacement. Roster + rationale: `scripts/data/scalar-media-roles.json`; re-asserted at module load and gated by a value-identity assertion in `check_value_identity.py` (renamed from `check_row_floor.py` 2026-08-07; its row-count floor was deleted and replaced by the report-only `dbschema/seed_history.py`). The role's single holder is `sgs/hero.splitMediaType` — the real, rendered media-family selector (D919, live-verified). Routing covers all 3 device tiers (desktop/tablet/mobile) and all 3 media types (image/video/SVG); the type-selector attr must be written alongside stored video/SVG content or that content is never selected for render. |
...
| `attribute_gap_candidates.(proposed_action)` | **RETIRED 2026-09-02** — the DB-backed gap-detection mechanism was never finished and the project owner confirmed it was never wanted as a completed feature. The "honest gap" mechanism is preserved via in-memory ledger tracking (not this table). |
...
- **GAP** — no destination attr or no property_suffixes row (→ seed via the canonical path; tracked in in-memory ledger, DB-backed `attribute_gap_candidates` retired 2026-09-02).
...
2. **Hardcoded property→attr dict — R-31-1.** `_SUFFIX_ATTR_OVERRIDES` (convert.py:972) and `prop_map` (convert.py:1519) are legacy violations in the FROZEN `convert.py` (§12.0/§12.4); the completeness gate BASELINES them rather than requiring their removal, since they vanish when the modular rebuild replaces that code path. Do NOT edit convert.py to remove them.
...
5. **Mirror-emit / `sourceMode='bound'` / BEM-element className — gate WIRED (D238, `2341e761`).** `check_no_mirror.py` auto-runs post-clone via the orchestrator (after `stage_9_report()`, pre-deploy) and HARD-HALTS on a NEW mirror violation; 13 legacy violations grandfathered via baseline. A commit-time static `sourceMode='bound'` AST tripwire (`scripts/cheat-gate/check_bound_emit.py`) was added D241.
6. **D2-when-D1-exists — DB cross-join at pipeline close, not grep.** A Python check at pipeline end: for each property stranded in `variation-d0-d2.css`, query `property_suffixes`+`block_attributes`; if a D1 destination exists for that (block, property), FAIL. *(D3 gap-candidate DB-tracking retired 2026-09-02; the honest-gap check persists via in-memory ledger.)* *(HOLE 6.)*
...
- **the DB-backed `attribute_gap_candidates` ledger is RETIRED 2026-09-02** — the gap-detection mechanism was never finished and the project owner confirmed it was never wanted. The "honest gap" concept is preserved via in-memory ledger tracking. `leftover-buckets.json` remains a debug aid. Do NOT trust input-side counts as a per-clone fidelity signal (Spec 20 §FR-20-7). The rendered signal is Stage 11.6.
...
| **D** dropped max-width | §3 step 3 — max-width by layer. RESOLVED for the OUTER box via the D230/D231 width-model upgrade (SHIPPED 2026-06-18): OUTER `max-width` → `maxWidth` literal (exact) or `align:"full"`; `widthMode`/`customWidth` retired. L2 band → `contentWidth` (token/literal). |
...
No-suffix-row property class (seed row OR exclude-with-reason; 2026-07-04: only transform/filter/transition still lack rows — the rest were seeded D250) · pseudo-elements (`::before`/`::after` — the `::`-as-media-separator PARSE bug FIXED 2026-07-04 `3d7e7d42`; the LIFT destination remains, EXECUTION Step 13) · broad-except fail-silent → fail-CLOSED on every ledger/DB path · `has_inner_blocks` handling SUPERSEDED by FR-31-2.6 `emit_shape` (source-derived seeder; readers migrated EXECUTION Step 8) · scalar-media `<video>`/caption swallow (no content-dropping `continue`) · classification exhaustiveness (unknown slug → hard fail, no empty-content emit) · non-device-tier breakpoints (600px) preserved or UNACCOUNTED.
...
>
> Build history D250→D274 (frozen-engine rollout, `SGS_NEW_ENGINE` flag, STOP-28 gate, and the D256 §2 layer-extraction landing) is entirely superseded by D276 below — see `decisions.md` for the full narrative. The `min-width:X` cross-device tier handling D256 listed as its remaining gap is specified at §"BOUND — a residual is confined to the DEVICE TIER its threshold falls inside" (F-ii / `serialise_residual_bands`).
>
...
> - **New failure contract (Rule 4).** A section that fails to recognise, or whose `build_block_markup()` call raises or returns empty/non-`wp:` markup, returns `status:'failed'` + a `failure_reason` string and logs at ERROR — it is never silently swallowed into an empty `sgs/container`. **Known gap (found by post-programme QC) — RESOLVED.** The original wording here said `sgs-clone-orchestrator.py` (~lines 1510-1530) did not yet branch on `status:'failed'`. Re-verified against the live file: it now DOES branch on `result.get("status") == "failed"` (`sgs-clone-orchestrator.py` lines 1478-1518) — it appends a descriptive line to `aggregate_errors`, emits a `stage_4_converter_failed` trace event, and keeps the section in `per_section_results` with `status: "failed"` so the Stage-9 operator queue surfaces it (the section is omitted from the emitted markup, but never silently — its comment there reads "Rule-4 loud path (post-programme QC fix, 2026-07-05)"). The entry.py contract and the orchestrator-side consumption of it are both correct.
...
>
> **BUILD LIST — SUPERSEDED BY D276 (2026-07-05): the completion programme it was governed by EXECUTED IN FULL.** Read each item below as historical, and verify against the current tree before treating any of it as open. Retained for the per-item reasoning, not as a work queue. The original framing follows: each is a named §12 stage, NOT an ad-hoc gap; governed by the approved completion plan + its EXECUTION plan (`plans/2026-07-04-*`) — items 2/3/5/6 map to EXECUTION Steps 12 / 11 / 13 / 15; item 4 (A1) is RESOLVED.
> 1. **DONE (D254):** the new engine defaults a no-name-match section to `sgs/container` + recurse-descends children (2/9 → 9/9), LANDED on page 8. Universal full-width + chrome-skip also shipped.
> 2. **CSS-branch resolver completeness (§3.A / §5) — UPDATED, resolvers now exist.** Re-verified against `converter/resolvers/outer_box.py` (EXECUTION Step 12, 2026-07-04): `order`/`z-index`, `overflow`/`overflow-x`/`overflow-y`, `object-fit`, `position`/`inset`/`top`/`right`/`bottom`/`left`, `aspect-ratio`, `opacity`, gradient `background-image`, and flex-item `flex-grow`/`flex-shrink`/`flex-basis` ALL have a live LIFT resolver now (all routed through the same generic `attr_resolve`/`attr_for_layer_property` dispatch — no per-property special-casing). The residual is NOT a missing resolver: it is that most of these properties have no block declaring a matching DESTINATION ATTR yet (e.g. no block declares `overflowX`/`overflowY`, or any of the six `position`/`inset`/`top`/`right`/`bottom`/`left` attrs) — each such case is an honest, named `NO_DESTINATION` gap, not a silent drop. Container-level `flex` IS lifted via `arrangement.py`. LIVE resolvers today (verified 2026-07-04; **`grid_area` REMOVED 2026-08-16, D642 — it was dead code, its trigger `ctx.area_name` never set by any production Ctx-builder; the live grid-per-area path is `fold_helpers.route_area_css_to_block_attrs`, a different mechanism, keyed on the draft's BEM element token via `services/assembly.py` step 3d**): `outer_box` (box/bg/box-shadow + the Step-12 additions above), `root_supports` (native `style.*`), `grid` (full — template+count+gap), `content_band`, `typography`; `scalar_content`/`scalar_media` are honest REGISTRY stubs; `styling_content` is live but UNREGISTERED (its path is `lift_styling_content` via extraction.py). EXECUTION Step 12.
...
> 7. **DONE (D254):** the new engine is LANDED via `/sgs-clone` with `SGS_NEW_ENGINE=1` on the sandybrown homepage (page 8). Production-wire + retire `convert.py` is DONE — the frozen tree was deleted at D276 and the `SGS_NEW_ENGINE` gate now exists only in comments recording its removal (no `os.environ` read anywhere in `converter/`). Spec 22 ABSORBED (§13) + archived — DONE (D253).
...
| Pseudo-elements `::before`/`::after` never collected | **Stage 4b** (cascade resolver) | PARSE FIXED 2026-07-04 (`3d7e7d42` + regression tests); remaining work is the LIFT destination (uid-scoped passthrough or EXCLUDED-with-reason) — EXECUTION Step 13 |
...
**Implementation-gap fix, 2026-09-18 (D1106):** `converter/recognition.py::recognise_section`
was found deviating from FR-31-4 for a top-level boundary with ZERO BEM classes at all (as
opposed to classes present but unmatched) — it stayed `unrecognised` instead of taking the
container default, contradicting this FR's own "a slug-None node at any depth defaults to
`sgs/container`" (§2). Fixed via a new `is_boundary_root` parameter (default `False`,
preserving the correct FR-31-11 pass-through behaviour for a nested classless node inside an
already-recognised composite); only the boundary's own top-level root passes `True`. Real
measurement on a genuinely classless (Claude-Design) draft: 17→38 of 70 boundaries now
convert. Full trail: `.claude/decisions.md` D1106.
...
- **FR-31-2.6 — the nested-vs-child fork is a source-derived `emit_shape` fact; ONE universal per-attr walk replaces the `has_inner` block-level dispatch + Mechanisms A/B (added 2026-07-04, Bean-agreed; CANONICAL).**
  ...
  - **Guards RE-HOME (MEASURED 2026-08-01).** The per-mechanism invariants become explicit per-attr checks in the ONE walk, each with a test: the D212 mutual-exclusion (an attr cannot be both nested and child), per-unit conservation (every content unit yields ≥1 of {scalar-lift, child-block, tracked-gap} — no silent drop), G3 `accepts_allowed_blocks` validation on emitted children, and the `sgs/container` default-recurse. **Art-directed `*Mobile`/`*Tablet` media routing is a SHIPPED invariant (D919).** `content_attr_for_element` (`db_lookup.py:4986`) resolves purely on the draft's BEM element token and never inspects a `--mobile`/`--desktop` modifier; the device-tier routing is the scalar-media mechanism instead (Branch A, `extraction.py`), covering all 3 device tiers (desktop/tablet/mobile) and all 3 media types (image/video/SVG), so a draft's `--mobile`/`--tablet` art-directed image routes correctly. The type-selector attr (`splitMediaType`/`…Tablet`/`…Mobile`) must be written alongside the media value — stored video/SVG content with no type selector is never selected for render.
...
- **FR-31-5 — three-destination CSS router (D0/D1/D2, **D3 retired 2026-09-02**):** D0 global tokens → theme.json; **D1** typed-attr lift → block attr (when a `property_suffixes` row matches an attr the emitted block declares); D2 scoped variation CSS (`variation-d0-d2.css`, inline at Stage 10). (**D3 `attribute_gap_candidates` was never finished and the project owner confirmed it was never wanted; the honest-gap mechanism is preserved via in-memory ledger.**) The D1 child-attribution rule routes a `.sgs-X__Y` rule to the CHILD block when `equivalent_block_for(X, Y)` is non-NULL (the FR-31-2 fork).
...
- **FR-31-22 — box-object routing (added 2026-07-09, `no-inline-styling-design-gate` + `box-object-interface-contract`; ROLLOUT ONGOING).** Every genuine 4-side or 4-corner box-property family (§2.9 Axis 1 note; the 8 four-side + 2 four-corner families enumerated in Spec 32 §6.1) routes to ONE named-object attr `{top,right,bottom,left}` / `{topLeft,topRight,bottomLeft,bottomRight}`, not N flat per-side/per-corner attrs — driven by the §3.A step-3b cross-declaration accumulator. The 10 keep-scalar families (single-side margins + `shapeDivider*`) are explicitly excluded and stay flat scalar attrs.
  ...
  - **Per-side border-width longhand transfer (2026-07-24, `38f7c30a`):** a draft's `border-{top,right,bottom,left}-width` now ACCUMULATES into the `borderWidth` box-object `{top,right,bottom,left}` on the 8 blocks declaring it — exactly as per-side padding accumulates into `contentBandPadding` (§3.A step 3b accumulator). Shared `services/border_side.py::border_side_write` called from BOTH `outer_box.resolve` AND `content_band.resolve` (a border-carrying block routes to OUTER when its base layer is OUTER, to CONTENT when it is a leaf — D307). Gated on `box_family_for(block,attr)==attr` (DB classification, never a name regex — passes the FR-31-22.2 AST collision gate); name-free (`attr_resolve`+`box_family_for`); base-only (a Tablet/Mobile longhand honestly gaps — no `borderWidthTablet`); no render.php change (render already reads the 4-side object). Known edge (loud, not silent): `border-width` shorthand + a per-side longhand on one element collide on a shared key → hard conservation error; resolving cleanly needs shorthand→longhand expansion at extraction (follow-up).
...
- **FR-31-23 — preset-absence transfer (added 2026-07-24, `5807205c`; AUTO-DERIVE).** A cloned card-like block used to keep its `block.json` default preset value (`cardStyle=elevated` / `effectHover=lift`) regardless of the draft — painting a shadow/hover the draft never had. These **preset-selector** attrs are un-routed by the normal attrMap (no `css_property`), so the converter now resolves them from the cloned element's COLLECTED CSS.
  - **FR-31-23.1 — auto-derive seed (R-31-1, reseed-durable):** `/sgs-update` (`sgs-update-v2.py::_populate_preset_implications`) parses each block's OWN `style.css` — for each preset attr named in `block.json supports.sgs.presetSelectors`, it reads each enum value's class rule (the class prefix + resting/hover state are derived from the block's `render.php`, the authority: `.sgs-{block}--{value}` for `cardStyle`, `.sgs-{block}--hover-{value}:hover` for `effectHover`; google-reviews' descendant-scoped `.sgs-{block}--card-{value} .{child}` is handled by the generalised selector-token matcher), classifies the implied signal property (box-shadow / border / transform present-or-absent), and seeds one row per `(block, preset_attr, enum_value)` into the new **`preset_implications`** table `(block_slug, preset_attr, enum_value, implied_property, presence, is_neutral)`. No dict, no manual SQL — a full re-run reproduces the seed from the block.json declarations. The convention dicts `_PRESET_STATE_BY_ATTR` / `_PRESET_NEUTRAL_FALLBACK_NAME` are per-ATTR-NAME (universal, R-31-1-acceptable); move to a DB column only when a 3rd preset-selector attr beyond `cardStyle`/`effectHover` is added.
  ...
  - **FR-31-23.3 — fidelity floor:** a real draft shadow whose value the box-shadow token-snap (§3 step 3, `outer_box.py`) cannot match to a `design_tokens` preset — so `cardShadow` gaps + stays unwritten — still resolves to `elevated` via the raw-decl fallback. A real shadow must never silently become the neutral (no shadow) on the clone (qc-council finding, 2026-07-24).
  - **FR-31-23.4 — reconciliation scoping (`4ff379f1`):** `db_lookup.attrs_for_css_property_state` returns ≤1 companion attr unchanged (preserves e.g. card-grid's `card-tile`-scoped `cardShadow` byte-identical) but narrows a ≥2-match ambiguity by the same `_BASE_ELEMENTS`∪`css_layer=OUTER`+base-tier predicate as the sibling lookups — closing a LIVE bug where a per-child `imageZoomHover` leaked into the card's `effectHover`.
...
- **`wrapper-rendered-styling`** → assigned role `styling`, via TIER 2.4 (D499). The decorative
  families (`overlayGradientFrom`/`To`, `shapeDividerTop`/`Bottom`, `bgSvgContent`) are ones
  `sgs/container` — the block every composite mirrors (R-31-9) — deliberately declines to map: its
  `decorative` element declares `"clusters": []` with a written note that these are governed by
  dedicated controls outside the style clusters (`container/block.json`). Declaring attrMaps for them
  would REVERSE a standing architectural decision to close a reporting nuisance. Only `gridItemBorder`
  was a genuine attrMap case. Proof the opt-out does not by itself discharge a row: `sgs/container`
  declares `decorative` and still showed its OWN `overlayGradientFrom`/`To` in the bucket — the only
  thing that removes a row is a non-NULL `css_property`, which the decorative families will never
...
- **Wiring status:** D6 + D7 verdicts ARE consumed — `fingerprint_content_roles.compute()` runs both
  ...
  verdict can never be overwritten by a styling/technical one (D502). The reason wiring was paused
  for one day is worth keeping: D7 CONTRADICTED the 2026-08-05 hand investigation on 3 of its 7 rows
  (`separator.contentIconName`, `site-header-row`/`site-footer-row.rowSlot`) — a genuine judgement
  disagreement on identical evidence, not a bug. Auto-seeding over a considered human verdict is the
  failure this vocabulary exists to prevent; the resolution was Bean's call, not the detector's.
...
`.claude/plans/archive/cloning-pipeline-tier-migration-requirements.md` left undocumented at spec
level (that file is INPUTS — captured evidence, not a spec — and decisions.md compresses over
time; archived 2026-09-11 once R1/R8/R9/R10 all shipped and nothing load-bearing remained uncaptured).
