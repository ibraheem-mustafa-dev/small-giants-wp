...
**OPEN GAP (must be closed in the build, flagged by the 2026-06-01 qc-council Raters A+B).** The DB does not yet carry a mapping from a composite's **media-column BEM element** (`__media` / `__split-image`) to its **scalar attr target** (`splitImage` / `splitImageMobile`), nor the `--mobile`→mobile / `--desktop`→desktop disambiguation. The build must add this DB-driven mapping (candidate: a `block_composition` column or a `slots` convention row) — NEVER a hardcoded `if slug=='sgs/hero'` (R-22-1). Until that mapping exists DB-first, FR-22-19 is DESIGN-only.
