...
**FR-22-2 reconciliation.** A composite's scalar-pipeline image slot (`splitImage`, `splitImageMobile`) is classified with a **`image-pipeline` role** (styling-behaviour class) in the `roles` table, so `equivalent_block_for(slug, attr)` returns NULL for it → the FR-22-2.4 unresolved-attr path lifts it to scalar instead of emitting a `sgs/media` child. This is the FR-22-2.2 positive-allowlist gate working as designed (styling/behaviour roles return NULL), NOT a per-block bypass. One DB row per slot, no code branch.
...
3. **Walker preference rule** (inside the `is_class_section_block` emit path; R-22-3-compliant — an FR-22-2-routing refinement applied to composite interiors, NOT a 4th branch): for a composite's interior element resolving to slot S, if the composite has a `scalar-media`-role attr at slot S → lift the element's content into that scalar attr (the `--mobile`/`--desktop` BEM modifier picks the base attr vs its `+Mobile` breakpoint sibling via `modifier_suffixes`); else (the content column, unclaimed by any scalar) → fold its children into the bare `$content` InnerBlocks. R-22-1 clean (no per-block slug conditional — the rule reads `block_attributes` for the current composite).
...
**Build sequence (R-22-5; each a separate commit, FR-22-18 structural-parity gate).** (1) DB: add `image-pipeline` role + reclassify `sgs/hero.splitImage`/`splitImageMobile`; verify `equivalent_block_for` returns NULL. (2) DB: add the media-column-BEM→scalar-attr mapping mechanism (closes the OPEN GAP). (3) convert.py: composite slot-router inside the `is_class_section_block` path (DB-driven). (4) `/sgs-clone --debug-trace` canary; verify extract.json shape + **live-DOM gate**: `section.sgs-hero` has exactly 1 `.sgs-hero__content`, exactly 2 imgs (`--mobile` + `--desktop`), `h1` present, 2 `.sgs-button`, `.sgs-hero__content` innerText > 50; AND no other section's extract.json changed. Roll back fast (STOP #19) on any failure. (5) Bean visual sign-off (R-22-13).
