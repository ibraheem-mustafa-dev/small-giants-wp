...
> - **D108** — `block_composition` table CREATED. PK `block_slug`. Columns: `wraps_block` (TEXT), `composition_role` (CHECK in 'section-root'|'wrapper-shell'|'content-block'|'leaf'), `has_inner_blocks` (INT), `accepts_allowed_blocks` (TEXT JSON). 188 rows populated (one per registered block). Initial composition_role distribution: 2 section-root, 1 wrapper-shell, 165 content-block, 20 leaf. 15 blocks have has_inner_blocks=1. `wraps_block='sgs/container'` for 4 blocks: sgs/hero, sgs/cta-section, sgs/modal, sgs/quote. **(D152 2026-06-02: now 189 rows; 28 blocks have wraps_block='sgs/container' + the new `container_kind` column populated — the container roster.)** **Walker consumption code DEFERRED** (see D109 / §FR-22-17).
...
| Column | Type | Notes |
|---|---|---|
| `block_slug` | TEXT PK | One row per registered block (189 rows currently — post-D152). |
| `wraps_block` | TEXT | Slug of the block this one wraps as its outer rendered container, or NULL. Populated for the **28-block container roster** (all wrap `sgs/container`) post-D152 (pending 29th: content-collection register, D163). (Pre-D152 it was 4 rows: hero/cta-section/modal/quote.) |
...
| `container_kind` | TEXT CHECK | `section` \| `layout` \| `content` \| NULL. Added + populated D152 for the 28-block container roster (4 section / 13 layout / 11 content) (pending 29th: content-collection register, D163). NOT walker-read — it is the propagation key for mirroring sgs/container capabilities onto composites per §FR-22-21. |
...
Populated 2026-05-30 by `/sgs-update` Stage 1 extension. 189 rows (post-D152). Refresh on every `/sgs-update` run.
...
**PASS test (data layer — updated D152 2026-06-02).** `SELECT COUNT(*) FROM block_composition` returns 189. `SELECT COUNT(*) FROM block_composition WHERE wraps_block='sgs/container'` returns **28** (was 4 pre-D152; `sync-container-wrapping-blocks.py` rewrite validated "wraps children" detection + 3-KIND model; `container_kind` column added). `SELECT COUNT(*) FROM block_composition WHERE composition_role='section-root'` returns 2.
...
### FR-22-21 — Universal wrapper-conversion procedure (Bean-directed, 2026-06-02; the canonical TARGET that FR-22-4.1 + FR-22-5 + FR-22-19 implement together; status DESIGN — build-pending workstreams in the standardisation plan; A3/A4 built-uncommitted + A5/A6/B1 dispositions refined by /qc-council 2026-06-03 (D163); content-collection = pending 29th roster block)
...
- ~~**content-width attr not built**~~ — **SHIPPED 2026-06-03 (A1, commit 2f86d9e6, D159).** `sgs/container` gained `contentWidth` attr (string, default ""); render.php emits a guarded `<div class="sgs-container__inner" style="max-width:<contentWidth>;margin-inline:auto">` (only when `'' === $layout`); style.css/editor.css gain `.sgs-container__inner` rule; edit.js "Content width" control added. Container block.json 0.1.0 → 0.2.0. Step 3 now has a destination.
- ~~**fold drops `__inner` max-width**~~ — **SHIPPED 2026-06-03 (A2, commit 2f86d9e6, D159).** `_fold_layout_into_attrs` now lifts the folded `__inner`'s max-width into `contentWidth` (previously discarded). No longer strands in `variation-d0-d2.css`.
- ~~**outer max-width transfer broken**~~ — **SHIPPED 2026-06-03 (A2, commit 2f86d9e6, D159).** The slug-None top-level section path (convert.py ~line 2104) now sets `widthMode` from the section's OWN max-width (ABSENT → "full"/alignfull which escapes WP's `:not(.alignfull)` 1200px cap; PRESENT → "custom" + `customWidth`). **Note:** the `widthMode:"full"` band-aid at `db_lookup.py:2461` is the slug-RESOLVED path (C1, WS-3) and is still present — A2 only fixed the slug-None path.
- **gap forced to spacing-token** (A4, WS-1c) — step 4's `gap` (raw px allowed) is currently snapped to a spacing token, losing raw values. Still pending.
- **gridItem\* never written** (A6, WS-1c) — step 4's uniform-item path has no writer; `gridItem*` defaults are never emitted. Still pending. (D163: → WS-4 lift-only-gated sub-mechanism)
- **D1 typed-attr sidecar written-but-not-consumed** (B1, WS-2) (`seed_d1_sidecar` stub) — the D1 attr-lift sidecar carries layout CSS that the container emit does not consume back into native attrs; layout CSS strands in variation CSS instead of landing on block attributes. Still pending.
- **composites don't mirror + no auto-propagation** (WS-4) — composites lack the mirrored container capabilities and `/sgs-update` does not yet propagate them. Still pending.
...
**KIND-specific:**
- **content** (info-box, quote, testimonial, team-member, notice-banner, accordion-item, …) — SIMPLEST. Helper emits width/spacing ONLY → **no bg/overlay/grid → no double-emit, no renames**. Mirror the 8 width attrs (`widthMode`+3 viewport variants, `customWidth`, `customWidthUnit`, `contentWidth`, `maxWidth`). The block's own bg/border/colour stays on its own classes (untouched).
...
### FR-22-21.2 — Auto-propagation: `/sgs-update` Stage 11 (DESIGN → build-pending)
On every `/sgs-update`, after the `sgs/container` block.json is read, Stage 11 (`sync-container-wrapping-blocks.py`) MUST detect whether `sgs/container`'s attr surface changed since the composites were last mirrored (e.g. container `version` bump or attr-set diff) and, for each block in the `block_composition` roster with `wraps_block='sgs/container'` AND NOT `containerMirror:false`, re-mirror the KIND-scoped attr subset into the composite's block.json (idempotent; `--apply` gated). This makes a new container capability propagate to all composites with one `/sgs-update` run — zero per-composite manual edits (D2/D3).
...
| `block_composition` | 189 | **D108 2026-05-30 — NEW table** (+ `container_kind` column added D152 2026-06-02). PK `block_slug`. Columns: `wraps_block` TEXT, `composition_role` TEXT CHECK in ('section-root','wrapper-shell','content-block','leaf'), `has_inner_blocks` INT, `accepts_allowed_blocks` TEXT JSON, `container_kind` TEXT CHECK in ('section','layout','content'). 189 rows post-D152 (188 seeded D108 + sgs/option-picker). **28 rows** wraps_block='sgs/container' + container_kind populated (the container roster — D152, was 4 pre-D152) (pending 29th: content-collection register, D163). Populated by `/sgs-update` Stage 1 + `sync-container-wrapping-blocks.py`. wraps_block/container_kind NOT walker-read; composition_role consumption DEFERRED per D109 / FR-22-17 / parking P-XS-3-TRIGGER-REFINEMENT. |
| `block_attributes` | 2,110 (live DB post-`/sgs-update` 2026-06-03; +9 attrs incl. container `contentWidth` now synced. Counts drift — `/sgs-db` or `02-SGS-BLOCKS-REFERENCE.md` are authoritative) | Central for FR-22-2. Walker reads `canonical_slot` + `derived_selector` + `role` to resolve equivalent_block at query time. **D110 2026-05-30 backfill (with XS-4 follow-ups):** `canonical_slot` coverage 52 → 692 rows (2.5% → 33.4%); `role` coverage 110 → 689 rows (5.3% → 33.2%) via D99-ported `assign-canonical.py`. |
