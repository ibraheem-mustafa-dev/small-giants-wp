...
**built_status:** PARTIAL — rule written D118; padding-lift A1 (1cf0692d, D178); widthMode:full BUILT — set points verified 2026-06-14 at `convert.py:4372/4376/4380` (the A2 slug-None section block) + `convert.py:4477/4481/4485/4489` (the R1.3 section-kind mirror block); the old `convert.py:4240` reference is stale. gridItem* uniform per-item CSS writer SHIPPED D224 (`_lift_uniform_grid_item_css` `convert.py:2734`, wired at `:4414` top-level + `:5574` nested wrapper, R-22-9 universal). REMAINING: 'border' shorthand has no `property_suffixes` row so gridItemBorder is not lifted from a shorthand (DB-data follow-up).
