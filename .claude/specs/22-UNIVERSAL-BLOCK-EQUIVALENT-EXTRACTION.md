...
| `wraps_block` | TEXT | Slug of the block this one wraps as its outer rendered container, or NULL. Populated for the **28-block container roster** (all wrap `sgs/container`) post-D152. (Pre-D152 it was 4 rows: hero/cta-section/modal/quote.) |
...
| `container_kind` | TEXT CHECK | `section` \| `layout` \| `content` \| NULL. Added + populated D152 for the 28-block container roster (4 section / 13 layout / 11 content). NOT walker-read — it is the propagation key for mirroring sgs/container capabilities onto composites per §FR-22-21. |
...
- **gridItem\* never written** (A6, WS-1c) — step 4's uniform-item path has no writer; `gridItem*` defaults are never emitted. Still pending.
