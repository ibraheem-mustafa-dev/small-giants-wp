...
| Column | Type | Notes |
|---|---|---|
| `block_slug` | TEXT PK | One row per registered block (189 rows currently — post-D152; container roster = 29 rows). |
| `wraps_block` | TEXT | Slug of the block this one wraps as its outer rendered container, or NULL. Populated for the **29-block container roster** (all wrap `sgs/container`) — D167 (was 28 pre-D167; `sgs/content-collection` added). (Pre-D152 it was 4 rows: hero/cta-section/modal/quote.) `sgs/modal` + `sgs/mobile-nav` have `containerMirror:false` and are excluded from mirroring despite wrapping sgs/container. |
...
**PASS test (data layer — updated D167 2026-06-04).** `SELECT COUNT(*) FROM block_composition` returns 189. `SELECT COUNT(*) FROM block_composition WHERE wraps_block='sgs/container'` returns **29** (was 28 pre-D167; `sgs/content-collection` registered commit `40a9e03d`; 4 section / 14 layout / 11 content). `SELECT COUNT(*) FROM block_composition WHERE composition_role='section-root'` returns 2.
...
- ~~**outer max-width transfer broken**~~ — **SHIPPED 2026-06-03 (A2, commit 2f86d9e6, D159).** Slug-None top-level section path sets `widthMode` from the section's own max-width. (`widthMode:"full"` band-aid at slug-RESOLVED path `db_lookup.py:2461` (C1, WS-3) still pending.)
...
- **gap forced to spacing-token** (A4, WS-1c) — step 4's `gap` (raw px allowed) currently snapped to a spacing token, losing raw values. Still pending.
...
**PASS test (block-side):** every block in the 29-block roster carries the KIND-scoped mirrored attrs in block.json + edit.js ContainerWrapperControls + render.php via SGS_Container_Wrapper; `supports.sgs.containerMirror:false` on modal + mobile-nav; `/sgs-update` reports 0 orphans; block_attributes 2,739.
...
**FAIL test:** any layout CSS silent-dropped or stranded in variation CSS; a composite missing a mirrored container capability; a per-block layout conditional in the converter; `widthMode` hardcoded to `full` where the draft has a max-width; a composite below KIND (trust-bar stripped of grid attrs).
