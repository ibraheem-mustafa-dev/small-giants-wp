...
## 6a. Progress — verified status (updated 2026-07-25 — mega CORE shipped, D379)
...
| **mega-panel presets** (parent-paints-child) | `FRONTEND works / EDITOR-PREVIEW OPEN` | render.php emits distinct Columns/Cards/Minimal layouts per `data-mega-style` (frontend works BY CONSTRUCTION — read in code, not yet visually confirmed on a page). ⚠ **EDITOR CANVAS preview of the presets does NOT work** — WP 7.0's iframed editor canvas is not applying this block's `editor.css` (only `style.css` reaches the canvas). PROVEN FIX (not yet landed): move the preset rules into `style.css`, NO `!important` (a no-`!important` rule injected into the canvas applies — beats WP by specificity; `!important` in style.css would break the frontend mobile-stack). The `!important` added to editor.css (e5f70680) + the 0.1.1 version bump were aimed at this + are harmless-but-inert until the style.css move. Also fix the render.php mobile stack to collapse the cards GRID to 1 column. **Owed as the first follow-up task.** |
...
| 36-8, 36-17, 36-9a (mega) | `BUILT (code)` | 36-8 mode-switch cites nav-menu's collapse point; 36-17 crawlable (panel content server-rendered, `hidden`-by-CSS but in the HTML, no-JS = progressive-enhancement per FR-36-7); 36-9a null-degrades a trashed/missing/recursion panel to a plain link. **Owed:** live confirmation on a real page (axe on OPEN panel, drawer no-regression, crawl assertion, live recursion test, Bean's eye R-31-13) |
