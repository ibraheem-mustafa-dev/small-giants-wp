...
> **Customiser paint targets.** Paint targets are `header.wp-block-template-part` / `footer.wp-block-template-part` (NOT `.wp-site-header` / `.wp-site-footer` — SGS theme template parts do not emit those classes); CSS custom properties belong on `:root` so they cascade regardless of which wrapper exists. View Transitions wiring checks `function_exists('wp_enqueue_view_transitions_admin_css')` and falls back to an inline `@view-transition{navigation:auto;}` rule where the native function is absent.
...
|---|---|---|
| Spec 36 §Customiser migration | `Sgs_Site_Info_Customiser` | `postMessage` transport, `wp_options` backing, capability gate, sanitiser pattern |
...
**WP 7.0 View Transitions (Decision 27):** Future Customiser sections should call `wp_enqueue_view_transitions_admin_css()` for smooth panel navigation. The Spec 36 Site Info Customiser is the first SGS use of this WP 7.0 feature.
