...
**ROOT FIX for the WooCommerce Block-Hooks injection (noted D324, 2026-07-13, build-pending):** WooCommerce hooks `woocommerce/mini-cart` and `woocommerce/customer-account` onto ANY `core/navigation` block via the WP Block Hooks API — both `src/Blocks/BlockTypes/MiniCart.php` and `CustomerAccount.php` register `'anchor' => 'core/navigation'` via `add_filter('hooked_block_types', ..., 9)` (confirmed in WC source, incl. the `BlockHooksTrait.php` docblock). This is why a stray WooCommerce mini-cart + customer-account kept appearing in the P1 header alongside the intended `sgs/cart`. Replacing `core/navigation` with `sgs/adaptive-nav` in the header removes the anchor WooCommerce hooks onto — the injection stops **by construction**, no suppressor code needed. Bean rejected a `hooked_block_types` workaround filter (considered + validated, then reverted) in favour of this architecture fix. **Once `sgs/adaptive-nav` ships and the header no longer uses `core/navigation`, remove the interim nav-visibility CSS hack in `sgs/site-header/style.css`.** Full detail: `.claude/plans/2026-07-13-header-builder-remaining-work.md`.
...
- **(d) Indus slim-bar** — authored in `parts/header.html` + `framework-header-default.php`: desktop icon-only phone/email/social utility strip (`labelCollapse:"all"`, hidden ≤1024); **≤1024** a prominent "Call" button on the BOTTOM row (a `sgs/business-info` phone styled via `.sgs-header-call`, reads Site Info → `tel:`, 44px, theme-token colours), with **logo + cart + burger together on the top row** (Bean's row fix — the Call was moved off the middle row so it no longer forces a wrap); email/social move into the drawer; **footer 3→1 at 768** (already D325). The `sgs/site-info` binding on a `sgs/button` url did NOT resolve on WP 7.0 (bindings unproven on custom-block attrs) → pivoted to the business-info-as-button approach (proven, R-31-1-clean).

**2nd-client evidence (honest):** verified live on sandybrown; 2nd-client universality is by-construction (client-agnostic blocks reading the per-site Site Info store + tokens, grep-clean of client literals) — there is no live indus-foods deployment on the hosting account (same caveat as FR-S9-10, D330). Reports: `reports/visual-diff/business-info-2026-07-14.md` + `mobile-nav-2026-07-14.md`. Plugin 0.1.3→0.1.4, theme 1.5.17→1.5.19.
