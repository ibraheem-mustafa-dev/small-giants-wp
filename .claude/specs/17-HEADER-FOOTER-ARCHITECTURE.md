...
- `no-header-footer-block.py` allows `Write`/`Edit` on `src/blocks/site-header/`, `src/blocks/site-footer/`, `src/blocks/adaptive-nav/` (already true; add a fixture test asserting it stays true). `src/blocks/mobile-nav/` was DELETED at D336/Task 1 (2026-07-14) — removed from this list; the equivalent fixture already targets `src/blocks/adaptive-nav/`, which owns the drawer FR-S9-5 describes.
...
**Behaviour:** Layout-KIND composite, one menu source rendering a desktop nav bar that collapses to a burger triggering the `sgs/mobile-nav` drawer (FR-S9-5). Breakpoint set across **4 tiers: Desktop / Tablet / Mobile / custom-px** (Elementor + Bricks pattern — the tier is a per-nav setting, not hardcoded). **Default = one-tree-restyled** (desktop tree re-styled for mobile); **escape hatch = independent mobile tree** via the drawer (Bricks' explicit named choice) — document the dropdown `position:absolute`→`static` gotcha inside the drawer. **Mega-menu:** per-item nestable content (columns, a Content-Block for rich content), full-width or element-targeted; on mobile it expands in place via an **accessible ARIA disclosure/accordion** (`aria-expanded`, keyboard-operable). ALL links are server-rendered as real `<a href>` — **NO AJAX lazy-load** — so every link stays visible to crawlers and legitimate AI search (Perplexity/ChatGPT/Claude/Gemini). **(Bean-decided 2026-07-14, D334):** accordion + no-AJAX is the shipped design; drill-down + back-link is deferred to the roadmap for any future genuinely deep-menu client — for shallow menus (Mama's, Indus) the accordion is the more accessible + AI-visible choice, and AJAX lazy-load would actively harm AI-indexability. **Desktop overflow safety:** items that don't fit the available width auto-collapse into a "more" menu — the intrinsic never-overflow guarantee extended to navigation specifically.
...
> **AMENDED 2026-07-14 (D337, Bean-delegated).** Two criteria below described the D323 *workaround* rather than the *outcome*, so the better implementation would read as failing this spec and a future session would "fix" it back to the worse design. Both are now intent-based. Original wording preserved in the strike-through notes for audit.

- **Drawer is genuinely interactive while open, and the background is genuinely frozen** — verified by BOTH: (a) every link/button inside the open drawer returns itself (not `BODY` or an ancestor) from `document.elementFromPoint(x, y)` at its own centre point — the exact test that caught the original bug; and (b) background content outside the drawer cannot be focused, clicked, or reached by AT while open. **The MECHANISM is free** — a native `<dialog>` opened via `showModal()` satisfies this by construction (a modal dialog is the only element that escapes inertness, so it cannot self-inert regardless of DOM position; per MDN), as does the legacy re-parent-to-`<body>` + `inert` approach. ~~*(Superseded: "Drawer is a direct child of `<body>` at open time (verified: `drawerEl.parentElement === document.body`) — regression-guarded so this can't silently regress back under `.wp-site-blocks`". That mandated the specific hack the D323 fix used, not the goal. `showModal()` makes the re-parent unnecessary; requiring it would forbid the stronger design.)*~~ **Regression guard = the elementFromPoint sweep** (baseline: 18/18 Indus, 10/10 Mama's, measured 2026-07-14), not DOM position. **AMENDED by Spec 34 §3 (2026-07-15, Bean-approved):** criterion (b)'s "background content outside the drawer" EXCLUDES the header row carrying the toggle + `#wpadminbar`; and the mechanism is now the re-parent-to-`<body>` + selective-inert model — `showModal()` was retired by the disclosure redesign (the header must stay interactive, which a modal dialog forbids by construction).
- Focus trap: Tab from the last focusable element cycles to the first (and Shift+Tab from the first cycles to the last); focus never escapes to background content while open. **AMENDED by Spec 34 §3 (2026-07-15, Bean-approved):** the contained set is {live header row + drawer}, containment is EMERGENT from the selective freeze (everything else inert), and there is no hand-rolled wrap target — "never escapes to background" still holds; "cycles to the first" is superseded.
...
- Background content (everything outside the drawer) is unreachable (`inert`/modal-dialog inertness/`aria-hidden`) while the drawer is open; **the drawer itself is never inert**. **AMENDED by Spec 34 §3 (2026-07-15, Bean-approved):** "everything outside the drawer" EXCLUDES the header row carrying the toggle (and `#wpadminbar`), which remain live by design — this bullet and the (a)/(b) criterion above state the same requirement twice; both carry this exception.
- `aria-controls` + `aria-expanded` on the toggle button correctly reflect open/closed state; state is also signalled by a CSS class, **not colour alone**. **The drawer's LINKS carry `aria-current="page"` for the active page.** ~~*(Superseded: "state is also signalled by a CSS class AND `aria-current`". `aria-current` means "this is the current item in a set" — it is semantically wrong on an open/close toggle, where `aria-expanded` is the correct and sufficient ARIA APG disclosure signal. Keeping it on the toggle would be cargo-cult ARIA. It remains REQUIRED on drawer links, where it is meaningful.)*~~
- **No static `role="dialog"`/`aria-modal` on a closed drawer** (added 2026-07-14) — modality must be conferred at open time (`showModal()`), never authored as an always-on attribute. A permanently-present `aria-modal="true"` hides the ENTIRE page from assistive tech while the menu is shut; WP core fixed this exact bug in 2021 (gutenberg#37434) and the pre-D337 `sgs/mobile-nav` shipped it (verified on the live DOM 2026-07-14).
