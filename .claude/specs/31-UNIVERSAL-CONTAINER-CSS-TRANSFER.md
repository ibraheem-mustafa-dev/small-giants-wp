...
- **B4 — Array / repeater** (`attr_type=array` — FR-22-2.5). Sibling-class DOM traversal of the array container; emit one child block per item; per-item attrs lift via B1's role-aware mechanism.
...
| `blocks.parent_block` (forced-parentage, 18 rows) | **child-block CONTENT resolution (Axis 3 child-routing, added 2026-06-27 — Spec 22 §FR-22-5.3 §5)** — a draft child token under an `InnerBlocks` parent resolves to the registered child whose identity matches, overriding the global `slots` alias (`sgs/accordion __item → sgs/accordion-item`, not the `card`-alias). NOT a CSS-routing column. |
