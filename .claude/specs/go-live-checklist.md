...
| IF-5 | Account area and saved items set up (FR-30-14/15) | SSH: `wp post get $(wp option get woocommerce_myaccount_page_id) --field=post_content` and the same for `woocommerce_saved_items_page_id`. Then `wp option get sgs_wishlist_features --format=json` and `wp option get sgs_n8n_webhook_url` | The account page holds `wp:sgs/account` and the Saved items page `wp:sgs/wishlist-panel`, each inside an `sgs/container`. If any of priceAlerts / stockAlerts is on, or Notify me is offered anywhere, the webhook URL is a live https N8N URL with a workflow that emails `sgs_wishlist_alert` and `sgs_back_in_stock` events (otherwise the alerts queue and never send). | ☐ |
