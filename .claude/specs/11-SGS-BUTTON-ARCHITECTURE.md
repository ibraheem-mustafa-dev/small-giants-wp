...
>
> **2026-07-05 update — D280: per-device width + hover underline SHIPPED; preset-as-seed model DESIGNED (next).** `block.json` 1.1.0 → **1.2.0**. (1) **Per-device width** — `customWidth`/`widthType` gained `Tablet`/`Mobile` tiers (+ per-tier `customWidthUnit*`), so a button can be full-width on mobile only (the draft pattern); rendered in a uid-scoped `<style>` at 768/1024 (Pattern-A, no inline that beats `@media`). This extends the Section-3 responsive attr list (previously `fontSize`/`padding` tiers only) to width. (2) **`textDecorationHover`** (enum `none`/`underline`, default `none`) — the first text-decoration hover attr, added to the hover family alongside `colour*Hover`/`boxShadowHover`; renders `text-decoration:underline` on `:hover`/`:focus-visible`. (3) **Preset-as-seed (DESIGNED, build next session):** the Section-3 `inheritStyle` behaviour below — "when `inheritStyle !== 'custom'`, render.php outputs only the variant class and per-element controls are inert" — is being **REPLACED** by the WordPress block-VARIATION pattern: picking a preset SEEDS the block's attrs from that preset's defaults (via `edit.js`), then every value stays editable and render.php ALWAYS paints from attrs (the `$is_custom` gate removed). Rationale: the current gate makes typography silently inert on preset buttons (the H4 defect); a preset should be a STARTING POINT, not a lock. Values seed from the theme-snapshot preset vars (per-client channel). Design: `.claude/next-session-prompt.md` Task 1. Until it ships, the `inheritStyle`-gates-everything behaviour in Section 3 remains current.
>
> **2026-07-06 update — D283: preset-as-seed SHIPPED + full-width fix + built-in-CTA reuse.** `block.json` 1.2.0 → **1.3.0**. (1) **Preset-as-seed SHIPPED** (`9e6d9622`, Bean-approved: REMOVE the old locked-preset system). The `$is_custom` render gate + the `.is-style-{preset}` class mechanism are DELETED — `render.php` ALWAYS paints colour/border/hover/typography/box-shadow from the block's OWN attrs. A **"Style preset" dropdown + a separate "Apply preset" button** (edit.js) copies `BUTTON_PRESETS[preset]` (new `src/blocks/button/presets.js`, primary/secondary/outline token-slug shapes matching the old is-style fallbacks) into the block's attrs, then everything stays editable; the separate Apply button prevents accidental resets. Fresh buttons default to the primary preset (block.json defaults); `inheritStyle` enum trimmed to primary/secondary/outline + emitted only as a `data-preset` debug attr. **Existing secondary/outline preset buttons need re-applying** (D270 no-deprecations; pre-production). This SUPERSEDES the Section-3 `inheritStyle`-gates-everything behaviour + Section-4's `is-style-*`/theme.json value-source model (both now historical). `sgs/multi-button` gained an "Apply to all buttons" (`95e8f432`) iterating child `sgs/button` blocks. (2) **Full-width fix** (`2529c9a4`): `.sgs-button-wrapper--full { flex:0 0 100% }` so a full-width button fills the line even as a flex item (multi-button row / container flex); dead `.sgs-button--full` class removed; editor==frontend parity. (3) **Built-in-button reuse** (`d7039a79`): the shared `includes/helpers-button-style.php` (`sgs_button_element_style_css`) now gives ANY built-in button element (product-card's bound CTA first) the same preset-as-seed colour/border/radius/font/padding/width from a prefixed attr set — the canonical reuse path for built-in CTAs (buybox/whatsapp-cta next). Guarded by `check-hardcoded-render-defaults.js` E11 (prefixed-helper attrs govern only their call-site selectors).
