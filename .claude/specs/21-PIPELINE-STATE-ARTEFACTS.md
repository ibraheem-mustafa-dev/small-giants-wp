...
| `slots` (post-D99 replacement for retired `slot_synonyms`) | 89 element rows + 16 section rows + 1 new `inner` passthrough element row | Canonical slot vocabulary; resolves BEM elements to standalone blocks via `standalone_block` column | Stage 3 slot list + walker element-path recognition. Walker `inner` passthrough code reverted; slot row persists for future re-enable. |
...
| `block_attributes.role` | 676 / 2074 rows (32.6% — uplifted from 110 via XS-4) | Per-attribute role classification | Stage 3 + Stage 4 |
