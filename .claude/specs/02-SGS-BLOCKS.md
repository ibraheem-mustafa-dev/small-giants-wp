...
- **Never-overflow layout:** base layout is Cluster (`flex-wrap` + `min-width:0` on children + `flex-shrink:0` on the logo) + fluid `clamp()` spacing + container-query tiers — guarantees no overflow down to 320px by construction. Owning FRs: Spec 37 (FR-37-12, §3.6).
