...
What the Tier 4a upgrade above DOES change: detecting Three.js's `data-engine` attribute is cheap and reliable now — so the pipeline CAN cheaply and honestly know "this section has genuine WebGL rendering" as a PRESENCE fact. What it still cannot do on its own, purely from computed CSS, is know WHAT the shader draws — the attribute proves activity, not content. So presence-detection alone doesn't unblock auto-cloning; it upgrades the QUALITY of the flag handed to the human review step. **What DOES unblock the content question is Tier 4d below — a real, designed capability, not a deferred idea.**

**Tier 4d — operator-confirmed reference-file pulling (Bean-directed, 2026-09-10; `decisions.md` D1019).**

When Tier 4c's style-approximation can't match either of SGS's shipped Tier W effects (or when Bean simply prefers the real thing over an approximation), the pipeline offers pulling the actual reference file(s) behind the detected WebGL effect — same shape as the 2026-08-28 Stripe precedent (`decisions.md` D880), now made a named, repeatable option rather than a one-off exception.

**Mechanism:** gated on Tier 4a's confirmed-WebGL-presence signal (never fires on a guess); the pipeline surfaces the relevant file(s) (shader source, the JS driving it) it can reach from the source's publicly-served assets, alongside Tier 4c's style-classification, as **one Bean-confirmed choice per source** — never an automatic pull, never applied without the operator choosing it for that specific clone. Once chosen, the file is treated exactly as the Stripe precedent was: a starting point that gets reworked into SGS's own attribute/control system, not shipped verbatim.

⚠ **Legal framing — read this precisely, do not soften it in a future edit.** This capability is admitted as **Bean's own risk-tolerance decision, not a legal clearance.** Rebuilding pulled material into SGS's own modular system is what makes it a *derivative work* — under UK/US copyright that still generally needs the original owner's permission unless a specific exception applies; reworking it changes what ships, it does not retroactively license the initial copy. The original Stripe decision (D880) recommended a UK IP solicitor's opinion on the client-indemnity question before proceeding once; **that opinion was never obtained, and admitting this as a repeatable pipeline feature does not resolve that flag — if anything it raises the stakes**, since exposure is no longer bounded to one named, considered source. Full reasoning: `decisions.md` D1019. If real legal certainty is ever wanted rather than a documented risk-acceptance, the solicitor's hour is still the cheapest way to get it.

- *Cost:* low-moderate — reuses the file-fetching the pipeline already does for a source's public assets; the operator-confirmation UI is the main new surface.
- *Risk:* legal risk is real and explicitly Bean's to own, not mitigated by this design — the design's only job is making sure it's never silent or automatic. Technical/product risk is low: gated on real WebGL presence, never applied without confirmation.
- *Value:* covers the cases Tier 4c's approximation can't (like TAG Heuer's 3D viewer), for operators who'd rather have the real starting point than a stylistic stand-in.
...
5. When the visual character matches neither shipped effect, say so plainly and offer Tier 4d (reference-file pulling) as the alternative — never force a bad fit onto the closed list.
...
**TAG Heuer Eyewear's collection page** — `https://www.tagheuer.com/fr/en/eyewear/collection-eyewear.html`, Awwwards Site-of-the-Day recognised, scored 8.40/10 on Awwwards' own "Animations/Transitions" criterion. Still the right test case, with its role sharpened by the Tier 4 split above: the page's blur-text intro and scrolling carousel are Tier V-shaped (Tier 1/2 territory) — good coverage evidence. Its WebGL 3D product interaction is OUT of Tier 4a/4b/4c's automatic scope (no auto-inference, and it matches neither of SGS's 2 existing Tier W effects for approximation) — on a real clone of this page, that portion should surface as a flagged gap offering Tier 4d (pull the reference file, Bean-confirmed) as the option, not something the pipeline silently reproduces, silently drops, or force-fits to a bad approximation. A correct outcome on this test case is therefore: Tiers 1/2/3 recover the CSS-shaped motion faithfully, AND the WebGL section is honestly flagged with a real choice (approximate, pull the reference, or hand-author) — not attempted or ignored.

### Recommendation (revised 2026-09-10 — `/qc-council` rater D)

Build Tiers 1, 2, 3, and 4a together as **one phase-plan** (matches Bean's "not longer than a phase plan" framing and his "clone everything, don't gate me on cautious slices" instruction) — but with an INTERNAL measurement checkpoint inside that phase, not a fully unmeasured bundle: ship and measure Tier 1+2 against the TAG Heuer page (and a couple of others) first, confirm real coverage, THEN build Tier 3/4a within the same phase. This is the same discipline R-31-5 ("phases never ship as single commits") already asks for, just applied as a checkpoint rather than a separate phase — it honours full-coverage intent without abandoning this project's own measure-before-escalating habit (visible in this doc's earlier tiers and the sibling BEM doc's explicit tiered gating). **Tier 4c (style-approximation) and Tier 4d (reference-file pulling) are a fast-follow, not part of this base phase** — both depend on Tier 4a's presence-detection existing first, and are small/separable enough to be their own follow-on step once the base phase ships and Tier 4a is proven live. Tier 4d carries the D1019 legal-framing requirement (Bean's own risk decision, documented as such, solicitor question left open) — do not build its UI/copy without that framing intact. Tier 4b (pure auto-inference of WebGL content, no operator confirmation) stays banned outright — Tier 4d is a materially different mechanism (operator-confirmed, per-source) and does not reopen it. Tier 5 (semantic fallback) stays deferred until 1-4a are shipped and measured.
...
5. ~~Is Tier 4b's exclusion acceptable as a standing answer, or worth a dedicated conversation?~~ **Resolved with two paths, not one: Tier 4c (style-approximation) covers sources matching SGS's existing 2 effects; Tier 4d (reference-file pulling, Bean-directed, `decisions.md` D1019) covers the rest, including TAG Heuer's 3D viewer, via operator-confirmed reference pulling rather than auto-inference. The narrower question — should Tier W ever grow a NEW admitted effect type built INTO SGS itself (vs. pulling a reference per-source) — stays confirmed OUT of scope for now (Bean, 2026-09-10): "system can wait for later."**
