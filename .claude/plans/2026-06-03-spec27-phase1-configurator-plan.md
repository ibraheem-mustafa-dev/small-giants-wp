...
TRACKED FOLLOW-UPS (build-time, not blockers): (a) **Store-API direct-bypass** (FR-MISSING-3) — the `woocommerce_store_api_validate_add_to_cart` enforcement is a commented stub; a client can still POST `/wc/store/v1/cart/add-item` directly, skipping the per-SKU cap → so this proxy is not yet the "single add-to-cart path" FR-27-G1 wants. Verify the filter name + implement. (b) **"Any"-attribute term validation** — when a variation axis is "Any", validate the client value is a real `term_exists()` (else an arbitrary slug shows in the cart line). (c) **product-LOCAL (non-`pa_`) attributes** — value compare is case-sensitive + passes the client value not the stored value; Mama's uses global `pa_` attrs so Phase-1-OK, but fix before a local-attr client. (d) **guest nonce** = shared-per-tick (acceptable WC-parity; document). (e) **shared-IP guest fingerprint** can over-throttle concurrent NAT buyers (UX, not security).
