...
- **Universal converter-lift** (post-WS-4, reinstated — `universal-lift-was-premature-not-falsified`): transfers draft CSS onto the now-mirrored wrapper attrs; rebuild with `property_suffixes` + CSS-signature layer detection (name-free, NOT a `canonical_slot` routing key — D194) + min-height align-gate.
...
- **#4a** converter grid-lift; **#4b** product-card fill (✅ **DONE block-side, D204 2026-06-10** — the card was rebuilt to a built-in-element block [FP-H] AND now yields its 380px cap in a grid track via `.sgs-card-grid > .product-card{max-width:none}` + the standalone B2 tie; its built-in content attrs are DB-mapped/converter-routable; converter routing of the `.sgs-product-card` section is the only remaining piece); **#5** image sideload; **#6** notice-banner converter content-synthesis; **#7** announcement-bar PARKED (mis-route, not a container-roster block); **#8** slider committed w/ the slider work — residuals (pause-float, broken icons, track-fill) need a live 4-card visual verify.
...
- **Universal converter-lift (WS-1c A5/A6 + B1):** NOT falsified — **premature**. Rebuild AFTER WS-4 with name-free layer detection via `property_suffixes` + CSS signature (NOT a `canonical_slot` routing key — D194; not a blind suffix fingerprint) + min-height align-gate. A5/A6/B1 all land once their wrapper-attr destinations exist (WS-4).
