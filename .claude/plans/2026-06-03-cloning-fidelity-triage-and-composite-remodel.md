...
- **#8 slider.** BUILT + deployed (uncommitted): arrows → flex siblings (no overlap), controls markup underneath, slidesVisible default 1→3, infinite loop via `wrapIndex`. **3-up VERIFIED live; nav/loop NOT verified** (page 144 slider has only 3 testimonials → nav correctly hidden). **TODO next session: (a) verify whether the draft has >3 testimonials the converter dropped; (b) verify arrows/controls/loop on a >3-slide page; (c) the pause-btn renders as a floating slider child not inside `__controls` (controls only render with dots) — tidy; (d) then commit + visual-diff report.**

### WS-4 mechanism design (from the foundation audit — the build spec)
- **Section-kind roster (container_kind='section'):** hero, cta-section, modal, trust-bar. **MODAL = HARD EXCLUSION** (its `<div>+<button>+<dialog>`+`::backdrop` shell has no sgs/container mapping — tag `containerMirror:false`). Real targets: **hero, cta-section, trust-bar**.
