...
### MF-D [Ship-PM] — this is a 2-SESSION build; re-sequence + front-load the visible win
**FOLD the build order (supersedes the Step 0→6 order below):**
- **Session A (converter core):** (A1) build the DB-driven map + value passthrough + FLAG + responsive [MF-A] → (A2) **Step 3-refined FIRST** (composite path via the param, MF-C — highest-risk + the first visible win: hero flat-pink + min-height) → (A3) Step 1 slug-None lift (reuses the map) → (A4) Step 2 sidecar deletion (mechanical cleanup). QA Gate 1.
- **Session B (block + polish, mostly parallel):** MF-B min-height block-layer fix ∥ FS-4 sgs/media CSS ∥ D6+D7 converter variant/modifier (quick) ; THEN D5 layer-diagnosis (verify block-vs-converter first, STOP #31; if converter, it's already covered by A3's grid lift) ; THEN FS-5 image sideload (**should-ship, not must** — if it stalls on auth/idempotency, defer without blocking the gate) ; THEN the synthetic universality fixture (Step 0, **deferred to last** — it's the regression harness, not a gate for the first win). QA Gate 2 + Bean R-22-13.
- **Context checkpoint:** if orchestrator context >70% before Session-A QA Gate 1, STOP #19 + handoff.
