...
## Follow-on phase (after this plan + the cloning converter) — FP-E / FP-H product-block rebuild (added 2026-06-09, D193)

Once this wave plan reaches 100% (R4+F2, P2/P3) **and** the cloning converter Stage 1 lands, the cloning-thread Wave-2 backlog has two product-block items that belong to the Spec-27 product layer, NOT the clone-fix waves:
- **FP-E** — give `sgs/card-grid` a real WooCommerce product capability (product query + product-aware controls + responsive product grid). Today it's a visual skin with no product machinery.
- **FP-H** — rebuild `sgs/product-card` so its core elements (image / name / price + per-pack value ladder / pack-size picker / CTA / badges) are **built-in to the block** (typed attrs + inspector controls), not loose child InnerBlocks — a curated, client-unbreakable, feature-rich commerce card faithful to the Mama's draft. This deliberately diverges from FR-22-2's "sub-elements → child blocks" for product-card and MUST be design-gated (the built-in-vs-child decision, recorded in the DB so the converter routes correctly).
- **Why here + why after:** both edit the SAME files this plan's Waves 2/3 touch (`product-card/render.php`/`edit.js`/`block.json`, `class-configurator-meta.php`, `render-helpers.php`), and FP-H builds ON this plan's value-ladder/configurator/pricing/legal layer. Running them before = collision + building on shifting foundations. They also must be cloneable, so the converter Stage 1 should exist first.
- **Self-contained build prompt (ready):** `.claude/reports/wave2/FP-E-FP-H-SPEC27-REBUILD-PROMPT.md` (full requirements + 3-audience building philosophy + the FR-22-2 design-gate + systems/long-term-viability + gates). Ledger: `.claude/reports/wave2/SIGN-OFF-LEDGER.md` (FP-E/FP-H rows, milestone-gated — cannot be VERIFIED until this phase runs). Slot the FR IDs into Spec 27 when built.
