...
status: active
progress: "WS-1a (A1 contentWidth attr + guarded __inner render wrapper) + WS-1b (A2 outer max-width→widthMode transfer absent→full/present→custom + fold lifts __inner max-width→contentWidth) SHIPPED 2026-06-03, commit 2f86d9e6 (D159), live-DOM verified @1440. REMAINING WS-1c: A3/A4/A5/A6 (A7 likely MOOT). **WS-4 BLOCK-SIDE COMPLETE (D167, 2026-06-04 PM):** all 29 composites in the container roster mirrored via shared helper (includes/class-sgs-container-wrapper.php). hero SHIPPED commit bacbde57 (double-emit guard + overlayColour→backgroundOverlayColour rename + extra_styles opt + wrap_inner:false); product-card SHIPPED commits f68bdc6f + 82fd3b45 (all 5 source-mode branches; sourceMode='wc' configurator live-preserved + verified page 589); 25 composites from D166; content-collection REGISTERED as 29th block (layout KIND, commit 40a9e03d); mobile-nav + modal EXCLUDED (containerMirror:false, commit 391e6cb1 — Popover/dialog shell, no container mapping). Helper gained extra_attr_html opt (compact data-wp-context). /sgs-update reconciled: block_attributes 2110→2739; roster 29; 0 orphans. Stage-11 auto-apply still REPORT-ONLY (PENDING). Mirror is BLOCK-SIDE ONLY — does NOT fix page-clone fidelity (converter routes to sgs/container @conf 0.10, not the composite BLOCK). Remaining converter-side work (next session 'Method 2'): routing fix (.sgs-hero→sgs/hero) + universal converter-lift (post-WS-4; premature-not-falsified per D163/Bean correction; canonical_slot precision + min-height align-gate) + #6 notice-banner synthesis + #4a grid-lift + image sideload (#5). WS-2/WS-3 de-cheat debt also pending."
...
On top of this, 29 composite blocks each re-implement their own wrapper CSS independently of `sgs/container`, so when a capability is fixed on `sgs/container` it has to be manually replicated 29 times. There is no propagation mechanism.
...
4. **WS-4 — Standardise all 29 composite blocks** to mirror `sgs/container`, and build the auto-propagation path so a future container capability propagates via the `block_composition` table. Gaps D1–D3.
5. **WS-5 — Docs** — embed the 6-step wrapper-conversion procedure in Spec 22; update decisions/parking/state throughout.

**Scope — universal, not section-level.** This fix applies to every wrapper element in the draft HTML at any nesting depth, every `sgs/container` instance at any depth, and every composite block with a built-in `sgs/container` wrapper (all three KINDs: section/layout/content). The four Mama's Munches sections are the measurement gate, not the scope. Faithful transfer also covers a property's absence (no `max-width` → full-width, overriding the theme default).

**Acceptance for the programme:** The four target sections (featured-product, ingredients, gift, social-proof) render full-bleed background + capped centred content matching the draft's `__inner` max-width. Every active SECTION-KIND composite (cta-section, hero, trust-bar — modal EXCLUDED via `containerMirror:false`) mirrors `sgs/container`'s full attr surface. A new container capability added via `block.json` + `/sgs-update` auto-propagates to all 29 composites via `sync-container-wrapping-blocks.py`. All live-DOM verified (R-22-11) + Bean sign-off (R-22-13).
...
| WS-4b | PHP shared-helper + composite block.json/render.php updates | ✅ BLOCK-SIDE COMPLETE 2026-06-04 (D167) — shared helper `includes/class-sgs-container-wrapper.php` built; all 29 composites call it (mobile-nav + modal EXCLUDED containerMirror:false); hero (bacbde57) + product-card (f68bdc6f, 82fd3b45) + 25 composites (D166) + content-collection registered (40a9e03d). extra_attr_html opt added. CONVERTER-SIDE REMAINING: routing fix + universal lift + #4a + #6 + image sideload. | WS-4a ✅ | Live-editor validated per composite; page 589 sourceMode='wc' verified; Stage-11 auto-apply still pending |
...
| ~~WS-4a: KIND-scoped diff writer~~ | ✅ SATISFIED — writer report-only SHIPPED; Stage-11 auto-apply pending | Claude Code |
...
> **WS-4 BLOCK-SIDE COMPLETE (D167, 2026-06-04 PM).** All 29 composites KIND-scoped: shared helper `includes/class-sgs-container-wrapper.php` built; hero + product-card + 25 composites (D166) + content-collection registered (29th, layout KIND). mobile-nav + modal EXCLUDED (containerMirror:false — Popover/dialog shell). /sgs-update reconciled: 2739 block_attributes; 29-block roster; 0 orphans. Stage-11 auto-apply still REPORT-ONLY (pending). **The block-mirror does NOT fix page-clone fidelity** — converter routes to `sgs/container` @conf 0.10, not the composite BLOCK. Remaining converter-side "Method 2" work: routing fix + universal converter-lift (post-WS-4 per memory `universal-lift-was-premature-not-falsified`) + #4a grid-lift + #6 notice-banner synthesis + image sideload (#5). Live triage + original WS-4 build spec in `.claude/plans/2026-06-03-cloning-fidelity-triage-and-composite-remodel.md`.
>
> **NOTE 2026-06-10 (D204, theme thread — product-card SUPERSEDED, mirror intact):** `sgs/product-card` was rebuilt to a BUILT-IN-ELEMENT block (FP-H) — its typed mode no longer renders InnerBlocks (the "5 source-mode branches" framing above is stale for typed mode; bound modes unchanged). The WS-4 container-wrapper MIRROR is UNCHANGED and still applies (the card still calls `SGS_Container_Wrapper::render`); only the INTERIOR content rendering changed (now from typed attrs, not child blocks). Two consequences for the remaining converter Method-2 work: (1) the card's built-in content attrs (productName→heading, priceLarge→price, image→media, ctaText→button, trial/featuredTag→label, description→text) now carry `canonical_slot`/`role` in the DB (confirmed via /sgs-update D204) — **the typed-attr routing destinations Method-2 needs now EXIST**, so routing a cloned `.sgs-product-card` section into the card is wired DB-side; (2) `#4a/D5 product-card grid-fill` shipped block-side — the card now yields its 380px cap inside a grid (`.sgs-card-grid > .product-card{max-width:none}` + the WP-tie B2 rule, D204). Canonical draft BEM vocabulary for the converter: Spec 02 §"Canonical draft BEM vocabulary". The Stage-11 auto-apply REPORT-ONLY drift now also lists product-card missing / F2 product-faq extra — reconcile the roster detection here when this WS resumes.

**Purpose:** Every composite block with a wrapper mirrors `sgs/container`. When container gains a new capability, one `/sgs-update` run propagates the capability diff to all 29 composites (content-collection registered as 29th block, layout KIND, D167).
...
2. ✅ Rewrite `sync-container-wrapping-blocks.py` — detect via `has_inner_blocks` + attr names + `accepts_allowed_blocks`; derive and write `wraps_block` + `container_kind`. **SHIPPED `0d746073` (report-only; Stage-11 auto-apply PENDING).**
...
4. ✅ KIND→attr-scope map built (section=full; layout=grid+width/contentWidth; content=width/contentWidth/spacing).
5. ✅ Fix composition_role `leaf`→`content-block` for post-grid, gallery, card-grid. **SHIPPED.**
6. ✅ Dry-run reviewed; 29-block roster + KIND assignments verified; `--apply` reconciled.
7. ✅ Extract shared PHP render helper `includes/class-sgs-container-wrapper.php`; all 29 composites call it (mobile-nav + modal EXCLUDED, containerMirror:false). **SHIPPED D166 + D167.** Helper has extra_attr_html opt.
8. ✅ Live-editor validated per composite. product-card verified on page 589 sourceMode='wc'. Hero deprecated.js migration done (overlayColour→backgroundOverlayColour).
9. ⏳ PENDING: Stage-11 auto-apply gate (writer currently report-only → Bean sign-off to flip). CONVERTER-SIDE remaining: routing fix (.sgs-hero→sgs/hero) + universal lift + #4a + #6 + #5.

**Acceptance:**
- D1: ✅ SATISFIED — `sync-container-wrapping-blocks.py` report-only SHIPPED; `wraps_block=1` + `container_kind` written for 29-block roster. Stage-11 auto-apply (--apply writer) PENDING.
- D2: ✅ BLOCK-SIDE SATISFIED — all 29 composites carry the KIND-scoped container attr surface. The auto-propagation writer (Stage-11 auto-apply) is still report-only; flip to writer pending Bean sign-off.
- D3: ⏳ PENDING — proof-of-propagation test (dummy attr → /sgs-update → all composites) requires Stage-11 auto-apply to be enabled. Deferred to Stage-11 flip.

**Estimate:** WS-4a (DB + sync writer) ~45 min. WS-4b (PHP helper + 29 block updates) ~90 min.

**Risk:** High — touching 29 block.json + render.php files; build with `/dispatching-parallel-agents` + a pre-/qc-council on the sync writer before `--apply`. Live-editor validation is mandatory (R-22-13, R-22-11).
...
> ✅ SATISFIED 2026-06-04 D167 — `sync-container-wrapping-blocks.py` report-only shipped; 29-block roster + KIND assignments verified; `--apply` reconciled with 0 orphaned rows. Stage-11 auto-apply (flip report-only→writer) PENDING Bean sign-off.

**Pass:** Bean reviews dry-run output; 29-block roster + KIND match the D150 validated list; `--apply` succeeds with no orphaned rows.

**Fail:** Any KIND mismatch from the validated list without a recorded justification.

### Gate WS-4b — composites validated

> ✅ BLOCK-SIDE SATISFIED 2026-06-04 D167 — all 29 composites call the shared helper and carry KIND-scoped attrs. Live-editor validated per composite; product-card page 589 `sourceMode='wc'` verified; hero `deprecated.js` migration complete. mobile-nav + modal EXCLUDED (`containerMirror:false`). CONVERTER-SIDE work (routing fix + universal lift + #4a + #6 + #5) remains pending.

**Pass:** Live-editor: load an existing post with each of the 29 composite blocks; no deprecation warnings (or deprecation path resolves correctly via deprecated.js). Create new post; blocks save and load correctly.

**Fail:** Any composite block loses content on re-save.

### Programme acceptance gate

> ⏳ PARTIALLY SATISFIED — block-side composite mirror COMPLETE (D167); converter-side routing + universal lift + Stage-11 auto-apply flip still pending. WS-1c / WS-2 / WS-3 also pending.

**Pass:** All WS-1–4 gates cleared. The 4 target sections + brand render faithfully at 1440/768/375 px. A new dummy attr on `sgs/container` propagates to all 29 composites via a single `/sgs-update` run. Bean visual sign-off on canary page 144. R-22-11 (live-DOM) + R-22-13 (Bean's eye) both satisfied.
...
| ~~29-block block.json + render.php changes introduce regressions across composite blocks~~ | WS-4b | M | H | ✅ RESOLVED D167 — live-editor validated all 29 composites; 0 regression | `/dispatching-parallel-agents` (one agent per block family); /qc-council before `--apply`; live-editor validation per block. Owner: Claude Code |
...
| WS-4b (PHP helper + 29 blocks) | ✅ COMPLETE D167 | 90 min | 345 min |
...
| 3 | ~~WS-4b (29 composites — parallel agents)~~ ✅ COMPLETE D167 + WS-5 close | Programme acceptance gate |
...
**Entry context:**
- Workstream A spec: `.claude/scratch/2026-06-02-container-roster-db-table-handoff.md` (6 build steps, validated 29-block roster — 28 original + content-collection registered D167, safety notes)
- Classifier script: `/tmp/d6_classify2.py` (the working 28-block classifier from D150; content-collection added as 29th per D167)
...
| ~~D1~~ | ✅ DONE 2026-06-04 D167 — `sync-container-wrapping-blocks.py` report-only SHIPPED; `wraps_block=1` + `container_kind` written for 29-block roster. Stage-11 auto-apply (flip report-only→writer) **PENDING**. | `sync-container-wrapping-blocks.py` | ~~High~~ → Pending auto-apply |
| ~~D2~~ | ✅ DONE 2026-06-04 D167 — all 29 composites carry KIND-scoped container attr surface via shared helper + mirrored block.json attrs. mobile-nav + modal EXCLUDED (containerMirror:false). content-collection registered as 29th block. block_attributes 2110→2739. | All 29 composite block.json + render.php files | ~~High~~ |
| D3 | Composites (especially SECTION-KIND) get confidence 1.0 via class-section tier; the 4 sgs/container sections get confidence 0.0 deferred-no-match — converter-side routing fix (.sgs-hero→sgs/hero etc.) is "Method 2" work, PENDING | `convert.py` tier logic | Medium |
...
2. **Composite mirror:** ✅ BLOCK-SIDE COMPLETE (D167) — all 29 composites (4 section / 14 layout / 11 content) carry the KIND-scoped `sgs/container` attr surface via `includes/class-sgs-container-wrapper.php`. mobile-nav + modal EXCLUDED (containerMirror:false). CONVERTER-SIDE (routing + universal lift) still pending.
3. **Auto-propagation proved:** ⏳ PENDING — Stage-11 auto-apply writer must be enabled (currently report-only). Proof-of-propagation test (dummy attr → /sgs-update → all composites gain it) deferred to the Stage-11 flip session.
4. **No regressions:** ✅ Live-editor validated per composite during D166/D167 builds. product-card page 589 sourceMode='wc' verified. Hero deprecated.js migration complete (overlayColour→backgroundOverlayColour).
