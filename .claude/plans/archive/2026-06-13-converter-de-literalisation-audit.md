...
created: 2026-06-13
status: ARCHIVED/SUPERSEDED 2026-06-23 (doc audit) — convert.py FROZEN by D229 (D-MODULAR); the de-literalisation goal is inherited by the clean modular rebuild (Spec 31 §12). This per-literal register (13 literals + verdicts) is preserved as reference intel for the rebuild architect, not a live build guide. Parking P-CONVERTER-DE-LITERALISATION closed (→ memory/parking-archive.md). ORIGINAL: SCOPED — next-session programme. Needs its own design + /adversarial-council before building (Rule 7: convert.py is the highest-blast-radius shared mechanism).
