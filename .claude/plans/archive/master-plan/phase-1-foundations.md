...
Generated by `/verify-loop` self-demo on 2026-04-29 (Phase 1c Step 14b). Each step's commandable proof, runnable verbatim. Pass condition is exit-0 unless otherwise stated.

| Step | Test type | Command | Pass condition |
|------|-----------|---------|----------------|
| 1 — Scaffold + canary_split.py | shell | `python ~/.agents/skills/shared-references/optimisation_toolkit/tests/smoke_canary_split.py; echo $?` | exit 0; "OK canary_split smoke" line present |
| 2 — QA gate: canary smoke | shell | `python ~/.agents/skills/shared-references/optimisation_toolkit/tests/smoke_canary_split.py 2>&1 \| grep -q "OK canary"; echo $?` | exit 0 |
| 3 — Build dspy + certainty + few_shot | shell | `for u in dspy_signature certainty_calc few_shot_injector; do python ~/.agents/skills/shared-references/optimisation_toolkit/tests/smoke_$u.py \|\| echo "FAIL: $u"; done` | No "FAIL:" lines; 3 "OK ..." lines |
| 4 — QA gate: all 4 utilities | shell | `for u in canary_split dspy_signature certainty_calc few_shot_injector; do python ~/.agents/skills/shared-references/optimisation_toolkit/tests/smoke_$u.py >/dev/null 2>&1 \|\| echo "FAIL: $u"; done; echo done` | No "FAIL:" lines printed; "done" prints last |
| 5 — Optional handoff | file assertion | `git log -1 --format=%s` | Latest commit subject contains "feat(toolkit)" or equivalent Phase 1a marker |
| 6 — 3 writer skill rubric-gen updates | file assertion | `for s in skill-writer pipeline-writer command-writer; do grep -lE "Rubric generation\|HARD GATE" ~/.claude/skills/$s/SKILL.md \|\| echo "MISSING: $s"; done` | All 3 grep, no "MISSING:" lines |
| 7 — Cross-tier QC reports for writers | file assertion | `ls .claude/reports/phase-1-cross-tier-qc/20260429-084939/{skill,pipeline,command}-writer.md 2>/dev/null \| wc -l` | Output: `3` |
| 8 — 2 optimiser updates | file assertion | `for s in skill-optimiser pipeline-optimiser; do grep -lE "Lens 6\|rubric.*Stage 1" ~/.claude/skills/$s/SKILL.md \|\| echo "MISSING: $s"; done` | Both grep, no "MISSING:" lines |
| 9 — Cross-tier QC reports for optimisers | file assertion | `ls .claude/reports/phase-1-cross-tier-qc/20260429-084939/{skill,pipeline}-optimiser.md 2>/dev/null \| wc -l` | Output: `2` |
| 10 — gap-analysis Lens 6 always-on | file assertion | `grep -E "Lens 6.*always\|always.*Lens 6" ~/.claude/skills/gap-analysis/SKILL.md \| head -1` | Match returned (non-empty stdout) |
| 11 — qc + qc-inline certainty_calc wired | file assertion | `for s in qc qc-inline; do grep -l "from optimisation_toolkit.certainty_calc" ~/.claude/skills/$s/SKILL.md \|\| echo "MISSING: $s"; done` | Both grep, no "MISSING:" lines |
| 12 — Cross-tier QC for gap-analysis + qc + qc-inline | file assertion | `ls .claude/reports/phase-1-cross-tier-qc/20260429-084939/{gap-analysis,qc,qc-inline}.md 2>/dev/null \| wc -l` | Output: `3` |
| 13 — QA gate: 8/8 cross-tier QC clean | shell | `ls .claude/reports/phase-1-cross-tier-qc/20260429-084939/*.md \| wc -l && grep -li "drift\|contradiction\|ambiguity" .claude/reports/phase-1-cross-tier-qc/20260429-084939/*.md \| wc -l` | First number ≥ 8; second number 0 (or all flagged ones triaged + patched) |
| 14a — Build /verify-loop | file assertion | `test -f ~/.claude/skills/verify-loop/SKILL.md && grep -q "^## Stages" ~/.claude/skills/verify-loop/SKILL.md && test -f ~/.claude/skills/verify-loop/references/end-goal-rubric.md && test -f ~/.claude/commands/verify-loop.md; echo $?` | exit 0 |
| 14b — End-to-end demo | file assertion | `test -f .claude/reports/phase-1-end-to-end/phase-1-end-to-end-2026-04-29-001/verdict.md && grep -q "^Verdict: PASS" .claude/reports/phase-1-end-to-end/phase-1-end-to-end-2026-04-29-001/verdict.md; echo $?` | exit 0 |
| 15 — G1 POST + handoff | API + file assertion | `curl -sf -X POST http://localhost:5050/api/knowledge -H "Cookie: blub_auth=$BLUB_AUTH" -H "Content-Type: application/json" -d @<(cat .claude/reports/phase-1-end-to-end/phase-1-end-to-end-2026-04-29-001/g1-payload.json) && grep -q "current_phase: phase-1.5-tooling-triage\|current_phase: phase-2" .claude/state.md` | curl returns 2xx; state.md frontmatter updated |
