...
Split from `.claude/architecture.md` on 2026-05-24 as part of Phase 10 D'-1 (architecture.md size reduction). Content unchanged; only the hosting location moved. The feature audit lives here permanently as a historical reference.

**Note:** The percentage scores in this document are a 2026-02-26 → 2026-04-18 snapshot. The framework has changed significantly since. Do not quote any percentage without re-running `/sgs-update`. The _architecture_ sections that reference specific block counts and shipping status are in `.claude/architecture.md` (Part A).
