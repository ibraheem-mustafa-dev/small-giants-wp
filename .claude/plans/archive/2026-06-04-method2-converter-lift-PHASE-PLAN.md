...
| **D5** product-card fill — ✅ **SHIPPED BLOCK-SIDE 2026-06-10 (D204)** | (done) the card yields its 380px cap inside a grid via `.sgs-card-grid > .product-card{max-width:none}` + the standalone-scoped B2 tie rule; confirmed by live computed-style probe (380px standalone, fills track in a grid) | Playwright — card computed width = its grid track (640/384), standalone still 380 ✅ | (no further block work — converter only needs to land cloned cards inside a grid wrapper) |
