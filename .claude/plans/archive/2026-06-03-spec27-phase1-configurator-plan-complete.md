...
created: 2026-06-03
status: COMPLETE — ARCHIVED 2026-06-04 (Phase 1 SHIPPED D165, Bean R-22-13 sign-off; all 13 units + 7 ship-gate UX fixes live; acceptance 1-6 met). Phase 2 continues in plans/2026-06-04-spec27-phase2-display-seo-plan.md. HISTORY: SELL-LOOP (U0+U6 backend + U3+U4+U7+U5 pill-swap) SHIPPED D164. Hardening: U9 (a11y `<button>` + moat-evidence sheet) SHIPPED `afb7a65a`; U8 (cache/tax; M-C1 staleness gap found-and-fixed, proven FRESH) SHIPPED `f15fa067`; U10 (perf budgets) MEASURED `d5af2c67`; U1 (per-site capability flag + toggle + backward-compat migration) SHIPPED; U11 (WC<9.8 read-only degradation + admin notices) SHIPPED; U12 (cloning/schema-compat tests — Jest 14/14 + PHPUnit) SHIPPED. **PHASE 1 SHIPPED — Bean R-22-13 sign-off GRANTED 2026-06-04 (D165(7)); Spec 27 acceptance 1-6 all met + 7 ship-gate UX fixes (card width/pills/hover/product-page links/near-black desc/header cart+feedback/no-image placeholder) live-verified.** Fast-follows: P-CONFIGURATOR-JS-WEIGHT-DEQUEUE, M-C10 dual-tax, WCAG-public-audit, P-JEST-BLOCK-EDIT-MOCK-ROT. Next = Phase 2 (display/SEO/schema) — separate chapter.
...
**U0 — 48-SKU dev fixture** · FR-27-I-MVP · ✅ BUILT + LIVE-VERIFIED 2026-06-03 (pulled-forward)
...
FILES: `plugins/sgs-blocks/scripts/seed-48-sku-fixture.php` (idempotent delete-and-recreate, keyed on `_sgs_fixture` postmeta; WC PHP API).
STATUS: run on the canary (WC 10.8.1) → **product 540** "Mama's Test Box — 48 SKU fixture", 48 variations verified via WP-CLI + Store API. OOS: 546/576/586 (12-coffee/48-honey/96-pistachio). stock-1: 563 (24-lemon). scheduled-sale: 565 (48-vanilla, £34.99→£24.49, 2026-01-01→2027-01-01). pa_size(id1,4 terms)+pa_flavour(id2,12 terms).
NOTE (deferred, no silent cap): no "Any"-attribute variation + no deliberate availability-gap structure yet — needed for U5 (availability engine, still gated). Add a fixture-v2 when U5 is un-gated (Cynic MF-2 / Spec-Lawyer #11).

> **M-C2 RESOLVED via U0 (the single highest-divergence risk, closed empirically).** Live add-to-cart of variation 541 returned HTTP 201, correct variation + price (£9.99), cart echoed the attributes. **Canonical WC 10.8.1 add-item wire format: `variation:[{attribute:"<DISPLAY NAME>", value:"<term slug>"}]`** — e.g. `{"attribute":"Size","value":"12-pack"}`. NOT `pa_size`, NOT `attribute_pa_size` (corrects spec FR-27-G1 + the plan's earlier guess). The variation `id` alone resolves the correct variation+price server-side; the array is the attribute-match payload. **U6 proxy mapping:** the variation object's `get_attributes()` returns `pa_size=>slug` (taxonomy form); the parent's `get_attributes()` carries the display-name↔taxonomy map — the proxy translates the client's display-name key → taxonomy to attribute-match against the variation. Combo-key (internal manifest): `{taxonomy}:{slug}` sorted, `|`-joined (stable, slug-based, not display-name).

**U1 — Per-site capability flag** · FR-24 #9 (resolved) · ✅ SHIPPED + PROVEN 2026-06-04 (D165)
STATUS: `Sgs_Content_Types_Settings` owns the autoloaded `sgs_content_types` option (default `[]` lean) + a `manage_options` toggle page; `Product_CPT` gates all 5 init handlers on `is_enabled()`. One-time init@1 backward-compat migration auto-enables content types that already have content (never a silent break) — auto-enabled the canary (detected product 522). Proven: REST route present (200) when enabled, absent (404) when `[]`, both directions; admin toggle page renders with correct checkbox state (Playwright). NEW file `includes/class-sgs-content-types-settings.php` + edits to `class-product-cpt.php` + `sgs-blocks.php`.
...
**U6 — `/sgs/v1/cart/add-item` proxy (security spine)** · FR-27-G1/G2/G3/G6 · ✅ BUILT + HARDENED + LIVE-VERIFIED 2026-06-03 (pulled-forward)
STATUS: `includes/class-cart-proxy.php` built (Sonnet) + wired into `class-sgs-blocks.php`; in-process `WC()->cart->add_to_cart()` model (validate → WC recomputes price + re-validates stock; client never sends a price). **2-rater cross-model security+logic review (qc gate, blub.db 255)** → 5 must-fixes folded: parent-variable-id reject, empty-`variation:[]` reject, draft/private publish gate (FR-27-G2), fixed-window rate-limit (anchor timestamp, no slide), removed the non-existent `woocommerce_scheduled_sale_action` hook (→ `woocommerce_product_set_price`). **Live adversarial suite on product 540 — ALL PASS:** no-nonce 403 · valid 200 · IDOR(page) 400 · parent-variable-id 400 · OOS 409 · attr-mismatch 400 · empty-variation 400 · unknown-attr 400 · qty-cap clamp 50→30 (proven via cart read-back). Verdict from both raters: safe to commit with the should-fixes tracked.
TRACKED FOLLOW-UPS (build-time, not blockers): (a) **Store-API direct-bypass — ✅ CLOSED 2026-06-03.** A `woocommerce_add_to_cart_validation` filter (`enforce_add_to_cart_limits`) now enforces the per-SKU cap + the global fixed-window rate-limit on EVERY add-to-cart path (the proxy AND a direct `/wc/store/v1/cart/add-item` call — WC runs this filter from `WC_Cart::add_to_cart()` for both). It is also the single incrementer of the global window (handler no longer increments). Live-verified: direct over-cap add (50 of a 30-cap variation) → 400 blocked + nothing added; legitimate direct add (3) → 201; proxy path → 200. (b) **"Any"-attribute term validation** — when a variation axis is "Any", validate the client value is a real `term_exists()` (else an arbitrary slug shows in the cart line). (c) **product-LOCAL (non-`pa_`) attributes** — value compare is case-sensitive + passes the client value not the stored value; Mama's uses global `pa_` attrs so Phase-1-OK, but fix before a local-attr client. (d) **guest nonce** = shared-per-tick (acceptable WC-parity; document). (e) **shared-IP guest fingerprint** can over-throttle concurrent NAT buyers (UX, not security).
...
### 8d. GO / NO-GO
**CONDITIONAL GO — now PARTIALLY BUILT (2026-06-03).** The plan is sound, code-grounded, the prior re-scope was right. **Decisions RESOLVED (Bean, D162.5):** (1) KEEP the single whole gate — all 13 units before "shipped"; the MVP subset is BUILD ORDER, not the ship line. (2) PULL the server-side units forward — DONE: **U0 + U6 BUILT + live-verified + committed**, M-C2 PINNED (display-name wire format), M-C5 bypass CLOSED, M-C1's bad-hook part fixed in U6. (3) Build the VISIBLE units on a SEPARATE test page (589) bound to fixture 540 → this UN-GATES U3/U4/U5/U7 from cloning (different page, no pixel-diff impact). **Remaining must-fixes to fold (before the unit each touches):** M-C1 render-time staleness guard (U3/U8), M-C3 per-instance context + separate price literals (U3), M-C5 manifest escaping (U3), M-C7 image fallback (U3), M-C9 cut prefetch (U3), M-C10 tax discipline (U8). §8c should-fixes at build-time. **FULL REMAINING PHASE-1 SEQUENCE to the ship gate (single whole gate):** U3 → U4 → U7 → U5 (the pill-swap, on page 589) → U9 (WCAG sprint + the 4 a11y gates + evidence sheet) → U10 (perf/INP budgets) → U8 (cache + tax correctness) → U11 (WC<9.8 degradation + activation prompt) → U1 (per-site capability flag + settings toggle) → U12 (cloning-compat + schema-compat tests). THEN Phase 2 (FR-27-B2/B3/C2/A4/E1-3/F1/PREFLIGHT/I2/I3 — swatches/per-unit/galleries/JSON-LD/SEO/go-live-check). Acceptance = Spec 27 §"Acceptance criteria (configurator — Phase 1)" 1-6. Honest estimate: 3-4 AI-agent sessions for the full Phase-1 sell-loop.
