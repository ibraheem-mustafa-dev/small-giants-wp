...
| 12 | Rewrite `_lift_inner_blocks(node, pattern)` to take `(node, parent_slug)` and consult `blocks.parent_block` + `slot_synonyms.standalone_block`. Add adjacent-slot grouping logic (when 2+ adjacent siblings need the same parent, wrap them in one parent block, not N). Delete `INNER_BLOCK_PATTERNS` dict from convert.py | `plugins/sgs-blocks/scripts/orchestrator/converter_v2/convert.py` | ~1 hr | Replaces hardcoded dict with DB-backed lookup. The Wave 2 hardcoded hero entry from commit `ad706d0d` becomes obsolete and gets removed. **NOTE (2026-05-23 fact-check + REVISED FRAMING):** Decision 12 implementation (commit `79158da5`) landed step 4 of Spec 16 §15 only (the `_lift_inner_blocks` rewrite + DB-driven InnerBlocks emit). Step 4 fires from BOTH the FR1 fast-path branch AND the normal-route branch — but the **NORMAL ROUTE** doesn't have the universal walker that steps 1-3 build. Per Spec 16 §FR1 + §FR4 (reframed 2026-05-23), every section that isn't matched by FR1's fast path (registered composite block OR registered pattern slug) takes the normal route — start with `sgs/container` (FR4), build inner blocks element-by-element via the universal walker. G1+G3+G5 are normal-route gaps (with G1 also having an FR1-path component — composite blocks emitting self-closing instead of OPEN with InnerBlocks). The **earlier framing of "5 sections fall through to fallback" was wrong** — falling through to `sgs/container` is the correct architectural default per FR4 + Decision 3. The actual gap is that the normal route's universal walker doesn't exist yet. Walker steps 1-3 + G1 OPEN-block emit + G3 slot_list visual extension + G5 per-block DOM fixes are scoped together as Phase 1 of `2026-05-24-strategic-plan.md` (revised 2026-05-23). |
...
### 6.4 Spec 17 — Header/Footer Architecture (SHIPPED 2026-05-18 + Option A CSS-custom-property emission SHIPPED in Wave B1, verified 2026-05-23: `class-sgs-header-renderer.php:73-78` + `class-sgs-footer-renderer.php:68` emit `:root{--sgs-header-bg/text/link/width; --sgs-footer-bg;}` via `wp_head`. Architectural cleanup of inline-style paint vs theme.json consumption remains as P-PHASE-5B-THEMEJSON-CONSUMPTION-PURITY — non-blocking)
