...
## 2. Variant detection — the CANONICAL Spec 31 §line-80 mechanism via `variant_slots`

**Spec 31 line 80 is the authority (NOT the frozen engine's emit-seam habit, NOT the council's "can't run here").** `blocks.variant_attr` names the selector attr; `variant_slots(block_slug, variant_value, unique_slot)` holds each variant's **discriminating** slots (set-difference vs siblings). The detector counts how many of each variant's `variant_slots` were **populated from the draft extract THIS run** and sets the highest-count variant. Universal, DB-driven, "query never guess." The limiting factor per the spec is **DATA** (block.json `variants` unpopulated for most blocks) — but hero's data IS populated (6 `variant_slots` rows verified 2026-06-24), so hero detects cleanly.

**The universal code already exists on the PERMITTED-import side** (import-ban allows `db_lookup`): `detect_variant(block_slug, populated_attrs)` ([db_lookup.py:1892](plugins/sgs-blocks/scripts/orchestrator/converter_v2/db_lookup.py#L1892)), `variant_attr_for(slug)` ([db_lookup.py:1825](plugins/sgs-blocks/scripts/orchestrator/converter_v2/db_lookup.py#L1825)), `_variant_slots_map` ([db_lookup.py:1849](plugins/sgs-blocks/scripts/orchestrator/converter_v2/db_lookup.py#L1849)). The new `converter/` imports them directly — no reach into `convert.py`, D-MODULAR holds.

**Rule.** The variant step (a thin component the recognition→Ctx adapter calls for a variant-bearing block):
1. `variant_attr = db_lookup.variant_attr_for(slug)`; null → no variant step.
2. **Bounded discriminator extract:** populate `populated_attrs` for EXACTLY the slots `variant_slots` names for this block (a small, named set from the table — split→splitImage/splitImageMobile, standard→backgroundImage, video→backgroundVideo/bgVideo, svg-animated→svgContent) by running the existing per-slot extractor on the draft sub-tree. This is NOT the full Stage-4 attr extraction — only the named discriminators, straight from `variant_slots`. The slot list is **queried from `variant_slots`, never a hand dict** (R-22-1).
3. `detected = db_lookup.detect_variant(slug, populated_attrs)` — the universal counter.
4. **Exactly one** (strictly-highest count) → use it. **Zero / tie** → `detect_variant` already returns `None` (leaves the block default) AND logs `variant_detect_miss`/`variant_detect_tie`; a tie surfaces as a loud RECOGNITION-GAP RED row for Bean (cynic SF-3 — never a silent plausible-wrong variant). Write `variant_attr`/`variant_value` onto the `Ctx`. The discriminating slot's *content* becomes child InnerBlocks at Stage 4f (out of scope here).
