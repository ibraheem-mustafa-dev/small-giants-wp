...
- A3 to A8, one small commit each: `<sc-if>` gap rows and branch tagging; `style-hover` to `state_value_lift`; `data:` URI sideload; `data-reveal`/`data-rm` recognised; call `draft-manifest` read-only at Stage -2 and attach its rows to the gap report; fix the halt message.
...
> 3. Where the README and the files disagree, do not choose: list each difference for me.
> 4. Then shorten the README to intent and rules that cannot be read from the files.
...
1. **The 10px snap.** Keep your rule (760 rounds to 768, an 8px band where the clone differs from the draft, logged and sampled), or treat 760 as a bounded residual like the other thresholds (no difference, one extra rule in Additional CSS per affected block)? Two reviewers preferred the residual. My recommendation: keep your rule; the band is 8px and nothing measured says it matters.
2. Will you run the closing prompt on the current Eye Care draft in Claude Design (Phase 0)? Nothing else needs you until Phase 2's ticker design gate.
