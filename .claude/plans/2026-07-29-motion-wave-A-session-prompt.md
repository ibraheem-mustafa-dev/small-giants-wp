...
| ID | Item | Tier | Est | QC |
|---|---|---|---|---|
| A1 | `npm i gsap` + webpack externals (`gsap`, `gsap/*` → WP script-module IDs, `externalsType: 'module'`) + per-plugin module registration in PHP (core/ScrollTrigger/SplitText each a registered module). **⚠ qc-council 2026-07-29: the externals wiring is NET-NEW — nothing in the repo resolves a bare `gsap` specifier today (`DependencyExtractionWebpackPlugin` covers `@wordpress/*` only; Vivus is a per-block bundled chunk, the opposite shape). Treat as a build+verify task, not config plumbing.** | SONNET | 45m | build green + `wp_script_modules()` lists the modules; canary proves `gsap` resolves via the browser import map AND appears in NO consuming chunk (grep the built bundles); NO page enqueues them yet (network check) |
...
| A5 | §4.3 exclusivity — TWO paths (qc-council 2026-07-29): dynamic blocks = OMIT in the `animation-attributes.php` render_block injection; static blocks = the attrs are BAKED into stored post_content at save time (`animation.js:209-233`), so STRIP them via `WP_HTML_Tag_Processor` (reuse the leading-`<style>` skip at `animation-attributes.php:95-111`); editor Disabled+Notice mirror | SONNET | 40m | one DYNAMIC + one STATIC stored instance, each carrying BOTH attr families, render scrub-only (LIVE DOM both); Notice shows in editor |
