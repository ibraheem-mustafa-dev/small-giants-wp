...
status: Phases 0/1/1b/2/3 COMPLETE (D464, D470–D478) · Phase 4 COMPLETE (2026-09-04) · Phase 5: 4/5 FIXED (2026-09-04, D954), Defect 3 pending DB reseed
...
> | **5 — loop defects** | ✅ **4 of 5 FIXED + shipped (2026-09-04, D954).** `/qc-council` validated every proposed fix-shape before dispatch — caught two wrong assumptions (Defect 2 was worse than diagnosed; Defect 3 falsified as sharing Defect 2's mechanism) and one undersized fix-shape (Defect 5's original proposal needed real architecture changes; a cheaper correct alternative shipped instead). **Defect 1 (CRITICAL, section annihilation) FIXED** — per-column exception scoping in `run_mechanism_b`. **Defect 2 (padding shorthand) FIXED.** **Defect 4 (duplicate dissolve mechanisms) FIXED** — shared recursive helper. **Defect 5 (false array-lift warnings) FIXED** — scoped to the triggering attr's own item schema. **Defect 3 (margin routing): code built, DB reseed still pending** — blocked by a concurrent track's uncommitted changes to `sgs-update-v2.py`; margin routing is inert until `/sgs-update` runs. Full detail + two process incidents (a subagent's unauthorised git stash, a hardcoded-dict cheat-gate trip resolved via a real DB-query refactor rather than baselining): `decisions.md` D954. |
>
