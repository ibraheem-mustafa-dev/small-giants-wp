...
status: Phases 0/1/1b/2/3 COMPLETE (D464, D470–D478) · Phase 4 COMPLETE (2026-09-04, see status table) · Phase 5 diagnosed, fixes still OPEN (2026-09-04, see status table)
...
> | **5 — loop defects** | 🔶 **DIAGNOSED (2026-09-04), fixes still OPEN.** Root-cause investigation ran fresh against current code (not the 2026-08-01 snapshot — the converter has been substantially rewritten since; `walk.py` is now a `NodeSignature`-keyed handler registry, not an if-chain). Defect 2 (shorthand `padding`→dead `gridItemPadding`) and Defect 5 (false array-lift drop gaps on hero CTAs) are fully confirmed with file:line root causes. Defect 4 surfaced a real duplicate-mechanism gap (two independent "dissolve a transparent wrapper" implementations, only one fixed in a 2026-07-25 refactor). Defect 1 (section annihilation) and Defect 3 (band margin drop) have plausible-but-unconfirmed mechanisms — flagged for a live repro before design, not guessed at. **No fixes have been built** — this system (the walker) is Rule-7/hand-back-#9 territory, so the actual fix-shapes need a `/qc-council` validation pass + Bean's sign-off before any code changes, per the 2026-09-04 session's design-gate ruling. Full per-defect diagnosis recorded in `decisions.md` (2026-09-04 entry, this Phase 4/5 pickup). |
>
