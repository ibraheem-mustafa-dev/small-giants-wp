...
> | **1 — regenerative seeders** | 🔶 **PARTIAL — the seeders are done, Bean's stated bar is NOT.** His ask was *"I want the vast majority of all migrations deleted and replaced"*; **0 of 30 migrations have been deleted.** F2 assessed 24 as deletable once seeders existed. Closing this on "they COULD now be deleted" is a weaker criterion than the one set. Seeder work: **COMPLETE (D470).** `property_suffixes` 154/154 · `slots` 104/104 · `excluded_properties` 10/10 (all order-exact, D470) · `roles` 29/29 (D464) · `modifier_suffixes` 19/19 · `html_tag_to_core_block` 17/17 · `legacy_role_lookup` 15/15 · `markup_examples` wired. **No converter-load-bearing table is unreproducible.** `rebuild_compare.py`: identical 12→15, `empty (known Phase-1)`=0, `KNOWN_UNREPRODUCIBLE` now empty. Residue is NON-converter: T1.5 floor gate · T1.6 Group-4 decision · T1.7 WP corpus restore. |
...
> | **4 — purge** | 🔶 **PARTIAL.** ✅ 15 dead scripts deleted + 4 tables retired (`variations` D469, `block_styles`/`_meta_schema_version` D472, `array_item_fields` D475) + 2 dead accessors. ⛔ **NOT done:** the WIRE-OR-DELETE item (`check_preset_absence_no_slug_literal.py`), the `orchestrator.py`→`dispatch_spine.py` rename, the "Bean's call" file list, and `walk.py:20-26`'s false claim — all still untouched and previously untracked anywhere. |
