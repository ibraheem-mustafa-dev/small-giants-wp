...
- **DP6 (QC-added 2026-07-18) — the block is a CONVERTER EMIT-TARGET by construction (requirement §0a.3, made structural).** Every capability attribute must be settable by the cloning-pipeline converter, and attribute shapes must map to features the converter extracts from a scraped/draft header (Spec 31 + Spec 33 Part 2). "Converter-emittable" is a **design constraint on the P2–P4 block architecture, NOT a P6 bolt-on.** Rationale: a QC of this plan found Req §0a.3 was used to *justify* BUILD but never made a *design constraint*, leaving the differentiator gated behind the last (most stall-prone) phase — the same trap DP1 fixes for D328. **Prove it EARLY:** the DP4 vertical slice must **clone a real header INTO the new block**, not just hand-build it — so Req 3 is proven in the first slice, not discovered broken in P6.
