...
| 7 | ◑ **PARTIAL 2026-08-21 (D729) — 3 of 4 measured, 1 unanswered, and the claim itself was split.** "Good" has a mechanical half and an aesthetic half. **Mechanical (SAFE by default — does dropping it untuned leave content visible?): `scrub`, `scramble`, `split-reveal` all PASS** on canary page 2103 after scrolling mid-page and to the bottom — effective opacity 1.0 up the whole ancestor chain, non-zero boxes, real text. That is the failure mode competitors ship (content stranded at `opacity:0` for anyone landing mid-page) and SGS does not have it. Negative control: a planted `opacity:0` node measured through the SAME code path reported 0. **`pin` is UNANSWERED, not passing — no page on the canary uses it** (DB search 2026-08-21); it needs a fixture that does not exist. **The aesthetic half stays open and is Bean's eye, not a script (R-31-13).** Evidence: `plugins/sgs-blocks/scripts/motion-qa/probe-good-by-default.mjs`. | Verification | pin fixture + Bean's eye |
