...
status: IN EXECUTION — Waves A and B complete and pushed to main (steps 1-17 + step 16a's QA-7 council, all GO). Wave C re-audited 2026-09-14: steps 18, 20, 21, 22, 23 DONE (mostly via ad-hoc work outside this plan's own dispatch, evidence-checked, not assumed); steps 19 and 24 WITHDRAWN (D1036); step 25 (Bean's formal sign-off) and step 26 (flip G20c — unsafe, nav-menu itself has 7 findings) are GENUINELY STILL OPEN; step 27 (living docs) is PARTIAL. See §Execution Progress Log immediately below for the authoritative per-step state — read it before resuming, do not assume the step bodies further down reflect final committed reality (several steps landed with disclosed deviations from their original text).
---
...
### Wave C — re-audited 2026-09-14 (evidence-based, see below); most of it landed via ad-hoc work, NOT via this plan's own dispatch process

**Why this re-audit exists.** Between the 2026-09-11 "not yet started" note above and now, an
entire day and a half of ad-hoc work happened OUTSIDE this plan's process: Bean's full live
review of the deployed nav-menu (D1039), the terminology/architecture redesign (D1041), the
smart-contrast/gradient/sweep wave (D1042), and a `/qc-council` diagnostic pass (D1043). Several
Wave C steps' INTENT is now satisfied by that work even though the plan never dispatched them.
Verdicts below are evidence-checked against the step's OWN original pass condition, not assumed
from the ad-hoc commits' own descriptions.

| Step | Verdict | Evidence |
|---|---|---|
| 18 (FR-41-13 parent-stays-hovered) | ✅ **DONE-via-later-work** | Mechanism fully present in `nav-menu-css.php::` FR-41-13 block (4 rules: mouse+keyboard × bar+drawer, `:has()` on `ul.sgs-nav-menu__submenu` per the step's own edge-case warning). Landed via `99aa0aae0` (built) + `bdd80254e` (fixed the border-only-hover gating bug, G7 from step 22/23's own gate sweep) + D1039's same-session fix for "the rescue rule never covering WordPress core's own ambient link-hover default" (beyond the original step's scope, not a substitute for it). |
| 19 (FR-41-34 migration notice) | ⛔ WITHDRAWN 2026-09-11 | D1036 — no stored content anywhere uses the pre-3-state shapes; nothing to notify about. Unchanged. |
| 20 (QA-6 static gate sweep) | ✅ **DONE-just-now (2026-09-14)** | Ran every listed check live: `audit-inline-styling.js --check` PASS (0 violations/83 blocks) · `check-dead-controls.js --check` PASS (0 net-new; 5 pre-existing advisory findings, all `sgs/team-member`/`sgs/card-grid`, unrelated to nav-menu) · `check-empty-inspector-containers.js --check` PASS (0/133 files) · `check-dead-pattern-attrs.py --check` PASS · `check-duplicate-controls.js` PASS (0 net-new, 24 baselined) · `npm run audit:element-manifest` exit 0 · `placement-reach.py --block sgs/nav-menu` exit 0 · `python scripts/run-gates.py --tier all` → **99/99 PASS** (235.7s) · `node scripts/hover-guard/check.js` → **PASS, 0 findings** (the plan's own "named pre-existing exception" for G2 is STALE — it now passes clean on rebuild, no bypass needed). The `grep -rnE "hoverStyle|underlineColour|..."` sweep is NOT clean — 16 files still match, but every match read is a **comment/doc-string referencing the deleted names historically** (`render.php`'s own "were deleted" comment, `style.css`'s migration-history comment, `setting-reclassification.json`'s classification ledger, `.pyc` cache) — zero live code re-declares or reads a deleted attribute. ⚠ **New finding, not on the original checklist:** the reuse-ledger re-check (S13) surfaces real drift — `ColourRowExtras.js` is now **320 lines** against the ledger's recorded 248, and `edit.js` is **892 lines** against the plan's own disclosed-deviation note of 625 (further growth from D1039/D1041/D1042/D1043, undisclosed anywhere) — both still exceed the ≤250 cap; `nav-menu-css.php` (415 code lines) and `nav-menu-submenu-css.php` (339 code lines) both exceed the stated ≤300-code-line cap (raw `wc -l` 1104/1305, heavily comment-documented). These are NOT gate failures (no automated check enforces JS/PHP-file-length as a hard gate) but they are real, undisclosed growth past this step's own stated thresholds — flagging for a future split/ledger-refresh pass, not fixed in this audit dispatch. |
| 21 (build + deploy to canary) | ✅ **DONE-via-later-work, re-verified live 2026-09-14** | The canary has been redeployed dozens of times since via later commits' own deploy steps — most recently the `d9f90b875` smart-contrast-default fix (D1043). Confirmed NO DRIFT live: the deployed static block bundle (`.../nav-menu/style-index.css`) carries the D1039 burger-icon/drill-back-btn CSS, and the deployed per-instance CSS (`.../uploads/sgs-css/sgs-3593-*.css`) contains the literal `surface-pink` token that `d9f90b875` introduced as the new submenu-background default — that string cannot appear unless the very latest nav-menu commit is live. No PHP fatal on page load. |
| 22 (Playwright lane 1 — mechanism gates) | ✅ **DONE-via-later-work** | D1038, 2026-09-11: two parallel Playwright agents asserted G6/G13(1,2,4)/G14(a-g)/G15/G17/G18 against the live canary — 22/26 mechanism sub-checks passed, 4 FAILs root-caused and fixed same-session (`bdd80254e`), full gate:all (99/99) re-run clean, evidence at `.claude/verify/spec-41-gates-mechanism.md`. G13 scenario 3 correctly NOT asserted here (owned by withdrawn step 24). Residual INCONCLUSIVE items named explicitly in D1038 (G7 BAR/DRAWER independence, G9 disable-and-revert, G14(g) UI-half, G18(a)/(b), G20(a) editor round-trip) were NOT closed by D1038 and were not re-attempted in this audit — genuinely open sub-items, not blocking, per D1038's own framing. |
| 23 (Playwright lane 2 — interaction/accessibility gates) | ✅ **DONE-via-later-work** | Same D1038 sweep covered G7/G8/G9/G10/G16/G19/G20 — 15/22 interaction sub-checks passed, root-caused, fixed same-session; G19(b)'s reported FAIL investigated and found to be a verification-agent artefact, not a real defect. Evidence at `.claude/verify/spec-41-gates-interaction.md`. |
| 24 (G5a stored-content migration + G20b) | ⛔ WITHDRAWN 2026-09-11 | D1036 — no stored content anywhere uses the pre-migration shapes; G20b and G13 scenario 3 withdrawn with it. Unchanged. |
| 25 (Bean's visual sign-off, R-31-13) | ⚠ **GENUINELY STILL OPEN — Bean's call, not mine to close** | Bean's live review (D1039) is real evidence he has looked at the deployed site closely and in depth — 26 reported issues, ~18 fixed. But this step's own pass condition is narrower and more formal than that: (a) concatenate steps 22+23's evidence into ONE merged `.claude/verify/spec-41-gates.md` sign-off record — **this file does not exist**, only the two separate lane files do; (b) present the canary at 375/768/1440px with the new controls exercised, alongside the four FR-41-17a residual risks named explicitly; (c) get an explicit yes on THAT presentation. D1039's review was Bean finding defects unprompted, not Bean being shown a structured summary and residual-risk list and saying yes. **What's needed to close:** merge the two evidence files (mechanical, ~5 min), then present the 3-viewport + residual-risk summary to Bean and get an explicit yes/no — I am not authorising this step closed on his behalf. |
| 26 (flip G20c WARN→HARD, scoped to `sgs/nav-menu` only) | ⛔ **GENUINELY STILL OPEN — unsafe to flip right now** | Ran `check-ungated-paint-rules.py` live: it is currently WARN-ONLY and **`sgs/nav-menu` itself has 7 ungated-paint findings** (indicator background×2, burger background, burger-bar×2, drill-enhanced submenu, drill-back-btn) plus `sgs/nav-drawer` has 5 more — 50 blocks framework-wide carry findings in total. Step 26's own `On-Fail:` is explicit: "a gate that fails on day one is a gate nobody reads." Flipping now would hard-fail the build on nav-menu's OWN pre-existing findings. Did not flip. **What's needed to close:** the 7 nav-menu findings need triage + fix (or an explicit accepted-exception list) before the flip is safe — that's real remaining work, not a formality. |
| 27 (living-docs update) | ⚠ **PARTIAL — some done by sibling agents today, real gaps remain** | Confirmed landed (commits `ee3758bb9`, `0cfd9a499`, `a7ace41ff`, plus two parking-archive commits): `decisions.md` D1040-D1043 backfilled, `LEDGER.md` brought current, Spec 41 frontmatter/FR-status corrected (v0.4.9, `itemColourHoverGradient`, FR-41-37/38 folded in), one new STOP-CATALOGUE entry (DOM-reparent-breaks-`:has()`), two parking entries archived. **Still open:** `.claude/specs/README.md` row 41 is stale on the exact terms step 27 itself names — it still reads **v0.4.7** (spec is now 0.4.9) and still says "Wave C ... NOT started" (contradicted by the rows above). The **`colourRows` ATOMICITY + two-corpus-blindness structural defence** step 27 explicitly calls out as a new STOP-CATALOGUE entry to add (S4) was NOT added — today's STOP-CATALOGUE commit added a different, unrelated entry. `plugins/sgs-blocks/CLAUDE.md` not checked for staleness in this pass — out of scope for this audit (not touched, per this dispatch's file-ownership boundary). |

Steps 19 and 24 remain withdrawn (D1036), skipped permanently, downstream `Deps:` voided per
their own WITHDRAWN boxes. **Net position:** of the 8 live steps (18, 20-23, 25-27), 5 are DONE
(18, 20, 21, 22, 23), 2 are genuinely open (25 — needs Bean's formal sign-off presentation, 26 —
needs the 7 nav-menu findings triaged before it's safe to flip), and 1 is partial (27 — two
concrete doc gaps named above). Nothing in this table was marked done without a command output,
live check, or commit hash to back it.
