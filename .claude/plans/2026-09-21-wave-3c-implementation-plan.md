...
| 14 | U-16 — design at council revision | Header and footer entrance animation. Step 0d measured it: most of M-11 is already expressible through the universal animation extension; the build adds a distance preset, 500/800ms delays and makes the entrance its own layer on every block (keyframes on the standalone translate/scale properties, so it never collides with a block's own transitions or transform) (design `.claude/reports/2026-09-26-u16-entrance-design.md`, council GO WITH FIXES applied; the revision goes back past the same two reviewers before the build, plan §5 step 2a) | M-11 | eye | medium | `src/blocks/extensions/animation.js`, `src/components/AnimationControl.js`, `includes/animation-attributes.php`, `assets/css/extensions.css`, `scripts/generate-extension-attributes.js` |
