...
| 3.20 | ⚠ **PARTIALLY RE-VERIFIED 2026-09-08.** `flex-grow: 1` on two buttons and the hero content | **Hero content — confirmed NOT a defect (TOOL artefact).** `.sgs-hero__content`'s parent (`.wp-block-sgs-hero`) is `display:grid` (`grid-template-areas`, matching the draft's own grid model at `index.html:255`) — `flex-grow` is inert inside a grid layout, contributes zero visual effect. Live-measured content/media split is an exact 50/50 (712px/712px), consistent with the draft's split-hero grid. **The two product-card CTA buttons — NOT YET RESOLVED**, flagged rather than guessed: live's `.sgs-product-card__cta-row` is `display:flex` with a single child at `flex-grow:1` (renders 600px, full row width); the draft's `.sgs-button` is `display:inline-flex` with no explicit width (content-sized) — whether the draft's equivalent CTA context (inside `.sgs-gift-section__card`, a flex column) stretches its button to full width by cross-axis `align-items:stretch` or leaves it content-sized was not checked before time ran out on this pass. Needs a direct draft-vs-live width comparison on the equivalent gift-section/product-card CTA before calling it fixed or closed. |
