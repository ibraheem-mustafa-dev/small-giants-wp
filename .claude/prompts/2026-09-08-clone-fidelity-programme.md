...
| 3.21 | ⏸ **CODE FIXED + BUILD-VERIFIED 2026-09-08, DEPLOY BLOCKED (shared-worktree conflict, not mine).** ~~Pill styling drift (weight 600→500/700, text-align, 1px padding)~~ | ⚠ **Not yet deployed/live-verified — do not treat as closed until a follow-up confirms it live.** `npm run build` passed clean locally with this change (exit 0). `build-deploy.py --payload plugins/sgs-blocks/src/blocks/option-picker/style.css` refuses to run: a concurrent session has `plugins/sgs-blocks/src/blocks/before-after/{block.json,edit.js,render.php,style.css}` staged-but-uncommitted in this shared worktree, which the dirty-tree gate correctly refuses to bundle into a deploy under my payload (I did not touch those files, and `--allow-dirty` is banned per CLAUDE.md's D336 lesson). **Next step for whoever picks this up:** once the before-after work is committed (or the worktree is otherwise clean), re-run the deploy command above, verify the selected pill's `font-weight` live (should read the same value as the resting pill, not a hardcoded 700), and only then mark this CLOSED. **Weight half — REAL, root-caused.** Draft (`index.html:407-427`): `.sgs-product-card__pill` declares `font-weight:600` on the base rule and the `.pill--active` (selected) modifier changes ONLY colour/background/border — weight stays 600 for every state. Live: the resting pills correctly computed 600, but the SELECTED pill computed **700** — `plugins/sgs-blocks/src/blocks/option-picker/style.css` hardcoded `font-weight: 700;` on the `:checked` selector for all 3 style variants (outlined/filled/ghost, lines 225/260/290 pre-fix) instead of reading the same `--sgs-op-pill-font-weight` custom property the resting pill already uses (line 98) — a CLAUDE.md CSS-Override-Rule violation (hardcoded typography in a stylesheet) as well as a genuine visual drift. **Fixed** (single-block CSS, not a shared wrapper/walker/converter mechanism — in scope to build directly): all 3 hardcoded `700`s replaced with `var(--sgs-op-sel-font-weight, var(--sgs-op-pill-font-weight, 500))`, so a selected pill now inherits the SAME weight as its resting state by default, with a new optional override var for a future case that genuinely wants the selected pill bolder. Universal — applies to every `sgs/option-picker` instance framework-wide, not special-cased to Mama's. **Padding half is a SEPARATE, ALREADY-LOGGED item — not duplicated here:** draft `padding:7px 13px` vs live `8px 16px` is item **3.25** (open, tracked below in its own row) — not part of this closure. **Text-align/justify-content half — confirmed NOT a defect:** draft's `.pill` uses `display:flex; align-items:center` (vertical centring only, no horizontal rule); live uses `justify-content:center` (horizontal centring) — visually identical here because the pill's width is padding-driven and content fills it either way (equivalent-by-different-mechanism, matching the doc's own prediction). |
