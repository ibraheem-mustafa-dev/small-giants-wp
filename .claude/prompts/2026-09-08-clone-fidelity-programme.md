...
| 3.12b | ⚠ **NEW, SIGNIFICANT — NOT YET INVESTIGATED FURTHER. NEEDS BEAN'S SIGN-OFF before any fix (walker/converter surface, CLAUDE.md rule 7).** The announcement-bar / "send to ward" section never converts to `sgs/notice-banner` at all | Found while investigating 3.12. Live DOM: `document.querySelectorAll('[class*="notice-banner"]').length === 0` — zero notice-banner elements anywhere on the page. The section instead renders as a generic `sgs/container` + `sgs/text` + `sgs/button` composition. `sgs/notice-banner` (with `displayMode=announcement`) is the block this section is supposed to become (per `plugins/sgs-blocks/CLAUDE.md`'s own block-status table: `sgs/announcement-bar` was retired and absorbed into `sgs/notice-banner`'s announcement mode). This is a walker/BEM-matching miss — likely a `slot_synonyms`/`standalone_block` lookup gap for the retired `announcement-bar` root class — not a CSS bug. **Separately, a genuine latent bug was ALSO found sitting inside `notice-banner/style.css:96-98`: `.sgs-notice-banner__text a:hover{text-decoration:none}` hardcodes underline OFF unconditionally** — this would block hover-underline fidelity for any REAL notice-banner instance even after the conversion gap is fixed, and matches this project's own documented "hardcoded wrapper defaults are cheats to remove" pattern. Query `slot_synonyms`/`standalone_block` via `/sgs-db` before proposing a walker fix-shape. Do not build without a design-gate. |
