...
| 3.19 | ⚠ **PARTIALLY RE-VERIFIED 2026-09-08 — description too imprecise to act on; needs the parity tool's exact instance list, not guessed.** ~~`max-width: 1280px` applied to 3 elements the draft never capped~~ | Live-verified the candidate: `sgs/container`'s `contentWidth` default DOES resolve to `max-width:1280px` on `.sgs-container__inner` — but this is not isolated to 3 elements, it's the theme-wide default and currently applies to **10** separate `.sgs-container__inner` instances across the page (grid/flex/stack variants, including the site footer). Whether any specific subset of those 10 is a real defect depends on whether the DRAFT's matching sections explicitly declared no cap — which needs a fresh parity-tool run naming the exact 3 instances, not a blanket "does 1280px exist anywhere" check. Not fixed — re-verify with the parity tool's instance-level output before acting; do not guess which 3 of the 10 are wrong. |
| 3.20 | ✅ **CLOSED 2026-09-08 — fully re-verified, both halves confirmed non-issues (TOOL artefacts).** ~~`flex-grow: 1` on two buttons and the hero content~~ | **Hero content:** `.sgs-hero__content`'s parent (`.wp-block-sgs-hero`) is `display:grid` (`grid-template-areas`, matching the draft's own grid model at `index.html:255`) — `flex-grow` is inert inside a grid layout, contributes zero visual effect. Live-measured content/media split is an exact 50/50 (712px/712px), consistent with the draft's split-hero grid. **The two product-card CTA buttons:** the draft's actual product-card CTA (`index.html:861`, `<a class="sgs-button sgs-button--primary">Add to Cart — £10</a>`) sits directly inside `.sgs-product-card__body`, which the draft declares `display:flex; flex-direction:column` (`index.html:397`) with NO `align-items` override — CSS default is `align-items:stretch`, so the draft's button ALSO stretches to fill the full cross-axis width by browser default, exactly matching live's explicit `flex-grow:1` mechanism. Live-measured: button width 600px = card width 640px − 2×20px body padding, i.e. genuinely full content-width, same visual result the draft's implicit stretch produces. Equivalent-by-different-mechanism (draft: implicit `align-items:stretch`; clone: explicit `flex-grow:1`) — not a real defect. No code change made. |
| 3.21 | ✅ **CLOSED 2026-09-08 (second pass, genuinely root-caused this time)** ~~Pill styling drift (weight 600→500/700, text-align, 1px padding)~~ | **Honest note on the false start:** the first same-day fix (`2cf126f3e`) edited `option-picker/style.css`'s `:checked` rules to read the SAME `--sgs-op-sel-font-weight`/`--sgs-op-pill-font-weight` var chain the resting rule reads. Live re-measurement still showed resting=600/selected=500 — the fix changed nothing, because neither custom property is EVER emitted by any caller anywhere in the codebase; the var chain was a red herring the whole time, on both attempts. A fresh, from-scratch investigation (not trusting the prior framing) traced the REAL cascade via live CSSOM enumeration on page 2742: `sgs/product-card`'s own `render.php:209` injects `.{uid} .sgs-option-picker__pill{font-weight:600;}` (via the shared `sgs_typography_css_rule()` helper, product-card's own `pillFontWeight` attribute, stored `"600"` in this instance's post_content) at specificity (0,2,0) — this wins on the RESTING pill (which only competes against option-picker's own base rule at (0,1,0)), but LOSES on the SELECTED pill against option-picker's own `:checked ~ .pill` rules in `style.css` (lines 234/269/299), which declare their own `font-weight` at specificity (0,5,0) regardless of the value. That is the actual, provable mechanism — confirmed live via `document.styleSheets` enumeration finding `.sgs-pc-3 .sgs-option-picker__pill{font-weight:600}` in the page's collected CSS, and the base var-chain resolving to empty. **Root cause:** the `:checked` rules unconditionally declared their own `font-weight`, which will always out-specify ANY external caller's typography injection into `.sgs-option-picker__pill`, no matter what mechanism sets it. **Fix (universal, not product-card-specific):** removed the `font-weight` declaration entirely from all 3 style-variant `:checked` rules (`outlined`/`filled`/`ghost`) in `option-picker/style.css`. Since the selected pill is the SAME DOM node as the resting pill, removing the competing declaration means whatever won the resting cascade (shared default, a standalone `sgs/option-picker` instance's own `pillFontWeight`, OR a host block's typography injection like product-card's) now simply persists unchanged on selection — exactly matching the draft, where `.sgs-product-card__pill--active` never redeclares `font-weight` (`sites/mamas-munches/mockups/homepage/index.html:407-427`). **Live-verified post-deploy:** resting=600, selected=600 (was 500) — exact match, both states and the draft's value. No other real published page on the canary uses `sgs/option-picker`/`sgs/product-card` (checked via `wp db query`; the only other hits are dev scratch/test pages with no rendered content), so no regression surface beyond page 2742. Deployed via `build-deploy.py --blocks-only --skip-build --payload plugins/sgs-blocks/src/blocks/option-picker/style.css`; all gates + motion-QA + payload-checksum verify passed. Commit: see git log for `fix(option-picker): stop the :checked rule's own font-weight beating a caller's typography override`. **Padding half is a SEPARATE, already-logged item** — draft `padding:7px 13px` vs live `8px 16px` is item **3.25** (open, tracked below), not part of this. **Text-align/justify-content half — confirmed NOT a defect** (equivalent-by-different-mechanism, unchanged from the original finding). |
| 3.22 | ✅ **CLOSED 2026-09-08 — confirmed TOOL noise, no code change.** ~~Image-background cluster: `background-size`/`background-repeat`/`background-position`/`border-image-slice`~~ | Live-verified across every `<img>` on the page (hero split-media ×2, product-card images ×2, `sgs/media`): `background-size:cover`/`background-repeat:no-repeat`/`border-image-slice:1` are all present in computed style, but these properties are semantically INERT on `<img>` elements (a replaced element paints via `src`, never a CSS background layer or border-image) — they have zero visual effect regardless of value. The properties that DO apply to an `<img>` — `object-fit`/`object-position` — were checked on the same elements and are correctly `cover`/`50% 50%` throughout. Confirms this session's own hypothesis exactly; no fix needed, no visible defect. |
| 3.23 | ✅ **CLOSED 2026-09-08 — already correct, not a current defect.** ~~Product-card gaps wrong~~ | Live-verified: the products grid (`.sgs-container__inner` parenting the two product cards) computes `gap:16px`, an exact match to the draft's `.sgs-products { display:grid; grid-template-columns:1fr; gap:16px }` (`index.html:386`). Cards render flush at the correct 640px/384px 5:3 ratio (the 3.5 fix from earlier this session). No gap defect present live; likely already resolved alongside 3.5's grid-item fix. No code change made. |
...
| 3.16 | ✅ **CLOSED 2026-09-08.** ~~The mobile hero image has the wrong alt text~~ | BOTH | Was carrying the DESKTOP image's alt instead of its own draft alt. Root cause: the stored per-tier alt attributes (`splitMediaImageAlt`/`Tablet`/`Mobile`) were already correct — `hero/render.php` collapsed them onto the desktop value alone at the point of rendering, before handing one string to the shared `sgs_tier_media_render()` helper (also used by `sgs/timeline`). **Fixed as the approved shared-mechanism exception (design-gated, Bean-approved):** widened `sgs_tier_media_render()`'s `$alt` param to `string|array`, additive — a plain string (every other caller) is byte-identical; an array opts into a per-tier fallback-UP cascade mirroring the existing media-source fallback rule. Wired hero's already-resolved per-tier values through it, and added a tier-aware "Alt text" editor control (mirroring the existing tiered picker pattern) since the attributes existed but had no manual override control before. Reverses Spec 35 D5's "alt is NOT tiered" assumption for this one block's use case; `sgs/timeline`'s call site confirmed unaffected (still passes a plain string). Live-verified page 2742: 375px shows the mobile-specific alt on the visible `<img>`, 1440px desktop unchanged, zero console errors. Commit `a53a83df7`. Full detail: `reports/visual-diff/hero-2026-09-08.md` (second section) + `reports/visual-diff/hero-alt-text-2026-09-08.md`. **Still open on the tool side** (unchanged from before) — the parity tool has no art-direction pairing mechanism, tracked separately below. |
...
| 3.18 | ✅ **CLOSED 2026-09-08 — confirmed already working, live-verified.** ~~Borders not transferring~~ | Live-verified via a full-page computed-style sweep: 35 elements across 6+ distinct block families currently render a non-zero, non-`none` border on page 2742 — `sgs/button` (×7), `sgs/trust-bar` circles (×4), `sgs/option-picker` pills (×4), `sgs/info-box` (×6 across two instances), `sgs/container` (×3), `sgs/testimonial` cards (×9, colour-verified exact against the draft's `--border` token, see 3.14 above), `sgs/testimonial-slider`'s pause button (×1). Was an OLD-tool false positive (`::after`/pseudo-element paint blindness), now fixed. No code change made. |
