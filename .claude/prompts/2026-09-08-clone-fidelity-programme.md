...
| 3.21 | ⏸ **DEPLOYED + LIVE-VERIFIED 2026-09-08, BUT NOT ACTUALLY FIXED — a different mismatch remains, do not close.** ~~Pill styling drift (weight 600→500/700, text-align, 1px padding)~~ | The before-after worktree conflict cleared and commit `2cf126f3e` deployed cleanly as part of this session's payload (confirmed live in `option-picker/style-index.css`: both the resting rule and the `:checked` rule now read `var(--sgs-op-sel-font-weight, var(--sgs-op-pill-font-weight, 500))`). **But live measurement on page 2742's product-card pills (Mama's Zookies, the instance 3.21 was written against) shows resting=600 / selected=500 — NOT the intended "selected matches resting" outcome**, and arguably further from the draft's `600` than the old 700 was in the opposite direction. Root cause of the NEW mismatch (not yet fixed, not yet root-caused past this first pass): `getComputedStyle(pill).getPropertyValue('--sgs-op-pill-font-weight')` returns EMPTY on this instance — the custom property is not set at all — yet the resting rule still computes 600, not its own declared fallback default (500). That means the resting pill's 600 is being painted by something OTHER than the `--sgs-op-pill-font-weight` var chain (an inline per-instance `<style>` block on a MORE specific selector, most likely — not yet located), while the selected rule's identical-looking var chain has no such override and falls through to its innermost default, 500. The fix's assumption — "reading the same var as resting" — does not hold when resting isn't actually reading that var on the instance that matters. **Needs its own root-cause pass** (find what actually sets 600 on the resting pill first, per this project's mandatory method) before a second fix attempt — not done in this session, time-boxed out. Do not mark CLOSED. **Padding half is a SEPARATE, ALREADY-LOGGED item** — draft `padding:7px 13px` vs live `8px 16px` is item **3.25** (open, tracked below), not part of this. **Text-align/justify-content half — confirmed NOT a defect** (equivalent-by-different-mechanism, unchanged from the original finding). |
