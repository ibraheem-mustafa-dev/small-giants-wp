...
| 3.13 | ✅ **CLOSED 2026-09-08 — no code defect found, live-verified.** ~~Trustpilot bar has too much white space at the bottom~~ | Root cause investigated in full: `.sgs-social-proof`'s computed padding is `64px`/`64px` top/bottom — an EXACT match to the draft's `padding: 64px 20px`. The `.sgs-social-proof__trustpilot-bar` sub-element (the small "★ Trustpilot ★★★★★ Leave us a review" pill) has computed `padding:18px/18px` + `margin-bottom:28px` — also an exact match to the draft's `.sgs-social-proof__trustpilot-bar` rule. Measured the full vertical chain live: gap from section-top to `<h2>` = 64px; gap from the slider's last visible element (the pause/play button) to section-bottom = 64px — symmetric, matching the draft. The apparent "excess white space" is the carousel's dots + play/pause controls row (`.sgs-testimonial-slider__controls`, `margin-top:16px` + WCAG 2.5.8-mandated 44px touch targets, ≈108px of real height) — content the draft's static (non-slider) testimonial grid never had. This is the already-accepted slider divergence (see 3.14/3.12b: "we legitimately diverge by using a slider"), not a padding/margin bug. No code change made. |
