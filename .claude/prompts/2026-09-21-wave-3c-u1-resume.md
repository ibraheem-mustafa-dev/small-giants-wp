...
Residuals to tell Bean: the client theme snapshots `sites/eye-care-ward-end/theme-snapshot.json` and `sites/mamas-munches/theme-snapshot.generated.json` still carry the OLD four shadow presets and would put them back if pushed (not edited: generated artefacts); the canary's scratch page 2526 still stores `shadow: subtle`; converter fixtures and the `design_tokens` shadow rows still name the old presets until the reseed; explicit-props ShadowControl mounts (about 20) still make two writes, not one; dark mode shadows are now black at the preset opacity (weaker than the old per-preset dark values), tune by eye; the panel writes the colour attribute as a list, so a block that also shows that attribute in `SgsColourPanel` (trust-bar icon circle and badge image) shows a garbled swatch for a multi-layer stack.
