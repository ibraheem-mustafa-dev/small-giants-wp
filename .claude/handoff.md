...
4. **Section-root row routing for Mamas-mockup-classes** — section-scope rows in slots table currently route: `featured-product → sgs/featured-product`, `social-proof → sgs/testimonial`, `gift-section → sgs/feature-grid`, etc. Per Bean's analysis (Cases 1/2/3 earlier in session), these should likely all default to sgs/container (the walker's FR-22-4 default) instead of routing to specific content-block primitives that collapse sibling structure. Empirical test via /sgs-clone measurement will show whether this matters.
