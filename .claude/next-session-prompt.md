...
**Also observed live (same defect class — theme-thread cross-confirmation 2026-06-11):** desktop hero columns compute 199px/1169px. Theme thread confirms (WS-4 `bacbde57` + `64fc1f3a`): the hero's section IS the container; the split grid is DESIGNED to be driven by **`splitColumnRatio`** (its `:where()` default deliberately loses to the attr) and the split columns consume **`contentPadding*`/`mediaPadding*`** — NOT `gridItemPadding`/raw `gridTemplateColumns*`. The converter emitted the GENERIC container attrs instead of the owning block's DESIGN attrs — the SAME wrong-destination class as the padding. The universal router's destination lookup must resolve per owning block from `block_attributes` (DB-driven), so `grid-template-columns: 1fr 1fr` → `splitColumnRatio` on a split hero and `padding` on the content area → `contentPadding*`. **Bean design steer (2026-06-11, fold into the router's design gate):** (a) the destination lookup must not be prefix-family-hardcoded — match by property-suffix SEMANTICS within the owning block's REAL attr set (the DB already holds suffix + layer + the block's attrs; "padding on the content area" must find `contentPaddingTop` despite shorthand/prefix technicalities); (b) `contentPadding*` is NOT bespoke (it's FR-22-21's CONTENT-layer canonical name — the padding failure was a layer-CHOICE bug), but **`splitColumnRatio` IS pre-merge legacy duplicating the mirror's `gridTemplateColumns` capability** — propose a block-side normalisation sweep across the 29-block mirror roster for capability-DUPLICATING bespoke attrs (migrate to mirror names + deprecated.js shims; THEME-thread work, Bean gate) so the converter needs zero per-block intelligence for mirrored capabilities; genuinely-new capabilities (e.g. `mediaPadding`) stay block-specific and the suffix matcher handles them. Separate smaller leftover: the `__ctas` wrapper emits as a container with a draft BEM className around the multi-button (pre-existing Rule-1 leftover, not fold-related). Probe scripts: `c:/tmp/verify-hero.py` + `verify-hero2.py`.
