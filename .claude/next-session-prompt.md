...
- **STOP-22 — The frozen `convert.py` is NEVER the design reference or the fact-check oracle for a rebuild stage — WITH ONE CARVE-OUT (D246).** The authority is Spec 31/22 + the modern DB tables + the draft. `convert.py` is consulted for exactly two purposes: (1) to NAME the bug being killed (BROKEN CSS-side assumptions — never the design reference); (2) to **READ-TO-PORT a function that Spec 31 §1's "ONE lift path" inventory names as WORKING and in-scope for modularisation** (`_lift_scalar_attrs_by_selector`, `_lift_scalar_media_from_img` — the content side). Reading-to-port is NOT importing-brokenness. If §1 names a function in the one-dispatch list, **reading it in full to modularise it faithfully is MANDATORY before designing its replacement, not forbidden.** Tell council/qc raters to fact-check against Spec+DB+draft, never the BROKEN engine logic — but the working content functions ARE the port source. (`feedback_rebuild_stage_authority_is_spec_and_db.md`, `do-not-recreate-working-functionality-modularise-it`.)
- **STOP-26 (NEW, D246) — Before designing ANY rebuild stage, read the WHOLE target spec holistically (not greps), and state Spec 31 §1's COMPLETE function inventory in plain English BEFORE the first design sketch.** A section-by-section / grep read CANNOT satisfy this — §1 "ONE lift path" must be read in full, first, because it is the SYSTEM MAP that names what already exists + works (incl. the content functions) vs what is genuinely absent. A council "X is missing from the spec" finding is a HYPOTHESIS about the spec TEXT — verify against the §1 inventory BEFORE adding any parallel mechanism. (This session's exact failure: §1 lists `_lift_scalar_attrs_by_selector` + `_lift_scalar_media_from_img` in the one-dispatch list; a grep-section read missed them; a separate content engine got built instead of porting the two functions. The reading gate enforced §12 build-direction + §2 architecture but NOT §1's component inventory.)
...
| `/adversarial-council` · `/qc-council` | Rule-7 design-gate on a new SHAPE; **pre-commit qc-council on the BUILT converter code (STOP-23, blub 255)**; FACT-CHECK the council against ground truth (STOP-15). **MANDATORY architectural-completeness pre-question on EVERY rebuild-stage design (D246): "Does this design RECREATE something Spec 31 §1's one-dispatch inventory already names as working (a new module / new DB column / new mechanism for functionality a §1 function already provides)? PASS = re-houses/modularises existing logic; FAIL = recreates it." A STOP-25 FAIL from any rater = NO-GO. The councils this session validated mechanism SOUNDNESS but were never asked architectural COMPLETENESS — that gap is why the separate-content engine passed every gate.** |
