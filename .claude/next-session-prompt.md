...
each commit: path-scoped (`git commit -m "msg" -- <paths>`, -m BEFORE --); Gate A conformance; the F5 commit-gate runs the gates; D-ceiling check before any new D (→ D241); convert.py UNTOUCHED (D-MODULAR)
```
