...
**Also observed live (same defect class — theme-thread cross-confirmation 2026-06-11):** desktop hero columns compute 199px/1169px. Theme thread confirms (WS-4 `bacbde57` + `64fc1f3a`): the hero's section IS the container; the split grid is DESIGNED to be driven by **`splitColumnRatio`** (its `:where()` default deliberately loses to the attr) and the split columns consume **`contentPadding*`/`mediaPadding*`** — NOT `gridItemPadding`/raw `gridTemplateColumns*`. The converter emitted the GENERIC container attrs instead of the owning block's DESIGN attrs — the SAME wrong-destination class as the padding. The universal router's destination lookup must resolve per owning block from `block_attributes` (DB-driven), so `grid-template-columns: 1fr 1fr` → `splitColumnRatio` on a split hero and `padding` on the content area → `contentPadding*`. The hero block needs ZERO changes (2-thread convergent). Separate smaller leftover: the `__ctas` wrapper emits as a container with a draft BEM className around the multi-button (pre-existing Rule-1 leftover, not fold-related). Probe scripts: `c:/tmp/verify-hero.py` + `verify-hero2.py`.
