...
## Done at D293 close (no action needed — noted so you don't re-do them)
- **Converter finding #4 DONE** (`3abe7fc2`): the box-object dict-merge now gates on
  `db_lookup.box_family_for(slug, attr)` (both merge sites), 439 tests pass.
- **CSS-keyword sanitiser DONE for hero/quote/label** (`4787b95d`) as a standalone security fix
  (media needed none). The remaining blocks get the sanitiser (contract §D) during their migration.
