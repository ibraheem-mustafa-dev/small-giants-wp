...
**Fix-shape (within the approved ROUTING-CATEGORISATION design — its worked example names `contentPadding*` as the destination):** pass `is_content_band=True` from the net-sole grid fold branch; expand padding shorthand→longhands before the CONTENT lookup (scoped to the content-band path, zero blast radius elsewhere); identify + suppress the OUTER duplicate writer. `/qc-council` per commit; live page-8 re-verify: `.sgs-hero__content` computed padding = 28/20/40 → 56/48 → 72/64 at 375/768/1440 AND the hero root has none.
