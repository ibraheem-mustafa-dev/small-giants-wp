...
### READ FIRST (corrected + adversarial-council-checked; v1 reports DELETED — use v2 only)
1. **`.claude/reports/2026-06-05-32pt-rootcause-QC-corrections.md`** — START HERE (QC verdict + the Family-A correction: **CSS-inheritance-not-simulated**, NOT "class-CSS-not-extracted").
2. **`.claude/reports/2026-06-05-32pt-rootcause-v2-part{1,2,3}-*.md`** — the EVIDENCE-GROUNDED per-point reports (v1 deleted — they contained falsified conjecture).
3. **`.claude/reports/2026-06-05-taskB-clone-verification-methodology.md`** — `clone-parity.js` design (see the corrected coverage below).

### ⚠ ADVERSARIAL-COUNCIL CORRECTIONS (2026-06-05 — these OVERRIDE the WS-A/WS-B text below; do not act against them)
A 3-persona council (conjecture-hunter + WS-B feasibility-skeptic + consistency-auditor) stress-tested both outputs. Binding corrections:
- **DO NOT ACT on these 7 points until live-DOM-grounded** (UNVERIFIED — may not even be real defects): **#8** (product image height — bound-path CSS already hardcodes 220px), **#12C** (brand image radius — query `sgs/media` block_supports first; "almost certainly" is a guess), **#25** (gift card heights — no evidence of a real defect), **#11a/#11b/#11c** (buttons-full-width / same-height / price-superscript — no draft rule found), **#19** (gift-sub `textAlign:center` origin unconfirmed — a wrong fix could break an intentional centre).
- **Three SYSTEMIC roots (fix once, not per-point):** (i) **`__experimentalBorder` missing on multiple blocks** (`sgs/product-card` #6/#7, likely `sgs/media` #12C) → **codebase-wide audit** of blocks lacking it, not per-block PRs. (ii) **CSS-inheritance-not-simulated** (#13/#16/#27/#28a) → ONE generalised `_collect_css_decls_for_element` fix. (iii) **Own-class-property-map incompleteness** — a SEPARATE gap from (ii): `margin-bottom`/`align-items`/`justify-content`/`font-family`/`background` are dropped even for OWN-class rules (#27 proves it: `text-align` extracts from the rule but `margin-bottom` doesn't). **Do NOT conflate (ii) and (iii) — two distinct fixes.**
- **CORRECTED prior claims (don't repeat):** `productId=0→empty-state` is FALSE (cards render real content; the defect is missing CSS — `border-radius:0`/`border:none` via the `__experimentalBorder` gap; productId binding is a SEPARATE concern, not the cause). Multi-card "slot-collision / wrong label / wrong price" is FALSE (emit is per-card correct; the real card gaps are font-family + margin-bottom). `backgroundShape:"circle"` is a REAL sgs/icon attr (not invented); the draft mechanism is `border-radius:50%`+`background:white` on the `.sgs-trust-bar__icon` span (partially dropped by the D2 lift). The hero image-gap (#2) is the **double-wrap DOM layer, NOT the gradient**.
- **#31 is NOT "routing correct" (corrected 2026-06-05 — I trusted a DB column instead of reading the block):** `sgs/card-grid` block.json has **`"items": {"type":"array"}`** (+ query mode) — it holds cards via an array ATTR, not InnerBlocks. `has_inner_blocks=0` is accurate but was the WRONG gate signal; the router treated "no InnerBlocks" as "can't hold cards" and fell back to `sgs/container`. **Fix = recognise + route-to + populate ARRAY-DRIVEN content blocks** (`card-grid.items`, `announcement-bar.messages` #26, `trust-bar` badges) — this is the SAME systemic gap as R7 (the router's capability gate must consider array-content attrs, not only `has_inner_blocks`; + the array extractors). IMPL.
- **Alignment (#13/#16/#19) solution (Bean-directed):** NOT just "simulate inheritance." The draft sets `text-align:center` on ancestors (`.sgs-ingredients-section__inner` line 507) AND some own-classes; absence = left (LTR default). Robust fix = **a per-instance `textAlign` control on each text block (heading/text/label), theme default left, converter sets it from the draft's effective alignment** (own-class first, else inherited ancestor) — owner/AI can override per instance. (Spans R2 ancestor + R3 own-class.)
- **General lesson (binding):** do NOT conclude "correct/no-fault" from a single DB column or a script's logic without reading the block's ACTUAL capability (block.json attrs + render.php). A DB value can be accurate yet the WRONG signal for the question asked.
- **WS-B is NOT ready to build as specified — NO-GO without a proof-spike first.** Honest coverage is **~55-65% basic / ~70-75% robust, NOT 90%** (that number was invented). Four unproven assumptions must be proven FIRST (short spike): (1) BEM+structural-path correspondence actually pairs correctly on the real clone HTML (WP injects `wp-block-*` wrappers; the double-container makes two `.sgs-container`; composites render as `sgs/container` *wearing* the BEM class → **false-PASS on every composite defect until Method-2 routing exists**); (2) draft images 404 locally → all image checks silently skip (so #11/#12D are NOT covered in basic); (3) canary computed styles come from `wp_global_styles` post-7 not the draft CSS-vars → token noise-FAILs need a reconciliation step (unspecified); (4) the 40+-FAIL first-run triage is bounded. Build the BASIC version first (cap coverage ~55-60%, treat run 1 as calibration not ground truth), extend only after it's proven on page 144. **Residual it can NEVER catch:** converter routing (symptom not cause), JS-interactive/slider, a11y/semantics, text-content completeness, font-render fidelity, perception-gestalt.

### 🎯 TARGETED BRIEF (read THESE for this session — the full STOP catalogue + reading list further down is REFERENCE-ONLY, not a required read)
**Read (in order):** (1) the 3 docs in READ-FIRST above; (2) Spec 22 **§FR-22-21** (transfer procedure) + **§FR-22-5** (CSS routing) — the spec the fixes must honour; (3) the draft `sites/mamas-munches/mockups/homepage/index.html` (the truth) + the latest run artefacts `pipeline-state/mamas-munches-homepage-2026-06-05-103529/` (extract.patched.json, leftover-buckets.json, convert-trace-b*.jsonl); (4) memories `llm-eyeball-clone-verification-unreliable`, `diagnosis-without-delivery-needs-conformance-gate`, `composite-mirror-is-separate-from-cloning-fidelity`.
**STOP entries that actually bind this session** (the rest of the catalogue is historical reference): **#33** faithful-transfer-not-detection-hacks · **#34/#44** verify-every-subagent-finding-vs-ground-truth (this session's whole lesson) · **#50** confirm the target CSS reaches the targeted path BEFORE building · **#51** not-done-until-live-DOM-verified-AND-on-main · **#52** "exists"≠"applied" (grep call-sites) · **#53** one-snapshot/one-viewport is untrustworthy + pixel-diff under-reports collapses · **#19** roll back fast on regression · **#32** design-gate sensitive/high-blast-radius changes · **#41/#45** shared-tree: commit by explicit path, `git log -1 --stat`, re-read shared docs before editing. New this session: **never assert a causal mechanism without a quoted evidence chain** (draft CSS + emit + code) — mark UNVERIFIED otherwise.
**Minor consistency nits to clear early:** parking P-TRUSTBAR top `Status:` label (body already RESOLVED→archive); `.claude/CLAUDE.md` D-ceiling D167→D178.
...
**META ROOT CAUSE (confirmed, 3 agents converge — NOT poor docs):** the specs (Spec 22 §FR-22-21/§FR-22-5) correctly describe the full lift. The recurrence is **good documentation + UNDELIVERED or MIS-WIRED code**, in four modes: (1) **written-but-never-wired** — `_lift_core_block_style`/`_lift_styling_attrs` (typography) existed with correct maps but ZERO call sites (dead code); (2) **split-mechanism misapplication** — `_detect_grid_container_from_css` sets `layout:"grid"` correctly but is only wired into the FOLD path, NOT the slug-None/composite emit path (which uses the A1/A3 `property_suffixes` helper where `display` has no DB row); (3) **consciously deferred** — `convert.py:~3191` comment literally says nested-grid attr-lift is "a deferred Phase-2 enhancement"; (4) **whole workstreams zero commits** — WS-2/WS-3 (B2/B4/C2-C5) planned + documented, never built; A3 grid lift "built-uncommitted" D163, never merged. **Systemic cause: NO spec↔code conformance gate** — nothing catches a dead lift / skipped path / forgotten defer, so each session re-diagnoses the same symptoms.

**DELIVERED + committed + live-verified last session (broke the diagnosis-without-delivery pattern):**
- **Padding lift on slug-None sections** (`1cf0692d`) — un-gated `_lift_root_supports_to_style` for slug-None; live: generic sections render their draft padding (64px 20px, was 0).
- **Trust-bar de-hardcode** (`e75db509`) — removed the static `repeat(4,1fr)` CSS; grid now via the universal container `layout=grid`/`gridTemplateColumns` wrapper mechanism + self-sufficient defaults; live: badges render a horizontal row (were vertically collapsed). visual-diff PASS.
- (Earlier: A4 sidecar delete `70bf6a37`, A5 modifier-carry+variant-harden `107723be`, FS-5 real image sideload `51e9ab13`, universality fixture `0d56fee5`.)

**✅ SHIPPED + LIVE-VERIFIED + on main (2026-06-05 — items 1-3 below are DONE, do NOT re-do):** typography descendant/responsive collector — hero H1 renders 58px, section H2 36px (`642cad61`); grid→`layout:grid` bridge on nested wrappers — products cards fill 640/384, trust-bar horizontal, brand/gift 2-col NOT regressed (`c97f85f1`); D5 product-card fills its grid track (`b3e3b284`). Plus earlier: padding lift (`1cf0692d`), trust-bar de-hardcode (`e75db509`). **9 fixes + the meta-diagnosis delivered — the diagnosis-without-delivery pattern was broken.**

**REMAINING (next session):**
- **(4) Conformance gate** + **(5) doc-drift reconcile** (below) — STILL OPEN.
- **NEW from this session's live verify:** typography-lift value nits (the colour token serialises malformed — `var(--wp--preset--color--varpresetcolortext)` instead of `…--text`; `line-height:1.15px` should be unitless `1.15`); social-proof renders a CAROUSEL where the draft is a static 3-col grid (block-choice mismatch); the A2 (composite) + A3 (top-level section) grid paths were DELIBERATELY excluded from the grid bridge (setting layout:grid there emits inline grid at (1,0,0) that beats the responsive media-query and collapses desktop 2-col — the D109 regression class) — leave excluded unless a different mechanism is designed.

**ORIGINAL REMAINING (now reduced to 4+5 above):**
1. **Typography descendant/responsive collector (the headline defect).** `_lift_typography_to_block_attrs` (NEW, `convert.py`, shipped `1cf0692d`) lifts a leaf element's OWN class-targeted CSS — but the hero H1 (58px) + section H2 (36px) come from DESCENDANT-combinator + `@media` rules (`.sgs-hero__content h1{font-size:58px}`) the leaf CSS collector doesn't gather → hero H1 renders 32px not 58px. **FIX:** extend the leaf-element CSS collector (`_collect_css_decls_for_element`/`collect_css_for_classes`) to gather descendant-combinator + responsive (`@media`) typography for the element, then the existing lift maps it. SERIAL on convert.py.
2. **Grid→`layout:"grid"` bridge (Batch 2 — highest value, highest regression risk).** 0/42 emitted containers set `layout:"grid"` (only `gridTemplateColumns` as a value). **FIX (DB-first, R-22-1):** add a `display` row to `property_suffixes` (→`layout`) OR wire `_detect_grid_container_from_css` into the slug-None/composite emit path (the A1/A3 helper), AND un-defer the nested-grid lift in `_emit_wrapper_container` (`convert.py:~3169`). Sets `layout:grid` universally → unblocks held D5 (cards fill their track via the `sgs-container--grid` selector) + faithful grids + trust-bar receiving a draft's grid. SERIAL on convert.py. **/qc-council pre-build (STOP #50) + verify HARD (this is the layout-emission change prior sessions regressed — D109).**
3. **Commit held D5.** `product-card/style.css` is UNCOMMITTED on disk (the `.sgs-container--grid > .product-card{max-width:none}` rule) — correct CSS, inert until Task 2 emits `layout:grid`. Once Task 2 lands, re-clone + verify cards fill (640/384) → commit D5 with a visual-diff report.
4. **Conformance gate (the structural fix to stop recurrence).** Add a test asserting every section-emit path runs the full §FR-22-21 lift + no lift function is dead (grep call-sites) + no "deferred" lift silently un-wired. This is what stops diagnosis-without-delivery.
5. **Doc-drift reconcile.** Close the misclassified hero-gradient parking entry (P-CSS-TRANSFER-FIDELITY gap 3 — D172 resolved it as a block-default, not a converter bug); fix D172's stale "REMAINING" list (A5/FS-5 shipped same-session). Per the doc-walk discipline.

**Verified ground truth for this session (do NOT re-derive):** hero IS `sgs/hero` conf 1.0, renders 2-col + 520 min-height + image loads (my prior "collapsed" read was a wrong-element snapshot — STOP #53). Sections correctly route to `sgs/container` (R-22-9, NOT a routing bug — do not "register composite blocks"). The hero gradient is a BLOCK CSS default (hero/style.css), not a converter emission. pixel-diff ~56-58% is informational (FR-22-18); trust live DOM.

**Orchestration:** SERIAL on convert.py (Tasks 1, 2 — one Sonnet subagent each, verify each on live DOM before the next); D5 + conformance-gate + doc-reconcile can follow. /qc-council pre-build on Task 2 (layout-emission, high blast radius). Live-DOM verify (re-clone --deploy-target page:144 + Playwright @1440/768/375) BEFORE every commit. Commit by explicit path (theme thread shares the tree — `git log -1 --stat`, STOP #41/#45).

> The 2026-06-04 PM4 opener below is DONE (all 5 items — FS-5/A5/A4/D5-attempt/fixture — shipped or superseded; D5 deferred to Task 3 above). The structural-defence sections further down REMAIN BINDING.

## ⚡⚡⚡⚡⚡ 2026-06-04 PM4 — METHOD 2 BUILD: Phase A CORE + FS-4 SHIPPED → FINISH FS-5/A5/A4/D5/fixture/gate [SUPERSEDED by the 2026-06-05 opener above; kept for history. Structural-defence sections further down REMAIN BINDING]
...
| 51 | **NEW 2026-06-05. Treating a fix as "done" because the spec/decision/plan says so, without grepping the converter code** | Recurring clone defects were diagnosed + designed repeatedly but the code never landed — written-but-dead (`_lift_core_block_style` zero call sites), built-but-never-committed (A3 D163), or whole workstreams with zero commits (WS-2/WS-3). A fix is NOT done until it is live-DOM-verified AND on `main`. Verify every "shipped/resolved" claim by grep before trusting it (feedback_shipped_claims_need_grep_verify). |
| 52 | **NEW 2026-06-05. Assuming a lift/transfer happens because the function EXISTS** | "There's a function that lifts font-size" ≠ "font-size is lifted." `_lift_core_block_style`/`_lift_styling_attrs` had correct maps but ZERO call sites — dead code. ALWAYS grep call-sites; "exists" ≠ "applied". A capability defined but un-wired is the recurring failure mode here. |
| 53 | **NEW 2026-06-05. Trusting a single computed-style snapshot on a guessed selector over a DOM-walked per-section comparison** | I read `1425px 0px` off a guessed `.sgs-hero` and reported the hero "collapsed" — it was the outer wrapper, not the hero block (which renders 2-col + image fine). Walk the DOM to the ACTUAL block element; one snapshot on one selector is untrustworthy. Also: pixel-diff UNDER-reports a layout collapse when the bg colour still matches (trust-bar scored 10.8% while visibly collapsed) — trust live DOM, not the % (STOP #20 extended). |
