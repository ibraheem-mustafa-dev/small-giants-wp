...
primary_goal: "LANDED-verify the new DB-recognition array field-lift on the REAL homepage (sandybrown page 8): deploy an SGS_NEW_ENGINE=1 clone of the Mama's draft, confirm the trust-bar renders its 4 draft badges (icons + captions, no phantom row, no client-copy default) at 375/768/1440 vs the draft + Bean's eye. THEN complete the slot-vocabulary so the 5 blocks whose item field NAMES don't resolve (icon-list iconName/iconSource, social-icons platform, card-grid badge, pricing-table cta/ribbon/priceYearly) lift their content — reseed-surviving slot aliases, following Spec 31 §3.B4/§13.3 FR-31-2.5 in EVERY detail. Every pipeline change follows the spec exactly; no design-gate."
---
