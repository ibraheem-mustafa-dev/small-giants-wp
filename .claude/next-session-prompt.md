...
## Task 1 — Testimonial slides render EMPTY [ROOT CAUSE FOUND + BUILD READY — execute the build plan]

**STATUS (D212, 2026-06-11):** investigation + design-gate COMPLETE. Root cause CONFIRMED via 3 evidence sources: `block_composition.has_inner_blocks=1` is STALE for `sgs/testimonial` (D8 typed rebuild left it hardcoded `1` in `seed-composition-roles.py:60`) → converter emits `sgs/star-rating`+2×`sgs/text` CHILDREN → typed render.php ignores them → empty slides. `/adversarial-council` ran (6 personas); its arch objection was overturned (Bean + Spec 22 §FR-22-2), its security/Rule-4/flag-child-only/variant must-fixes retained.
**EXECUTE:** `.claude/plans/2026-06-11-testimonial-universal-lift-build.md` — the self-contained 7-step build (flip flag child-only + populate the universal DB lift data + wire the one missing universal multi-scalar lift + esc_html names + clamp stars + Rule-4 card chrome + re-clone + live-verify). UNIVERSAL DB-driven lift, NOT a bespoke handler, NOT a new table (STOP-A/STOP-B in that plan). `/qc-council` MANDATORY per converter commit.
**Acceptance:** live page-8 `.sgs-testimonial__quote`/`__name`/`.sgs-testimonial__stars` innerText > 0 at 375/768/1440 (2-attestation); ledger SP-C/D/E flip VERIFIED. Aggregate differ NOT the gate.
