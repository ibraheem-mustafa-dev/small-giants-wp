...
> **WS-1 A1+A2 is DONE (shipped 2026-06-03, `2f86d9e6`).** The new opener is one (or a sequenced mix) of: **(a) PAGE-TRIAGE** — root-cause the 8 issues in `P-CLONE-PAGE-VISUAL-TRIAGE` (parking) via `/systematic-debugging` on the run artefacts (`pipeline-state/mamas-munches-mamas-homepage-ws1-2026-06-03-060940/`) + draft-vs-clone live-DOM; #3/#4 already verified pre-existing. **(b) WS-4 composite-remodel** (Bean SHARPENED directive D159.2) — remove each composite's drifted built-in wrapper, replace with an EXACT mirror of the current `sgs/container`, then add an `/sgs-update` step that walks `block_composition` + re-mirrors every composite on container version-bump; hero/trust-bar (#1/#2) are drift symptoms; this is sensitive/large → design-gate + /qc-council first. **(c) WS-1c** — A3 custom-width centring / A4 raw-px gap / A5 min-height / A6 gridItem* (A7 likely MOOT, dead code). Serve the mockup on localhost + diff computed styles vs page 144 (STOP #35) before any fix. The original WS-1 build detail below is retained for WS-1c reference.
