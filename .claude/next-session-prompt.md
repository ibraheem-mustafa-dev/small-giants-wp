...
### Side task — the scalar branch's resolution-path inconsistency (a UNIVERSAL fix, NOT a 6-slot patch)
**Diagnostic (D244 universal-recognition audit):** NAMED (74/74 sgs blocks) + ATOMIC (16/16 tags) recognition is fully universal. The SCALAR branch resolves 34/40 *populated* slots — the 6 misses are EVIDENCE of ONE mechanism gap, not 6 bugs: `bem_element_to_canonical_slot` goes `token → db_lookup.canonical_slot_for (synonym map) → standalone_block_for`, which DIVERGES from the slots table's direct `slot_name → standalone_block` for any slot whose name isn't a synonym-map key (`backgroundMedia`, `buttonSecondary` resolve to None; `cta` resolves to `sgs/button` while the slots table says `sgs/cta-section` — a DB-consistency divergence; `hero`/`cta-section`/`core/group` are composite/structural slots that correctly belong to the NAMED branch, not scalar).
**⛔ ANTI-CHEAT (Bean's directive — the fix MUST be universal):** Do NOT special-case `backgroundMedia`/`cta`/etc. A fix that only handles the 6 named slots is a per-slot carve-out (Rule 3 / R-22-9 / STOP-7) and FAILS. The fix is to make the scalar element-token→block resolution a SINGLE DB-driven path consistent with the slots table (e.g. resolve `slot_name → standalone_block` directly from `slots`, with the synonym map as a fallback for aliases) + reconcile any DB divergence via F6 / a dated migration. **Acceptance is UNIVERSAL:** every populated genuine-scalar slot resolves correctly through the one path, tested across the FULL `slots` table (not the 6) — and the 6 are just the currently-failing rows that flip green when the path is unified. Then seed the 63 unmapped slots via `/sgs-update` (dated migration + reseed, never manual — `db-changes-reproducible-via-migration`); baseline today's unmapped-slot UNRECOGNISED set before arming `coverage_report --check` as a build-blocker (STOP-14).
