...
1. **[DESIGN-GATE FIRST] Remove `sgs/mobile-nav`; `sgs/adaptive-nav` handles the menu/nav-bar GLOBALLY** (Task 1). HIGH-BLAST-RADIUS — adaptive-nav must absorb the whole off-canvas drawer incl. the FR-S9-5 a11y contract (focus-trap, ESC, body-scroll-lock, the D323 P0 re-parent-to-`<body>` fix, the socials/tagline zones, the toggle). **Rule 7: design-gate + Bean approval BEFORE building.** Do NOT start building until the design is approved.
...
- **STOP-DESIGN-GATE-HIGH-BLAST-RADIUS (D335, NEW)** — a change that removes/merges a whole block or re-homes an a11y-critical mechanism (the mobile-nav→adaptive-nav drawer absorption) is HIGH-BLAST-RADIUS: design-gate + get Bean's approval BEFORE building (Rule 7). Never rush it at the tail of a long session.
...
2. For Task 1 (mobile-nav removal): is this DESIGN-GATED + Bean-approved BEFORE I build (STOP-DESIGN-GATE-HIGH-BLAST-RADIUS)? Have I mapped how adaptive-nav absorbs the FR-S9-5 a11y contract (focus-trap/ESC/scroll-lock/P0 re-parent/socials/toggle)?
...
## Task 1: DESIGN-GATE + build the mobile-nav→adaptive-nav re-architecture
**What:** remove `sgs/mobile-nav`; `sgs/adaptive-nav` becomes the single global nav system (desktop bar + mobile collapse + the off-canvas drawer). **Why:** Bean — one nav system, no duplication. **DESIGN-GATE FIRST (Rule 7):** with `/brainstorming` design mode, produce a design for how adaptive-nav absorbs the FR-S9-5 drawer a11y contract (focus-trap, ESC, backdrop-dismiss, body-scroll-lock, the D323 P0 re-parent-to-`<body>` fix, the socials zone reading Site Info per Task 2, the toggle + `aria-controls`/`aria-expanded`), what happens to `sgs/mobile-nav` + `sgs/mobile-nav-toggle` (retire? absorb?), the header.html template change, and the CPT/DB reseed. **Get Bean's approval before building.** **Orchestration:** design inline (Opus) → Bean sign-off → parallel/sequential subagents to build → live-verify the a11y contract (keyboard-only traversal, `elementFromPoint` on every drawer link, ESC+focus-return, scroll-lock) at 375 on BOTH clients. **/qc gate after:** yes (a11y-critical). **Acceptance:** adaptive-nav renders the full nav + drawer with the complete FR-S9-5 a11y contract intact on 2 live clients; `sgs/mobile-nav` removed with no regression; spec reconciled.
