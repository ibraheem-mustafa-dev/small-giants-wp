...
primary_goal: "P5/P7/P8 block-quality cluster CLOSED 2026-06-12 (commit 9275328c, 10 rows live-verified: TB-A/B, FP-N/O/I, BR-C, SP-D.1, SP-E, IN-D + SP-F non-defect). Remaining: (1) IN-C feature-grid render-gap — emit fixed but the shared container-wrapper renders `verticalAlign` (default 'start') not the declared `alignItems`; PROPER FIX = wrapper `verticalAlign ?? 'start'`→'stretch' default (Rule-7 DESIGN-GATE, ~29 mirror blocks); (2) IN-F notice-banner empty content (bg fixed) → the gated universal-lift mechanism; (3) earlier-deferred P3 (IN-B/H-C1 content-band max-width) + P6 (IN-E/GF-B.2 text-align re-verify); (4) run /sgs-update to register the 2 new testimonial attrs (ratingSize/nameFontWeight) into the DB reproducibly (manually inserted as a stopgap this session). Acceptance = ledger rows flip VERIFIED with per-row live page-8 evidence."
---
...
## State recap (plain English, 2026-06-12 PM close — P5/P7/P8 cluster)

**The P5/P7/P8 block-quality cluster is DONE + live-verified + on main (commit `9275328c`).** 10 ledger rows closed: TB-A/B (trust-bar gap 12px + white icon circles), FP-N (price baseline), FP-O (cards equal height), FP-I (product image 220px via maxHeight — NOT the dead imageHeight attr), BR-C (ghost outline button via theme.json `custom.buttonPresets.outline`, Bean-approved), SP-D.1 (star 15px via new ratingSize attr), SP-E (quote italic/14px/text-muted + author 600 via new nameFontWeight attr), IN-D (emoji icons via new sgs/icon handler), SP-F (non-defect). The bulk were CONVERTER non-transfers (not block-defaults as the prior handoff guessed) — draft CSS values lifted onto native attrs. A UNIVERSAL grid-stretch mechanism (emit verticalAlign/alignItems:stretch for absent-align-items grids = CSS grid's true default) was qc-council-GO'd (2 cross-model raters) + regression-swept clean; 21 conformance goldens re-baselined. **2 rows still open:** IN-C (feature-grid emit-fixed but the shared container-wrapper renders verticalAlign not alignItems — the proper fix is the Rule-7 shared-wrapper default change; cards render equal anyway), IN-F content (bg fixed, empty content → universal-lift). **Follow-up: run /sgs-update** to register ratingSize/nameFontWeight into the DB (stopgap-inserted this session). Lesson: `feedback_converter_attr_must_match_the_attr_render_reads` (emit-green≠rendered; grep render.php+wrapper for the attr before lifting).
...
## Task 1 — P5 block-quality cluster [✅ DONE 2026-06-12, commit 9275328c]

**CLOSED (10 rows live-verified):** TB-A/B, FP-N, FP-O, FP-I, BR-C, SP-D.1, SP-E, IN-D + SP-F (non-defect). FP-P (CTA full-width) was NOT in the final cut — re-assess if still a defect on the current clone. **2 rows carried forward:** IN-C (feature-grid render-gap → Rule-7 shared-wrapper `verticalAlign ?? 'start'`→'stretch' DESIGN-GATE; emit already correct), IN-F content (gated universal-lift). See primary_goal + State recap for the new priority list. **Also run /sgs-update** (register ratingSize/nameFontWeight attrs).

**The original Task 1 brief (for reference — most now closed):** BR-C outline button, SP-F slide-card, TB-A/B gap+circle, FP-N/I/O/P, IN-C feature-grid. The handoff guessed "block-defaults" but the bulk were CONVERTER non-transfers — lift draft CSS onto native attrs (the text-CSS pattern), NOT block-default edits.
...
- **Container `maxWidth` vestigial-attr cleanup (2026-06-12, Bean):** `sgs/container` carries THREE OUTER-width mechanisms — `widthMode` (named: default/wide/full/custom), `widthMode=custom`+`customWidth` (number), AND a legacy string `maxWidth`. The converter NEVER emits the container's `maxWidth` (0× in live clones; `max-width → widthMode` at convert.py:1445), so it overlaps `widthMode=custom`+`customWidth` and is dead weight. widthMode IS the OUTER layer of the 3-layer model and is correct (theme-composable alignfull/alignwide; zero client px literals — Rule 14) — KEEP it + `contentWidth`. Retire only `maxWidth` (string) via deprecated.js (most-used block → verify nothing authored or in the 29 mirror blocks reads it first). **TEST CASE: the Brand section is the ONLY non-full-width section on the homepage clone** — it's the canary for OUTER-width handling. FIRST verify how Brand's constrained width (re-baseline noted max-width:1000px / BR-A 2-col verified) currently resolves (widthMode=wide? contentWidth? a literal?) and that it renders correctly; that same Brand probe is the regression gate before AND after pruning `maxWidth`. Low priority; harmless, just untidy.
