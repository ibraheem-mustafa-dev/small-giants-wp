...
**Task 1 — Execute the doc-fix register (the adversarial doc-council is DONE; findings in `.claude/reports/2026-06-06-doc-council-findings.md`).** The council found the root cause of repeated drift: the docs CONTRADICT themselves and ENFORCE the wrong thing — they bless the bound-mode cheat as ratified architecture (Spec 27 FR-24-10, decisions D151/D167, root CLAUDE.md), mandate className-mirroring (FR-22-4.1 rule#5 / FR-22-11), ship CSS verbatim instead of converting (FR-22-5 D2), and waive the only homepage visual gate (FR-22-18 "informational-only"). So cheating is the documented path of least resistance that PASSES the acceptance gate. Execute the must-fix register in that report, FATAL items first: (1) un-bless the cheat in the permanent record; (2) add an ENFORCED anti-mirror gate (R-22-15 + a pipeline-stage-gate.py PASS test that fails on an emitted draft-class container / sourceMode='bound' / D2-stranded layout CSS); (3) restore the homepage visual gate (FR-22-18 amend, R-22-11 enforced); (4) put the 7 rules as a named STOP block at the top of CLAUDE.md + handoff + this prompt + mistakes.md. Then the HIGH items (single-source status, shrink/archive, goal statement, hero-gradient reconcile). This is the meta-fix — without it the next session cheats again.
...
- **D-NUMBER CONTENTION (shared decisions.md — added 2026-06-06):** theme + cloning both append to one `.claude/decisions.md`. BEFORE assigning any new D-number, verify the live ceiling: `grep -oE 'D[0-9]+' .claude/decisions.md | sort -V | tail -1`. Never assume the ceiling from state.md/a handoff (they drift). Commit decisions.md by `git commit -- .claude/decisions.md` only; append your entry at the top, never interleave the other thread's.
