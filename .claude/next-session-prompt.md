...
## ⚡⚡⚡⚡ 2026-06-04 PM3 — METHOD 2 INVESTIGATED + DESIGNED + COUNCIL-VALIDATED → BUILD NEXT [READ THIS FIRST — supersedes ALL openers below; the structural-defence sections (STOP catalogue, pre-flight ritual, mandatory reading list, orchestration model) further down REMAIN BINDING and carry forward]

**You are Opus = the ORCHESTRATOR (see §ORCHESTRATION MODEL below).** Method 2's investigation + design + hardening are DONE. This session BUILDS the converter-lift.

**State recap (plain English):** The Method-2 converter investigation is complete and the premise was CORRECTED (verified vs DB + match.json): routing is essentially solved — hero already routes `sgs/hero`@1.0, the trust-bar routing fix SHIPPED this session (`c3443e03` — restored `is_section_root`), and the 5 layout sections are correctly `sgs/container`. The real lever is the **converter-lift** (the 560 "no value extracted" — copying the draft's CSS onto the now-mirrored block controls). A 5-fix-shape design was authored, qc-council-validated (caught the slug-None-vs-composite **path-split** trap, STOP #50), turned into an executable phase plan, then a 5-persona `/adversarial-council` pre-mortemed it and surfaced the **universality must-fixes** (the plan as first written was a Mama's-shaped band-aid). All folded. **The build is ready.**
...
2. **`.claude/plans/2026-06-04-method2-converter-lift-PHASE-PLAN.md` — the "⚡ FINAL PLAN — post-adversarial-council" section IS the build map (it SUPERSEDES the step bodies below it).** Build order: **Session A (converter core)** = MF-A DB-driven lift map (from `property_suffixes` + CSS-function passthrough + FLAG-not-drop + responsive `bp_decls`) → Step 3-refined composite path **hero-first** (MF-C: compute attrs once, pass into `emit_sgs_container_wrapping` as a param — do NOT make `db_lookup.py:2461` read CSS) → Step 1 slug-None lift → Step 2 sidecar delete. **Session B (block/polish)** = MF-B min-height block-layer fix (gate `--has-min-height` on `verticalAlign` — `class-sgs-container-wrapper.php:431`) ∥ FS-4 sgs/media grid-child CSS ∥ D6/D7 variant+modifier ; then D5 layer-diagnosis (verify block-vs-converter FIRST) ; image sideload (FS-5, should-not-must) ; synthetic universality fixture LAST.
