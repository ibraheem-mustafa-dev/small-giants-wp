...
> 2. **WS-4 (FOUNDATIONAL)** — shared PHP wrapper helper (`includes/class-container-wrapper.php`) + KIND-scoped block.json attr-mirror + `sync-container-wrapping-blocks.py` report-only→WRITER (/sgs-update Stage 11). **MODAL excluded.** **BLOCKER FIRST:** rename cta-section `layout`→`contentLayout` (+deprecated.js); hero `overlayColour`→`backgroundOverlayColour` (+deprecated.js); trust-bar additive. Build order: cta rename→helper→trust-bar→cta→hero→writer. Sensitive → design-gate + /qc-council. (hero #1 / trust-bar #2 are drift symptoms that resolve here.)
