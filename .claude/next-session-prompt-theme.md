...
# Next Session — SGS THEME thread — Spec 30 P1 COMPLETE + MERGED; next = EXECUTE the P2 consolidated plan (orchestration-first)

> ## SESSION CLOSE (2026-06-11 EVENING, theme thread — READ FIRST) — Spec 30 P1 COMPLETE + P2 PLANNED
> **Spec 30 P1 is DONE, Bean R-22-13 signed off, MERGED to `main`.** `origin/main` is now `1c13a08c` (P1 + the cloning thread's 25 commits reconciled via merge `9f357129` — both threads in sync; their next pull FFs clean). Working PDP + cart loop live on canary product **540**: new `sgs/buybox` (option-picker→cart bridge), `sgs/tabs` first deploy (PDP details tabs), shop = SGS product-cards (`showPickers` off). **FR-30-12 (product-page cloning) UNGATED.** Evidence: `.claude/reports/spec30-p1/wave2-final-*.png`.
>
> **THE PLAN TO EXECUTE: `.claude/plans/2026-06-11-spec30-p2-differentiators-shop-schema.md`** (docscore A) — ONE consolidated phase for ALL remaining Spec 30 work: FR-30-8 (price coupling) → FR-30-10 (reviews) → FR-30-3 (shop archive UX) → FR-30-6 (searchable filter) → FR-30-5 (product search — effort tentpole, own design gate) → FR-30-9 (schema) → FR-30-13 (go-live) + the 2 parked P1 follow-ups. Start at **Step 1 (branch + ground-truth probe, SESSION-START)**. The P1 plan is archived at `.claude/plans/archive/2026-06-11-spec30-p1-wc-chassis-COMPLETE.md`.
>
> **⚙️ BINDING ORCHESTRATION MODEL (Bean directive 2026-06-11 — read the plan's ORCHESTRATION MODEL block):** the Opus main agent does ONLY plan / delegate / QC / live-test / commit. ALL block/PHP/JS/CSS implementation is DELEGATED to sonnet subagents (`/subagent-driven-development` = implementer + 2 reviewers; `/dispatching-parallel-agents` for disjoint steps). Subagents have NO commit/deploy authority — they return uncommitted diffs; the main agent QCs (`/qc-council` + `/adversarial-council`), deploys + live-verifies on the canary HIMSELF (never on a subagent self-report), then commits. Do NOT hand-write implementation inline; do NOT retype a subagent's file to "quickly fix" it (SendMessage the correction back).
>
> **Open P1 follow-ups (folded into the P2 plan as Step 10):** per-variation gallery image-swap (Beta-gallery coupling severed by the classic-gallery fallback — decision in P2); notify-me capture (deferred, needs PECR-guarded path). Two canary fixture products lack photos (data only).
>
> **CO-ACTIVE MAIN:** `main` is held in the cloning thread's worktree `C:/tmp/sgs-p4` + that thread has UNCOMMITTED staged converter work in the primary worktree index — LEAVE IT UNTOUCHED; commit theme work path-scoped + merge to main via temp-worktree cherry-pick (D210-cont precedent), never disrupt their tree.

> ## PREVIOUS SESSION CLOSE (2026-06-11 AM) — D209
...
- **STOP — WC 10 ships `woocommerce_coming_soon=yes` by default: it masks ALL store pages behind a Coming Soon template for guests — a "template not rendering" symptom with a config cause. Check the option FIRST when store pages look wrong; it's now also an FR-30-13 go-live item.** (Found 2026-06-11; canary set to `no`.)
- **STOP — WC fixture = product 540 (`mamas-test-box-48-sku-fixture`, published 48-SKU variable); product 1017 is a DRAFT (no slug, invisible to guests). Verify a fixture's post_status before writing tests against it.**
