...
> **STOP — WC product-panel CSS force-columns EVERY label, panel-wide (NEW, session 18 close, D201).** WooCommerce applies `float:left;width:150px;margin-left:-150px` to every `<label>` in the product-data panel — not just the field row's own label. Any hand-rolled NESTED label (per radio/checkbox/input) collapses onto the same 150px column and paints over its siblings; section labels in plain divs CLIP off the left edge (no column padding). Reset nested labels inline (`float:none;width:auto;margin:…` — no !important in WC's rule, inline wins; width/margin hit FLEX ITEMS too). `woocommerce_wp_*` field labels must be SHORT — sentences go in description. Bean's eye caught this after the visual pass mis-triaged it as polish; treat any rater 'crowding/overlap' finding on WC admin markup as a DEFECT, not polish. ROUND-2 mechanisms (same class, D201 `ffadce85`): a half-width `form-row-first` wrapper with NO `form-row-last` partner lets the NEXT row float up beside it (its help-tip painted onto that row's label) — keep rows `form-row-full` unless deliberately paired; and WC FLOATS panel selects/inputs, so a `display:block` description after a floated select stays on the SAME line (text wraps the float) — bound the select width AND `clear:both` the description.
