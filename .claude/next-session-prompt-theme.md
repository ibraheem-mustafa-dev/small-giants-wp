...
- **Doc-currency sweep + `/qc-council`** (`352bd804` + content in `603cbaaf`) — all of `.claude/`/specs/plans/registry brought current + consistent so this session inherits clean docs. Documented the cloning thread's work; fixed block counts (66 SGS), Spec 02 trust-bar (ACTIVE), FR-22-20 (partially shipped), struck an R-22-14-violating plan instruction. You should NOT need to re-verify doc currency.
- **⚠ Cloning thread moved under this branch (parallel):** since the last theme code commit it shipped **D145** (is-style carry + tag-authoritative leaf routing), **D146** (`sgs/button` replaces `core/button` everywhere + `sgs/multi-button` grouping), **D147** (button `inheritStyle` presets + `video`/`iframe`→`sgs/media`). Net: bare `<a>`/`<button>` clone as `sgs/button`; CTA groups as `sgs/multi-button`. **Read `.claude/decisions.md` D145-D147 before Task 2 Phase-D** (clone-emit) — the converter's button/multi-button routing is the surface your `sgs/option-picker` emit-path plugs into.
