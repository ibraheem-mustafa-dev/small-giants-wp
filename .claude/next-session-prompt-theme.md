...
**NEXT OPENER (pick with Bean):** the remaining pulled-forward server-side hardening = the **Store-API direct-bypass enforcement** (FR-MISSING-3 — the `woocommerce_store_api_validate_add_to_cart` stub; until done, a client can skip the proxy by POSTing `/wc/store/v1/cart/add-item` directly) + the "Any"-attr `term_exists` + product-local-attr handling (plan §U6 tracked). **THEN, gated on cloning closing:** the card-rendering units **U3** (manifest+SSR seed, opus) → **U4** (view.js store) → **U7** (wire client add-to-cart to the proxy) → **U5** (availability). **Open question for Bean:** could U3/U4 be built on a SEPARATE test page bound to fixture 540 (not page 144) so they don't touch the Mama's pixel-diff, un-gating them earlier? **Orchestration (Bean-locked):** Opus inline orchestrates + fact-checks + runs qc gates (adversarial-council/qc-council/2-rater review); Sonnet subagents do the mechanical build; dispatched agents have NO commit authority; live-verify everything yourself. **Honest estimate: 3-4 sessions for the full sell-loop.**
