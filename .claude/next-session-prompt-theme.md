...
Run `git log --oneline -10` + `git status` + `git branch --show-current`. Confirm HEAD is on `main` at/after `395c76c9`. Confirm the only uncommitted files belong to the cloning thread (hero/*, sgs-framework.db, phase4 reports, lucide, theme-snapshot) PLUS the pre-existing spec27/phase1-plan doc tangle — leave all untouched (reconcile the doc tangle only if Bean asks). Then open `https://sandybrown-nightingale-600381.hostingersite.com/sgs-configurator-test-540/` + click a few pills to re-ground (auto mode, tax OFF baseline). Then read the live plan `.claude/plans/2026-06-04-spec27-phase2-display-seo-plan.md` (v2) §Cluster A remaining + the SEC-1..9 cross-cutting amendments.
