...
**THIS SESSION'S OPENER (Bean-authorised, pixel-diff-safe — does NOT wait on cloning):** build **U0** `seed-48-sku-fixture.php` first, then **U6** the `/sgs/v1/cart/add-item` proxy (the security spine — fold M-C1 freshness/purge + M-C2 attribute-key + M-C4 inventory-denial + M-C5 CSP/escaping; `/qc-council` per commit; dispatched agents have NO commit authority). After U0 exists, pin M-C2's exact `attribute` key string by adding the fixture variation via the Store API and reading the cart line back. **Verified already:** canary = WooCommerce 10.8.1; `variation` is an array of `{attribute,value}` (spec shape correct). **STILL GATED on cloning closing:** the card-rendering units U3/U4/U5/+ (they re-baseline the pixel-diff — check `.claude/next-session-prompt.md` + `state.md`). **Honest estimate: 3-4 sessions for the sell-loop, not 2.** First commit the uncommitted theme-thread docs (plan + decisions.md D162 + handoff + this file) by EXPLICIT PATH (the cloning thread's `testimonial-slider/*`/`reports/phase4-*`/`theme-snapshot.json`/`lucide-icons.php` stay untouched; verify `git log -1 --stat`).
