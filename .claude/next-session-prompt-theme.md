...
## ⚡ SESSION 9 UPDATE (2026-06-03, D162) — read this, then the STOP boxes above still all apply
**Task A DONE + LIVE:** the product-meta IDOR fix (`d07a7e05`) is deployed to the canary + proven 3 ways (contributor REST write → 403; admin → 200; live file matches). **Task B DONE:** the Spec 27 Phase-1 build plan is written + stress-tested → **`.claude/plans/2026-06-03-spec27-phase1-configurator-plan.md`** (13 units U0-U12, gates, grounded risk register, `/adversarial-council` CONDITIONAL-GO with the **must-fix register at §8b** — fold those before building the unit each touches). The two build open-questions are resolved (plan §1). **Bean's two decisions (D162.5):** (a) KEEP the single whole gate — all 13 units before "shipped" (declined the council's re-cut; it doesn't fit the completionist profile); (b) PULL the server-side units forward to build DURING the cloning wait.

**✅ U0 + U6 DONE this session (committed `43ecfce1`, pushed; D162.7).** U0 = `seed-48-sku-fixture.php` → canary product **540** (48 variations, 3 OOS / 1 low-stock / 1 scheduled-sale). M-C2 PINNED live: WC 10.8.1 add-item format = `variation:[{attribute:"<display name>",value:"<slug>"}]` (e.g. `{"attribute":"Size","value":"12-pack"}`). U6 = `class-cart-proxy.php` (`POST /sgs/v1/cart/add-item`, in-process `WC()->cart->add_to_cart`) — 2-rater security+logic review folded 5 must-fixes; full live adversarial suite passes (CSRF/IDOR/parent-id/OOS/attr-mismatch/empty-variation/qty-cap-clamp 50→30). Tracked follow-ups in plan §U6.

**✅ Store-API direct-bypass CLOSED (`a02b5ac3`).** A `woocommerce_add_to_cart_validation` filter enforces the per-SKU cap + global rate-limit on EVERY add-to-cart path. Live-verified (direct over-cap 50→400; legit 3→201; proxy 200). U6 is now security-complete. Residual §U6 follow-ups (small, build-time): "Any"-attr `term_exists`, product-local-attr case-handling, guest-nonce doc.

**✅ Test page LIVE + card baseline POLISHED (`9717a069`).** Canary page **589** (`/sgs-configurator-test-540/`) renders a Bound product-card → fixture 540 (real WC variable-product data, 0 errors). Card design polished (Bound-scoped, page-144-safe): "From £9.99" (was the bare range), cart icon on the button, eager image. `/qc` PASS 96/100 (all contrast ≥4.5:1, 0 errors, no regression). product-card block.json 1.4.0. Use page 589 for the U3/U4 build.

**NEXT OPENER — Bean APPROVED the SEPARATE-TEST-PAGE build (un-gates the visual configurator now, no wait on cloning):** build the card-rendering units on a NEW canary test page bound to **fixture product 540** (NOT page 144 — so the Mama's pixel-diff is untouched). Order: **U3** manifest + SSR seed (OPUS — the SSR-wipe trap lives here: every `data-wp-bind`/`data-wp-text` resolves against server-seeded `data-wp-context` whose default == the SSR literal; per-instance context NOT shared `wp_interactivity_state`; combo-key `{taxonomy}:{slug}` sorted/`|`-joined; price slot = separate seeded literal keys since there is NO innerHTML binding — fold M-C3/M-C5) → **U4** view.js multi-axis store (`ctx.selectedAxes` object; namespace-qualified `getContext('sgs/product-card')`) → **U7** wire client add-to-cart to `POST /sgs/v1/cart/add-item` (send `id` + `quantity` + `variation:[{attribute:"Size",value:"<slug>"}]` + `X-WP-Nonce`; the proxy is built + verified) → then U5 availability. **DEPENDENCY ANSWER (Bean asked):** U3/U4/U7 are NOT functionally dependent on the cloning thread's WS-4 (composite-wrapper-mirror) — they are ORTHOGONAL layers (WS-4 = the outer wrapper/padding/max-width; configurator = the interior price/pill/cart data). BUT both `sgs/product-card` + `sgs/option-picker` are on WS-4's content-KIND roster, so BOTH threads edit those two `render.php` files → COORDINATION overlap (concurrent-edit/commit-race), not a build dependency. Edits land in different parts of each file; commit by explicit path + sequence the product-card/option-picker edits with the cloning thread. WS-4's `#4b` (product-card fills its cell) is the wrapper-sizing layer — won't collide with the interior data work. **Orchestration (Bean-locked):** Opus inline orchestrates + fact-checks + runs qc gates; Sonnet subagents do the mechanical build; dispatched agents have NO commit/deploy authority; live-verify everything yourself. **Honest estimate: 2-3 more sessions for the full sell-loop.**
