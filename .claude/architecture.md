...
**DB-first architecture rule (blub.db row 260, Rule 11 HARD-GATE in /sgs-clone SKILL.md):** Converter / recogniser scripts read canonical vocabulary from sgs-framework.db tables (`property_suffixes` 117 rows, `block_supports` 1223 rows, `modifier_suffixes` 19 rows, `slot_synonyms` 89 rows, `block_attributes` 2230 rows, `blocks.parent_block` for InnerBlocks emission) via `db_lookup.py`. No hardcoded Python dicts duplicating DB data. /sgs-update keeps the DB in sync; manual dicts do not sync. `block_selectors` (74 rows) holds block.json `selectors` API editor-scoping data — NOT read at /sgs-clone runtime per `cloning-pipeline-flow.md:1486-1488`; the walker uses `block_attributes.derived_selector` instead. `block_compositions` (37 rows) is PATTERN-LEVEL and WRITE-ONLY at runtime (only `pattern-register.py` + `seed-block-compositions.py` write; nothing reads at /sgs-clone time). (Row counts + table-role refresh 2026-05-23.)

**Session B 2026-05-22 (Phases 5a + 5b + 6 + WP 7.0 upgrade shipped):**

- **Phase 5a** (commits `43a93df9` + `96df3dde`) — WP style-variation overlay system retired. 24 per-client variation JSONs (8 clients × 3 axes) moved from `theme/sgs-theme/styles/` to `sites/<client>/{theme-snapshot,theme-snapshot-colours-axis,theme-snapshot-typography-axis}.json`. Push CLI `plugins/sgs-blocks/scripts/push-theme-snapshot.py` deploys snapshots over SSH+SCP with auto-`--no-push` safety on shared dev surfaces. `wp_theme_json_data_user` + `block_editor_settings_all` filters hide Browse-styles UI when styles dir is empty. Three PHP picker classes + 1 test moved to `plugins/sgs-blocks/_retired/` (Commit B = `rm -r _retired/` deferred to a future session after soak). Verified live: variations REST endpoint returns `[]`; Mama's branding renders unchanged on sandybrown.
