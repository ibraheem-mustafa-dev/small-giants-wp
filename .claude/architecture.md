...
**Deterministic converter (slot-aware DOM walker, Spec 16 2026-05-14):** Spec 16 is the concrete implementation of Spec 15 §7 Stages 3-7. Phase 1 shipped 2026-05-14: prototype at `.claude/scratch/converter-prototype/` (1,136 lines across db_lookup + convert + convert_page) emits clean WP block markup with typed-attr lifts on the Mama's Munches homepage (10 SGS block types from 9 sections). Replaces ~1,942 lines of legacy `extract.py` + `extract_strategies.py` + `overrides/hero.py` at Phase 6 of the rollout.

**Phase 7 (closed 2026-05-13):** Converter promoted from scratch/ to production. 9 of 9 sections route through cv2 lift logic.

**Phase 8 (closed 2026-05-16, 22 commits across 3 sessions):** Walker precedence + DB-driven slot routing + composite-element fast path + bucket-router accuracy + severity_totals dashboard + role backfill + heritage-strip retirement + 7 static→dynamic block conversions + universal BEM-child array lifter + voter retired-block remap + walker grouping preservation + hero mockup migration + parse_css @media regex fix + block-root supports lift + DB-first refactor for property_suffixes + modifier_suffixes lookups.

**Phase 9 (active as of 2026-05-17):** Brand + hero evidence-driven walkdown to attribute-coverage% ≥ 95% (universality) + pixel-diff% ≤ 5% (fidelity). Pre-work adds per-decision tracing (`convert-trace-<boundary>.jsonl`) + expected-rules baseline (`expected-rules-<boundary>.jsonl`) to surface SILENT MISSES (the @media regex bug pattern). Section-by-section workflow: targeted run → /systematic-debugging → /qc council only when no clear hypothesis → DB-first universal fix → verify split metric. Active plan: `.claude/next-session-prompt.md` v8 (post P-WP-ALIGNMENT-WIDTH-SYSTEM close).

**P-WP-ALIGNMENT-WIDTH-SYSTEM (CLOSED 2026-05-18, 3 commits):** Architectural milestone within Phase 9. (1) cv2 pipeline targets WP PAGES (`page.html` template, `is-layout-flow` main) not POSTS (`single.html`, `is-layout-constrained` with `.entry-content { max-width: 800px }`) — `c7f42003`. Canary surface moved from post 65 to page 131 (`/cv2-output-mamas-munches/`); brand-cropped pixel-diff at 1440 dropped 58.0% → 43.7% (14.3-point reduction from this template change alone). `reports/brand-walkdown-2026-05-19/upload_and_patch.py` rewritten with `argparse` — `--target page|post --target-id N`, defaults `--target page --target-id 131`. (2) sgs/container gains `widthMode` per-viewport enum (default/wide/full/custom) composing with WP-native `alignfull`/`alignwide`; converter detects mockup section widths from SGS-BEM block-root selectors and writes `settings.layout.contentSize`/`wideSize` into per-client style variation JSON idempotently; `_lift_root_supports_to_style` emits semantic `widthMode` when lifted max-width matches theme widths within ±5%, arbitrary literals fall back to legacy inline `style.dimensions.maxWidth` — `86172812`. Editor InspectorControls expose the full surface. Universal-benefit held — zero client literals; per-client style variations now inherit auto-detected layout widths. NOTE (2026-05-23): page 131 was deleted between 2026-05-20 and 2026-05-23. Current canary is page 144 (`/rc-fix-verification-mamas-munches/`). upload_and_patch.py default `--target-id` should be updated to 144 before next run.

**DB-first architecture rule (blub.db row 260, Rule 11 HARD-GATE in /sgs-clone SKILL.md):** Converter / recogniser scripts read canonical vocabulary from sgs-framework.db tables (`property_suffixes` 117 rows, `block_supports` 1223 rows, `modifier_suffixes` 19 rows, `slot_synonyms` 89 rows, `block_attributes` 2230 rows, `blocks.parent_block` for InnerBlocks emission) via `db_lookup.py`. No hardcoded Python dicts duplicating DB data. /sgs-update keeps the DB in sync; manual dicts do not sync. `block_selectors` (74 rows) holds block.json `selectors` API editor-scoping data — NOT read at /sgs-clone runtime; the walker uses `block_attributes.derived_selector` instead. Pattern composition lives on the `patterns.block_composition` JSON column (35 populated of 53 patterns) and is WRITE-ONLY at /sgs-clone runtime — only `pattern-register.py` + `seed-block-compositions.py` write; the walker never reads it. (Row counts + table-role confirmed by independent verify-loop 2026-05-23, updated 2026-05-24. Canonical counts: blocks 194, block_attributes 2230, block_supports 1223, slot_synonyms 89, modifier_suffixes 19, property_suffixes 117, block_selectors 74, patterns 53 (35 with block_composition populated), legacy_role_lookup 18, design_tokens 184, hooks 5234.)

**BEM-is-canonical recognition (2026-05-24, D48–D54):** Walker recognises block types via BEM element name → slot_synonyms canonical → standalone block. HTML tag is rendering shape only; tag-based routing rejected as a side-channel that conflicts with Spec 00 §3 BEM-as-canonical. `__quote` / `__blockquote` BEM routes to sgs/quote via aliases moved into the quote canonical (was wrongly aliased to text). Section-root transparent wrappers (BEM-named, no gap/positioning CSS, not registered blocks) absorbed via `_absorb_transparent_wrappers` pre-pass — one-section-one-container architecture. /sgs-update Stage 4 (`assign-canonical.py`) now auto-runs from `sgs-update-v2.py:stage_1_sgs_codebase_scan()` tail with singularise + Tier B (registered-block reverse-lookup) fallback for plural collection-attr names (testimonials→review, logos→logo, plans→card, etc.). 12 previously-NULL array-attr canonical_slots populated; ARRAY_LIFT_PATTERNS dict deletion deferred (count_stars special extractor + multi-selector fallbacks not yet in universal path — Phase 1 follow-on F1).

**Session B 2026-05-22 (Phases 5a + 5b + 6 + WP 7.0 upgrade shipped):**

- **Phase 5a** (commits `43a93df9` + `96df3dde`) — WP style-variation overlay system (`.json` files only) retired. 24 per-client variation JSONs (8 clients × 3 axes) moved from `theme/sgs-theme/styles/` to `sites/<client>/{theme-snapshot,theme-snapshot-colours-axis,theme-snapshot-typography-axis}.json`. Push CLI `plugins/sgs-blocks/scripts/push-theme-snapshot.py` deploys snapshots over SSH+SCP with auto-`--no-push` safety on shared dev surfaces. `wp_theme_json_data_user` + `block_editor_settings_all` filters hide Browse-styles UI when styles dir is empty. Three PHP picker classes + 1 test moved to `plugins/sgs-blocks/_retired/` (Commit B = `rm -r _retired/` deferred to a future session after soak). Verified live: variations REST endpoint returns `[]`; Mama's branding renders unchanged on sandybrown. Note: Phase 5a retired only the `.json` overlay system. The `.css` path (`theme/sgs-theme/styles/<client>.css`) is being relocated to `pipeline-state/<run>/variation-d0-d2.css` by the Q3 css_router subagent (updated 2026-05-23).
- **Phase 5b** (commits `60220b13` + `012c4b75` + paint-fix `0ef032fe`) — Three Customiser sections register at `Appearance → Customise → SGS Header / SGS Footer / SGS Site Info`. 10 settings registered with `postMessage` transport for live preview of colour/spacing/sticky; refresh transport reserved for conditional-rules controls. Pattern follows `Sgs_Floating_UI_Customiser` (Spec 18 §8b). Old admin pages preserved (additive overlay, not replacement — Customiser is 5-field subset; rules-table UI still managed via admin). Button-presets CSS bridge deleted as redundant after coverage audit (`.claude/reports/phase-5b-button-property-coverage.md`) confirmed WP natively generates 100% of consumed `--wp--custom--button-presets--*` properties. Paint targets are `header.wp-block-template-part` / `footer.wp-block-template-part` (verified empirically — the initial `.wp-site-header` selector targeted zero DOM elements in the SGS theme). Verified live via chrome-devtools: `wp.customize('sgs_header_bg_colour').set('#E68A95')` → header inline `backgroundColor: rgb(230, 138, 149)`.
- **Phase 6** (commits `d307c8b0` + `a3109e3b`) — Markup examples seeded for 69 SGS blocks (56 auto-generated from block.json defaults via `generate-markup-examples.py`, 13 hand-authored composites). Spec target was 73; 4 DB rows reference blocks with no source `block.json` file (parked as P-6-MISSING-BLOCK-JSON). Block-supports audit found ZERO gaps — original 2:1 under-documentation prediction (404 vs core 819) was wrong; existing 404 rows are all accurate. 360 active + 44 flagged `is_stale=true`. 87 content-bearing attributes across 40 blocks now carry `"role": "content"`. `wp_set_script_module_translations()` wired for 25 blocks (works natively on WP 7.0). All 69 source blocks already at `apiVersion: 3`. Lucide REST registration shipped defensively (double `class_exists`/`function_exists` guard) — pending discovery of correct WP 7.0 entry point.
...
- **Phase 4 — `/sgs-update` rebuilt as `sgs-update-v2.py`.** 9-stage single entrypoint replaces the legacy 3-script chain. Stage 2 live-scrapes 10 canonical upstream sources every invocation. Source 2 counter rewritten to gate on extraction-count (scraper-health signal), not insert-count. GitHub PAT rotated to working `ghp_*` classic format — verified 10/10 sources end-to-end. Commits `39d32799` → `99081252` (6 commits). Entrypoint: `plugins/sgs-blocks/scripts/sgs-update-v2.py`. New `schema_metadata` table in sgs-framework.db. Shadow token type added to `design_tokens` CHECK constraint; `design_tokens` now holds 184 rows post Phase 5a + Phase 4 Stage 4 (was 39). **Post-shipment update 2026-05-24 (doc-op session):** Mode A/Mode B distinction collapsed — Stage 2 is now always live-scrape; the `--refresh-upstream` CLI flag was removed (the refresh IS the default). Stage 3 retired (Stage 2 Source 3 covers the same wp-cli/handbook scrape). Full SonarLint cc sweep landed across 10 functions in the same session (see D55+D56 + parking entry close-out). See D56 in decisions.md.
...
1. **DB consolidation (Phase 1) — CLOSED 2026-05-24.** Three databases (wp-blockmarkup-mcp blocks.db, wp-devdocs-mcp hooks.db, sgs-framework.db) merged into sgs-framework.db with a `source` column ('native_wp' / 'sgs' / 'third_party'). Source files + their MCP cache directories deleted (~24 MB DBs + 623 MB cache). Three read-path consumers ported: `~/.claude/hooks/wp-blocks.py`, `~/.claude/hooks/wp-docs.py`, `plugins/sgs-blocks/scripts/sgs-update-v2.py`. One source of truth for all WP + SGS knowledge. After this: `/sgs-update` Stage 2 live-scrapes 10 canonical sources every invocation (gutenberg repo, wp-develop repo, wp-cli handbook, `developer.wordpress.org/reference/since/<version>/`, field guide, dev blog, 4 handbooks). Per-release verification gate checks DB completeness against `since/<version>/` API list. The Mode A / Mode B distinction was collapsed in the same close-out commit and the `--refresh-upstream` flag retired — every `/sgs-update` invocation refreshes upstream by default.
...
20. **Recogniser v2 / `/sgs-clone` pipeline — schema-driven, all-CSS-harvest, forward-only (orchestrator shipped 2026-05-09, hero verified at 100% PoC parity, multi-section walker is M9)** — Replaces v1 recogniser at `tools/recogniser/`. Three structural defects in v1 corrected: (a) **fingerprints auto-derived from `block.json`** so attribute extraction can never silently skip declared attributes (v1 hand-written → 12% coverage on sgs/hero); (b) **all CSS rules harvested every run** (BS4 native selector engine + recursive @media block parsing + Playwright cascade resolution) and classified into block-attribute / universal-handled / one-time-custom buckets — zero silent loss; (c) **forward-only emission** (no reverse template, no round-trip "lossless" claim — three independent reviews converged on round-trip being a tautology that doesn't prove WP correctness). Pipeline emits `sgs/multi-button` + `sgs/button` markup for any CTA pattern detected (Decision 19 / spec 11 button architecture is shipped).
    ...
    **Skill surface (shipped 2026-05-09):** `/sgs-clone` (orchestrator) + 5 siblings (`/uimax-scrape`, `/uimax-sgs-scrape-pattern`, `/uimax-mood-board`, `/uimax-scrape-animation`, `/uimax-classify-naming`) at `~/.claude/skills/`. All 6 graded >=B (skillscores 87-98%). Each carries rubric file at `<skill-dir>/references/end-goal-rubric.md` with `bean_signoff: confirmed_via_m7_brief_2026-05-08`. The deprecated `/animation-harvest` skill stays in repo as reference; replacement `/uimax-scrape-animation` enforces the SGS-block-attribute mapping the deprecated skill silently dropped.
    ...
    Spec at `.claude/specs/15-DETERMINISTIC-DRAFT-TO-SGS-CONVERTER.md (former Spec 12 absorbed 2026-05-12)`. Salvage matrix REVISIONS at `.claude/plans/archive/cloning-skill-salvage-matrix-2026-05-05.md`. Pattern dedup mechanics REVISIONS at `.claude/plans/archive/pattern-dedup-classify-mechanics-2026-05-05.md`.
