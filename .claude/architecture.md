...
> Last updated: 2026-06-02 (theme-thread Wave 1: sgs/option-picker + sgs/cart added = 68 total SGS blocks; notice-banner FR-22-6 migrated; product-card Phase B; D148/D149. No new architectural primitives — block count update only; authoritative counts in `../CLAUDE.md` "Framework stats"). Prior: 2026-05-30 (D107-D113 XS-batch: section-root tier flag + `blocks.tier` column + `block_composition` table LIVE; XS-3 walker reverted post-regression; XS-4 canonical_slot port complete at 31.8% coverage; XS-5 slot vocabulary cleanup; D6 inheritance script shipped, threshold tuning deferred). Prior: 2026-05-29 (D93-D100 architectural cleanup batch SHIPPED — commit `bcbafe09` on origin/main). Architecture programme CLOSED (2026-05-22, 31 decisions). Cloning-pipeline canonical spec is **Spec 22 (Universal Block-Equivalent Extraction)** — Spec 16 retired and archived. NEW universal walker at `plugins/sgs-blocks/scripts/orchestrator/converter_v2/convert.py` (1873 LoC vs retired 4803 = 61% reduction); R-22-3 PASS test self-runs in `__main__`. See `.claude/specs/22-UNIVERSAL-BLOCK-EQUIVALENT-EXTRACTION.md` + `.claude/handoff.md` for D93-D100 detail.
>
...
22. **`block_composition` table tracks ALL container-wrapping blocks, not just section-roots (D108, 2026-05-30)** — Data layer LIVE (188 rows seeded); walker consumption DEFERRED. Schema columns differentiate section-root wrappers from non-section container wrappers (e.g. card-shells, group wrappers), enabling div-class-level routing in a future walker pass. Companion script `plugins/sgs-blocks/scripts/sync-container-wrapping-blocks.py` (D6) emits inheritance audit + diff. Current threshold flagged 4 blocks with `wraps_block='sgs/container'`; threshold tuning DEFERRED (target 20-30 blocks per follow-up).
