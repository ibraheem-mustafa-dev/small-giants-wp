...
| Blocks plugin | `sgs-blocks` | Block count is DB-authoritative — query `/sgs-db` (79 SGS blocks as of 2026-07-14; do not hard-code). Extensions in `src/blocks/extensions/`. `Sgs_Ai_Connector` wraps WP 7.0 AI Connectors API. |
...
- **Never-overflow layout (D455, 2026-08-01)** — the header Cluster row is locked `flex-wrap: nowrap` and NEVER wraps or stacks; `min-width:0` on children lets flexbox shrink them proportionally, each stopping at its own floor (interactive controls 44px, logo `min-width: min(100%, var(--sgs-header-logo-min, 7.5rem))`). This solves the sub-400px WCAG 1.4.10 header-overflow bug intrinsically rather than per-element. ⚠ `flex-shrink:0` on the logo was REMOVED — unshrinkable it overflows a 320px viewport once wrapping is gone. Fluid `clamp()` spacing is **NOT shipped**: `sgs_container_gap_value()` strips parentheses and commas, so a `clamp()` gap emits as invalid CSS.
