...
1. **DB consolidation (Phase 1) — CLOSED 2026-05-24.** Three databases (wp-blockmarkup-mcp blocks.db, wp-devdocs-mcp hooks.db, sgs-framework.db) merged into sgs-framework.db with a `source` column ('native_wp' / 'sgs' / 'third_party'). Source files + their MCP cache directories deleted (~24 MB DBs + 623 MB cache). Three read-path consumers ported: `~/.claude/hooks/wp-blocks.py`, `~/.claude/hooks/wp-docs.py`, `plugins/sgs-blocks/scripts/sgs-update-v2.py` (Mode A no-ops gracefully when source cache absent). One source of truth for all WP + SGS knowledge. After this: `/sgs-update --refresh-upstream` pulls from 10 canonical sources (gutenberg repo, wp-develop repo, wp-cli handbook, `developer.wordpress.org/reference/since/<version>/`, field guide, dev blog, 4 handbooks). Per-release verification gate checks DB completeness against `since/<version>/` API list.
