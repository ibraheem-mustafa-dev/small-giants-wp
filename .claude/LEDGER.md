...
**⭐ CURRENT (2026-07-26 PM — Track 1b: Spec-32 NO-INLINE ROLLOUT CLOSED. The "2805-GAP Wave B" front was a PHANTOM (GAP count = semantic noise); the real 5-fix backlog is landed. 4 commits on main: `3e98861b` `6adc932f` `33272bd3` `23d27246`. D385.).**
- **11-condition DONE audit (D385)** — proved the Spec-32 no-inline PRIMARY DELIVERABLE was already complete across accessible blocks: 0 inline-via-render, 0 supports lacking skip-serialization, 0 box-family violations, 0 dead controls. Even 100%-DONE exemplars carry 23–151 GAPs (object-fit on a button) — the GAP metric is NOT work-remaining. Trustworthy backlog: `.claude/reports/2026-07-26-spec32-11-condition-done-audit.md`.
- **5-fix backlog landed:** product-card (`6adc932f`, stale F3 dead-code drained) · feature-grid (`33272bd3`, device-tier bp 1024/768→1023/767, live-verified) · content-collection + pricing-table + form (`23d27246`, false-flags resolved by a durable **F3-gate E13** element-aware exemption + form `:where()`). Nearly regressed by "remove the vestigial attrs" — safety battery caught they're wrapper-consumed BEFORE any edit.
- **F3 baseline now holds only `sgs/mega-menu`** (Track 2). E13 code-reviewer-gated (2 issues fixed); empirical regression empty-baseline 3→0.
- **2 pre-existing findings still parked (NOT ours):** `P-CONFORMANCE-GOLDEN-DRIFT` (27 stale goldens — blind re-seed forbidden) + `P-ARCHIVE-PRODUCT-WC-VALIDATION` (WC-core drift, frontend fine).
- **⚠ LEDGER over-cap (>24.5KB) — a multi-track sweep is owed** (the Track-2 mega block below → trim to a pointer next sweep). Rotate hook snapshots to `memory/session-2026-07-26.md` on Stop.
- **Prev session (D383/D384, commits `b9114844` `4234e26e` `586f5e9f` `68a70260`):** box-object A1+A2 (grid-item + product-card CTA) COMPLETE — closed the box-flat genuine-upgrade set; sgs/container stale-wrapper editor-validation fix (34 wrappers, 12 templates).
...
- **Track 1b — Spec 35 block-inspector-UX + Spec 32 no-inline — CLOSED as of 2026-07-26 (D385).** (Track-specific `next-session-prompt-spec35-track1.md` DELETED at session close — the F3/no-inline front is done; there is no live Track-1b prompt.) Spec 35: cluster/element vocabulary rework shipped (D354/D355), FR-35-1..6 ALL BUILT + rolled out (74/74 in-scope manifested); box-flat genuine-upgrade set COMPLETE (D383). **Spec 32 no-inline: COMPLETE + RE-VERIFIED (D385/D346).** The "2805-GAP wave" was a PHANTOM — the `check-element-manifest-conformance.js` GAP count is semantic noise (100%-DONE exemplars carry 23–151 GAPs), NOT work-remaining. The real 5-fix backlog is LANDED (product-card/feature-grid/content-collection/pricing-table/form; F3 baseline now = `sgs/mega-menu` Track-2 only, via the new element-aware E13 gate exemption). Audit + trustworthy matrix: **`.claude/reports/2026-07-26-spec32-11-condition-done-audit.md`**. **Only open Spec-35/32 residual: a separate inspector-QUALITY layer (12 WARN in `audit-inspector-conformance.js` — 10 raw-url-link controls that should be `SgsLinkControl`, 1 missing MediaUploadCheck on product-card, 1 reduced-motion gap)** — logged in the audit report; a future Spec-35 session, NOT this track's no-inline front. Canonical: `plans/block-migration-DONE-checklist.md` + Spec 32 §6.1.
...
- **Track 2b — Spec 37 header/footer PER-ROW identity** (prompt: `next-session-prompt-header-footer-per-row.md`; design: `plans/2026-07-26-per-row-sticky-mini-design.md`). **Phase 1 + Phase 2 CLOSED + live-verified 2026-07-26 (D386–D389).** Each row carries its own transparent / hide-on-scroll / shrink (device-tier, inherit-upward). **Shrink shipped a GROW bug and was fixed (D386):** an absolute `padding-block` in the shared stylesheet out-specified each row's own padding rule, so an unpadded row went 0px→4px; now emitted per instance as `calc(own padding / 2)` via `sgs_emit_responsive_css()` — proportional by construction. New prebuild gate `check-shared-css-state-rules.js` (nothing scanned `assets/css/` before). Hide-a-chosen-element ships with a DECLARATIVE `supports.sgs.headerEssential` guardrail, proven server-side (D387). Footer parity verified on the ACTIVE footer **CPT 1654** (not the obvious 1571). 44px floor measured and deliberately NOT built. **D388 [INCIDENT]: two editor-killing crashes shipped past ALL-GREEN gates** — only opening the real editor caught them. **NEXT = the sticky build (SA-1 APPROVED, D389):** sticky stays HEADER-level, rows COLLAPSE (height→0) rather than translate; per-row `position:sticky` REJECTED (short-parent trap); the D2 offset chain must NOT be built. Task 1 of that build is a LIVE WCAG defect worth shipping alone — `scroll-padding-top` is applied unconditionally, so a non-sticky header already reserves 252px for in-page anchors.
