...
| **P5** | agent refresh + skills refresh + WCAG 2.1 AA baseline | 🔶 in progress. **§4c WCAG 2.1** DONE (global `8f96c24` + project `c59355bf`, pushed). **§4 roster cleanup** DONE (`97bb6ce`): `seo-geo` dereferenced to a skill; 2 shadowing archived agents deleted. **skillscore ruler RECALIBRATED** — it was scoring agents against *skill* rules, so the "56–59%" was mostly scorer bugs; agent path fixed + verified (honest 65–98% spread). ⚠ LIVE but UNVERSIONED (`~/.agents` is not a git repo). **§4 agent refresh 4/6 DONE + LIVE** (Sonnet builds → graded inline vs fixed ruler): seo-auditor 100%, seo-schema 98% (fabricated dates + invented file-ref caught & removed on inline judge), project-manager 100%, site-reviewer 100% — all A, faithful. **REMAINING:** the 2 HEAVY agents — wp-sgs-developer (77%; de-hardcode 2 paths + trim 296→≤200 + dangling `sgs-rollback` ref) + design-reviewer (65%; 562-line restructure, move methodology to a skill); then §4b generic-skills refresh. ⚠ ALL refreshed agent files are LIVE but UNTRACKED (`agents/` not in the `~/.claude` git repo). |
