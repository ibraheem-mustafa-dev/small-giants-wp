...
- **Branch:** `main` (2026-07-24; a co-active Spec-31/35 track commits between handoffs, so real HEAD is
  likely higher — re-check with `git log -1`). **D-ceiling: D392** (2026-07-26, Track 2b sticky build;
  verify with the grep below rather than trusting this line). The paragraph immediately below describes
  the 2026-07-24 Track-2 mega session and is retained as history — it is NOT the current front.
  **D-ceiling: D378** was that session's figure. THIS session was DESIGN/SPEC only (no
