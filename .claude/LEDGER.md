...
| **P5** | agent refresh + skills refresh + WCAG 2.1 AA baseline | 🔶 §4c WCAG 2.1 DONE (`8f96c24`+`c59355bf`, pushed). §4 roster cleanup DONE (`97bb6ce`). **skillscore ruler RECALIBRATED** — agents were scored against *skill* rules, so "56–59%" was scorer bugs; agent path fixed + verified (honest spread). ⚠ LIVE but UNVERSIONED (`~/.agents` not a git repo). **§4 agent refresh 6/6 DONE + LIVE + A-grade** (Sonnet builds → inline-judged vs fixed ruler; each frontmatter-diffed + faithfulness-read): seo-auditor 100 / seo-schema 98 (caught+removed fabricated dates & invented file-ref) / project-manager 100 / site-reviewer 100 / wp-sgs-developer 100 / design-reviewer 93 (methodology MOVED to `agents/references/design-reviewer-methodology.md`, verified verbatim). ⚠ all refreshed agent files LIVE but UNTRACKED (`agents/` partially tracked in `~/.claude` git) — **git-persist pending**. **REMAINING: §4b generic-skills refresh only** (own session, judgement-heavy). FLAG: design-reviewer still says WCAG 2.2 (left per Bean's ③ decision — revisit if aligning to §4c 2.1). |
