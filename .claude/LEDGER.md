...
- **Lane A:** U-9+U-11, U-5, U-3+U-8 and U-6+U-7 CLOSED; `main` and sandybrown at 3458c9d5c or later (all lane A code live). U-6+U-7 (3aae1950c, 777ee5dd4; live PASS `reports/visual-diff/nav-drawer-2026-09-25.md` section U-6 + U-7, plus Bean's review fixes F1 drawer scrollbar 5e8d0bb9f/6a64c1992 and F2 wheel over an open drawer 3458c9d5c): a mega item's panel renders inside its drawer accordion; sibling dim, label roll with the trigger's hover/open words, drawer row ornament and expander glyph, per-item media from the linked page's featured image; M-30, M-24, M-25, M-22, M-15 covered. Wearecollins' two-bar burger (M-27 residue) is live too: `burgerBarCount` (a19a5c6ee, c16bcb949; live PASS in the same report, section "Two-bar burger"). U-4 is closed, cut by Bean to `vw`/`vh` font-size units in the shared typography control (61ae4cf99, live PASS; design note `.claude/reports/2026-09-25-u4-type-scaling-design.md` records the decision and each agency reference's custom-CSS formula for its Wave 4 clone). **Next: U-10, plus the owed lane A items.** Resume from plan §4, the U-6 + U-7 paragraph (owed items listed there). Open fixture on sandybrown: `plugins/sgs-blocks/scripts/nav-qa/qa-item-markup-fixture.php` (the `two-bar` case applied: exit-cells plus the two-bar burger; `restore` also removes the menu-119 page link and page 2742's featured image). Batched for one later pass (Bean: no heavy testing per edit): axe with the drawer open, keyboard sibling dim, reduced-motion emulation, editor round-trips of the U-9/U-11, U-5 and U-6/U-7 controls, the `vw`/`vh` size units and the heading preview fix, Bean's eye on the gallery arrows, motion shapes and the new item effects. U-16 waits on plan step 0d (headed footer capture; ask Bean before opening Chrome). Deploys and DB reseeds are serialised across sessions (`.claude/dev-setup.md`).
- **Lane B:** not started.
- **Lane C:** not started.
