...
- **Lane A:** U-9+U-11, U-5 and U-3+U-8 CLOSED. U-3+U-8 (9f3fc5071, 51d4be574, dd2db8a1f; live PASS `reports/visual-diff/nav-drawer-2026-09-25.md`): drawer side and header-content positions plus `anchorOffset`, per-tier menu `gap` (other sites run `scripts/migrate-nav-gap-tier.php` before taking the deploy), one panel placement vocabulary with the gap measured from the header, three structured starter patterns (a mega panel post and the drawer body take any blocks). M-17, M-46, M-16, M-20 covered. Found and not fixed (container owner): an `sgs/container` grid set to `contentWidth: full` drops its column count. **Next: U-6+U-7.** Fixture `scripts/nav-qa/qa-geometry-fixture.php <exit-cells|restore>` (currently exit-cells, collapse point 1024). U-5 (motion: how the drawer and every dropdown/mega panel arrive and leave, plus item stagger; Spec 36 "Motion") is 01e4b5a5f, live PASS on `/qa-scrim/` (`reports/visual-diff/nav-drawer-2026-09-24.md` section U-5): away's drawer slide 300ms each way, the drafts' panel fade-lift 340ms with 28ms/55ms item staggers, panel close holds pointer-free then leaves. Also fixed: Escape now closes a panel opened by hover (35d98e413). M-31 and M-32 covered. Batched for one later pass (Bean: no heavy testing per edit): reduced-motion emulation, axe with drawer open, editor round-trips of the U-9/U-11 and U-5 controls, Bean's eye on the gallery arrows and the motion shapes. U-16 waits on plan step 0d (headed footer capture; ask Bean before opening Chrome). Older fixtures: `scripts/nav-qa/qa-close-fixture.php`, `scripts/nav-qa/qa-motion-fixture.php` (their backups' gaps are folded, so a restore is safe).
- **Lane B:** not started.
- **Lane C:** not started.
