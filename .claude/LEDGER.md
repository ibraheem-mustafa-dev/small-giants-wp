...
- **Lane A:** U-9+U-11, U-5, U-3+U-8, U-6+U-7, U-4 and U-10+U-14 CLOSED; `main` at 5781740e7 or later, all lane A code live on sandybrown. U-10+U-14 (design `.claude/reports/2026-09-25-u10-u14-design.md`, commit 0fbe085f1): `sgsCollapseVisibility` (hide a header block exactly while its menu shows the burger), `headerPassThrough` (float over the page), `triggerSurface` (whole row opens the menu; DEC-14 amended to a separate attribute) and buck's detaching chip (`triggerDetach*`); M-19, M-52, M-39, M-08 covered; live PASS in `reports/visual-diff/nav-drawer-2026-09-25.md`. Earlier today: two-bar burger (`burgerBarCount`), U-4 cut to `vw`/`vh` units, link padding (`itemPadding`), per-run deploy names, `sgs/mega-panel`'s opaque default fill (translucency from `surfaceOpacity` only), `sgs/icon-list`'s per-item description line with the numbered compact-links pattern (`sgs/mega-compact-links-numbered`), and the container's "Scroll sideways" setting (`scrollSideways`, `scrollItemWidth`; away's 375 tile row). The menu video source is closed by GIF (Bean). **Next: the batched QA pass (below); no owed lane A build items remain.** Eye Care's header phone keeps its 1160px Additional CSS rule (Bean: a rare one-off, the right use of Additional CSS). Fixture on sandybrown: `plugins/sgs-blocks/scripts/nav-qa/qa-item-markup-fixture.php` (`two-bar` applied; cases `header-row` and `detach-chip` added). Batched for one later pass (Bean: no heavy testing per edit): axe with the drawer open (and opened from the chip), keyboard sibling dim and order past the chip, reduced-motion emulation, editor round-trips of the container's Scroll sideways controls, the U-9/U-11, U-5, U-6/U-7 controls, burger Bars, Link padding, `vw`/`vh` units, heading preview and the four U-10/U-14 controls; Bean's eye on the gallery arrows, motion shapes and item effects. U-16 waits on plan step 0d. Before a deploy, run the full local `npm run build` (all 120 gates); a new `supports.sgs.elements` entry or override row trips DB gates that block every session's deploy. A DB reseed or a same-target deploy is still taken in turns across sessions (`.claude/dev-setup.md`).
- **Lane B:** not started.
- **Lane C:** not started.
