...
→ U-6+U-7 → U-4 → U-10 (+U-14). Lane B (header behaviours): U-13 → U-14 → U-16, after A passes U-9+U-11. Lane C
(independent): U-12, U-15, U-17 (`prompts/2026-09-24-wave-3c-lane-c.md`). Each lane edits only its own line below.

- **Lane A:** U-9+U-11, U-5, U-3+U-8, U-6+U-7 and U-4 CLOSED; `main` at f06b7133f or later, all lane A code live on sandybrown. Since U-6+U-7: wearecollins' two-bar burger (`burgerBarCount`, a19a5c6ee); U-4 cut by Bean to `vw`/`vh` font-size units in the shared typography control plus a heading editor-preview fix (61ae4cf99; design note `.claude/reports/2026-09-25-u4-type-scaling-design.md` keeps each agency reference's custom-CSS formula for its Wave 4 clone); `sgs/nav-bar-menu` link padding as a setting (`itemPadding`, 4cf0b9069); per-run upload and unpack names in `build-deploy.py` (7c61b7d16). All live PASS in `reports/visual-diff/nav-drawer-2026-09-25.md`. **Next: U-10 + U-14 (header-row structure), plan §4 rows 9 and 12 and "Pairs and lanes"; owed lane A items in plan §4's U-6 + U-7 paragraph.** Fixture on sandybrown: `plugins/sgs-blocks/scripts/nav-qa/qa-item-markup-fixture.php` (`two-bar` case applied; `restore` also removes the menu-119 page link and page 2742's featured image). Batched for one later pass (Bean: no heavy testing per edit): axe with the drawer open, keyboard sibling dim, reduced-motion emulation, editor round-trips of the U-9/U-11, U-5, U-6/U-7 controls, the burger Bars, Link padding and `vw`/`vh` size units, and the heading preview; Bean's eye on the gallery arrows, motion shapes and item effects. U-16 waits on plan step 0d. A DB reseed or a same-target deploy is still taken in turns across sessions (`.claude/dev-setup.md`).
- **Lane B:** not started.
- **Lane C:** not started.
