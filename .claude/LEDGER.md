...
hidden, free delivery pre-selected, staff download link for uploaded prescriptions). Spec 43 is v1.8.0; the lens configurator (post 463) runs the showcase layout; its buying path works but its look and motion
do not yet match the draft (15 differences, the plan's "Lens-flow parity gap" bullet, found 2026-09-26). The final design review ran; its owed items are in the plan's Status block. No
...
- **Lane B:** not started.
- **Lane C:** CLOSED. U-12 (furniture: clock, language switch, store selector, two-tier wishlist with Save for later, theme toggle with an automatic dark palette, Button back-to-top and account, audio sound toggle), U-15 (self-changing banner messages) and U-17 (the Lottie player as a media type, wrapper background and logo substrate; Spec 38 Tier H, D1151) are built, deployed to sandybrown and checked live (`plans/2026-09-21-wave-3c-implementation-plan.md` lane C paragraph; fixture trees `plugins/sgs-blocks/scripts/nav-qa/lane-c/`). Live on sandybrown at 5879ee945 (deploy verified; the banner's full-width strip mode checked live at `/qa-notice/`: edge to edge, square, still rotating). Open, Bean: go or no on the dark-palette framework fix (Mama's refusal is three name-guessed false alarms plus one text-on-yellow-button conflict; diagnosis and fix in the plan's lane C paragraph); icon size and background shape on the banner icon (offered). The customer account area and saved-item alerts are Spec 30 FR-30-14/15 (not started).
...
| **Form CPT + choice-flow** — COMPLETE: Phases 0-5 and the v1.8.0 follow-up live on sandybrown (Phase 5 at 07121d92e: in-use forms and flows can't be trashed or deleted from any surface; outside a saved form, `sgs/form` only picks, creates or converts to a saved form). eye-care-test still runs the pre-Phase-5 plugin. Parked: cloning-pipeline gap and analytics in `plans/2026-09-26-form-choiceflow-pipeline-and-analytics.md` | `specs/42-SGS-FORM-CPT-AND-PRICING.md` + `specs/43-SGS-CHOICE-FLOW.md` + `plans/archive/2026-09-14-spec42-43-form-choiceflow-phase-plan.md` |
