...
- **Lane A:** every lane A unit and owed build item is CLOSED and live on sandybrown; `main` at e1023f10f or later (code through a71fde293 deployed). Closed: U-9+U-11, U-5, U-3+U-8, U-6+U-7, U-4, U-10+U-14 (design `.claude/reports/2026-09-25-u10-u14-design.md`), the two-bar burger, link padding, `sgs/mega-panel`'s opaque default fill, `sgs/icon-list`'s per-item description line with the `sgs/mega-compact-links-numbered` pattern, and `sgs/container`'s "Scroll sideways" (design `.claude/reports/2026-09-25-container-scroll-sideways-design.md`); the menu video source is closed by GIF (Bean). Live evidence: `reports/visual-diff/nav-drawer-2026-09-25.md`, `icon-list-2026-09-25.md`, `mega-panel-2026-09-25.md`, `container-2026-09-25.md`. **Open: the batched QA pass** (Bean: no heavy testing per edit), scope in the plan's §4 U-10 + U-14 and U-6 + U-7 paragraphs: axe with the drawer open (incl. opened from the detaching chip) and on a Scroll sideways row, keyboard sibling dim and tab order past the chip, focus rings inside a Scroll sideways row, reduced-motion emulation, editor round-trips of every new control (U-9/U-11, U-5, U-6/U-7, burger Bars, Link padding, `vw`/`vh` units, heading preview, the four U-10/U-14 controls, icon-list Description, container Scroll sideways); Bean's eye on the gallery arrows, motion shapes and item effects. Eye Care's header phone keeps its 1160px Additional CSS rule (Bean). Fixture on sandybrown: `plugins/sgs-blocks/scripts/nav-qa/qa-item-markup-fixture.php` (`two-bar` applied; also `header-row`, `detach-chip`, `restore`). U-16 waits on plan step 0d. Run the full local `npm run build` before any deploy (a new `supports.sgs.elements` entry or override row trips DB gates that block every session's deploy); a DB reseed or a same-target deploy is taken in turns across sessions (`.claude/dev-setup.md`).
- **Lane B:** not started.
- **Lane C:** not started.
