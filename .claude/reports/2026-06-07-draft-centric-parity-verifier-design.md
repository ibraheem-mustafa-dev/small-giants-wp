...
- **Graded CSS + dimension de-weighting:** verdict uses a CSS match-RATE (≥80% authored-rule props = transferred), and `width`/`height` (computed-layout consequences) are de-weighted out of the verdict (reported as `dim_dropped`).
- **LAYOUT as a weighted sub-signal (Bean 2026-06-07):** the layout-DETERMINING props (`display`, `gridTemplate*`, `gap`/`columnGap`/`rowGap`, `align*`/`justify*`, `flex*`, `maxWidth`/`minHeight`, `margin*`) were being DILUTED in the single css% across ~60 props — a node could drop every layout prop and still score ≥80% and pass, hiding the trust-bar's real badge-grid/spacing difference. Now scored as `layout_pct` and REQUIRED for a TRANSFERRED verdict. Reports **content% / layout% / css% / full%** separately. Surfaces the real signal: brand layout 48%, products 56% (mirrored, poor layout transfer) vs trust-bar layout 95% (faithful, one badge flagged).
