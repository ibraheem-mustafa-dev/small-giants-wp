...
**Commit 0a — `seed-canonical-slots.py` (blocks Commit 2). NB: a DATA fill of an EXISTING column, not new infra.** `canonical_slot` is the existing, correct `block_attributes` column that maps attr→slot (no other column does this — `role`=kind, `derived_selector`=selector, `default_value`=attr-value). It's already populated for most attrs; the cross-node dispatch just needs the per-slot box attrs tagged too. **~41 rows untagged** (28 blocks' `contentWidth` + `sgs/hero.contentPadding*` all `canonical_slot=NULL`; `sgs/hero.mediaPadding*` already tagged). `/sgs-update` does NOT write `canonical_slot` — build `scripts/seed-canonical-slots.py` writing BOTH DBs (the `seed-slot-synonyms.py` pattern), verify per-row. Gate: `SELECT count(*) FROM block_attributes WHERE canonical_slot IS NULL AND (attr_name LIKE 'contentPadding%' OR attr_name LIKE 'contentWidth%' OR attr_name LIKE 'mediaPadding%')` returns 0. Low-risk (data, not structure).
