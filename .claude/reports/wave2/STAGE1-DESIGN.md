...
**Commit 0a — `seed-canonical-slots.py` (blocks Commit 2).** The cross-node dispatch finds a parent's per-slot box attrs by their `canonical_slot`. **~41 rows are untagged** (28 blocks' `contentWidth` + `sgs/hero.contentPadding*` all `canonical_slot=NULL`; `sgs/hero.mediaPadding*` already tagged). `/sgs-update` does NOT write `canonical_slot` — build `scripts/seed-canonical-slots.py` writing BOTH DBs (the `seed-slot-synonyms.py` pattern), verify per-row. Hard gate: `SELECT count(*) FROM block_attributes WHERE canonical_slot IS NULL AND (attr_name LIKE 'contentPadding%' OR attr_name LIKE 'contentWidth%' OR attr_name LIKE 'mediaPadding%')` returns 0.

**Commit 0b — `block_defaults` table + seeder (blocks Commit 3 / F6a).** F6a's absence-detection half needs to know each block's own CSS default for an inheritable property. Build a `block_defaults(block_slug, css_property, default_value)` table + `scripts/seed-block-defaults.py` that parses each block's `style.css` `:where()` rules (e.g. `heading/style.css:7` `:where(){text-align:center}`) and writes BOTH DBs — NOT a runtime file-read, NOT a hardcoded set (R-22-1). Hard gate: the table exists + has a row for every block with a `:where()` default. (Needs its own tiny FR or a note under FR-22-5.1.)
