...
2. **F1-cross-node (NET-NEW code, the headline).** Route an interior element's CSS to its slot's destination: parent container attrs (`contentPadding`/`contentWidth`/gap) or child-block attrs. Today `_collect_css_decls_for_element(node)` (`:2871`) collects the node's OWN css only — nothing walks `__inner`/`__content` child CSS upward. This is real new code; size it as such.
