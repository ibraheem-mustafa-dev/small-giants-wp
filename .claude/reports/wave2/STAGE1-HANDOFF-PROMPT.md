...
4. **Remove the carve-outs** — replace `_route_composite_interior` (`:2404`) + the trust-bar atomic handler (`:2236`) with the universal dispatch (FR-22-19 retirement). **PRESERVE** the sole-element-child guard at `_process_container_children:2956` (it prevents the +13pp XS-3 regression). **NOT in your scope:** the array per-item routing (FR-22-2.5 — TB-A badges/TB-B) is Stage 1b, a separate medium build; sequence it after this lands.
