...
Built as one coherent workstream (split into commits per R-22-5), design-gated (Rule 7), **regression-OK but live-DOM-verified-universal**:
- **Consolidate the lift paths** — `_lift_typography_to_block_attrs`, `_lift_wrapper_css_to_container_attrs`, `_lift_root_supports_to_style`, the scalar-media path → ONE DB-driven dispatch keyed on `role`/`canonical_slot`/`attr_type` (the `equivalent_block_for` machinery). **Delete the dead `_lift_styling_attrs` + `_slot_attr_prefix`.**
- **Add cross-node routing** — an interior element's CSS routes to its slot's destination: child-block attrs (walked), parent container attrs (`contentPadding`/`contentWidth`/gap), or per-item array fields. This is the F1 gap (paths exist for a node's OWN css; the gap is child→parent).
...
- **Remove the carve-outs** — replace `_route_composite_interior` + the trust-bar atomic handler with the universal dispatch. **Verify on the live DOM that the replacement routes correctly for EVERY composite + array block** (not just that a score held). Roll back only if it's genuinely broken (non-universal), not if the draft's number dips.
- Array per-item routing (FR-22-2.5) depends on the universal-walker piece — sequence it after the styling-lift consolidation lands.
