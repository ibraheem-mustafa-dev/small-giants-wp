...
2. **`_route_composite_interior`** (`:2404`, FR-22-19) — a **per-composite carve-out** gated by `is_class_section_block` (`:2415`). Routes content/scalar-media for *named* composites (hero/cta-section/…) but **drops the interior wrapper's CSS**. A passing-grade shortcut for specific composites, not the universal rule (Rule 2 / Rule 3).
