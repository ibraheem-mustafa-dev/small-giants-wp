/**
 * Q6 — what does a frame of this effect actually cost?
 *
 * The anatomy report says "NOT MEASURED — no figure should be quoted". This is the harness
 * that replaces that line with a number, and with the configuration the number belongs to.
 *
 * Usage:  node perf/measure-frame-cost.mjs            (from the stripe-hero-poc directory)
 *         node perf/measure-frame-cost.mjs --frames 400
 *
 * Output:  perf/frame-cost.json  + a table on stdout
 *
 * ── Deliberate choices, each with its reason ────────────────────────────────────────────────
 *
 * 1. NO --ignore-gpu-blocklist. Gate B ran with it and the adversarial council flagged that it
 *    bypasses the very gate Stripe uses to decide who sees this effect. A cost measured on a
 *    configuration Stripe refuses to serve is the wrong cost. The rig's own capability verdict
 *    (window.__capability) is recorded per run — if it declines on this machine, that refusal
 *    is itself part of the answer to Q6.
 *
 ...
 *
 * 3. Its OWN static HTTP server. The rig uses ES modules and fetch(), so file:// will not do,
 *    and depending on an externally-started server makes the run non-reproducible.
 *
 ...
 *    DPR. FR-38-31's resize() clamps DPR to 1.5 by design, so "DPR 2" means 2x for the Stripe
 *    rig and 1.5x for ours. Comparing on requested DPR would manufacture a false result.
 *    Every cost is therefore ALSO reported per megapixel, which is size-independent.
 */
...
const CONFIGS = [
	{ id: 'stripe-dpr1', label: 'Stripe rig — both passes', url: 'index.html', dpr: 1 },
	{ id: 'stripe-dpr2', label: 'Stripe rig — both passes', url: 'index.html', dpr: 2 },
	{ id: 'stripe-nopost-dpr1', label: 'Stripe rig — wave pass only', url: 'index.html?nopost', dpr: 1 },
	{ id: 'stripe-nopost-dpr2', label: 'Stripe rig — wave pass only', url: 'index.html?nopost', dpr: 2 },
	{ id: 'fr3831-dpr1', label: 'FR-38-31 as shipped', url: 'perf/fr3831-perf.html?w=1393&h=761&dpr=1', dpr: 1 },
	{ id: 'fr3831-dpr2', label: 'FR-38-31 as shipped (DPR capped at 1.5)', url: 'perf/fr3831-perf.html?w=1393&h=761&dpr=2', dpr: 2 },
	{ id: 'negative-control', label: 'NEGATIVE CONTROL — glClear only', url: 'perf/negative-control.html?w=1393&h=761&dpr=1', dpr: 1 },
];
...
if ( g( 'stripe-dpr1' ) !== null && g( 'stripe-nopost-dpr1' ) !== null ) {
	derived.postPassCostMs_dpr1 = g( 'stripe-dpr1' ) - g( 'stripe-nopost-dpr1' );
	derived.postPassShare_dpr1 = derived.postPassCostMs_dpr1 / g( 'stripe-dpr1' );
}
if ( g( 'stripe-dpr2' ) !== null && g( 'stripe-nopost-dpr2' ) !== null ) {
	derived.postPassCostMs_dpr2 = g( 'stripe-dpr2' ) - g( 'stripe-nopost-dpr2' );
	derived.postPassShare_dpr2 = derived.postPassCostMs_dpr2 / g( 'stripe-dpr2' );
}
if ( g( 'stripe-dpr1' ) !== null && g( 'fr3831-dpr1' ) !== null ) {
	derived.stripeVsFr3831_dpr1 = g( 'stripe-dpr1' ) / g( 'fr3831-dpr1' );
}

// ── Controls ────────────────────────────────────────────────────────────────────────────────
const controls = {};
if ( g( 'stripe-dpr1' ) !== null && g( 'stripe-dpr2' ) !== null ) {
	controls.positive = {
		test: 'DPR 2 must cost measurably more than DPR 1 on a fillrate-bound effect',
		dpr1Ms: g( 'stripe-dpr1' ),
		dpr2Ms: g( 'stripe-dpr2' ),
		ratio: g( 'stripe-dpr2' ) / g( 'stripe-dpr1' ),
		pass: g( 'stripe-dpr2' ) > g( 'stripe-dpr1' ) * 1.2,
	};
}
if ( g( 'negative-control' ) !== null && g( 'stripe-dpr1' ) !== null ) {
	controls.negative = {
		...
		realEffectMs: g( 'stripe-dpr1' ),
		ratio: g( 'stripe-dpr1' ) / g( 'negative-control' ),
		pass: g( 'negative-control' ) < g( 'stripe-dpr1' ) * 0.25,
	};
}
