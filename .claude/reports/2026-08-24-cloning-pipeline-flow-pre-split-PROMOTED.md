...
companion_docs:
  - .claude/specs/16-DETERMINISTIC-CONVERTER-V2.md - SINGLE end-goal spec (Spec 31 absorbed into §12 on 2026-05-21)
...
| **Spec 16** | **Single end-goal spec** — full pipeline architecture (Stages 0-9 + L0-L3 layers + canonical vocabularies + /sgs-update + QA gates). Spec 31 absorbed into §12 Appendix A. | `.claude/specs/16-DETERMINISTIC-CONVERTER-V2.md` |
...
│ Skills:       none                                                          │
│                                                                             │
│ WEAKNESS: Stage 0.7 isn't in Spec 31 stage list (was added during        │
...
Added 2026-05-19 in response to scattered-info findability gap; supersedes ad-hoc lookups across §"Direct file accesses inventory", §"DB table heat-map", and Spec 00 §3.1.
...
The convention is stored in `uimax.naming_conventions` row `"SGS WordPress"` with `is_canonical_for_sgs_drafts=1`. Stage 0.1 BEM lint hard-rejects non-conforming drafts on production runs. Spec 00 §3.1 (now part of Spec 16 §12) is canonical reference.
...
8. **Stage 0.7 isn't in Spec 31 stage list** - implementation drift from spec. Doc fix.
...
- **Single end-goal spec:** [.claude/specs/16-DETERMINISTIC-CONVERTER-V2.md](specs/16-DETERMINISTIC-CONVERTER-V2.md) — Spec 31 absorbed into §12 Appendix A on 2026-05-21
- Historical Spec 31 (absorbed, retained for git-blame): [.claude/specs/15-DETERMINISTIC-DRAFT-TO-SGS-CONVERTER.md](specs/15-DETERMINISTIC-DRAFT-TO-SGS-CONVERTER.md)
...
v1 recogniser (tools/recogniser/ -- 7 files): all TO-RETIRE Spec 31 Phase 5; none wired.
...
|---|---|---|
| tools/recogniser/*.py (7 files) | Spec 31 Phase 5 | EXISTS |
...
| fingerprint-builder/build-catalogue.py | Spec 31 Phase 3 | MISSING |
| fingerprint-builder/step*.py (2 files) | Spec 31 Phase 3 | MISSING (.pyc only) |
| fingerprint-builder/qa-gate.py | Spec 31 Phase 3 | MISSING |
