...
**D5. Forced colours everywhere.** Stylesheet shadows are covered by the existing `scripts/shadow-fallback/` (it appends the `CanvasText` outline to built block CSS at postbuild and to the theme's stylesheets, skips transient states such as `:hover` and `:focus` by design, and its `--check` runs in postbuild); composed PHP writers are gated by `scripts/check-shadow-fallback-php.py`. The new detector gates only what neither sees: a shadow serialised by the style engine (`supports.shadow`) without its fallback. Fixes: `--sgs-card-shadow` consumers, `supports.shadow` serialisation (append the fallback where the block scopes its style-engine output), gallery, and each style.css rule (a nested `@media (forced-colors:active)` beside it). The fallback stays tone-independent: forced-colours mode replaces author colours with system ones, so the tone check does not apply there.
