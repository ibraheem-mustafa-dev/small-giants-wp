...
- **One-line where-we-are (2026-06-14, D225):** **SDD build run — 4 converter fixes SHIPPED to `main` (conformance-green, PENDING live-verify) + the rest correctly DEFERRED as design-gate phases + a selector-seeding architecture decision.** Shipped (commit `a8bf5616`, each with a fire-proof lock-in test): **GF-B.2** selector-scope matcher (`convert.py:619-622`, no cross-section CSS bleed), **H-C1** per-slot max-width (`convert.py:2399` via `attr_for_area_property`), **IN-B** co-declared `var()` resolver (`convert.py:384`), **gridItem\*** uniform per-item CSS (`_lift_uniform_grid_item_css` `:2734`, wired top-level `:4414` + nested `:5574`). Both conformance suites green (84/84). **DEFERRED (verify-first = design-gate PHASES, NOT built):** array→child (FR-22-2.5), draft breakpoints (FR-22-5.2), D1 sidecar; IN-E needs Bean's live-probe. **nameFontWeight hardcode REVERTED** (Bean — no hardcoded selectors). **ARCHITECTURE DECISION (top next task): block.json-declared, auto-seeded selectors** (P-BLOCKJSON-SELECTOR-AUTOSEED, parking) — assign-canonical (`:485 WHERE canonical_slot IS NULL`) + fingerprints both skip canonical_slot-populated rows, so selectors got hardcoded; the fix is block.json `supports.sgs` + `/sgs-update` seeding (Rule-7, design-gate). **Spec-22 built_status TRUTH-SYNC** applied (FR-22-5.1/5.3 were mislabelled PLANNED but BUILT; etc.). **D-CEILING: D225.** **NEXT:** (1) block.json selector auto-seed (design-gate + council); (2) **Bean deploy + live-verify** the 4 shipped fixes on canary page 8; (3) the 3 deferred design-gate phases; (4) IN-E after the live-probe. Gate A (`plugins/sgs-blocks/scripts/tests/test_converter_conformance.py`) + `converter_v2/tests/` — run BOTH.
