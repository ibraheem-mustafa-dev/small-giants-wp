...
project_id: 14
current_phase: "CONTAINER/WRAPPER STANDARDISATION PROGRAMME (cloning thread) + THEME-BLOCKS (theme thread) — TWO active threads on main. CLONING: Workstream A shipped (0d746073); the container/wrapper CSS-transfer system deep-analysed → 5-workstream standardisation plan written (1d846667, NOT built); OPEN WITH WS-1 (sgs/container 3-layer completion). Plan: .claude/plans/2026-06-02-container-wrapper-standardisation.md; canonical procedure Spec 22 §FR-22-21; D152. Two next-session-prompts: .claude/next-session-prompt.md (cloning) + .claude/next-session-prompt-theme.md (theme)."
...
current_subphase_step: "THEME (2026-06-03, Spec 26 design session, D158): Bean questioned the global-CSS setup → research-buddies (13 sources) + 4-persona council (all ranked Candidate C #1) + CLI/abilities + converter-routing investigations → **Spec 26 — SGS Global Styles & Per-Client Theming** written (docscore 100% A) + committed (15a2b183). Architecture: lean framework theme.json baseline + per-client style-variation DELTA (kills fork-tax) + scoped deploy with Browse-Styles suppressed (super-fixes D18) + EXTEND push-theme-snapshot.py to write the live wp_global_styles post via POST /wp/v2/global-styles/{id} + pipeline style derivation (converter emits RAW → post-pass derives globals from repetition + hero-button-position rule). BUILD DEFERRED until cloning phase closes, EXCEPT FR-26-D2 (REST-write extension, urgent low-risk). FR-26-D1 RESOLVED/MOOT (verified — canary theme.json + post 7 already mirror Mama's palette+WCAG CSS; do NOT clear post 7). Supersedes D156 'override precedence' framing (WP 7.0 merge is a data-layer merge, not a CSS override). NEXT (theme): see next-session-prompt-theme.md — FR-26-D2 build is the concrete next theme task; universal auto-contrast still deferred (P-AUTO-CONTRAST-LIGHT-PRIMARIES)."
current_subphase_step: "CLONING (2026-06-02, THIS session, D152): Task 1 4-block save-null editor-/qc'd LIVE (PASS); Workstream A SHIPPED (0d746073 — sync-container-wrapping-blocks.py rewrite + container_kind migration + roster --apply all 28, 4-rater qc-council fixed the UPDATE-only silent undercount). Container/wrapper system deep-analysed (4-branch + 6-step target/current compare + artefact-empirical: the fold deletes __inner + strands its max-width; composites confidence 1.0 vs containers 0.0). Bean reframe: NO composite evades R-22-9 — composites must MIRROR sgs/container (block_composition propagation). 5-workstream standardisation plan + Spec 22 §FR-22-21 + flow/stages + parking written + 2-rater doc-council (1d846667). NEXT: WS-1 sgs/container 3-layer completion (design-gate + qc-council before code), then WS-3 (parallel) → WS-2 (B1 decision) → WS-4 composite mirror+auto-propagation. Branch already merged to main pre-session; on main @ 1d846667. POST-WRAP: /sgs-update ran clean (DB current, 190 blocks); FULL doc-walk reconciled ~16 docs across .claude/specs/plans (CLAUDE.md/architecture/goals/mistakes/specs-02/00/19/21/22 + 3 container plans marked SUPERSEDED); 2nd qc-council fixed the Spec 22 stale 188→189 / 4→28 sweep + container_kind schema row; lesson-capture endpoint fixed (/api/corrections dead → /api/learning, content field 'learning')."
