...
current_subphase_step: "Next action = Spec 22 Phase 0.1 (DB enrichment — Tier B ≤72-row backfill per D84 scope correction 2026-05-27). DB audit 2026-05-27 found 1,142 of 1,214 'NULL canonical_slot' rows are correctly-NULL behavioural attrs by design (block_attributes catalogues every block × every attr; canonical_slot sparsely populated by intent); real backfill scope is ≤72 Tier B candidates, not 1,214. Golden corpus DROPPED — dry-run JSON diff is the review surface. Pre-rewrite DB snapshot captured 2026-05-26 at pipeline-state/_snapshots/sgs-framework-pre-spec22.db (SHA256 d088...0017bc). Tier C ships dormant (0 candidates). Then 0.2 (wp-blocks.py extension), 0.3 (pixel-diff.py hardening), 0.4 (hybrid-block audit roster) — Tasks 0.2/0.3/0.4 can run in parallel with 0.1 per dependency graph. Phase 1 walker rewrite starts at Commit 1.1 only after Phase 0 closes."
