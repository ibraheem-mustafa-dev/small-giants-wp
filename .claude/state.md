...
project_id: 14
current_phase: hero-fix-shape-LOCKED-FR22.19-H-conv + trust-bar-dual-mode-FR24.10 — BUILD PENDING (focused session)
...
current_subphase_step: "NEXT SESSION (see next-session-prompt.md): READ §FR-22-19 + the council brief end-to-end FIRST. Build hero §FR-22-19 (H-conv): (1) DB add image-pipeline role + reclassify sgs/hero.splitImage/splitImageMobile (direct sqlite3, row-by-row); (2) CLOSE THE OPEN GAP — DB-first mapping of a composite's media-column BEM element → its scalar attr target + --mobile/--desktop disambiguation (NEVER a per-block slug conditional, R-22-1); (3) convert.py composite slot-router inside the is_class_section_block path (DB-driven, NOT a 4th branch); (4) live-DOM gate (1 .sgs-hero__content, 2 art-directed imgs, no other section changed) + roll-back-fast; (5) Bean sign-off. Then trust-bar §FR-24-10 dual-mode. /qc-council per converter/block commit."
last_updated: 2026-06-01
latest_commit: "docs(2026-06-01): hero §FR-22-19 H-conv fix-shape (qc-council REJECT H2) + trust-bar §FR-24-10 dual-mode + D125-D127 + parking + next-session-prompt STOP #1-32 (branch feat/fr22-4-1-universal-wrapper; NOT main)"
working_tree: "Docs committed 2026-06-01; lucide-icons.php auto-regen (never committed); stray src/blocks/trust-badges/ deletions still untracked-pending (clean up on next code commit)"
github_branches: "feat/fr22-4-1-universal-wrapper (8+ commits ahead of main; NOT merged) + main (clean)"
