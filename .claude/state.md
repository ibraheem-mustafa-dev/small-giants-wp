...
project_id: 14
current_phase: spec-22-architectural-cleanup-batch-SHIPPED-stream-a-canary-measurement-pending
current_subphase: "D93-D100 architectural cleanup batch SHIPPED 2026-05-29 (commit bcbafe09 + handoff e2df4eca on origin/main). 10 subagents, 99 files, +5346/-2275. Highlights: slot_synonyms + legacy_role_lookup UNIFIED into new `slots` table (composite PK on slot_name+scope; 89 element + 16 section = 105 rows); new `roles` table (20 rows) replaces slot_synonyms.role_classification — FIXES link-href content-bearing gap (equivalent_block_for sgs/media.videoUrl was returning None, now returns sgs/media); sgs/svg-background merged into sgs/container as bgSvg* attrs (D93); sgs/certification-bar merged into sgs/trust-badges as badgeStyle variants + auto-scroll (D95); sgs/media extended image-only → unified image/video block (D97); sgs/container ship-blockers (grid-item defaults + providesContext + advanced grid controls + templateMode, 13 new attrs total) (D98); block_capabilities wired into walker as FR-22-15 capability-aware BEM tiebreaker (D96); /sgs-update Stage 1 UPDATE-on-drift + Stage 10 v2/v3 aggressive default + attr-level orphan detection + retired-blocks cleanup (D94/D100); 4 retired blocks purged from blocks table; 205 attr ghosts + 26 stale supports purged; html_tag_to_core_block INSERT OR REPLACE; property_suffixes.kind_override column replaces _KIND_BY_SUFFIX hardcoded dict; 51 render.php WP_Block PHPDoc fixes; mojibake fix; phpunit intelephense stub. Stream A2 partial: price slot fixed (sgs/text). Stream A5 canary measurement DEFERRED to next session."
...
Now:
- `slots` table (105 rows, composite PK `(slot_name, scope)`, columns: slot_name + scope + aliases + standalone_block + notes) — unified element + section scope mappings
