...
- **One-line where-we-are (2026-06-20, D237):** **Phase-F F2 + F3-core + F4 + F6 SHIPPED; F5 PARTIAL; NEXT = F5-remaining.** F2 (`f8a746c7`, D232), F3-core (`6b430dae`, D234), F4 (`excluded_properties` ships EMPTY, `870f48aa`, D235) SHIPPED. **F6 SHIPPED (D237):** the DB-as-code consistency suite `plugins/sgs-blocks/scripts/db-consistency/` — **7 checks, 51 tests, 0 live violations, wired prebuild+prestart** (`1293c24f` 3 checks + `502f849e` 4 follow-ups + `b533690e` rating-role migration + `fdcf70ab` hero discriminator fix). Re-grounded on the lift surface after a `/qc-council`→`/adversarial-council` NO-GO + fact-check (the qc-council "validated" the hero fix on a SYNTHETIC unreachable input — the adversarial-council caught it); retired `check-composition-sync.py`. convert.py UNTOUCHED. **F5 PARTIAL (D236):** `check_no_mirror.py` armed with a committed legacy baseline (`6193f3e9`); **KNOWN GAP (STOP-6):** the orchestrator does NOT call `pipeline-stage-gate.py` → not auto-run. The OTHER 5 F5 gates (`check-converter-cheats.py`, `generate-coverage-matrix.py`, the pipeline-close ledger checker, the EXCLUDED-literal gate, the PreToolUse git hook) + the orchestrator-wire are OPEN — tracked `P-F5-REMAINING`. **NEXT SESSION = F5-remaining** (smallest = the check_no_mirror orchestrator-wire) then the stage-by-stage rebuild. **D-CEILING: D237.** convert.py FROZEN (D-MODULAR). Gate A green. F3-RUNTIME DEFERRED.
