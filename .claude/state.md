...
project_id: 14
current_phase: converter-renders-content+layout-SHIPPED-FR22.4.1-spec-impl-pending
current_subphase: "2026-05-31 session SHIPPED to main (squash-merge 1761eb35): the cloning pipeline now RENDERS content AND side-by-side card layout on Mama's page 144 (live-DOM verified, NOT pixel-diff which mis-scores). D114 XS-3 reconciled+merged (branch=superset). D115 root cause: walker never ran FR-22-2 content-routing for leaves. D117 G1 (leaf content-routing + the _atomic_attrs_for attr_type fallback-bug fix — all 7 sections render) + G2/FR-23-6 (depth-2 grid-wrapper preservation, council-designed + trace-confirmed: featured-product + gift cards side-by-side, duplicate nesting fixed, mean -0.66pp). D118 FR-22-4.1 canonical universal wrapper/container rule written + propagated to all docs + ROOT-CAUSE METHODOLOGY baked into CLAUDE.md (D26/D27). Spec 24 query-driven cards + sgs_product CPT built. blub.db dashboard DOWN (lessons in file layers, blub.db sync pending)."
current_subphase_step: "NEXT SESSION (see next-session-prompt.md): READ key docs + recent git log FIRST, apply the root-cause methodology (CLAUDE.md). Priorities: (1) HIGHEST — implement FR-22-4.1 fully (Spec 22 §FR-22-4.1): replace the depth-2 gate + walk_passthrough-drop + _absorb_transparent_wrappers patchwork with the one coherent fold/container/block + per-item-CSS mechanism. Sensitive walker rewrite — /qc-council + /verify-loop + per-section live-DOM measurement. (2) Image sideload (real WP media IDs, not Stage 4i dry-run) — biggest remaining pixel-diff lever. (3) Migrate sgs/info-box FR-22-6 hybrid (gift content). (4) Reach pixel-acceptance. (5) Deploy+seed sgs_product CPT. See parking P-FR226-FIDELITY-AND-MERGE."
last_updated: 2026-05-31
latest_commit: "1761eb35 (squash-merge feat/fr22-6-content-render → main: G1+G2 converter render+layout fix, FR-22-6 block migrations, FR-22-4.1 spec, methodology bake-in, Spec 24 CPT)"
