...
phase_3_5_summary: "Phase 3.5 (coverage + drift remediation + long-tail fanout) shipped 2026-05-12 via TWO 5-way parallel subagent dispatches. Final outcomes: drift violations 833→0, canonicalised 309→1210 (90.1%), gap queue 942→37 (96% reduction). Slot vocab grew 20→74; property_suffixes 48→99. Spec §11 amended to include layout/state/motion. Hero --verify-against PASS preserved end-to-end. Fanout produced 5 reports (phase-3.5-fanout-a1..a5.md) classifying 506 attrs across 57 blocks; tight-vocab rule applied (cross-cutting slots accepted, show*/payment*/security* collapsed to `options`). Modifier check refined twice: (1) skip when canonical=`options`, (2) only flag when stem-without-trailing matches the slot (distinguishes missing suffix from compound noun). 28 new property_suffixes added for trailing-token patterns (Type/Title/Size/Mode/Date/etc.). buttonSecondary kept as canonical (Bean correction); secondaryCta is an alias, not a separate slot."
