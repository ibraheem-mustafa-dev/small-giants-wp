...
project_id: 14
current_phase: "CSS-TRANSFER FIDELITY (cloning thread) + THEME-BLOCKS (theme thread) — TWO active threads. FR-22-20 variant detection SHIPPED+verified (hero). Branch feat/fr22-4-1-universal-wrapper, NOT merged. Two next-session-prompts: .claude/next-session-prompt.md (cloning) + .claude/next-session-prompt-theme.md (theme)."
...
latest_commit: "b83cd312 fix(hero): \$is_split fires on present split media → media column renders + spec role-name scalar-media. Session commits: 83a55820/5859c42d (hero scalar-media+gate) → d6358f32 (cta-section+trust-bar+converter) → b83cd312 (hero media+spec) + doc commits. Branch feat/fr22-4-1-universal-wrapper, NOT merged."
