...
## ACTIVE WORKSTREAM — Cloning pipeline: root-cause diagnosis session (the ONLY active work)
- **SoT for current status:** `.claude/handoff.md` (top entry, D277+D278) + `.claude/next-session-prompt.md` (the diagnosis-session orchestration plan with the MANDATORY READING GATE). READ THOSE.
- **One-line where-we-are:** the QC is FULLY closed (D277 + D278) — programme verified, 3 defect classes fixed (`4a35134a`), then ALL 8 parked findings cleared same-day per Bean (`a5161cc1`+`f31e1149`: tiebreaker deleted/FR-31-15 amended, 40 byte-compare goldens restored, real-draft metamorphic legs PASS, instrument tightened with numbers held, +2 bonus drop-bugs fixed). A2 5 keys; parity 90/67-69-76. Next front (Bean-directed at close): a DIAGNOSIS session — fresh clone run → full defect register → parallel root-cause investigation → cause-groups presented to Bean for agreement before fixing (card residuals + stylesheet defaults ride as known priors).
