...
| `block-validation-recovery` | When block attribute changes don't render on the frontend, check for block validation errors in the editor first — invalid blocks fall back to stale stored HTML. **Extended 2026-05-01:** any synthesised-markup pipeline (recogniser, importers, migration scripts) hits the same failure for static core blocks; mandatory fix is a `parse_blocks()` + `serialize_blocks()` round-trip via `wp eval-file` after `wp post create`, plus a zero-validation-error check before declaring deploy success. See `specs/common-wp-styling-errors.md` row B4. | [feedback_block_validation_recovery.md](~/.claude/projects/c--Users-Bean-Projects-small-giants-wp/memory/feedback_block_validation_recovery.md) |
