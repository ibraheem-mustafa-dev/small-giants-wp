# LEDGER snapshot — 2026-09-07 (auto-rotated by ledger-rotate.py)
...
⛔ **DEPLOY BLOCKED — stranded content on the LIVE HOMEPAGE, caused by this session's hero
rename (`41ac811e5`).** `post 2742` carries SIX pre-rename attrs (`splitImageAlt`,
`splitImageAltMobile`, `splitImageId`, `splitImageIdMobile`, `splitImageUrl`,
`splitImageUrlMobile`) and `post 3355` one (`splitImageUrl`); `hero/block.json` declares none of
them. Verified over SSH, not inferred. If deployed and anyone then opens 2742 in the editor and
SAVES, WP drops all six — the split image AND its alt text. `oldshape-audit` correctly refuses.
`621482e0e` fixed the CONVERTER emitting old names; it does not touch already-stored content.
⚠ Bean's call: re-clone 2742 (D554-B: canary pages are TRASHED not migrated — a correct fresh
clone already exists at page 3405) vs a stored-attr migration. Do NOT rewrite the homepage
unilaterally.
