...
## ▶ CLIENT-CONTROLS TRACK — 2026-08-30: dead-control bugs fixed, new scatter detector, NONE DEPLOYED
...
settable from two places, unsynced — now one. New tool finds a thing whose controls are
...
deploy-blocking gates (`08-raw-url-link` re-anchor, `01-tab-group` fixed in accordion-item);
the other two (`check-render-undefined-vars` on pricing-table, `check-editor-render-parity`
178→177) are the BORDER track's, already closed (D889).
