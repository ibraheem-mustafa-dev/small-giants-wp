# LEDGER snapshot — 2026-08-02 (auto-rotated by ledger-rotate.py)
...
⚠ **IN FLIGHT, UNCOMMITTED** — `plans/2026-08-01-shared-css-length-validator.md`. `includes/helpers-css-safety.php` (`sgs_css_length_value`) accepts `clamp/min/max/calc/var` via WP
core's grammar; `sgs_container_gap_value()` delegates to it. ⚠ Review CORRECTION: the validator is
NOT what enables the header clamp — that gap uses the OBJECT path
(`sgs_responsive_sanitise_css_value`), a pre-existing allowlist that already passed parens/commas.
The validator is load-bearing for the ~19 FLAT-scalar callers only. **Task 3 (live canary) BLOCKED — a co-active track
holds STAGED files in deploy scope (`form/`, theme CSS); do NOT `--allow-dirty` (D336), do NOT add
their files to `--payload`.** Proven locally: gap varies 16px→8.56px over 1400→320px, bounds held,
control constant. Review fixed an IMPORTANT hole — remainder-only validation passed
`calc(}body{color:red)`, `calc(1px;…)`, `clamp(<script>,…)`; raw input is now checked too.
...
### Track 2c task — finish the CSS-length validator [inline, Opus]
**What:** Task 3 of `plans/2026-08-01-shared-css-length-validator.md` — deploy + prove the fluid gap
on the canary. **Blocked only by** the co-active track's staged `form/`+theme files clearing deploy
scope. **Acceptance:** served CSS carries an intact `clamp(...)` AND the computed gap differs at
1400 vs 380px. **/qc gate:** yes. ⚠ The plan's `decision:` is UNASSIGNED — D457/D458 went to a
co-active track mid-session; re-read the live ceiling (D460 at close) before numbering.
