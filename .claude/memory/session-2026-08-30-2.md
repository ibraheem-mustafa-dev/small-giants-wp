...
## ▶ CLIENT-CONTROLS TRACK — 2026-08-30: two dead-control bugs fixed, new scatter detector; NOTHING DEPLOYED

⭐ **NEXT: the prompt named at the top of this file.**

**Plain English:** ~30 settings on two blocks (cta-section, trust-bar) did nothing — gone.
One trust-bar colour was settable from two places, unsynced — now one. New tool finds a
thing whose controls are scattered across panels.
...
built: 69 findings/48 blocks, self-test 32/32, advisory-first (not wired into `rules.json`).
Trust-bar's duplicate `textColour` writer collapsed to one. `74a203e8d` cleared 2 of 4
deploy-blocking gates (`08-raw-url-link` re-anchor, `01-tab-group` fixed at source in
accordion-item); the other two (`check-render-undefined-vars` on pricing-table,
`check-editor-render-parity` 178→177) are the BORDER track's, already closed (D889).

**Colour standard (D890):** `SgsColourPanel` default (65/83 blocks); own-panel colour row
only where a purpose-built paired composite exists (`SgsBorderControl` alone, today).
**Two detector gaps, unfixed (D892):** `check-dead-controls` can't see "read but feeds no
matching CSS" (how grid-item survived); `check-duplicate-controls` can't see a duplicate
writer inside a config-prop object literal (how trust-bar's survived).

⛔ **NOTHING TONIGHT DEPLOYED.** Pushed to `main` only. Carries the 2026-08-29 debt below.
...
HAND check is the only real gate.
...
C16 COMPLETE: all 48 `ResponsiveBoxControl` mounts pass `presets` (`18806e6b0`), detector
`inspector-scan/rules/36-box-control-presets-missing.js` + `migrate-box-control-presets.py`.
C19 rolled out to the full `aspectRatio` surface (card-grid, gallery, image-sequence, media,
post-grid). ⛔ `detector-first-commit-gate.py` HAS A NAMED HOLE: `MIN_SHARED_LINES = 3` rejects
a genuine 4-file repeat sharing only ONE line — a rollout IS a one-line repeat. Prompt Task 7.
...
rest unverified. A WRONG element is worse than NULL — NULL reads unknown, wrong reads
authoritative and misroutes CSS silently.

### ⚠ DEPLOY SAFETY — two near-misses, both fixed
`stackable/` (278MB competitor GPL source) was one deploy from landing web-accessible —
untracked files are invisible to the dirty gate, and `--exclude=…/src` missed `stackable/src`
(path-anchored). Now gitignored + excluded, tarball ceiling fails closed. Deploy went
**114MB → 29MB**. Ceiling 150 → 45MB.
