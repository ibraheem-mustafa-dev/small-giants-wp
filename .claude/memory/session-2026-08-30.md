...
**Plain English:** a client could set ~30 settings on two blocks (cta-section, trust-bar)
that changed nothing — gone. One trust-bar colour could be set from two places with nothing
syncing them — now one. New tool finds a visual thing whose controls are scattered across
panels, so a client shouldn't have to hunt.

**Commits (`b59f8cd3f`..`99d2204da`), pushed, none deployed:**
`b59f8cd3f`/`637385cfa` grid-item controls removed (only paint a direct `.sgs-container`
child, which neither block's cells are) + DB pruned (31 orphan attrs, 44 stale supports) +
roster regen (83 blocks). `91574fc05`/`5ee7a1d95` `scattered-element-controls.js` built: 69
findings/48 blocks, self-test 32/32, **not wired into `rules.json`** (advisory-first).
`99d2204da` trust-bar's duplicate `textColour` writer collapsed to one. `74a203e8d` cleared
2 of 4 deploy-blocking gate items (`08-raw-url-link` re-anchor, `01-tab-group` fixed at
source in accordion-item); the other two (`check-render-undefined-vars` on pricing-table,
`check-editor-render-parity` 178→177) are the BORDER track's, already closed there (D889).

**Colour standard (D890):** `SgsColourPanel` is default (65/83 blocks); an element gets its
own panel colour row only where a purpose-built paired composite exists (`SgsBorderControl`
alone, today).

**Two detector blind spots named, not fixed (D892):** `check-dead-controls` can't see "read,
but the CSS it feeds matches nothing" (how the grid-item controls got past every gate);
`check-duplicate-controls` can't see a duplicate writer inside a config-prop object literal
(how trust-bar's duplicate survived). Both open.

⛔ **NOTHING FROM TONIGHT DEPLOYED OR LIVE-VERIFIED.** Pushed to `main`, never sent to the
canary. Carries forward the 2026-08-29 debt below, unchanged.
