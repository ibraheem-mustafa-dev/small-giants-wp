# LEDGER snapshot — 2026-08-12 (auto-rotated by ledger-rotate.py)
...
- **A separate FX bug fix is fully built, self-tested, and ready to deploy** — a decorative shape
  ...
  just needs a deploy window.
- **The bespoke "Advanced" tab for the block inspector is RULED, not built.** A same-day mis-step
  nearly cancelled this decision on the grounds that WordPress itself has no native third tab — wrong
  reasoning, corrected same day. A ready-to-paste build prompt was handed to you directly.
...
- **Branch:** `main`, HEAD `8598ac73` (D591 — hero revert, shipped this session). ⛔ **This will
  drift — run `git log -1` AND `git status` AND `git branch --show-current`, don't trust this line.**
  Local and `origin/main` are in sync as of this HEAD (verified via `git push`, fast-forward).
  Commit by EXACT PATH — this checkout is shared with at least one other concurrent session (proven
  twice tonight — see D591 for both collision incidents).
- **Uncommitted, staged, ready to deploy:** the FX route-box fix — `assets/css/fx-motion-path.css`,
  `assets/css/fx-shape-routes.css`, `includes/fx-path-routes.php`, `includes/fx-shape-routes.php`,
  `src/blocks/container/style.css`. Fully built and self-tested; only blocked on a deploy window and
  live verification. Full detail in `go-track-1b-playful-hamster.md` C1/C2.
...
- **Verify every session:** `git log -1 --stat` · `git status` · `git branch --show-current` ·
  ...
  (currently 591) · `git merge-base --is-ancestor <claimed-shipped-commit> HEAD` before trusting any
...
| Decisions (D-numbered) | `decisions.md` — D591 is this session; D590/D589/D588 are recent siblings |
...
- **FX route-box fix — built, staged, ready to deploy.** Just needs a deploy window + live
  verification (outside/inside `sgs/container`, plus forcing the `@supports` fallback path).
- **The bespoke Advanced inspector tab (D4) — ruled, not built.** A cold-start prompt was handed to
  Bean directly for a fresh session. Scope note: this is explicitly NOT a one-session job for all 83
  blocks — build the tab mechanism, prove it on one block, then roll the four placement rules out as
  a separate pass.
