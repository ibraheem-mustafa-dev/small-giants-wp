# Handoff archive — older cloning-thread session sections (superseded; full detail also in decisions.md + git history)
...
## Archived cloning-thread sections (2026-06-03 and 2026-06-04) — moved 2026-06-06

# Session Handoff — 2026-06-04 (CLONING thread, PM4) — Method 2 converter-lift BUILD: Phase A core + FS-4 SHIPPED + live-verified (D172)

> Archived 2026-06-06. This was the live handoff before the 2026-06-05 session superseded it. Full history in git.
...
> Archived 2026-06-06. Superseded by the PM4 section.
...
> Archived 2026-06-06. Superseded by the PM section.

# Session Handoff — 2026-06-03 (CLONING thread, LATE PM) — Wave-1 block fixes + composite-diff scanner · dedup audit (NO merges) · generic-lift FALSIFIED · WS-4 set up as the full-29 opener

> Archived 2026-06-06. Superseded by the 2026-06-04 section.

# Session Handoff — 2026-06-03 (CLONING thread, PM) — WS-1 A1+A2 SHIPPED + #1-#8 triage GROUNDED (council) + WS-4 designed

> Archived 2026-06-06. Superseded by the LATE-PM section.
...
> Archived 2026-06-06. Superseded by the PM section. The two dated sections (2026-06-02 roster/DB-table; 2026-06-03 editor-fixes D141-D147) were already in handoff-archive.md.


---

## Archived theme-thread sessions (sessions 7-15) — moved 2026-06-06

---

# Session Handoff — 2026-06-05 (SGS THEME thread, session 15 — Spec 27 Phase 2 CLUSTER C COMPLETE: R2 provisioning + R3 authoring/edit-safety + PREFLIGHT go-live gate SHIPPED + QA-AUTHORING gate PASSED + Bean R-22-13 sign-off GRANTED)

> Theme/blocks thread. Cloning pipeline → `.claude/handoff.md`. Next → `.claude/next-session-prompt-theme.md`. **D-number: theme Cluster C decision entry is DEFERRED (needs a new D + the R1 D176 from last session) — `.claude/decisions.md` was uncommitted ALL session (the parallel Spec-28 P1 session holds it). FIRST JOB next session: add both once the tree clears.** Commit was by EXPLICIT PATH throughout; never-commit artefacts (`lucide-icons.php`, `sgs-framework.db`, `theme-snapshot.json`, Spec-28's `decisions.md` + `product-card/style.css` + `spec28-p1-value-ladder.md`) left untouched.

## Completed This Session — CLUSTER C SHIPPED (Spec 27 Phase 2 final cluster)
1. **R2 — provisioning + cartesian generate + transactional rollback SHIPPED** (`e62f337f`). NEW `Product_Provisioning` (+`-args` +`-helpers`) + shared `Product_Authoring_Security` (extracted from R1; R1 now delegates, behaviour-identical). `POST /sgs/v1/products/{id}/provision` (conflict-safe pa_* taxonomy/term provisioning — only ADDS, never alters siblings; parent attrs MERGED by union, never replaced → re-run with fewer attrs can't orphan; cartesian generate; upsert dedup `_sgs_variation_upsert_key`; combo cap 300 BEFORE writes; transactional rollback via created-resources ledger + parent snapshot; injected-failure hook triple-gated = dead in prod) + `POST .../variations/bulk`. All commerce writes via `set_*()`+`save()`. **Cross-model review: Opus security GO no-blockers; spec pass caught 2 real gaps I fixed (combo-cap ordering, set_attributes re-run truncation).** **Golden-master live PASS: R2-built == native-WC byte-identical; injected failure → 0 orphans; shared-taxonomy subset add → sibling unchanged.**
2. **R3 — presentation authoring + edit-safety SHIPPED** (`dd9d0d7d`). Audited Cluster A: swatch/gallery/divisor/label UIs already shipped; the ONE gap was the Google `variesBy` `<select>` on the attribute term add/edit screens → filled (saved via `Configurator_Meta::sanitize_variesby`). NEW `Configurator_Edit_Safety`: warns (not blocks) on a pa_* term SLUG rename; warns on deleting a variation with order history; cleans orphaned Configurator_Meta postmeta on variation delete. Zero-raw-meta authoring achieved.
3. **PREFLIGHT — hard go-live gate + cart £0 guard + health cron SHIPPED** (`dd9d0d7d`; publish-gate fix `27e54132`). NEW `Product_Preflight::evaluate()` = 7 blocker checks (wc<9.8/draft/zero_price/no_image/manifest_over_cap/no_variesby/invalid_jsonld), variable products only. `transition_post_status` reverts a blocked publish to draft + `_sgs_preflight_issues` meta + admin notice (dual re-entrancy guard). `GET /sgs/v1/products/{id}/preflight` pre-check (nonce + per-object edit_post). Cart £0 guard two layers: proxy 422 `sgs_price_not_set` + `woocommerce_add_to_cart_validation` filter (Store-API path). Weekly `sgs_preflight_health_check` cron (batched ≤50). **Live PASS: £0/no-image product → reverted to draft; valid product → publishes; cart refuses £0.** **Fix `27e54132`: invalid_jsonld Check-7 is now publish-gated** (the manifest only builds for published products, so a draft pre-check falsely reported empty JSON-LD; still validates at publish-transition + every re-save of a published product).
4. **QA-AUTHORING gate PASSED + R-22-13 sign-off GRANTED** (`27e54132` adds `scripts/e2e-authoring-acceptance.php`). Consolidated E2E live PASS: provision (R2) → author swatch/variesBy/gallery/unit via R3 meta → PREFLIGHT ready → publish → **valid ProductGroup rich-results (variesBy=size, AggregateOffer 48 offers £9.99–£59.99, per-variant offers + OOS) + 4-combo manifest** — every step via controller/REST/sanitised callback, ZERO raw SQL, ZERO WP-CLI. Bean shown the live configurator screenshot (`.claude/reports/visual-diff/cluster-c-configurator-540.png`) + the live JSON-LD → **signed off.**

## Orchestration that worked (Opus orchestrator + Sonnet implementers)
Dispatched Sonnet for every mechanical build (NO commit/deploy authority — returned uncommitted); I fact-checked EVERY claim against live ground truth + deployed + ran token-gated webroot acceptance one-shots + committed by explicit path. Fact-checking caught what `php -l` + subagent self-reports MISSED: un-wired bootstrap routes; 3 pre-WordPress WP-function calls in a one-shot token gate; an invented `wc_register_attribute_taxonomies()`; a `plugin_dir_path` path-doubling; a golden-master native-reference slug mismatch (underscore vs hyphen). Fan-out stayed ≤3 concurrent.

## Current State
- **Branch:** `main` at `27e54132` (3 code commits this session: `e62f337f` R2, `dd9d0d7d` R3+PREFLIGHT, `27e54132` preflight-fix+e2e; all pushed).
- **Tests:** `php -l` + WPCS 0-errors on all new/changed files. 4 token-gated acceptance one-shots in `scripts/` (golden-master-harness/-acceptance, preflight-acceptance, e2e-authoring-acceptance) — committed with PLACEHOLDER tokens; server copies (real tokens) were `rm`'d after each run.
- **Build/deploy:** all LIVE on sandybrown canary, opcache-reset, live-verified. No block build needed (PHP includes only). Regression guard: configurator pills still render (16 radios), homepage/test-page 200.
- **Uncommitted (NOT mine):** Spec-28's `decisions.md` + `spec28-p1-value-ladder.md` + the 3 never-commit artefacts + `product-card/style.css` (another session). Left untouched.

## Outcome vs Completion (Gate 3.5)
**OUTCOME ACHIEVED** — Cluster C is genuinely DONE: every unit live-verified on the canary via real acceptance runs (not "code shipped"), the full authoring journey proven end-to-end with valid rich-results, and Bean's co-authoritative R-22-13 sign-off granted after seeing the live page + JSON-LD. A first client shop is complete, sellable, discoverable, and client-authorable.

## Known Issues / Blockers
- **None block next session.** Cluster C is shipped + live.
- **DEFERRED (tree race):** add the R1 `D176` + a new Cluster-C `D` entry to `decisions.md` once the Spec-28 session commits. Carry from this handoff + session-14's recorded D176 text.
- Cluster D (R4 slug-templates / R5 AI-builder / F2 llms.txt+feed) is DECISION-GATED — does NOT block a first client shop (Bean decides at the Cluster-C gate whether launch waits for D).

## Notes for Next Session (NEW lessons — fold into STOP catalogue)
- **Pre-`wp-load` one-shot token gates must use NATIVE PHP only** (no `wp_json_encode`/`wp_unslash`/`sanitize_text_field` before `require wp-load`). `php -l` does NOT catch this (syntax-only).
- **Token-swap placeholder guards must be a LENGTH check (`strlen < 32`), never a literal-placeholder `===`** — a `sed s/PLACEHOLDER/token/g` rewrites the comparison literal too and defeats a `=== 'PLACEHOLDER'` guard (caught 2× → permanent 503).
- **Fact-check subagent wiring + invented APIs against ground truth:** grep `sgs-blocks.php` for the `::register()` line (R2 routes were un-wired); a subagent invented `wc_register_attribute_taxonomies()`; `plugin_dir_path(__DIR__)` doubled a path. `php -l` passes on all of these — only a live run + grep catch them.
- **The configurator manifest/schema is PUBLISH-GATED** (`Product_Manifest` builds combos only for `post_status==='publish'`). Any readiness/JSON-LD check on a DRAFT must account for this (PREFLIGHT Check-7 now gates on publish).
- **Golden-master fairness:** the "native" reference must derive taxonomy slugs the SAME way the unit under test does — WC normalises an attribute name "Acctest Size" → slug `acctest-size` (hyphen via sanitize_title); a hardcoded underscore constant produced a false diff.
