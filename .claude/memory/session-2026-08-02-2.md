...
- **A SHARED GIT INDEX can hold another track's staged files.** 20 of theirs were staged mid-session;
  a bare `git commit` would have taken all of them. ALWAYS `git commit -- <exact paths>` (a partial
  commit ignores the index for unlisted files). `git diff --quiet -- <file>` compares working-tree to
  INDEX, so it reports "clean" on a file the other track has already staged — use `git diff HEAD --`.
- **`grep -c` exits 1 on zero matches**, which silently kills an `&&` chain — my amend never ran and
  I reported from a stale read. Never chain a verification `grep -c` with `&&`.
- **A fix that is correct can still never run.** The css_layer fix edited Task A while the reseed only
  ever invokes `--task-b-only`; the artefact it feeds had to be regenerated separately. Verify a fix
  EMITS, not merely that the emitting code exists.
- **A council reasons correctly from a wrong brief.** An adjudicator called 13 rows unexplained
  because MY pack quoted a file mtime where the committed content was 6 days older. Second time this
  session. Fact-check your own brief before a panel decides on it.
