...
**Bean asked for a deterministic codemod** (built from the 2 proven examples), not further
hand-editing per block — in flight when this session closed. **Do not apply it to the remaining
45 blocks until you've read its own self-test/proof-against-the-2-references output** — it was
built to be proven, not assumed correct.
