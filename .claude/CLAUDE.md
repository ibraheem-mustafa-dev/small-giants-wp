...
- Recent decisions: read `decisions.md` head (most-recent-first; D-ceiling verified via `grep -oE 'D[0-9]+' .claude/decisions.md | sort -V | tail -1`). Do NOT cache a D-summary here — it drifts (the previous D225 inline summary was 16 decisions stale by the 2026-06-23 doc audit). Key milestones: cloning CSS-transfer foundation (Phase F, F1–F6 + F5 gates) COMPLETE; convert.py FROZEN (D-MODULAR, D229); Spec 30 (WooCommerce) COMPLETE (D220). **W3 Steps 1–7 BUILT (D250): walker port + CSS↔content unification conductor + native-style.*/box-shadow/§5 fixes; the two inert halves now connect (Finding A fixed). All WRITTEN not LANDED — new engine still INERT in prod (STOP-28); next = LANDED proof + A1/A2 + §5 lift-path. D-ceiling D250.**
