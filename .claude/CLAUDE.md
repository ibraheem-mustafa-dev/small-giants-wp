...
- Recent decisions: see `decisions.md` (current through D212, 2026-06-11 — D212 = testimonial-empty ROOT CAUSE confirmed (stale `block_composition.has_inner_blocks=1` for `sgs/testimonial` after the D8 rebuild → converter emits children the typed render ignores) + universal-lift build plan adversarial-council-gated [`.claude/plans/2026-06-11-testimonial-universal-lift-build.md`]; ALSO branches/worktrees consolidated to main; D211 = container 4-layer programme close + 8 ledger rows VERIFIED live; D210 = Spec 30 P1 WC chassis COMPLETE + merged, FR-30-12 ungated; D209 = R-22-13 block-quality remediation MERGED to main (all 12 Bean-review points): shared `TypographyControls` component + `sgs_typography_css_rule()` helper (canonical sgs/text UI — responsive size slider+unit, weight/style dropdowns) adopted across 6 blocks (counter/whatsapp-cta/mobile-nav/option-picker/trust-bar/product-card); Wave-1 bug fixes (trust-bar circle border + placeholder + badge-size; testimonial italic; notice-banner iconColour; icon alignment); announcement-bar RETIRED → notice-banner `displayMode=announcement` (sticky/closeable/button); qc-council-gated (security MERGE-OK; dismiss-key content-hash fix); typography component now MANDATORY in plugins/sgs-blocks/CLAUDE.md; D208 = WooCommerce page-type solution signed off (Spec 30, build delegated to theme thread); D207 = Stage-1 fold_eligible root cause + product-page = WC page type; D206 = block-quality programme (A1–E9 + cleanup) on `feat/block-quality-mirror`: product-card B3-B6 + picker-label forwarding, option-picker C7 label controls, notice-banner E9 overridable + dismissible removed, testimonial D8 typed-attr 7-variant rebuild + v8 migration [page-8 live-verified]; D205 = plans-folder consolidation (on feat/stage1); D204 = FP-H/FP-E SHIPPED (sgs/product-card is a BUILT-IN-ELEMENT block in all modes — connect+override, price-never-overridable, image-override-keeps-swaps; sgs/card-grid wc-product mode; ONE page-level ItemList; card emits NO schema; adversarial-council closed a draft→public-JSON-LD leak); D203 = Spec 28 P4 WC-write SHIPPED → Spec 28 COMPLETE; D202 = Spec 27 R4 slug-templates + F2 feeds SHIPPED → Spec 27 complete bar gated R5; D196–D200 = council backlog #17 + Spec 28 P2/P3 + visual pass; D167 = WS-4 block-side complete (29-block roster mirrors sgs/container); D152 = container/wrapper standardisation WS-1 + composite-mirror rule; D150 = archive-on-resolve; D57-D65 = doc-op programme close)
