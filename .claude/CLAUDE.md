...
- Recent decisions: see `decisions.md` (current through D160, 2026-06-03 — D159 = WS-1 A1+A2 SHIPPED: `sgs/container` `contentWidth` attr + `__inner` wrapper + converter section max-width transfer; D160 = triage #1–#8 grounded + WS-4 all-composites scope confirmed; D153-D158 = theme thread (Spec 26 global styles + per-client theming, build deferred); D152 = container/wrapper standardisation WS-1 SHIPPED + composite-mirror rule locked; D150 = archive-on-resolve standard practice; D148/D149 = theme thread; D57-D65 = 2026-05-24 doc-op programme close)
